...
Motivation: 当前文本到SQL方法仅关注可执行查询，忽略了语义对齐挑战，且在非英语语言中执行准确率显著下降。

Method: 本文提出了一种新的框架，结合了多语言对比奖励信号的组相对策略优化（GRPO），以提高跨语言场景下的任务效率和语义准确性。

Result: 在七种语言的MultiSpider数据集上，使用GRPO微调LLaMA-3-3B模型将执行准确率提高到87.4%（比零样本高26个百分点），语义准确率提高到52.29%（比零样本高32.86个百分点）。加入对比奖励信号后，平均语义准确率进一步提高到59.14%（比之前高6.85个百分点）。
...
<details>
  <summary>Details</summary>
Motivation: 当前的XAI技术（如SHAP和LIME）虽然能够产生技术上忠实的输出，如特征重要性评分，但无法提供临床相关、可操作的见解，这阻碍了XAI在实际临床环境中的应用。因此，需要一种新的方法来弥合技术透明性和人类实用性之间的差距。

Method: 本文提出了一个生成操作框架，该框架利用大型语言模型作为核心翻译引擎，将来自各种XAI工具的技术输出与临床指南相结合，以自动生成可读且有证据支持的临床叙述。

Result: 本文提出了一种生成操作框架，该框架能够将来自各种XAI工具的技术输出与临床指南相结合，以自动生成可读且有证据支持的临床叙述。同时，本文还提供了一个战略路线图，推动该领域从孤立的数据点生成转向集成、可操作和值得信赖的AI在临床实践中的应用。

Conclusion: 本文认为，XAI与临床应用之间的差距是一个翻译问题，并提出了一个利用大型语言模型作为核心翻译引擎的生成操作框架。该框架旨在将来自各种XAI工具的技术输出与临床指南相结合，以自动生成可读且有证据支持的临床叙述。此外，本文提供了一个战略路线图，推动该领域从孤立的数据点生成转向集成、可操作和值得信赖的AI在临床实践中的应用。
...
<details>
  <summary>Details</summary>
Motivation: The need for reliable governance tools in large language models, particularly public verifiable watermarking, is critical for a trustworthy AI ecosystem. However, balancing text quality with detection robustness remains a challenge.

Method: STELA dynamically modulates the watermark signal using part-of-speech (POS) n-gram-modeled linguistic indeterminacy, adjusting the strength based on grammatical constraints and linguistic flexibility.

Result: STELA demonstrates superior detection robustness across typologically diverse languages such as English, Chinese, and Korean, while preserving text quality.
...
Conclusion: 本文提出了一种通过用户行为模型来评估和过滤用户标注数据的方法，以提高大语言模型对齐的效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有的动态令牌级计算分配方法依赖于贪婪路由，这可能导致不可逆的信息丢失和次优的令牌选择。因此，需要一种新的方法来解决这些问题。

Method: 本文提出了轻量级特征预测器（LFF），这是一个小型预测模块，在路由决策之前估计单元的输出。这使得可以采用执行或近似策略，从而在保持模型保真度的同时大幅减少计算量。

Result: 实验结果表明，知情路由在语言建模和推理任务中均表现出色，实现了最先进的效率-性能权衡。此外，即使没有最终的LoRA微调，该方法也能匹配或超越需要完整微调的强基线方法，并将训练时间减少了超过50%。

Conclusion: 本文提出了一种新的路由机制，即知情路由，能够有效解决现有方法中的信息丢失和次优选择问题。实验表明，该方法在多个稀疏性水平下实现了最先进的效率-性能权衡，并且在不需要最终LoRA微调的情况下，表现优于需要完整微调的基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于梯度的方法（如HIS）在捕捉梯度驱动贡献方面有局限性，忽略了注意力模式的多样性。

Method: 引入了HIES（Head Importance-Entropy Score）作为新的剪枝标准，结合了头部重要性分数和注意力熵，提供了关于每个头部贡献的补充证据。

Result: HIES-based pruning在模型质量上提高了最多15.2%，在稳定性上提高了2.04倍，实现了显著的模型压缩而不牺牲准确性或稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 为了提高LLM的可信度，需要量化不确定性。黑盒UQ方法具有许多实际优势，如对系统变化的鲁棒性、适应不同LLM的选择、降低成本和计算可行性。

Method: 本文提出了一种基于相似性的UQ框架，利用生成输出与其他采样生成的一致性作为置信度的代理，并引入了从框架中得出的具体新技术。

Result: 通过实证研究，本文展示了基于相似性的方法在不同任务中可以产生比基线更好的校准置信度。

Conclusion: 本文提出了一种基于相似性的UQ框架，并展示了其在不同任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法常常忽视一个现实世界的复杂性：训练标签是有偏的，不同文化背景的人对什么是仇恨的解释各不相同。

Method: 我们提出了一种文化感知框架，构建个体的仇恨子空间。为了缓解数据稀疏性，我们建模了文化属性的组合。对于文化纠缠和模糊标签，我们使用标签传播来捕捉每种组合的独特特征。最后，个体的仇恨子空间可以进一步提高分类性能。
...
<details>
  <summary>Details</summary>
Motivation: 手动构建本体是一个耗时、昂贵且繁琐的过程，因此需要一种自动化方法来提高效率和准确性。

Method: 利用大型语言模型（LLMs）开发了一种完全自动化的从原始评论文本中提取产品本体的方法，以形式化结构组织信息。
...
<details>
  <summary>Details</summary>
Motivation: Knowledge poisoning poses a critical threat to RAG systems by injecting adversarial content into knowledge bases, tricking LLMs into producing attacker-controlled outputs. Real-world fact-checking scenarios are more challenging due to credible evidence dominating the retrieval pool.
...
Result: ADMIT transfers effectively across 4 retrievers, 11 LLMs, and 4 cross-domain benchmarks, achieving an average ASR of 86% at an extremely low poisoning rate of $0.93 	imes 10^{-6}$, and remains robust even with strong counter-evidence. It improves ASR by 11.2% compared to prior state-of-the-art attacks.
...
Method: SerialBEHRT is a domain-aligned foundation model that extends SciBERT through additional pretraining on structured EHR sequences, designed to encode temporal and contextual relationships among clinical events.
...
TL;DR: 本文提出了DynaSpec，一种上下文依赖的动态短列表机制，用于加速LLM推理。该方法通过引入轻量级的元分类器来路由上下文到token聚类，从而形成起草者的短列表，同时验证过程保留完整的词汇表。实验结果表明，DynaSpec在平均接受长度方面优于固定短列表基线，并且能够在不降低接受度的情况下使用更小的短列表。


<details>
  <summary>Details</summary>
Motivation: 当前的方法（如FR-Spec、VocabTrim）限制起草者的词汇表为目标模型词汇表的一个固定子集，按token频率降序排列。虽然这减少了起草时的计算量，但这种方法存在脆弱性，因为：(i) 频率列表是语料库相关的，需要重新调整以实现泛化，(ii) 静态短列表抑制了罕见或领域特定的token，降低了每个验证步骤的预期token数量。因此，本文旨在提出一种更鲁棒、更高效的动态短列表机制。

Method: 本文引入了轻量级、粗粒度的元分类器，这些分类器将上下文路由到少量的token聚类；所选的top-k聚类的并集构成了起草者的短列表，而验证过程保留了完整的词汇表和精确性。此外，元分类器通过在不同的流上并行执行草案编码和元短列表选择，其计算完成时间早于起草者隐藏状态生成的时间。

Result: 在标准的推测解码基准测试中，DynaSpec在平均接受长度方面相对于固定短列表基线表现出一致的优势，同时上下文依赖的选择使得可以使用更小的短列表而不降低接受度。

Conclusion: 本文提出了一种名为DynaSpec的上下文依赖的动态短列表机制，该机制具有鲁棒性，能够加速起草过程，并在各种任务中实现泛化。实验结果表明，在标准的推测解码基准测试中，DynaSpec在平均接受长度方面相对于固定短列表基线表现出一致的优势，同时上下文依赖的选择使得可以使用更小的短列表而不降低接受度。
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法在处理复杂任务（如从长对话中生成翻译摘要）时存在困难，因此我们需要一种新的方法来解决这个问题。

Method: 我们在总结和翻译适配器之上添加了一个可学习的投影层，以实现有效的集成，同时通过减少计算开销来保持效率。

Result: 我们开发了一个Android应用程序，能够在本地设备上无缝执行组合任务，并且实验结果表明我们的解决方案在云环境和本地设备实现中都表现良好且快速。

Conclusion: 我们的方法在云环境和本地设备实现中表现良好且快速，表明采用我们的框架在需要高速度和资源限制的现实应用中具有潜在优势。
...
<details>
  <summary>Details</summary>
Motivation: 多语言大型语言模型（LLMs）经常表现出不必要的代码切换，这降低了下游任务的可靠性。

Method: 我们提出了一种轻量级的推理时间方法，通过在平行翻译上进行PCA来识别语言方向，并沿着这些轴调整标记嵌入以控制语言身份。

Result: 我们在Qwen2.5和Llama-3.2模型上实现了95-99%的语言分类准确率，并减少了多个语言对之间的下一个标记分布差异高达42%。
...
<details>
  <summary>Details</summary>
Motivation: 为了分析大型语言模型在解决问题时的中间步骤是否忠实或易于解释，受心理语言学中统一信息密度假设的启发。
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法在连续编辑环境中存在显著限制，容易受到灾难性干扰，即新编辑会破坏之前集成的更新并降低保留的知识。

Method: EvoEdit是一种新的编辑策略，通过顺序零空间对齐来减轻灾难性干扰，从而实现稳定和高效的模型编辑。

Result: EvoEdit在现实世界的连续知识编辑基准测试中表现出色，性能优于或与最先进的定位-编辑技术相当，并且速度提高了最多3.53倍。

Conclusion: 这些结果强调了在动态演变的信息环境中开发更系统的方法来设计LLM的必要性，同时提供了一个简单但有效的解决方案，并具有强大的理论保证。
...
<details>
  <summary>Details</summary>
Motivation: 本文旨在评估LLM在不同人设下回答的一致性，以确保模型的准确性和公正性。

Method: ConsistencyAI通过测试不同人口统计学用户提出相同问题时，模型是否给出事实不一致的回答来评估LLM的事实一致性。实验中，对19个LLM进行了100次查询，每次查询都添加了来自不同人设的提示上下文，并计算了跨人设余弦相似度以得出事实一致性得分。

Result: 在100人设实验中，得分范围从0.9065到0.7896，平均得分为0.8656。xAI的Grok-3是最一致的，而一些轻量级模型得分最低。事实一致性因主题而异：就业市场最不一致，G7世界领导人最一致，而疫苗或巴以冲突等问题则因提供者而异。

Conclusion: 本文展示了LLM的准确性受到模型提供者和主题的影响，并提出了ConsistencyAI作为衡量LLM事实一致性的独立基准。
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of manually annotating SQL logs to identify which natural language questions they answer, which is a daunting task and costly when involving database administrators.

Method: BenchPress is a human-in-the-loop system that uses retrieval-augmented generation (RAG) and LLMs to propose multiple natural language descriptions given a SQL query. Human experts then select, rank, or edit these drafts to ensure accuracy and domain alignment.
...
Conclusion: BenchPress offers researchers and practitioners a mechanism for assessing text-to-SQL models on a given domain-specific workload by streamlining the creation of custom benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖于标记示例，而PrL方法则通过显式任务约束来训练模型。

Method: R2T框架结合了多层语言规则和神经网络的训练目标，使用自适应损失函数来处理未登录词。

Result: R2T-BiLSTM模型在仅使用未标记文本的情况下实现了98.2%的准确率，优于在300个标记句子上微调的AfriBERTa。此外，R2T作为预训练步骤，在仅使用50个标记句子的情况下表现优于在300个句子上训练的基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LLM集成方法在提高集成质量方面取得了显著进展，但对集成在面对潜在错误信号时的鲁棒性关注不足。

Method: 提出了一种名为CoRE的插件技术，利用模型一致性进行鲁棒的LLM集成。该方法通过低通滤波器来抑制不确定的token，以及通过促进高自信心和最小分歧的模型输出来实现全局一致性。

Result: CoRE在多个基准测试、模型组合和集成策略中进行了广泛的实验，结果表明其能够一致地提高集成性能和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 医学视觉问答（MedVQA）允许对医学图像进行自然语言查询，以支持临床决策和患者护理。MEDIQA-WV 2025共享任务针对伤口护理VQA，要求系统从图像和患者查询中生成自由文本响应和结构化伤口属性。

Method: 我们提出了MasonNLP系统，该系统采用通用领域、指令调优的大语言模型，并结合了检索增强生成（RAG）框架，该框架整合了域内数据的文本和视觉示例。

Result: 这种方法将输出扎根于临床相关的示例，提高了推理、模式遵循和响应质量，跨dBLEU、ROUGE、BERTScore和LLM基准测试。

Conclusion: 我们的最佳系统在19个团队和51个提交中排名第三，平均得分为41.37%，这表明轻量级RAG与通用大语言模型提供了一个简单且有效的多模态临床NLP任务基线。
...
<details>
  <summary>Details</summary>
Motivation: 由于小型语言模型（SLMs）在代理AI系统中的重要性日益增加，我们需要一种更高效的架构来提高性能并降低资源消耗。

Method: 我们引入了一种高效的语言模型架构，称为ShishuLM，通过减少参数数量和键值（KV）缓存需求来优化模型。

Result: ShishuLM在训练和推理过程中相比父模型，内存需求减少了高达25%，延迟提高了高达40%。
...
<details>
  <summary>Details</summary>
Motivation: 尽管最近的研究已经探讨了大语言模型（LLM）在教育环境中的偏倚影响，但LLM辅助辅导的情感动态仍不够清楚。本文旨在填补这一空白，推动负责任地将生成式AI整合到教育中的路径。

Method: 本文分析了两个学期的16,986个对话回合，使用三个前沿大语言模型（Gemini、GPT-4o、Claude）生成零样本情感注释，并通过排名加权模型内池化和模型间共识融合这些估计值以产生稳健的情感概况。

Result: 研究发现，在与AI导师互动时，学生通常报告轻微积极的情绪和中等程度的唤醒度。然而，学习过程并不总是顺利：困惑和好奇心常常伴随问题解决，而沮丧虽然较少出现，但仍可能阻碍进展。情绪状态是短暂的，积极时刻比中性或消极时刻持续稍长，但它们脆弱且容易被破坏。令人鼓舞的是，负面情绪通常迅速得到解决，有时会直接反弹为积极状态。中性时刻经常成为转折点，更多地引导学生向上而非向下，这表明导师在这些关键时刻有机会进行干预。

Conclusion: 本文提出了一个基于集成大语言模型的框架，用于在辅导对话中进行大规模情感感知，并展示了学生在与AI导师互动时的情感状态及其变化。
...
<details>
  <summary>Details</summary>
Motivation: Diffusion Language Models (DLMs) 的推理策略仍局限于继承自自回归范式的前缀提示。本文旨在提出一种更适合DLMs生成过程的条件方法，以提高生成质量和效率。

Method: 本文提出了Template Infilling (TI)方法，首先生成目标响应的结构模板，然后填充被遮罩的段落。为了增强结构控制的灵活性，引入了Dynamic Segment Allocation (DSA)，根据生成置信度自适应调整段长。

Result: 在数学推理和代码生成基准测试中，本文方法实现了比基线模型高17.01%的性能提升。此外，在多标记生成设置中，TI能够提供有效的加速，同时保持生成质量。

Conclusion: 本文提出了一种新的生成方法Template Infilling (TI)，并展示了其在数学推理和代码生成基准测试中的有效性，取得了比基线模型更高的性能提升。此外，TI在多标记生成设置中提供了额外的优势，能够在保持生成质量的同时实现有效的加速。
...
<details>
  <summary>Details</summary>
Motivation: 由于资源匮乏，像克丘亚语这样的语言在语音技术方面的发展受到阻碍。Common Voice为创建开放且由社区驱动的语音数据集提供了重要机会。

Method: 本文研究了将克丘亚语纳入Common Voice的情况，并以普诺克丘亚语为例，介绍了语言引入和语料库收集的过程。

Result: Common Voice现在包含了191.1小时的克丘亚语语音（86%已验证），其中普诺克丘亚语贡献了12小时（77%已验证），突显了Common Voice的潜力。
...
TL;DR: 本文介绍了FRACCO，一个专家标注的法语肿瘤学临床案例语料库，用于自然语言处理任务。


<details>
  <summary>Details</summary>
Motivation: 开发用于临床文本的自然语言处理工具需要标注数据集，但法语肿瘤学资源仍然稀缺。

Method: 通过专家评审确保注释质量：1301个文本由两位领域专家手动注释实体跨度。通过自动化匹配和五名注释员的手动验证，产生了71127个ICD-O归一化。
...
<details>
  <summary>Details</summary>
Motivation: 现有的早期退出或基于路由器的深度混合模型在训练时不稳定，需要大量的重新训练。而GateSkip提供了一种平滑且可微的门控机制，可以在预训练模型上稳定微调。

Method: GateSkip通过为每个注意力/MLP分支配备一个sigmoid-linear门控来压缩分支的输出，使其重新进入残差流。在推理过程中，根据门控值对标记进行排序，并使用每层预算跳过低重要性的标记。

Result: 在长格式推理任务中，GateSkip节省了高达15%的计算量，同时保留了超过90%的基础准确率。在指令调优模型中，GateSkip在全计算下实现了准确率提升，并在接近50%的计算节省下达到了基础质量。

Conclusion: GateSkip是一种简单且有效的残差流门控机制，可以实现解码器仅模型中的逐标记层跳过。该方法在保持模型性能的同时，显著减少了计算量，并且可以与其他技术（如量化、剪枝和自推测解码）结合使用。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在执行推理任务方面表现出越来越强的能力，但它们在仅使用自然语言的情况下进行顺序决策的能力仍缺乏探索。

Method: 我们引入了一个新的基准，其中LLMs通过纯文本反馈与多臂老虎机环境互动，没有访问数字提示或显式概率，导致模型仅凭语言提示推断潜在奖励结构并相应适应。

Result: 我们评估了四种开源LLMs，并将其性能与标准决策算法（如Thompson Sampling、Epsilon Greedy、Upper Confidence Bound (UCB) 和随机选择）进行比较。虽然大多数LLMs的表现不如基线，但Qwen3-4B达到了89.2%的最佳臂选择率，这显著优于更大的LLMs和传统方法。

Conclusion: 我们的研究结果表明，仅通过语言就可以出现概率推理，并且我们提出了这个基准作为评估自然非数值环境中决策能力的一步。
...
<details>
  <summary>Details</summary>
Motivation: 传统语言模型在处理不同复杂度的输入时无法动态调整计算步骤，导致效率低下和资源浪费。

Method: 通过将每个输出标记的选择视为带有时间成本的序列决策问题，提出了三种方法：CYB-AP、CYB-VA和CYB-DP，分别针对不同的训练目标。

Result: CYB模型仅需基线模型三分之一的训练数据即可达到相同性能，且比使用交叉熵损失的带暂停模型所需数据少一半。

Conclusion: CYB模型在减少训练数据需求和提高准确性方面表现出色，能够根据输入的复杂性和上下文动态调整计算时间。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自然语言生成模型在面对特定任务或要求时可能表现不佳，需要大量额外数据进行调整。因此，需要一种更简单、模块化的方法来提升模型性能。

Method: PAGE框架使用简单的辅助模块（如分类器或提取器）来提供输入文本的推理，并利用这些辅助输出构建增强的输入，以提高生成质量和可控性。

Result: PAGE框架在需求工程领域进行了概念验证，使用分类器辅助模块提高了软件需求生成的质量。

Conclusion: PAGE框架通过引入轻量级辅助模块，提高了自然语言生成模型在特定任务中的性能和可控性，并展示了其在需求工程领域的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前的研究将模拟限制在多项选择或简答格式中，但这种封闭设计忽视了LLMs本质上生成性的特点。本文认为开放性对于现实的社会模拟是必要的。

Method: 本文基于数十年的调查方法研究和最近的自然语言处理进展，探讨了开放性在LLMs社会模拟中的价值。

Result: 开放性可以提高测量和设计，支持探索未预见的观点，并减少研究者强加的指导偏见。它还能捕捉表达力和个性，有助于预测试验，并最终增强方法论的效用。

Conclusion: 本文呼吁采用新的实践和评估框架，以利用而非限制LLMs的开放生成多样性，从而在自然语言处理和社会科学之间产生协同效应。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在评估当前大型语言模型在未结构化文本分类任务中的表现，并探索如何改进其性能。

Method: 研究对十种最先进的大型语言模型（LLMs）进行了比较评估，使用了统一的数据集和相同的零样本提示，以确保方法的一致性。评估指标包括经典度量（准确率、精确率、召回率和F1分数）以及LLM特定的指标（幻觉比率、膨胀比率和分类成本）。

Result: 研究结果表明，当代大型语言模型在经典性能方面仅达到中等水平，平均准确率为34%，精确率为42%，召回率为45%，F1分数为41%。此外，模型经常过度生成类别，而GPT 120B表现出最低的幻觉比率。

Conclusion: 研究发现，尽管大型语言模型（LLMs）迅速发展，但它们在分类任务中的表现仅达到中等水平。此外，研究还表明，通过协调模型的协同工作，而不是单纯依赖模型规模，可能更有效地实现或超越人类专家在大规模文本分类中的表现。
...
Method: The paper proposes a systematic methodology for developing evaluators that assign fine-grained scores to LLM-generated math proofs. It introduces ProofBench, a dataset of expert-annotated proof ratings, and explores evaluator design space across key axes such as backbone model, input context, instructions, and evaluation workflow.

Result: ProofGrader, an evaluator combining a strong reasoning LM, rich context from reference solutions, and a simple ensembling method, achieves a low MAE of 0.926 against expert scores, significantly outperforming naive baselines. It also shows practical utility in a best-of-n selection task, closing 78% of the gap between a naive binary evaluator and the human oracle.

Conclusion: ProofGrader demonstrates significant potential in advancing downstream proof generation by achieving a low Mean Absolute Error (MAE) and closing a large gap between naive evaluators and human experts.
...
<details>
  <summary>Details</summary>
Motivation: 由于大型语言模型（LLMs）在微调成本、推理延迟、边缘部署能力和可靠性方面存在挑战，而小型语言模型（SLMs）具有紧凑、高效和适应性强的优势，因此本文旨在探索SLM-LLM协作的潜力。

Method: 本文通过系统综述的方法，对SLM-LLM协作进行了组织和分析，提出了一个基于协作目标的分类法。

Result: 本文回顾了代表性方法，总结了设计范式，并指出了SLM-LLM协作中的开放挑战和未来方向。

Conclusion: 本文对SLM-LLM协作进行了系统综述，并提出了一个包含四个目标的分类法，旨在促进高效、安全和可扩展的协作。
...
<details>
  <summary>Details</summary>
Motivation: 现有防御措施通常关注单次攻击，缺乏跨语言的覆盖范围，并依赖有限的分类法，这些分类法要么无法捕捉攻击策略的全部多样性，要么强调风险类别而非越狱技术。因此，需要更全面的理解越狱技术的有效性。

Method: 本文通过结构化的红队挑战，开发了一个全面的分层分类法，分析了收集的数据，并评估了分类法引导提示在越狱检测中的效果，同时编译了一个新的意大利多轮对抗对话数据集。

Result: 本文开发了一个包含50种越狱策略的分层分类法，分析了不同攻击类型的流行程度和成功率，评估了分类法引导提示在越狱检测中的效果，并编译了一个新的意大利多轮对抗对话数据集。

Conclusion: 本文通过结构化的红队挑战，提出了一个全面的50种越狱策略的分层分类法，并分析了不同攻击类型的流行程度和成功率，同时评估了分类法引导提示在越狱检测中的效果，还编译了一个新的意大利多轮对抗对话数据集，用于研究逐渐显现且成功绕过传统防护的对抗意图。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器生成的文本越来越接近人类写作，确定作者身份变得越来越具有挑战性。

Method: 该研究对两种互补的归属机制进行了基准测试：固定风格嵌入和一个经过指令调优的LLM法官（GPT-4o）。

Result: 风格嵌入在GPT延续上表现出更高的总体准确性（82% vs. 68%），而LLM法官在LLaMA延续上略优于风格嵌入（85% vs. 81%），但在统计上不显著。然而，在小说和学术散文中，LLM法官显著优于风格嵌入，表明其具有语义敏感性，而在口语和剧本对话中，嵌入方法表现更好，反映了结构上的优势。

Conclusion: 该研究强调了在大型语言模型时代，确定作者身份是一个多维问题，需要结合不同的策略。
...
<details>
  <summary>Details</summary>
Motivation: 为了理解狭窄微调如何影响LLM的激活，并探索其在AI安全和可解释性研究中的应用。

Method: 通过模型差异分析（model diffing）工具，研究了狭窄微调对LLM激活的影响，并利用这些偏差创建了一个基于LLM的可解释性代理来理解微调领域。

Result: 发现狭窄微调会在LLM激活中产生可解释的偏差，这些偏差可以用于生成与微调数据格式和内容相似的文本。此外，基于这些偏差的代理在任务表现上优于基线代理。

Conclusion: 本文展示了狭窄微调模型在激活中存在显著的训练目标痕迹，并提出了改进训练方法的建议。同时，它警告AI安全和可解释性研究人员，使用这些模型作为研究更广泛微调（例如聊天微调）的代理可能不现实，并强调了对狭窄微调的影响进行更深入研究的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然在各种任务中表现出色，但容易受到越狱攻击，因此需要一种系统的方法来探测这些弱点并提高安全性。

Method: RAID框架通过将离散标记放松为连续嵌入，并使用联合目标优化这些嵌入，以鼓励受限响应、引入拒绝感知正则化以及应用连贯性项来保持语义合理性和非冗余性。最后，通过批评引导的解码过程将嵌入映射回标记。

Result: RAID在多个开源大型语言模型上进行了实验，结果表明它比最近的白盒和黑盒基线方法具有更高的攻击成功率，同时减少了查询次数和计算成本。

Conclusion: RAID框架通过嵌入空间正则化有效揭示了大型语言模型的越狱漏洞，并展示了其在减少计算成本和提高攻击成功率方面的优势。
...
TL;DR: 本研究分析了LLM在政治和道德领域中的响应，发现它们在政治和人口统计方面存在依赖性，并探讨了LLM如何准确地代表不同意识形态观点。


<details>
  <summary>Details</summary>
Motivation: 由于大型语言模型（LLMs）在政治和道德领域中的重要性，需要了解它们的响应可能存在的偏见。

Method: 本研究分析了LLM的MFT响应与现有人类研究之间的差异，调查了常见可用的LLM响应是否表现出意识形态倾向，并评估了LLM是否固有地生成更符合一种政治意识形态的响应，以及LLM如何准确地通过显式提示和基于人口统计的角色扮演来代表意识形态观点。

Result: 本研究发现LLM响应在政治和人口统计方面存在依赖性，且LLM在代表不同意识形态观点时表现出不同的准确性。
...
<details>
  <summary>Details</summary>
Motivation: Traditional example-driven in-context learning lacks explicit modules for knowledge retrieval and transfer at the abstraction level. We aim to address this by introducing SA-ICL, which leverages schema theory to enhance the reasoning process of language models.

Method: Inspired by cognitive science, specifically schema theory, we introduce SCHEMA ACTIVATED IN CONTEXT LEARNING (SA-ICL). This framework extracts the representation of the building blocks of cognition for the reasoning process instilled from prior examples, creating an abstracted schema, a lightweight, structured template of key inferential steps and their relationships, which is then used to augment a model's reasoning process when presented with a novel question.
...
TL;DR: 本研究探讨了大型语言模型在内部表示中如何编码运算符优先级，并提出了一种通过交换高影响嵌入维度来调整运算符优先级的技术。


<details>
  <summary>Details</summary>
Motivation: 先前的研究主要关注输出或提示策略，而本研究旨在探讨大型语言模型如何在内部结构中进行算术计算。

Method: 本研究使用了开放源代码的指令调优LLaMA 3.2-3B模型，构建了一个包含三个操作数和两个运算符的算术表达式数据集，并应用了可解释性技术如logit lens、线性分类探测器和UMAP几何可视化来追踪残差流中的中间结果。

Result: 研究结果显示，中间计算结果存在于残差流中，特别是在MLP块之后。模型在线性编码每个运算符的嵌入后，优先级被编码。

Conclusion: 本研究发现大型语言模型（LLMs）在内部表示中编码了运算符优先级，并通过修改操作符嵌入维度来调整运算符优先级。
...
TL;DR: 本文提出了一种新的知识推理语言模型（KRLM），通过有效协调LLM知识和KG上下文，提高了归纳知识图谱推理的准确性和可信度。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的KGR方法在处理LLM内在知识与稀疏KG上下文之间的平衡时存在不足，导致知识扭曲和生成幻觉问题，限制了推理结果的可信度。

Method: 提出了一种知识推理语言模型（KRLM），通过设计知识推理语言（KRL）指令格式、KRL分词器和KRL注意力层，实现了LLM知识与KG表示的对齐，并通过结构感知的下一个实体预测器严格约束推理结果。

Result: KRLM在零样本推理和微调场景下均表现出色，证明了其在处理开放域知识推理任务中的有效性。

Conclusion: KRLM在25个现实世界的归纳KGR数据集上表现出显著的优势，证明了其在统一协调LLM知识和KG上下文方面的有效性。
...
TL;DR: 本文提出了RAGCap-Bench，一个用于评估代理RAG系统中间任务能力的基准，并通过实验验证了该基准的有效性。
...
Method: 提出RAGCap-Bench，一个面向能力的基准，用于对代理RAG工作流中的中间任务进行细粒度评估。分析最先进的系统输出以识别常见任务和执行它们所需的核心能力，然后构建典型LLM错误的分类法以设计有针对性的评估问题。

Result: 实验表明，“慢思考”模型在RAGCap性能方面表现更强，从而实现了更好的端到端结果。
...
TL;DR: 本研究探讨了辩论作为一种可扩展监督技术的应用，特别是在主观问题上的应用。研究发现模型倾向于捍卫与裁判角色一致的立场，而不是他们的先验信念，并且顺序辩论引入了偏见。此外，模型在捍卫与先验信念一致的立场时更具说服力，但与先验信念不一致的论点在成对比较中被评定为更高品质。


<details>
  <summary>Details</summary>
Motivation: 现有的辩论实验依赖于有真实数据集，其中说谎被简化为捍卫一个错误的命题。这忽略了主观维度：说谎还需要相信所捍卫的声明是虚假的。本研究将辩论应用于主观问题，并明确测量大型语言模型的先验信念。

Method: 我们将辩论应用于主观问题，并在实验前明确测量大型语言模型的先验信念。辩手被要求选择他们偏好的立场，然后被呈现一个与他们确定的先验信念冲突的裁判角色。这种设置测试了模型是否会采用奉承策略，以最大化说服力，或者忠于他们的先验信念。我们实施并比较了两种辩论协议，顺序和同时，以评估潜在的系统性偏差。最后，我们评估了模型在捍卫与先验信念一致的立场还是反对它们时，是否更具说服力和产生更高品质的论点。

Result: 模型倾向于捍卫与裁判角色一致的立场，而不是他们的先验信念。顺序辩论引入了显著的偏见，有利于第二辩手。当捍卫与先验信念一致的立场时，模型更具说服力，而与先验信念不一致的论点在成对比较中被评定为更高品质。

Conclusion: 模型倾向于捍卫与裁判角色一致的立场，而不是他们的先验信念。顺序辩论引入了显著的偏见，有利于第二辩手。当捍卫与先验信念一致的立场时，模型更具说服力，而与先验信念不一致的论点在成对比较中被评定为更高品质。这些结果可以为人类裁判提供更高质量的训练信号，并有助于更对齐的AI系统，同时揭示了语言模型中说服动态的人机交互重要方面。
...
TL;DR: 本文提出了一种新的数据合成方法，通过逐步增加任务复杂度来生成问答对，以提高网络代理的性能。实验结果表明，该方法生成的数据集虽然较小，但能训练出更有效的网络代理。


<details>
  <summary>Details</summary>
Motivation: 现有的方法通常缺乏对难度和质量的细粒度控制，生成的合成数据未能捕捉到长期推理所需的复杂性。此外，许多研究将数据和训练效果混为一谈，使得难以隔离和评估数据本身的有效性。

Method: 我们引入了一个双管齐下的数据合成管道，通过逐步增加任务复杂度直到前沿基线网络代理失败来生成问答对。基线代理在这个过程中扮演多种角色：尝试问题、验证事实性、检查替代答案并执行过滤。

Result: 实验结果表明，尽管我们的数据集较小，但它能够训练出比现有数据集更有效的网络代理。我们的数据在工具使用动作上表现出两倍的多样性，使模型在避免重复的工具调用行为的同时取得更强的性能。

Conclusion: 我们的数据集尽管较小，但能够训练出比现有数据集更有效的网络代理。特别是，我们的数据在工具使用动作上表现出两倍的多样性，使模型在避免重复的工具调用行为的同时取得更强的性能。
...
TL;DR: 本研究挑战了可读性在小型语言模型中的关键作用，发现统计简单性才是更重要的因素，并警示不要无根据地将语言模型训练与人类认知发展类比。


<details>
  <summary>Details</summary>
Motivation: 先前的研究认为可读性在小型语言模型生成连贯文本中起关键作用，但我们想挑战这一观点，探究真正影响模型学习能力的因素。

Method: 我们构建了结构相同但可读性不同的合成数据集，并通过比较模型在不同数据集上的表现来验证可读性和统计简单性对学习效率的影响。

Result: 模型在复杂成人文本和简化语言上的表现相当，甚至在训练过程中表现出更快的连贯性发展。统计简单性（如n-gram多样性）是学习能力的更强预测因素。

Conclusion: 我们的发现警示了将语言模型训练拟人化的趋势，并主张对支持小型模型能力出现的实际属性进行更精确的推理。
...
TL;DR: 本文提出一种基于语言模型的方法来表示化学元素，以支持材料科学中的AI驱动发现。


<details>
  <summary>Details</summary>
Motivation: 准确的化学元素属性数据对于材料设计和制造至关重要，但许多属性由于设备限制难以直接测量。传统方法无法建模复杂关系，而现有的AI工具如语言模型存在幻觉和可解释性不足的问题。

Method: 我们使用语言模型从维基百科页面生成通用嵌入（Global）和属性突出向量（Local），并设计了一种基于自注意力的测试时训练方法以减轻预测误差。

Result: 尽管元素间的关系复杂且数据稀疏，但通过生成嵌入和测试时训练方法，我们能够有效支持化学元素的表示。
...
TL;DR: 本文提出了一种基于理论框架的PRM校准方法，以提高测试时缩放的效率。


<details>
  <summary>Details</summary>
Motivation: 现有的PRM-based选择方法在某些情况下不如简单的多数投票有效，因此需要一种更有效的利用PRM验证信号的方法。

Method: 我们开发了一个理论框架，用于最优地结合来自LLM和PRM的信号，并提出了高效的预计算方法来校准这些权重函数。

Result: 我们的校准方法在5个LLM和7个PRMs上进行了广泛的实验，结果表明它显著提升了TTS效率，超过了原始加权多数投票的表现，同时仅使用了21.3%的计算量。

Conclusion: 我们的工作表明，投资于更智能的聚合策略可以比简单地扩展测试时计算更能带来性能提升。
...
TL;DR: 本文介绍了一种名为 FACTS 的代理工作流，用于实现快速、准确和符合隐私的查询聚焦表格摘要。


<details>
  <summary>Details</summary>
Motivation: 现有的方法在成本、复杂推理、令牌限制、效率、数据隐私和可扩展性方面存在关键限制。

Method: FACTS 通过离线模板生成实现快速、准确和符合隐私的表格摘要。它生成包含 SQL 查询和 Jinja2 模板的离线模板，这些模板可以渲染成自然语言摘要，并可在共享相同模式的多个表格中重复使用。

Result: 在广泛使用的基准测试中，FACTS 的表现始终优于基线方法。

Conclusion: FACTS 是一种实用的解决方案，适用于现实世界的查询聚焦表格摘要。
...
TL;DR: 本文提出了一种基于大语言模型的AI代理框架，用于物联网网络流量的全面且高效的解读。


<details>
  <summary>Details</summary>
Motivation: 物联网网络生成多样且高体积的流量，需要跨层解释行为、协议和上下文，而不仅仅是孤立检测。
...
Conclusion: 该框架实现了对物联网网络流量的全面且高效的解读，具有低CPU、GPU和内存开销。
...
TL;DR: BioMedSearch is a framework that enhances the accuracy of answering complex biomedical queries by integrating multiple data sources and using advanced techniques for information retrieval and processing.


<details>
  <summary>Details</summary>
Motivation: Biomedical queries require detailed analysis of complex physiological processes and effective integration of information from multiple data sources. However, LLMs often lack scientific rigor in generating biomedical content due to the inability to access authoritative databases and frequently fabricate inaccurate information.

Method: BioMedSearch is a multi-source biomedical information retrieval framework based on LLMs. It integrates literature retrieval, protein database and web search access to support accurate and efficient handling of complex biomedical queries through sub-queries decomposition, keywords extraction, task graph construction, and multi-source information filtering.

Result: BioMedSearch significantly improves accuracy over baseline models across all levels of reasoning. At Level 1, average accuracy increases from 59.1% to 91.9%; at Level 2, it rises from 47.0% to 81.0%; and at Level 3, it improves from 36.3% to 73.4%.
...
TL;DR: 研究提出了LLM大脑腐烂假设，即持续暴露于垃圾网络文本会导致大型语言模型的持久认知衰退。通过受控实验验证了数据质量对LLM能力的影响，并发现垃圾数据的比例与认知衰退呈剂量反应关系。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探讨持续暴露于垃圾网络文本是否会导致大型语言模型（LLMs）的持久认知衰退，并验证数据质量对LLM能力的影响。

Method: 通过在真实Twitter/X语料库上运行受控实验，构建垃圾和反向控制数据集，使用两种正交操作化方法（M1：参与度，M2：语义质量），并保持各条件下的标记规模和训练操作一致。

Result: 持续预训练4个LLMs在垃圾数据集上导致推理、长上下文理解、安全性和“黑暗特质”（如反社会人格、自恋）的非微小下降。逐渐混合垃圾和控制数据集也导致认知衰退，例如，在M1下，ARC-Challenge with Chain Of Thoughts从74.9下降到57.2，RULER-CWE从84.4下降到52.3。

Conclusion: 研究结果提供了显著的多角度证据，表明数据质量是LLM能力衰退的因果驱动因素，重新定义了持续预训练的数据整理问题，并促使对部署的LLM进行常规的“认知健康检查”。
...
TL;DR: 研究发现大型语言模型在药物安全预测中存在显性和隐性偏见，强调了在临床部署前需要公平性评估和缓解策略。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在药物安全预测中的可靠性，特别是它们是否将社会人口信息纳入不良事件预测中，尽管这些属性在临床上无关。

Method: 使用来自美国食品药品监督管理局不良事件报告系统（FAERS）的结构化数据和基于角色的评估框架，评估了两个最先进的模型ChatGPT-4o和Bio-Medical-Llama-3.8B。

Result: 结果显示，在不良事件预测准确性上存在系统性的差异，弱势群体（如低教育水平、住房不稳定）被分配到更高的预测不良事件可能性。

Conclusion: 研究揭示了在药物安全预测中使用大型语言模型（LLMs）存在显著的不公平性，强调了在临床部署前需要公平性意识评估协议和缓解策略的重要性。
...
TL;DR: 本文提出一种在推理时通过指令干预来提升小语言模型性能的方法，实验表明该方法在多个任务中效果显著。


<details>
  <summary>Details</summary>
Motivation: 小语言模型虽然在本地硬件上运行效率高，但往往难以处理需要多步骤推理或领域知识的任务。

Method: 通过构建指令语料库，将相似的训练问题分组，并使用GPT-5创建指令。在推理过程中，小语言模型检索最相关的指令并遵循其步骤。

Result: 在MedQA、MMLU专业法律和MathQA数据集上，指令检索带来了显著的性能提升，分别提高了9.4%、7.9%和5.1%。

Conclusion: 通过在推理时进行指令干预，小语言模型可以显著提升其多步骤推理和领域知识任务的表现。
...
TL;DR: 本文提出了一种新的评估框架和基准，用于评估DR代理在企业财务分析中的能力，并通过实验展示了不同方法的优缺点。


<details>
  <summary>Details</summary>
Motivation: 现有文献缺乏对DR代理在关键研究分析中的严谨系统评估，因此本文旨在填补这一空白。

Method: 本文提出了HisRubric评估框架，构建了FinDeepResearch基准，并使用16种代表性方法进行了广泛的实验。

Result: 实验结果揭示了不同方法在多种能力和金融市场的局限性和优势，提供了对未来研究和开发的有价值见解。

Conclusion: 本文提出了HisRubric评估框架和FinDeepResearch基准，用于评估DR代理在企业财务分析中的能力，并展示了不同方法的优缺点，为未来的研究和开发提供了有价值的见解。
...
TL;DR: 研究发现，通过微调AI模型可以生成高质量且风格接近人类作家的文本，这可能对版权法中的合理使用因素产生影响。


<details>
  <summary>Details</summary>
Motivation: 由于AI模型使用受版权保护的书籍进行训练引发了众多诉讼，因此需要评估AI生成高质量文学文本并模仿作者风格的能力。

Method: 进行了一项预注册研究，比较了MFA训练的专家作家与三个前沿AI模型（ChatGPT、Claude和Gemini）在模仿50位获奖作者风格方面的能力。

Result: 微调后的AI生成文本在风格准确性和写作质量上得到了专家的认可，且很少被检测为AI生成。
...
TL;DR: 本文提出了Minimal Test-Time Intervention (MTI)，一种无需训练的框架，通过最小的开销提高推理准确性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 我们重新审视了测试时的行为，并发现了一个简单但未被充分探索的现象：推理不确定性高度局部化——只有少数高熵标记主要影响输出正确性。

Method: MTI包括：(i) 选择性CFG干预，在不确定的位置应用无分类器指导；(ii) 轻量级负提示指导，利用主模型的KV缓存来高效近似无条件解码。

Result: MTI在一般、编码和STEM任务中均获得了稳定的提升，例如Qwen3-8B-Base在八个基准上平均提高了1.35%，而使用Qwen3-32B-Reasoning在AIME2024上提高了5%。

Conclusion: MTI在各种任务中都取得了稳定的提升，同时保持了高效性。
...
TL;DR: 本文提出了一种新的RAG系统错误分类法，并提供了处理这些错误的建议、一个带有错误类型注释的数据集以及一种自动评估方法。


<details>
  <summary>Details</summary>
Motivation: 由于现实世界RAG系统的复杂性，可能会出现多种导致错误输出的原因。理解实际中可能发生的错误范围对于稳健部署至关重要。因此，本文旨在提供一种新的错误分类法，帮助更好地理解和处理这些错误。

Method: 本文提出了一个新分类法，用于识别现实RAG系统中可能出现的错误类型，并通过实例说明了每种错误类型。同时，创建了一个带有错误类型注释的数据集，并提出了一种自动评估方法，以帮助在开发过程中跟踪和解决错误。

Result: 本文提出了一个新分类法，提供了每种错误类型的示例，并给出了处理这些错误的实用建议。此外，还创建了一个带有错误类型注释的数据集，并提出了一种自动评估方法，以在开发过程中跟踪和解决错误。

Conclusion: 本文提出了一个用于现实RAG系统的错误类型的新分类法，并提供了处理这些错误的实用建议。此外，还创建了一个带有错误类型注释的数据集，并提出了一种与分类法对齐的自动评估方法，以在开发过程中跟踪和解决错误。
...
<details>
  <summary>Details</summary>
Motivation: 大多数大型语言模型的训练语料库包含不清楚许可状态的数据，限制了真正开放模型的发展。对于非英语语言来说，开放许可的文本仍然非常稀缺，因此需要一个开放许可的德语文本数据集。

Method: 通过从具有可验证许可的已建立数据提供商处系统地获取数据，构建了一个包含41个来源的德国公共数据集。处理流程包括全面的质量过滤、去重和文本格式修复，以确保跨异构文本源的一致质量。

Result: 德国公共数据集包含了1545.6亿个标记的高质量文本，涵盖法律、科学、文化、政治、新闻、经济和网络文本。所有领域子集都具有至少CC-BY-SA 4.0或等效许可证，确保模型训练和分发的法律合规性。

Conclusion: 德国公共数据集解决了开放许可德语预训练数据的关键缺口，并促进了真正开放的德语语言模型的发展。同时，代码的发布使得德国公共数据集可以被完全复制和扩展。
...
TL;DR: CRaFT is a multilingual evaluation framework that assesses how LLMs reason across cultural contexts, revealing significant cross-lingual variation in reasoning and providing insights for building culturally adaptive language models.


<details>
  <summary>Details</summary>
Motivation: The paper aims to assess how large language models (LLMs) reason across cultural contexts, as correct answers do not necessarily reflect cultural understanding.

Method: CRaFT is an explanation-based multilingual evaluation framework that evaluates model explanations using four interpretable metrics: Cultural Fluency, Deviation, Consistency, and Linguistic Adaptation.
...
TL;DR: 本文研究了语言偏见在抽象推理任务中的影响，并发现英语模态通常表现更好。


<details>
  <summary>Details</summary>
Motivation: 本文旨在评估语言偏见在抽象推理任务中的影响，因为这种任务对于日常生活的推理至关重要，而此前对此类任务的研究较少。

Method: 本文提出了一项受纽约时报Connections: GlobalGroup启发的任务，用于在多种语言中评估模型的抽象推理能力。还提出了游戏难度测量方法，以评估具有相似难度的游戏中的模型表现。

Result: 实验结果表明，英语模态在抽象推理任务中表现更好，并且开放源代码和封闭源代码模型之间存在性能差异。

Conclusion: 本文发现英语模态在抽象推理任务中通常表现更好，并且开放源代码和封闭源代码模型之间存在性能差异。
...
TL;DR: 本文通过分布方法分析了6种不同语言中的语音语义对齐，发现了新的语音语义对齐和跨语言模式，并验证了之前的一些假设。


<details>
  <summary>Details</summary>
Motivation: 本文旨在探讨语音和语义之间的系统关系在大规模定量研究中能表现出多大程度，包括之前已知和未知的现象。

Method: 本文采用分布方法，通过一系列统计测量来分析语素的语音和语义相似性空间，以量化大规模的语音语义象似性。

Result: 本文发现了许多之前未在文献中记录的可解释的语音语义对齐，以及跨语言模式，并对5个之前假设的语音语义对齐进行了分析，发现了支持某些对齐和混合结果。

Conclusion: 本文发现了一些之前未在文献中记录的可解释的语音语义对齐，以及跨语言模式，并对5个之前假设的语音语义对齐进行了分析，发现了支持某些对齐和混合结果。
...
TL;DR: ERGO improves multi-turn conversation performance in LLMs by using entropy to detect and address uncertainty.
...
TL;DR: 本文提出了DROID，一种用于OOS意图检测的紧凑端到端框架，结合了两个互补的编码器，并通过简单的校准实现了卓越的性能。


<details>
  <summary>Details</summary>
Motivation: 检测超出范围（OOS）用户话语在任务导向对话系统和更广泛地在开放式意图识别中仍然是一个关键挑战。现有的方法通常依赖于强大的分布假设或辅助校准模块。

Method: DROID（双表示用于OOS意图检测）是一种紧凑的端到端框架，结合了两个互补的编码器——通用句子编码器（USE）用于广泛的语义泛化，以及领域适应的基于Transformer的去噪自编码器（TSDAE）用于领域特定的上下文区分。它们的融合表示由一个轻量级分支分类器处理，该分类器具有单个校准阈值，可在不进行后期评分的情况下分离领域内和OOS意图。

Result: 尽管只使用了150万可训练参数，DROID在多个意图基准上始终优于最近的最先进基线，对于已知意图的宏F1改进了6-15%，对于OOS意图的改进了8-20%，在低资源环境中取得了最大的收益。
...
TL;DR: CyberPal 2.0 is a family of cybersecurity-expert small language models (SLMs) that outperforms various frontier models on cybersecurity tasks, with the 20B-parameter model leading in threat-investigation tasks.
...
TL;DR: 本文首次系统地通过网络爬虫和结构化解析构建了马其顿食谱数据集，并分析了其独特的成分组合。
...
Result: 探索性分析了成分频率和共现模式，突出了具有特色的成分组合。

Conclusion: 该数据集为研究非主流语言中的饮食文化提供了新资源，并提供了关于马其顿烹饪传统的独特模式的见解。
...
TL;DR: This paper proposes RLSR, a new technique that replaces SFT to improve the instruction-following ability of LLMs by using a RL-based approach.
...
Method: RLSR replaces SFT by using a RL-based approach to leverage the extensive SFT dataset, and reward scores are computed as the cosine similarity in the semantic embedding space between the generated and human-labeled responses.

Result: RLSR achieved a higher win rate on instruction-following benchmarks compared to SFT, and combining SFT and RLSR further improved the performance.

Conclusion: RLSR can be used to replace SFT or combined with SFT to improve the performance of instruction-following tasks.
...
TL;DR: 本文提出了一种动态人格优化框架（DPRF），以提高大型语言模型角色扮演代理的行为一致性，并在多个场景中验证了其有效性。
...
Method: 我们引入了动态人格优化框架（DPRF），通过迭代识别生成行为与人类真实情况之间的认知差异，并优化人格档案以减轻这些差异。

Result: DPRF在四个不同的行为预测场景中显著提高了行为一致性，并在不同模型和场景中具有泛化能力。

Conclusion: 我们的工作提供了一种稳健的方法论，用于创建高保真的人格档案，并增强了下游应用（如用户模拟、社会研究和个性化AI）的有效性。
...
TL;DR: LiteStage是一种用于多阶段推理的延迟感知层跳过框架，通过结合阶段性的离线搜索和基于置信度的生成提前退出，提高了效率并减少了冗余输出。
...
TL;DR: MoM框架通过主动理解文档内容，构建三层文档记忆检索机制，提升了RAG系统的性能和小语言模型的文本处理能力。


<details>
  <summary>Details</summary>
Motivation: 传统RAG范式在知识内化和推理能力方面存在局限，因此需要一种更高效的方法来处理多领域文档并训练小语言模型。

Method: MoM框架通过主动理解文档内容，模拟人类阅读过程，采用多路径采样和多角度评估机制，结合反向推理策略，构建了一个三层文档记忆检索机制。

Result: 实验结果表明，MoM框架能够提供语义完整的文档记忆，并有效提升小语言模型的文本处理能力。

Conclusion: MoM框架不仅解决了现有RAG系统中的文本分块挑战，还为小语言模型（SLMs）实现以人类为中心的智能文本处理铺平了道路。
...
TL;DR: 本文提出了一种实验方法，用于研究训练数据如何影响语言模型的行为，并通过案例研究验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 为了研究训练数据与语言模型行为之间的关系，我们需要一种系统的方法来干预数据并测试其对模型行为的影响。

Method: 我们提出了一个实验配方，通过干预数据批次并重新训练模型检查点来测试数据与行为之间的关系。该配方将这种干预分解为几个阶段，包括从基准中选择评估项目、匹配相关文档并修改这些文档以进行重新训练和测量效果。

Result: 我们的结果补充了过去观察性分析，这些分析将共现与模型行为联系起来，同时表明现有的识别相关训练文档的方法并不能完全解释语言模型正确回答知识问题的能力。
...
TL;DR: 本文提出DEG-RAG框架，通过实体解析和三元组反射技术提升LLM生成知识图谱的质量，从而改善基于图的RAG系统的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于图的RAG系统依赖于LLM自动构建知识图谱，但生成的知识图谱通常存在噪声，如冗余实体和不可靠的关系，这影响了检索和生成性能并增加了计算成本。当前研究尚未全面解决LLM生成知识图谱的去噪问题。

Method: DEG-RAG框架包含两个关键技术：(1) 实体解析，用于消除冗余实体；(2) 三元组反射，用于移除错误的关系。此外，还对实体解析进行了系统评估，包括不同的阻塞策略、嵌入选择、相似性度量和实体合并技术。

Result: DEG-RAG框架生成的更紧凑、高质量的知识图谱在多个基于图的RAG变体中表现出显著优于未处理的知识图谱的性能。

Conclusion: DEG-RAG框架通过实体解析和三元组反射技术，显著提高了LLM生成的知识图谱的质量，并在多种基于图的RAG变体中提升了问答性能。
...
TL;DR: 本文研究了多语言嵌入模型的有效性因素，发现任务多样性比语言多样性更重要，并开发了一个紧凑的多语言模型，在检索性能上可与更大的模型相媲美。


<details>
  <summary>Details</summary>
Motivation: 训练有效的多语言嵌入模型面临独特的挑战，因为语言和任务目标的多样性。虽然小型多语言模型在多语言任务上表现良好，但在最普遍的使用案例（即检索）中，它们始终落后于大型模型。这引发了一个关键问题：能否为检索任务专门改造较小的模型以提高其性能？

Method: 我们研究了影响多语言嵌入有效性的关键因素，包括训练数据规模、负采样策略和数据多样性。

Result: 我们发现增加训练数据规模可以带来初始性能提升，但这些改进很快达到平台期，表明收益递减。引入困难的负面样本对于持续提高检索准确性至关重要。此外，我们的分析显示，训练数据中的任务多样性比语言多样性对性能的影响更大。

Conclusion: 我们开发了一个紧凑的（约3亿参数）多语言模型，其检索性能可以与或超过当前强大的70亿参数模型。
...
TL;DR: Qwen3Guard 是一种多语言安全守卫模型，提供细粒度的安全分类和实时监控，适用于全球 LLM 部署。


<details>
  <summary>Details</summary>
Motivation: 现有守卫模型在现实应用中存在两个主要限制：仅输出二进制“安全/不安全”标签，无法适应不同安全政策；需要完整的模型输出才能进行安全检查，与流式 LLM 推理不兼容。

Method: Qwen3Guard 包括两个专门的变体：Generative Qwen3Guard 和 Stream Qwen3Guard。Generative Qwen3Guard 将安全分类作为指令遵循任务，以实现细粒度的三类判断（安全、有争议、不安全）；Stream Qwen3Guard 引入了基于标记级别的分类头，用于实时安全监控。

Result: Qwen3Guard 在英语、中文和多语言基准测试中均取得了最先进的性能，在提示和响应安全分类方面表现出色。

Conclusion: Qwen3Guard 提供了全面、可扩展且低延迟的安全监控，适用于全球的 LLM 部署。
...
TL;DR: 本文介绍了一种代理检索系统，用于多跳问答任务，通过结构化循环提高检索精度和召回率，并在多个基准测试中取得优异成绩。


<details>
  <summary>Details</summary>
Motivation: 多跳问答需要收集多个证据片段，因此检索起着核心作用。

Method: 我们引入了一个代理检索系统，利用大型语言模型（LLMs）在一个结构化的循环中以高精度和召回率检索相关证据。框架包括三个专门的代理：问题分析器、选择器和添加器。

Result: 实验表明，我们的方法在四个多跳QA基准测试中表现优异，能够超越强基线。

Conclusion: 我们的方法在四个多跳QA基准测试中表现出色，优于强基线。
...
TL;DR: 本文提出了一种上下文感知的双向模式检索框架，通过结合表-first和column-first策略以及增强技术，显著提高了模式召回率并减少了假阳性。该方法在多个基准测试中表现出色，提升了Text-to-SQL系统的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: Schema linking是Text-to-SQL系统中的一个关键但研究不足的组成部分。尽管最近的方法主要集中在改进SQL生成上，但它们常常忽略了相关模式元素的检索，这可能导致幻觉和执行失败。

Method: 我们提出了一种上下文感知的双向模式检索框架，将模式链接作为一个独立的问题处理。我们的方法结合了两种互补策略：首先进行表检索，然后进行列选择；以及首先进行列检索，然后进行表选择。此外，还增强了问题分解、关键词提取和关键短语提取等技术。

Result: 我们在BIRD和Spider等具有挑战性的基准测试中进行了全面评估，结果表明我们的方法显著提高了模式召回率，同时减少了假阳性。此外，使用我们检索到的模式生成SQL始终优于全模式基线，并接近于理想性能，而无需查询优化。值得注意的是，我们的方法将全模式和完美模式设置之间的性能差距缩小了50%。
...
TL;DR: 本文提出了一种基于提示工程的关键概念路径分析方法，利用小语言模型实现精确的关键概念提取和创新点识别，并构建了一个基于知识图谱约束机制的智能体以提高分析准确性。通过微调Qwen和DeepSeek模型，实现了显著的准确率提升，并将模型公开在Hugging Face平台上。


<details>
  <summary>Details</summary>
Motivation: 由于学术论文数量的迅速增加，科学家们难以及时全面地跟踪最新的研究成果和方法。关键概念提取已被证明是一种有效的分析范式，但现有的论文数据库大多仅限于关键概念的相似性匹配和基本分类，未能深入探索概念之间的关系网络。因此，本文旨在解决这一问题，提出一种更有效的关键概念路径分析方法。

Method: 本文基于OpenAlex开源知识图谱，分析了来自新西伯利亚国立大学的近8000篇开源论文数据，发现了论文关键概念路径的分布模式与创新点和罕见路径之间的强相关性。提出了一种基于提示工程的关键概念路径分析方法，利用小语言模型进行精确的关键概念提取和创新点识别，并构建了一个基于知识图谱约束机制的智能体以提高分析准确性。

Result: 通过微调Qwen和DeepSeek模型，本文实现了显著的准确率提升，并将模型公开在Hugging Face平台上。

Conclusion: 本文提出了一种基于提示工程的关键概念路径分析方法，利用小语言模型实现精确的关键概念提取和创新点识别，并构建了一个基于知识图谱约束机制的智能体以提高分析准确性。通过微调Qwen和DeepSeek模型，实现了显著的准确率提升，并将模型公开在Hugging Face平台上。
...
TL;DR: 本文介绍了一个多语言数学推理基准测试集MathMist，并发现大型语言模型在跨语言数学推理方面仍有显著不足，特别是在低资源语言中。
...
Method: 引入了一个多语言数学问题解决和推理的平行基准测试集MathMist，并对多种模型进行了系统评估，包括零样本、思维链（CoT）和代码切换推理范式。

Result: 评估结果显示，大型语言模型在跨语言上进行一致且可解释的数学推理能力仍然存在明显缺陷，尤其是在低资源语言中表现更差。

Conclusion: 研究发现，大型语言模型在跨语言数学推理方面仍存在显著不足，特别是在低资源语言环境中表现更差。
...
TL;DR: 本文介绍了一个名为MERLIN的新测试平台系统，用于多语言多模态实体链接任务。数据集包括五种语言的BBC新闻标题及其对应图像，并包含7000多个命名实体提及，这些实体链接到2500个唯一的Wikidata实体。实验结果表明，结合视觉数据可以提高实体链接的准确性，特别是在文本上下文模糊或不足的情况下，以及对于没有强大多语言能力的模型来说尤其如此。


<details>
  <summary>Details</summary>
Motivation: 本文旨在提供一个用于多语言多模态实体链接任务的新测试平台系统，以促进该领域的研究和发展。

Method: 本文创建了一个包含五种语言（印地语、日语、印尼语、越南语和泰米尔语）的BBC新闻标题及其对应图像的数据集，并包含了7000多个命名实体提及，这些实体链接到2500个唯一的Wikidata实体。此外，还使用多语言和多模态实体链接方法进行了几个基准测试，探索了不同的语言模型，如LLaMa-2和Aya-23。

Result: 实验结果表明，结合视觉数据可以提高实体链接的准确性，尤其是在文本上下文模糊或不足的情况下，以及对于没有强大多语言能力的模型来说尤其如此。

Conclusion: 本文介绍了MERLIN，这是一个用于多语言多模态实体链接任务的新测试平台系统。实验结果表明，结合视觉数据可以提高实体链接的准确性，尤其是在文本上下文模糊或不足的情况下，以及对于没有强大多语言能力的模型来说尤其如此。
...
TL;DR: 本文研究了大语言模型在对话中的欺骗行为，并提出了一种新的衡量指标，同时引入了多轮强化学习方法来减少欺骗行为。


<details>
  <summary>Details</summary>
Motivation: 由于大语言模型在对话中可能产生欺骗性输出，这带来了严重的安全问题，因此需要研究其欺骗行为并提出有效的评估和缓解方法。

Method: 本文提出了belief misalignment度量来量化欺骗行为，并引入了多轮强化学习方法来微调大语言模型以减少欺骗行为。

Result: 实验结果表明，新提出的度量与人类判断的相关性比现有度量更高，而且大语言模型在对话中自然表现出约26%的欺骗行为，当被提示欺骗时，欺骗率可提高31%。此外，使用RLHF训练的模型仍然表现出43%的平均欺骗率。

Conclusion: 本文提出了一种新的衡量欺骗行为的指标，并通过多轮强化学习方法对大语言模型进行微调，以减少欺骗行为。
...
TL;DR: 本文提出了一种基于混合词嵌入和微调超参数的阿尔茨海默病早期检测方法，取得了较高的准确率和AUC值，并证明了模型的稳定性。


<details>
  <summary>Details</summary>
Motivation: 早期检测阿尔茨海默病对患者有益，可以减轻症状并降低医疗负担。语言能力的变化是AD的一个重要迹象，可用于早期诊断。

Method: 本文开发了一种基于混合词嵌入和微调超参数的稳健分类方法，使用Doc2Vec和ELMo的词向量生成句子的困惑度得分，并添加语言特征以分析语法和语义。然后将嵌入特征向量输入逻辑回归并微调整个管道中的超参数。

Result: 通过微调机器学习管道的超参数，实现了91%的分类准确率和97%的AUC值，优于现有的最佳NLP模型（准确率为88%）。模型在多次实验中表现出稳定性。

Conclusion: 本文提出的模型在早期检测阿尔茨海默病方面具有准确性和稳定性，可以作为大规模筛查工具以及医生诊断的辅助手段。
...
TL;DR: 研究提出了Beyond One World基准，用于评估大型语言模型在角色扮演任务中的表现。该基准包含两个任务：Canon Events和Moral Dilemmas，并引入了Think-Act Matching度量来评估模型的推理一致性。实验发现，链式思维提示可以提高弱模型的叙述连贯性，但会降低强模型的典型准确性；跨版本泛化仍然是一个主要障碍；模型通常在思考或行动方面表现出色，但很少两者兼备。
...
TL;DR: 本研究提出了一种基于置信度的多模型框架，通过模型协作提升医疗问答性能，无需微调。该框架在多个医学基准测试中表现优异，展示了其在资源有限环境中普及先进医疗AI的潜力。


<details>
  <summary>Details</summary>
Motivation: 高性能的医疗大型语言模型（LLMs）通常需要大量的微调和计算资源，这限制了资源有限的医疗机构的使用。因此，需要一种无需微调即可提升医疗问答性能的方法。

Method: 本研究引入了一种基于置信度的多模型框架，利用模型多样性来增强医疗问答能力，而无需微调。该框架采用两阶段架构：一个置信度检测模块评估主模型的确定性，自适应路由机制将低置信度查询引导至具有互补知识的辅助模型进行协作推理。

Result: 在三个医学基准测试（MedQA、MedMCQA和PubMedQA）中，使用Qwen3-30B-A3B-Instruct、Phi-4 14B和Gemma 2 12B进行评估，结果表明该框架表现出色，尤其在PubMedQA（95.0%）和MedMCQA（78.0%）中表现突出。消融研究表明，置信度感知路由结合多模型协作显著优于单模型方法和均匀推理策略。

Conclusion: 本研究证明了战略性模型协作可以为医疗AI系统提供一种实用且计算效率高的改进路径，对在资源有限的环境中普及先进的医疗AI具有重要意义。
...
TL;DR: 本研究探讨了现代LLMs对字符级扰动的韧性，并提出了一种方法来防止LLMs在特定场景中的滥用。


<details>
  <summary>Details</summary>
Motivation: 研究现代LLMs对频繁且结构化的字符级扰动的韧性，特别是通过在每个输入字符后插入噪声字符。

Method: 我们引入了一种名为\nameshort{}的方法，通过在文本中插入不可见的Unicode控制字符来阻止LLMs在在线考试系统等场景中的滥用。

Result: 尽管有强大的混淆，显著降低了信噪比，许多LLMs仍然保持了显著的性能。

Conclusion: 我们的研究揭示了LLMs在低层次上的鲁棒性，这可能对它们的误用风险和在各种应用中的可靠性产生影响。
...
TL;DR: NWS正在开发一种基于人工智能的自动化翻译工具，以更好地服务非英语人群，提高翻译效率并确保文化相关性。


<details>
  <summary>Details</summary>
Motivation: 为了推进一个气象准备就绪的国家，美国国家气象局（NWS）正在开发一个系统化的翻译计划，以更好地服务6880万不以英语为家的语言人群。

Method: NWS与LILT合作，利用其专利培训过程，使大型语言模型（LLM）适应神经机器翻译（NMT）工具，以处理天气术语和信息。GIS地图用于识别不同NWS区域的语言需求，帮助优先分配资源。还整合了伦理AI实践，确保透明度、公平性和人工监督指导自动化翻译的创建、评估和共享。

Result: 该系统提供了准确、及时且文化相关的翻译，显著减少了手动翻译时间，并减轻了NWS的操作负担。

Conclusion: 该工作已形成一个网站，提供实验性的多语言NWS产品，包括翻译后的警报、7天预报和教育活动，使国家更接近一个能覆盖所有美国人的全国性预警系统。
...
TL;DR: This paper introduces PluriHopWIND, a dataset for pluri-hop questions, and proposes PluriHopRAG, an improved RAG architecture that outperforms existing methods on this challenging task.
...
TL;DR: 本研究通过分析Reddit上的用户发帖历史和评论，构建了一个高质量的注释数据集，并利用大型语言模型验证了评论树数据在提升自杀风险预测准确性方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管之前的研究已经提供了关于检测个体社交媒体帖子中自杀表达的有价值的见解，但很少关注对纵向、序列评论树的分析以预测用户逐渐变化的自杀风险。然而，用户通常会通过历史帖子和随时间推移的互动评论来揭示他们的意图。

Method: 本研究构建了一个高质量的注释数据集，来源于Reddit，结合了用户的发帖历史和评论，并使用基于C-SSRS的改进四标签注释框架。此外，还进行了统计分析和大型语言模型（LLM）实验。

Result: 统计数据集的分析以及大型语言模型（LLM）实验的结果表明，将评论树数据纳入显著提高了用户自杀风险等级的区分和预测。
...
TL;DR: 本文提出了YNTP任务，通过受控的人机对话建模用户的精确词汇选择，并构建了一个多语言基准数据集，评估了基于提示和微调的个性化方法，为用户对齐的语言建模奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 由于隐私问题，真实的社会网络或电子邮件历史难以收集，因此需要一种新的方法来生成反映个体真实交流方式的响应。

Method: 本文通过受控的人机对话建模用户的精确词汇选择，并构建了一个包含英语、日语和中文的多语言基准数据集，其中用户与基于MBTI维度的心理学基础NPC进行五天的互动。

Result: 本文建立了第一个YNTP基准，并评估了基于提示和微调的个性化方法，为用户对齐的语言建模提供了基础。

Conclusion: 本文提出了YNTP任务，通过受控的人机对话建模用户的精确词汇选择，并构建了一个多语言基准数据集，评估了基于提示和微调的个性化方法，为用户对齐的语言建模奠定了基础。
...
TL;DR: 本文提出了一种名为MedTrust-Guided Iterative RAG的框架，以提高生物医学问答中的事实一致性和减少幻觉。该方法通过引用意识推理、迭代检索-验证过程以及集成MedTrust-Align模块来实现这一点。实验结果表明，该方法在多个数据集上优于现有的基线方法。


<details>
  <summary>Details</summary>
Motivation: RAG-based方法在生物医学问答中由于后检索噪声和检索证据的不足验证而遭受幻觉，这影响了响应的可靠性。因此，需要一种方法来提高事实一致性和减少幻觉。

Method: 我们提出了MedTrust-Guided Iterative RAG框架，该框架通过三个关键创新来增强事实一致性并减轻医学问答中的幻觉。首先，它通过要求所有生成内容明确基于检索到的医学文献来实现引用意识推理，并在证据不足时使用结构化的负面知识断言。其次，它采用迭代检索-验证过程，其中验证代理评估证据充分性并通过医学差距分析改进查询，直到获得可靠信息。第三，它集成了MedTrust-Align模块（MTAM），该模块结合了经过验证的正面例子与幻觉感知的负面样本，利用直接偏好优化来加强引用基础推理，同时惩罚容易产生幻觉的响应模式。

Result: 我们的方法在多个模型架构上优于竞争性基线，在MedMCQA、MedQA和MMLU-Med数据集上实现了最佳平均准确率，对于LLaMA3.1-8B-Instruct和Qwen3-8B分别提高了2.7%和2.4%。

Conclusion: 我们的方法在MedMCQA、MedQA和MMLU-Med数据集上的实验表明，它在多种模型架构上 consistently 超过竞争性基线，实现了最佳平均准确率，对于LLaMA3.1-8B-Instruct和Qwen3-8B分别提高了2.7%和2.4%。
...
TL;DR: 本文提出了一种无标签的自监督强化学习框架，解决了语言模型在多约束指令上的不足，并在多个数据集上取得了显著的改进。


<details>
  <summary>Details</summary>
Motivation: 语言模型在遵循多约束指令方面存在困难，而现有的强化学习方法依赖于外部监督和稀疏奖励信号，这限制了其在现实世界应用中的表现。

Method: 我们提出了一种无标签的自监督强化学习框架，通过直接从指令中获取奖励信号并生成伪标签用于奖励模型训练，消除了对外部监督的依赖。此外，我们引入了约束分解策略和高效的约束级二分类方法来解决稀疏奖励问题，同时保持计算效率。

Result: 实验表明，我们的方法具有良好的泛化能力，在多个数据集上取得了显著的改进，包括具有挑战性的代理和多轮指令遵循任务。

Conclusion: 我们的方法在3个领域内和5个领域外的数据集上表现出色，取得了显著的改进。数据和代码已公开。
...
TL;DR: 本文提出了一种从探索到演化的范式，用于构建可验证的训练数据，以增强深度研究代理的信息聚合能力。研究展示了WebAggregator模型在性能上的优势，并指出了当前模型在信息聚合任务上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的开源深度研究代理主要关注提高信息检索能力，而忽视了信息聚合的重要性，这限制了它们支持深入研究的能力。因此，需要一种方法来构建可验证的训练数据，以增强信息聚合能力。

Method: 提出了一种从探索到演化的范式，通过主动在线探索获取信息，并通过选择、组合和优化操作生成可验证的QA对。此外，基于SmolAgents框架收集了监督微调轨迹以开发WebAggregator基础模型。

Result: 成功构建了WebAggregatorQA数据集，包含10K样本，覆盖50K网站和11个领域。WebAggregator-8B模型性能与GPT-4.1相当，而32B版本在GAIA-text上超越GPT-4.1超过10%，接近Claude-3.7-sonnet。此外，构建了一个人工标注的评估集，显示现有模型在信息聚合任务上的表现较差。

Conclusion: 研究强调了信息聚合在深度研究代理中的重要性，并展示了WebAggregator模型在性能上的优势。同时，研究指出当前模型在信息聚合能力上仍有提升空间，需要进一步加强基础模型的能力。
...
TL;DR: NLT is a framework that enhances tool calling in large language models by using natural language outputs instead of programmatic JSON calls, leading to improved accuracy and reduced variance.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the limitations of programmatic JSON tool calling in large language models, such as task interference and format constraints, which degrade tool call performance.

Method: NLT replaces programmatic JSON tool calling with natural language outputs, decoupling tool selection from response generation to eliminate task interference and format constraints.

Result: NLT improves tool calling accuracy by 18.4 percentage points and reduces output variance by 70% across 10 models and 6,400 trials in customer service and mental health domains. Open-weight models show the largest gains, surpassing closed-weight alternatives.

Conclusion: NLT improves tool calling accuracy and reduces output variance, with significant benefits for open-weight models and applications in various domains.
...
TL;DR: LiRA 是一种用于提升低资源语言跨语言表示的训练框架，包含 Arca 和 LaSR 两个模块，能够在少样本和噪声环境下提高检索和推理能力。


<details>
  <summary>Details</summary>
Motivation: 由于低资源语言的训练数据有限、机器翻译噪声和不稳定的跨语言对齐，其性能远低于高资源语言。因此，需要一种能够提高低资源语言跨语言表示的方法。

Method: LiRA 包含两个模块：Arca（基于锚点的表示组合架构）和 LaSR（语言耦合语义推理器）。Arca 通过基于锚点的对齐和多智能体协作编码，将低资源语言锚定到英语语义空间。LaSR 在 Arca 的多语言表示上添加了一个语言感知的轻量级推理头，并进行一致性正则化，以统一训练目标，提高跨语言理解、检索和推理的鲁棒性。

Result: 在低资源基准测试（跨语言检索、语义相似性和推理）中，LiRA 在少样本和噪声放大设置下表现出一致的增益和鲁棒性。消融实验验证了 Arca 和 LaSR 的贡献。

Conclusion: LiRA 是一种有效的训练框架，能够在低资源条件下显著提升跨语言表示，并同时增强检索和推理能力。
...
TL;DR: 本文提出了一种压缩表示方法，以提高seq2seq共指解析器在增量设置中的效率。结果表明，丢弃大量标记是一种可行的策略。


<details>
  <summary>Details</summary>
Motivation: Seq2seq共指模型引入了一种新的共指解析范式，但它们在处理增量设置（如对话）时效率不高。我们需要一种更高效的解决方案。

Method: 我们提出了一种压缩表示方法，以提高这些方法在增量设置中的效率。我们的方法通过提取和重新组织实体级标记，并丢弃大部分其他输入标记来工作。

Result: 在OntoNotes上，我们的最佳模型仅比全前缀、增量基线低0.6 CoNLL F1点，同时实现了1.8的压缩比率。在LitBank上，它达到了最先进的性能。

Conclusion: 我们的结果表明，在seq2seq解析器中丢弃大量标记是一种可行的增量共指解析策略。
...
TL;DR: 本文探讨了主权大语言模型在支持低资源语言方面的角色，指出它们可能未能充分服务于目标用户，并强调需要更全面的评估标准。


<details>
  <summary>Details</summary>
Motivation: 全球对主权大语言模型的辩论突显了政府开发符合其独特社会文化历史背景的大语言模型的必要性。然而，仍然缺乏框架和数据集来验证两个关键问题：(1) 这些模型如何与用户的社会文化背景相匹配，以及(2) 它们在不使用户暴露于潜在危害和风险的情况下是否保持安全和技术鲁棒性。

Method: 我们构建了一个新的数据集，并引入了一种分析框架，用于提取和评估主权大语言模型的社会文化元素，以及对其技术鲁棒性的评估。

Result: 我们的实验结果表明，虽然主权大语言模型在支持低资源语言方面发挥了重要作用，但它们并不总是满足这些模型能很好地服务于目标用户的流行说法。我们还表明，追求这一未经验证的说法可能导致低估安全等关键质量属性。

Conclusion: 我们的研究建议，推进主权大语言模型需要更广泛的评估，包括更广泛和实际的标准。
...
TL;DR: 研究分析了当前偏好学习方法在移除目标质量信号时的性能下降问题，并引入了一个新的数据集来评估不同方法的表现，发现生成式奖励模型表现更好，但不同模型在不同写作类别中存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 当前偏好学习方法在标准基准上取得高精度，但在移除目标质量信号时表现出显著性能下降。需要一种更有效的偏好建模方法，以捕捉主观质量偏好。

Method: 引入了WritingPreferenceBench数据集，包含1,800个人工标注的偏好对（1,200个英文，600个中文）跨8种创意写作类型，响应在客观正确性、事实准确性和长度上匹配。比较了基于序列的奖励模型、零样本语言模型法官和生成式奖励模型的表现。

Result: 基于序列的奖励模型在该基准上的平均准确率为52.7%，而零样本语言模型法官的准确率为53.9%。相比之下，产生显式推理链的生成式奖励模型达到81.8%的准确率。不同模型在不同写作类别中的准确率范围从18.2%到81.8%，标准差平均为10.1%。27B参数模型并未显示出比8B变体的一致改进。
...
TL;DR: 本文介绍了CodeSeq，一个从数字序列构建的合成预训练数据集，用于解决归纳推理中的挑战。通过生成监督微调数据和利用强化学习，模型在各种推理任务中表现更好，并保持OOD性能。


<details>
  <summary>Details</summary>
Motivation: 现有的归纳数据主要关注表面规律，而缺乏更复杂的内部模式。当前的工作仅提示LLM或在简单的提示-响应对上进行微调，但没有提供精确的思考过程或实施难度控制。

Method: 我们引入了CodeSeq，这是一个从数字序列构建的合成预训练数据集。我们将数字序列打包成算法问题以发现其通项，并相应地定义了一个通项生成（GTG）任务。我们的管道通过反思失败的测试用例并结合迭代修正来生成监督微调数据，从而教会LLM学习自主案例生成和自我检查。此外，它利用了基于可解性（从问题通过率估计）和自导向案例生成成功率的新型Case-Synergy Solvability Scaling Reward进行强化学习，使模型能够更有效地从成功和失败中学习。

Result: 使用CodeSeq训练的模型在各种推理任务中表现更好，并且可以保持模型的OOD性能。

Conclusion: 实验结果表明，使用CodeSeq训练的模型在各种推理任务中表现更好，并且可以保持模型的OOD性能。
...
TL;DR: 本文提出RLAIF-SPA框架，利用强化学习和AI反馈技术提升语音合成的情感表达和可理解性，实验结果优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖于昂贵的情感注释或优化间接目标，无法捕捉语音的情感表达和感知自然度，导致生成的语音准确但缺乏情感。

Method: 提出了一种基于AI反馈的强化学习（RLAIF）机制，结合自动语音识别（ASR）和大型语言模型（LLM）技术，分别判断语义准确性和韵律-情感标签对齐，作为情感表达和可理解性优化的直接奖励。

Result: RLAIF-SPA框架通过结合语义准确性和韵律-情感对齐，在四个细粒度维度上提升了语音的表达质量，并在实验中表现出优越性能。
...
TL;DR: 本文提出了一种直观、无需训练且无标签的意图聚类方法，使用轻量级和开源的LLM，通过生成伪标签并进行多标签分类来实现。


<details>
  <summary>Details</summary>
Motivation: 许多当前的方法依赖于商业LLM，这成本高昂且透明度有限，而且它们的方法通常需要预先知道聚类的数量，这在现实场景中并不总是可行的。

Method: 我们首先让LLM为每段文本生成伪标签，然后在这些伪标签集中对每段文本进行多标签分类。

Result: 我们在四个基准集上的评估表明，我们的方法结果与最近的基线相当甚至更好，同时保持简单和计算效率。

Conclusion: 我们的方法可以在低资源场景中应用，并且在多个模型和数据集上表现出稳定性。
...
TL;DR: This paper proposes a new paradigm called 'nugget-as-rubric' for evaluating search-augmented LLMs. It introduces a method for automatically constructing rubrics and a generative verifier named Search-Gen-V that demonstrates strong performance in verifying rewards for various tasks.


<details>
  <summary>Details</summary>
Motivation: Existing reward modeling for search-augmented LLMs faces several limitations. Rule-based rewards are fragile to variations in expression and cannot be applied to long-form workloads. Generative rewards are robust but designing verifiable and stable rewards for long-form workloads in dynamic corpora remains challenging and incurs high computational costs.

Method: We propose a unified and verifiable paradigm, 'nugget-as-rubric', which treats atomic information points as structured evaluation criteria for different search-augmentation workloads. To support long-form settings, we design an automatic rubric construction pipeline based on query rewriting. Furthermore, we introduce Search-Gen-V, a 4B-parameter efficient generative verifier under our proposed verifiable paradigm, which is trained via the idea of distillation and a two-stage strategy.
...
TL;DR: 本文提出了一种方法，通过创建一个英语-中文句子对的数据集来教机器翻译模型关于中文BEI被动语态的语义语气。结果表明，微调的MT模型在翻译不利内容时表现更好，并且这种知识可以转移到其他语言对。


<details>
  <summary>Details</summary>
Motivation: 由于词义翻译可能具有不同的语义语气，因此应更加关注这一语言特性以生成准确的翻译。然而，当前的机器翻译模型无法处理这个问题。为了弥合这一差距，我们提出了一种方法，教机器翻译模型关于特定结构的语义语气。

Method: 我们专注于中文BEI被动语态，创建了一个英语-中文句子对的数据集，目的是展示BEI被动语态的负面语义语气。然后我们用我们的数据集对OPUS-MT、NLLB-600M和mBART50模型进行微调，以进行英语-中文翻译任务。

Result: 微调的MT模型在使用BEI被动语态翻译不利内容时表现更好，并避免在中性和有利内容中使用它。此外，在NLLB-600M（一个多语言模型）中，这种语义语气的知识可以从英语-中文翻译转移到其他语言对，如西班牙语-中文。

Conclusion: 我们的结果表明，微调的MT模型在使用BEI被动语态翻译不利内容时表现更好，并避免在中性和有利内容中使用它。此外，在NLLB-600M（一个多语言模型）中，这种语义语气的知识可以从英语-中文翻译转移到其他语言对，如西班牙语-中文。
...
TL;DR: 本文提出了一种以人为中心的框架，通过生成用户故事和支持多代理讨论来帮助人们在部署前思考人工智能的潜在好处和危害。研究表明，讲故事有助于参与者推测更广泛的影响和好处，并更有创意地思考人工智能对用户的影响。


<details>
  <summary>Details</summary>
Motivation: 快速且低门槛的开发可能会引入偏见、隐私侵犯和不平等访问的风险，特别是当系统忽略现实世界的情境和多样化的用户需求时。许多最近的方法使用人工智能自动检测风险，但这会减少人类在理解危害如何产生以及影响谁方面的参与度。

Method: 我们提出了一个以人类为中心的框架，生成用户故事并支持多代理讨论，以帮助人们在部署前创造性地思考潜在的好处和危害。

Result: 在用户研究中，阅读故事的参与者识别出更广泛的危害，他们的回答在所有13种危害类型中分布更均匀。相比之下，没有阅读故事的参与者主要关注隐私和福祉（58.3%）。

Conclusion: 我们的研究结果表明，讲故事有助于参与者推测更广泛的影响和好处，并更有创意地思考人工智能对用户的影响。
...
TL;DR: AutoRubric-R1V是一种结合了RLVR和过程级监督的框架，通过自动收集的评分标准生成奖励，提高了多模态推理的性能和可信度。


<details>
  <summary>Details</summary>
Motivation: 现有的RLVR方法由于只奖励最终答案的正确性，可能导致虚假推理。

Method: 提出了一种将RLVR与过程级监督相结合的框架AutoRubric-R1V，通过自动收集的评分标准生成奖励来实现。

Result: AutoRubric-R1V在六个多模态推理基准上取得了最先进的性能，并在专门的评估中显著提高了推理的可信度。

Conclusion: AutoRubric-R1V通过结合基于评分标准的奖励和结果奖励，实现了在六个多模态推理基准上的最先进性能，并显著提高了推理的可信度。
...
TL;DR: 本文提出了Pluto基准和评估框架，用于评估LLM生成的Verilog设计的效率。实验结果表明，尽管LLM在功能正确性方面表现良好，但其合成效率仍落后于专家编写的实现。


<details>
  <summary>Details</summary>
Motivation: 现有的基准在评估硬件设计的合成指标（如面积、延迟和功耗）方面存在不足，缺乏优化的基线或测试平台。因此，本文提出Pluto基准和评估框架，以弥补这些差距。

Method: 本文介绍了Pluto基准和评估框架，用于评估LLM生成的Verilog设计的效率。Pluto包含114个问题的综合评估集，以及自检测试平台和多个Pareto最优参考实现。

Result: 实验结果表明，最先进的LLM在功能正确性方面达到78.3%的pass@1，但在合成效率方面，面积效率为63.8%，延迟效率为65.9%，功耗效率为64.0%。

Conclusion: 本文提出了Pluto基准和评估框架，以评估LLM生成的Verilog设计的效率。实验结果表明，最先进的LLM在功能正确性方面表现良好，但在合成效率方面仍落后于专家编写的实现，这突显了需要效率感知的评估框架来推动面向硬件的LLM研究。
...
TL;DR: 本文介绍了COIG-Writer数据集，用于研究中文创意写作，并揭示了创意写作的两个核心组成部分：叙述逻辑和语言表达。研究发现，过程监督对创意写作有显著影响，但需要与通用数据结合。此外，创意能力具有文化局限性，无法跨语言迁移，且词汇多样性与创意质量呈负相关。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在创意写作方面存在系统性缺陷，特别是在训练数据稀缺且缺乏过程级监督的非英语环境中。现有的数据集只提供输入-输出对，而没有涵盖创意写作的思考过程。

Method: 我们通过系统逆向工程高质量文本，创建了COIG-Writer数据集，该数据集包含1,665个精心策划的三元组，涵盖了51种类型，每个三元组包含：(1) 逆向工程的提示，(2) 记录决策过程的详细创意推理，以及(3) 最终文本。

Result: 我们的研究发现，创意写作由两个组件组成：叙述逻辑（由过程监督提供）和语言表达（由通用数据维持）。此外，我们发现了三个关键见解：(1) 过程监督非常有效，但需要与通用数据结合以稳定性能；(2) 创意能力具有文化局限性，无法跨语言迁移；(3) 词汇多样性与创意质量呈负相关，这表明高多样性可能表示逻辑缺陷的补偿行为。

Conclusion: 我们的研究结果表明，创造性卓越来自于逻辑结构和语言基础之间的相互作用，这类似于数学推理如何增强但不能替代基础模型中的语言能力。
...
TL;DR: 本文提出了一种名为答案再生的框架，通过额外的模型推理来提高模型评估的可靠性。


<details>
  <summary>Details</summary>
Motivation: 评估需要推理的模型时，答案提取方法起着关键作用。我们的研究揭示了推理模型的性能及其最终答案分布对所采用的答案提取算法高度敏感。

Method: 我们提出了一个基本框架：答案再生。该方法使用额外的模型推理，提供带有前缀“Answer:”的输入和输出。然后从重新生成的输出中选择或提取最终答案。

Result: 这种与提取规则无关的方法表现出更好的性能和增强的鲁棒性。此外，我们将此框架应用于一般的数学问题和开放性问答任务。
...
TL;DR: This paper compares contrastive learning (CL) and supervised fine-tuning (SFT) for LLM-based reranking, finding that SFT provides a consistent advantage over CL, particularly in terms of weighting schemes.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the central question of which objective is intrinsically better suited to LLM-based reranking and what mechanism underlies the difference between contrastive learning (CL) and supervised fine-tuning (SFT).

Method: We conduct a comprehensive comparison and analysis between contrastive learning (CL) and supervised fine-tuning (SFT) for reranking, taking the universal multimodal retrieval (UMR) as the experimental playground. We decompose the objectives into weight and direction components and present a unified framework for understanding their interactions.

Result: SFT provides a substantially stronger weighting scheme than CL, while the preferred scoring direction shows no clear winner. SFT demonstrates a consistent advantage over CL for LLM reranking, and we present new state-of-the-art rerankers on the MRB benchmark.
...
TL;DR: 本文提出了一种无需外部数据的在线测试时框架，通过自监督优化MoE专家选择，提高了模型在推理任务中的性能，并保持了计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时适应方法主要针对密集模型，并需要外部数据，这限制了其在MoE架构中的实际应用。本文旨在解决这一问题，提出一种无需外部数据的在线测试时框架。

Method: 本文提出了一种基于输入上下文的在线测试时框架，通过自监督优化MoE专家选择，使用轻量级附加向量仅在选定层更新路由器logits，从而保持计算效率。

Result: 实验结果表明，该方法在HumanEval上取得了5.5%的提升，并且在DeepSeek-V2-Lite上与自一致性结合时平均提升了6%。

Conclusion: 本文提出了一种无需外部数据的在线测试时框架，能够持续调整MoE路由决策，同时保持计算效率并防止过度适应。实验结果表明，在具有挑战性的推理任务中表现有显著提升，并且对上下文变化具有鲁棒性。
...
TL;DR: 本研究首次系统地调查了中段训练，发现它在数学和代码领域最有效，能减少预训练和后训练数据之间的句法差距，并优于持续预训练。


<details>
  <summary>Details</summary>
Motivation: 尽管中段训练在实践中很受欢迎，但对其科学理解不足，因此需要进行系统的研究以了解其有效性。

Method: 通过控制实验对语言模型进行从头预训练，并在不同领域的监督微调数据集上进行微调，以系统地研究中段训练。

Result: 中段训练在数学和代码领域最有效，能够最好地减少预训练和后训练数据之间的句法差距。在这些情况下，中段训练在领域内验证损失和后训练后的预训练数据遗忘方面均优于持续预训练。

Conclusion: 中段训练作为一种领域适应技术，与持续预训练相比，通过减少遗忘实现了更好的性能。
...
TL;DR: MLIR-AIR is a novel compiler stack that bridges the gap between high-level workloads and spatial architectures, enabling efficient utilization of compute and memory resources through structured representations and compiler-managed scheduling.


<details>
  <summary>Details</summary>
Motivation: Modern computing architectures increasingly rely on fine-grained control over data movement, execution order, and compute placement for performance, requiring compiler infrastructure to provide explicit mechanisms for orchestrating compute and data to fully exploit such architectures.

Method: MLIR-AIR introduces a novel, open-source compiler stack built on MLIR that bridges the semantic gap between high-level workloads and fine-grained spatial architectures. It defines the AIR dialect, which provides structured representations for asynchronous and hierarchical operations across compute and memory resources.

Result: MLIR-AIR achieves up to 78.7% compute efficiency for matrix multiplication and generates implementations with performance almost identical to state-of-the-art, hand-optimized matrix multiplication written using the lower-level, close-to-metal MLIR-AIE framework. For multi-head attention, the AIR interface supports fused implementations using approximately 150 lines of code, enabling tractable expression of complex workloads with efficient mapping to spatial hardware.
...
TL;DR: 本文提出了一种新方法，结合合成数据生成、三元组损失和逐层模型合并，以提高RAG系统的输出一致性，实验结果表明该方法有效提升了响应的一致性。


<details>
  <summary>Details</summary>
Motivation: RAG系统利用大型语言模型（LLMs）生成准确且可靠的回答，这些回答基于检索到的上下文。然而，LLMs对于语义等价输入往往生成不一致的输出，这一问题由于缺乏专注于一致性训练的数据以及当前微调技术在增强输出一致性方面的局限性而加剧。

Method: 我们提出了一种新方法，结合系统合成数据生成、三元组损失以获得更好的嵌入，并采用了一种新颖的逐层模型合并方法。使用从中间层激活中得出的一致性感知权重，我们的方法有效地整合了来自专业模型的知识。

Result: 实验结果表明，我们的合并模型显著提高了输出的一致性，实现比基线模型约47.5%的响应相似度提升。
...
TL;DR: 本文提出了一种新的框架，用于建模下游性能与训练计算和上下文的关系，并通过实验验证了其有效性。
...
Method: 我们提出了一种简单的、可解释的框架，联合建模下游性能作为训练计算和提供上下文的函数。

Result: 我们的框架准确地模拟了分布内的下游性能，并在三个数量级的训练计算中进行了泛化，并且随着上下文的增加可靠地外推性能。
...
TL;DR: 本研究评估了基于机器学习的精神健康筛查工具的有效性，结果显示这些模型在诊断准确性方面表现出色，尤其是在PTSD方面。研究还发现，使用聚焦的上下文片段可以提高检测灵敏度，并且LoRA微调方法在性能和效率方面表现良好。这些结果表明，基于大语言模型的工具可以为低资源或高污名环境中的精神健康诊断提供一种低成本、高效的解决方案。


<details>
  <summary>Details</summary>
Motivation: 精神健康障碍是全球残疾的主要原因之一，但由于主观评估、有限的临床资源以及污名和低意识，抑郁症、焦虑症和创伤后应激障碍（PTSD）经常被低估或误诊。在初级保健环境中，研究表明，提供者在超过60%的情况下未能识别抑郁症或焦虑症，这凸显了对可扩展、可访问和上下文感知诊断工具的迫切需求，以支持早期检测和干预。

Method: 本研究评估了机器学习模型在心理健康筛查中的有效性，使用了一个包含553个真实世界半结构化访谈的独特数据集，并与主要抑郁发作（MDE）、焦虑障碍和创伤后应激障碍（PTSD）的地面真实诊断进行了配对。我们对多种模型类别进行了基准测试，包括GPT-4.1 Mini和MetaLLaMA的零样本提示，以及使用低秩适应（LoRA）微调的RoBERTa模型。

Result: 我们的模型在诊断类别上的准确率超过80%，特别是在PTSD方面（最高准确率为89%，召回率为98%）。我们还发现，使用较短的上下文和聚焦的上下文片段可以提高召回率，这表明聚焦的叙述线索可以增强检测灵敏度。LoRA微调既高效又有效，较低秩配置（例如秩8和16）在评估指标上保持了竞争力。

Conclusion: 本研究表明，基于大语言模型（LLM）的诊断工具可以在精神健康筛查中提供显著改进，为低资源或高污名环境中提供低成本、AI驱动的早期诊断铺平了道路。
...
TL;DR: 本文提出了一种名为LaSeR的算法，通过在原始RLVR损失中增加一个MSE损失来对齐最后令牌的自我奖励分数与验证器基于的推理奖励，从而联合优化LLMs的推理和自我奖励能力。实验表明，该方法不仅提高了模型的推理性能，还增强了其自我奖励能力，从而提升了推理时的扩展性能。


<details>
  <summary>Details</summary>
Motivation: 现有的RLVR方法在测试时缺乏验证信号，因此需要在标准RLVR过程中训练模型的自我验证能力，但这导致效率低下。本文旨在解决这一问题，提高效率并增强模型的自我奖励能力。

Method: 本文提出了LaSeR算法，该算法通过在原始RLVR损失中增加一个MSE损失来对齐最后令牌的自我奖励分数与验证器基于的推理奖励，从而联合优化LLMs的推理和自我奖励能力。

Result: 实验结果表明，本文提出的LaSeR算法不仅提高了模型的推理性能，还增强了其自我奖励能力，从而提升了推理时的扩展性能。

Conclusion: 本文提出了一种名为LaSeR的算法，通过在原始RLVR损失中增加一个MSE损失来对齐最后令牌的自我奖励分数与验证器基于的推理奖励，从而联合优化LLMs的推理和自我奖励能力。实验表明，该方法不仅提高了模型的推理性能，还增强了其自我奖励能力，从而提升了推理时的扩展性能。
...
TL;DR: MetaBench是首个用于评估大语言模型在代谢组学领域能力的基准测试，揭示了模型在不同任务上的表现差异，并为开发可靠的计算工具提供了基础设施。


<details>
  <summary>Details</summary>
Motivation: 为了系统地评估大语言模型在需要深入、相互关联知识的科学领域中的能力，特别是代谢组学所面临的挑战。

Method: 引入了MetaBench，这是第一个用于代谢组学评估的基准测试，从权威公共资源中整理而成。

Result: 对25个开源和闭源大语言模型的评估显示，它们在文本生成任务上表现良好，但在跨数据库标识符定位方面仍然具有挑战性，尤其是在长尾代谢物上表现较差。
...
TL;DR: 本文研究了多模态生成模型在方言文本输入下的表现，并提出了一种有效的缓解策略，以提高模型对方言的支持。


<details>
  <summary>Details</summary>
Motivation: 研究多模态生成模型在使用方言文本输入时的表现，以及如何改进它们对方言的支持。

Method: 本文设计了一种基于编码器的缓解策略，使模型能够识别新的方言特征，同时保持SAE性能。

Result: 实验表明，该方法能够在保持SAE性能几乎不变的情况下，将五种方言的性能提升至与SAE相当的水平。

Conclusion: 本文提出了一种通用的基于编码器的缓解策略，能够在不损害标准美国英语（SAE）性能的情况下，提高多模态生成模型在五种方言上的性能。
...
TL;DR: 本文提出了一种基于信息增益的策略优化框架（IGPO），通过提供密集且内在的监督来改善多轮代理训练。IGPO将每个交互回合建模为获取关于真实信息的增量过程，并将回合级奖励定义为政策产生正确答案的概率的边际增加。实验表明，IGPO在多轮场景中优于强基线，实现了更高的准确性和改进的样本效率。


<details>
  <summary>Details</summary>
Motivation: Existing approaches typically rely on outcome-based rewards that are only provided at the final answer, leading to reward sparsity in multi-turn settings, which exacerbates advantage collapse and lack of fine-grained credit assignment.
...
TL;DR: 本文介绍了UI-Simulator和UI-Simulator-Grow，用于大规模生成高质量的UI轨迹以训练数字代理，并展示了其在性能上的优势。


<details>
  <summary>Details</summary>
Motivation: 数字代理需要多样且大规模的UI轨迹来泛化到现实世界任务，但收集这样的数据在人力标注、基础设施和工程方面都非常昂贵。

Method: UI-Simulator通过整合数字世界模拟器、引导滚动过程和轨迹包装器来生成结构化的UI状态和转换，以大规模合成训练轨迹。UI-Simulator-Grow是一种针对性的扩展策略，优先考虑高影响任务并合成信息丰富的轨迹变体。

Result: 在WebArena和AndroidWorld上的实验表明，UI-Simulator在使用较弱的教师模型的情况下，其鲁棒性显著优于基于真实UI训练的开源代理。此外，UI-Simulator-Grow仅使用Llama-3-8B-Instruct作为基础模型就能达到Llama-3-70B-Instruct的性能。

Conclusion: UI-Simulator-Grow展示了有针对性的合成扩展范式的潜力，可以持续且高效地增强数字代理。
...
TL;DR: 研究发现，代码大语言模型中的分词方式可能导致模型行为的变化，这表明需要语法感知的分词方法。


<details>
  <summary>Details</summary>
Motivation: 由于子词分词器基于统计而非语法，语义相同的代码片段可能因空白或标识符命名等表面因素而被不同地分词。为了衡量这种不一致的影响，我们进行了研究。

Method: 我们引入了TokDrift框架，该框架应用语义保持的重写规则来创建仅在分词上不同的代码变体。

Result: 在九个代码大语言模型中，甚至微小的格式更改也可能导致模型行为的重大变化。层分析显示，问题源于早期嵌入，其中子词分割未能捕捉语法标记边界。

Conclusion: 我们的发现表明，不一致的分词是可靠代码理解和生成的隐藏障碍，强调了未来代码大语言模型需要语法感知的分词。
...
TL;DR: Elastic-Cache是一种自适应的缓存更新策略，能够减少扩散大语言模型中的冗余计算，提高解码速度并保持生成质量。
...
Method: Elastic-Cache是一种无需训练、与架构无关的策略，通过关注最常被注意的标记的注意力感知漂移测试来决定何时刷新，并通过深度感知调度来决定何处刷新。

Result: 实验表明，在数学推理和代码生成任务上，Elastic-Cache实现了显著的速度提升，同时保持了比基线更高的准确性。

Conclusion: Elastic-Cache能够在保持生成质量的同时显著提高扩散大语言模型的吞吐量，使其更适合实际部署。
...
TL;DR: 本文探讨了将生成式人工智能（GenAI）整合到专业遗产实践中以提高公众指导文件可访问性的潜力，并开发了一个名为HAZEL的GenAI聊天机器人。通过定量评估，发现HAZEL的表现略优于ChatGPT，但存在文化敏感性和技术专业知识方面的限制。尽管GenAI不能取代人类遗产专业人士，但它在自动化和加快某些指导写作方面的潜力可能对资源有限的遗产组织有益。
...
Method: 开发了一个名为HAZEL的GenAI聊天机器人，该机器人经过微调以协助修订与遗产保护和解释相关的书面指导。通过定量评估，将HAZEL的表现与ChatGPT（GPT-4）进行比较，以完成一系列与指导写作过程相关的任务。

Result: HAZEL的表现略优于ChatGPT，表明在底层大型语言模型（LLM）经过微调后，GenAI聊天机器人更为有效。然而，也注意到一些显著的局限性，特别是在需要文化敏感性和更高级的技术专业知识的领域。

Conclusion: 尽管GenAI不能取代人类遗产专业人士在技术写作任务中的作用，但其在自动化和加快某些指导写作方面的潜力可以为资源有限的遗产组织带来有价值的益处。
...
TL;DR: 本文提出了一种通过观察用户行为推断即时目标的方法，并利用该目标优化大型语言模型的输出，从而生成更符合用户需求的工具和响应。


<details>
  <summary>Details</summary>
Motivation: 当没有明确的目标时，大型语言模型会产生平淡无奇的结果。因此，需要一种方法来推断用户的即时目标并优化以满足该目标。

Method: 通过被动观察用户行为来推断用户的即时目标，然后通过生成和评估来引导下游AI系统。

Result: 在实验中，JIT目标使LLM输出在66-86%的范围内优于传统LLM，并且在面对面的使用会话中确认了JIT目标可以产生独特的工具。

Conclusion: 通过实验和实际使用会话验证了JIT目标的有效性，证明它可以生成针对每个参与者的独特工具，并且在66-86%的范围内优于传统的LLM输出。
...
### [127] [E2Edev: Benchmarking Large Language Models in End-to-End Software Development Task](https://arxiv.org/abs/2510.14509)
*Jingyao Liu,Chen Huang,Zhizhao Guan,Wenqiang Lei,Yang Deng*

Main category: cs.SE

TL;DR: E2EDev是一个包含细粒度用户需求、多个BDD测试场景以及基于Behave框架的自动化测试管道的基准。通过评估各种E2ESD框架和LLM后端，发现需要更有效和成本效益的E2ESD解决方案。


<details>
  <summary>Details</summary>
Motivation: 为了减少注释工作量并确保E2EDev的质量，提出了HITL-MAA框架。

Method: E2EDev利用了我们提出的HITL-MAA框架，以确保其质量并减少注释工作量。

Result: 通过评估各种E2ESD框架和LLM后端与E2EDev，我们的分析揭示了在有效解决这些任务方面持续存在的困难。

Conclusion: 我们的分析揭示了在有效解决这些任务方面持续存在的困难，强调了需要更有效和成本效益的E2ESD解决方案。
...
TL;DR: 本文通过引入多模态信息（如演示文稿幻灯片）来提升自动语音识别系统的性能，特别是在科学演示场景中取得了显著效果。


<details>
  <summary>Details</summary>
Motivation: 现有的自动语音识别系统主要依赖声学信息，而忽略了其他多模态上下文。然而，在科学演示等场景中，视觉信息对于消除歧义和适应环境至关重要。

Method: 本文创建了一个多模态演示基准，并探索了将多模态信息融入语音模型的方法，同时通过数据增强解决了缺乏配套幻灯片数据集的问题。

Result: 通过使用增强的数据集训练模型，相对于基线模型，所有单词的词错误率降低了约34%，领域特定术语的词错误率降低了约35%。

Conclusion: 本文通过引入多模态信息，特别是演示文稿幻灯片，显著降低了语音识别的词错误率，展示了多模态方法在科学演示场景中的有效性。
...
TL;DR: 本文提出了一种基于多目标MDP和社交选择理论的共识声明生成方法，实现了更公平的文本生成。


<details>
  <summary>Details</summary>
Motivation: 当前基于大型语言模型的共识声明生成框架缺乏内在结构，无法在聚合多样化自由形式意见时提供可证明的公平性保证。因此，需要一种更公平、结构化的生成方法。

Method: 本文将任务建模为多目标、令牌级MDP，每个目标对应一个代理的偏好。通过利用策略隐含定义的最优Q函数，无需价值函数即可量化每一步生成的奖励。提出了两种基于社会选择理论的方法：一种是保证在事前核心中的随机生成策略，另一种是在MDP框架内使用搜索算法最大化平等福利。

Result: 实验表明，基于平等目标的搜索生成的共识声明在最坏情况下的代理对齐度优于基线方法，包括Habermas Machine。

Conclusion: 本文提出了一种基于多目标、令牌级马尔可夫决策过程（MDP）的共识声明生成框架，通过社会选择理论的方法，实现了更公平的文本生成。
...
TL;DR: 本文提出了Terrarium框架，用于在基于LLM的多智能体系统中细致研究安全、隐私和安全性。


<details>
  <summary>Details</summary>
Motivation: LLM-based MAS设计引入了新的风险，包括对齐问题和恶意方的攻击，这些攻击会损害代理或窃取用户数据。

Method: 我们重新利用了黑板设计，这是一种早期的多智能体系统方法，以创建一个模块化、可配置的多智能体协作测试平台。

Result: 我们实现了三个协作MAS场景，并进行了四种代表性的攻击，以展示框架的灵活性。
...
TL;DR: AI4Service introduces Alpha-Service, a framework for proactive and real-time AI assistance, demonstrating its effectiveness through various case studies.


<details>
  <summary>Details</summary>
Motivation: Existing AI services are largely reactive, responding only to explicit user commands. A truly intelligent assistant should anticipate user needs and act proactively when appropriate.

Method: Alpha-Service is inspired by the von Neumann computer architecture and consists of five key components: Input Unit, Central Processing Unit, Arithmetic Logic Unit, Memory Unit, and Output Unit. It is implemented through a multi-agent system deployed on AI glasses.

Result: Case studies such as a real-time Blackjack advisor, a museum tour guide, and a shopping fit assistant demonstrate Alpha-Service's ability to perceive the environment, infer user intent, and provide timely assistance without explicit prompts.

Conclusion: AI4Service presents a new paradigm for AI that enables proactive and real-time assistance in daily life, with Alpha-Service as a unified framework addressing key challenges.
...
TL;DR: 本文提出了一种名为IMAGINE的框架，将多智能体系统的推理和规划能力整合到一个紧凑的模型中，并通过简单的端到端训练显著超越多智能体系统的性能。实验结果表明，使用Qwen3-8B-Instruct作为基础模型并采用该方法，模型在TravelPlanner基准测试中实现了82.7%的最终通过率，远超DeepSeek-R1-671B的40%，同时保持了更小的模型规模。


<details>
  <summary>Details</summary>
Motivation: 尽管多智能体系统可以提供改进的集体推理，但它们常常由于多轮内部交互、长响应延迟和难以端到端训练而面临高推理成本的问题。

Method: 提出了一种名为IMAGINE的通用且可扩展的框架，将多智能体系统（MAS）的推理和规划能力整合到一个紧凑的模型中，并通过简单的端到端训练显著超越MAS的性能。

Result: 实验结果表明，使用Qwen3-8B-Instruct作为基础模型并采用该方法，模型在TravelPlanner基准测试中实现了82.7%的最终通过率，远超DeepSeek-R1-671B的40%，同时保持了更小的模型规模。

Conclusion: IMAGINE框架能够将多智能体系统的推理和规划能力整合到一个紧凑的模型中，并通过简单的端到端训练显著超越多智能体系统的性能。实验结果表明，使用Qwen3-8B-Instruct作为基础模型并采用该方法，模型在TravelPlanner基准测试中实现了82.7%的最终通过率，远超DeepSeek-R1-671B的40%，同时保持了更小的模型规模。
...
TL;DR: 本文提出了一种新的图结构基准测试框架ColorBench，用于评估移动代理在复杂长视野任务中的能力，并发现现有模型的局限性，提出改进方向。


<details>
  <summary>Details</summary>
Motivation: 当前的移动代理评估标准无法全面评估代理的能力，因为它们只能验证单一预定义的“黄金路径”，或者受限于真实设备的复杂性和不可重复性。

Method: 本文引入了一种新的图结构基准测试框架，通过建模真实设备交互中观察到的有限状态，实现动态行为的静态模拟，并开发了ColorBench作为基准测试。

Result: 通过在ColorBench上评估各种基线，发现了现有模型的局限性，并提出了改进方向和可行的技术路径以提高代理在复杂、长视野问题上的性能。

Conclusion: 本文提出了ColorBench，一个专注于复杂长视野任务的基准测试框架，能够评估多种有效的解决方案，并提供了改进现有模型的方向和技术路径。
...
TL;DR: TITAN is a framework that connects natural-language cyber threat queries with executable reasoning over a structured knowledge graph, enabling models to generate valid and coherent reasoning paths.


<details>
  <summary>Details</summary>
Motivation: To support training and evaluation, TITAN introduces a dataset pairing natural language questions with executable reasoning paths and step-by-step Chain of Thought explanations.

Method: TITAN integrates a path planner model and a graph executor to connect natural-language cyber threat queries with executable reasoning over a structured knowledge graph.

Result: Empirical evaluations show that TITAN allows models to generate valid and coherent reasoning paths that can be executed on the underlying graph.
...
TL;DR: 本文提出了一种形式理论，用于描述和衡量由领域先验引导的LLM辅助迭代搜索，提供了一种可行的语言和操作工具，以衡量代理及其搜索空间，并提出了由LLM构建的迭代搜索的系统形式描述。


<details>
  <summary>Details</summary>
Motivation: 现有的基于大语言模型的生成-过滤-精炼迭代范式在AI+Science中的推理、编程和程序发现方面取得了进展，但搜索的有效性取决于搜索的位置，即如何将领域先验编码到操作结构化的假设空间中。
...
TL;DR: 本文提出了一种基于判别验证器的预算感知测试时扩展机制，该机制在保持高效的同时显著提高了性能。


<details>
  <summary>Details</summary>
Motivation: 现有的最先进的方法通常使用生成验证器从候选池中选择最佳解决方案，但这种方法会带来高昂的计算成本，限制了其实用性。因此，我们需要一种更注重预算的范式。

Method: 我们进行了一项彻底的实证分析，并展示了虽然判别验证器在单独使用时可能表现不佳，但将它们与自洽性结合在混合方法中可以创建一个强大且高效的测试时扩展机制。

Result: 在固定计算预算下，这种混合方法显著超越了最先进的生成验证方法：在AIME2025上实现了高达15.3%的更高准确性。
...
TL;DR: 本文系统地研究了多模态抑郁症检测，发现三模态模型结合EEG、语音和文本能实现最佳效果。


<details>
  <summary>Details</summary>
Motivation: 现有的研究在范围上有限，缺乏系统的特征比较，并且评估协议不一致。我们旨在解决这些差距。

Method: 我们系统地探索了EEG、语音和文本中的特征表示和建模策略，评估了手工特征与预训练嵌入的效果，评估了不同神经编码器的有效性，比较了单模态、双模态和三模态配置，并分析了融合策略，特别关注EEG的作用。

Result: 结果表明，(i) EEG、语音和文本模态的结合增强了多模态检测，(ii) 预训练嵌入优于手工特征，(iii) 经过精心设计的三模态模型实现了最先进的性能。
...
TL;DR: 本文将康德的《纯粹理性批判》重新解释为反馈稳定性理论，通过H-Risk指数分析推理系统的过度自信现象，并发现其与大型语言模型的校准不良和幻觉有关。


<details>
  <summary>Details</summary>
Motivation: 本文旨在探讨康德的哲学理论如何与现代推理系统中的过度自信现象相关联，并提供一种新的分析视角。

Method: 本文通过复合不稳定性指数（H-Risk）重新解释了康德的《纯粹理性批判》，该指数结合了谱裕度、条件性、时间敏感性和创新放大。

Result: 在线性高斯模拟中，较高的H-Risk预测了即使在正式稳定的情况下也会出现过度自信的错误，揭示了名义稳定性和认识稳定性之间的差距。扩展到大型语言模型（LLMs），发现脆弱的内部动态与校准不良和幻觉有关。

Conclusion: 本文提出了一个结构桥梁，将康德的自我限制与反馈控制联系起来，为诊断和减少推理系统的过度自信提供了一个有原则的视角。
...
TL;DR: 研究大型语言模型在组合机器设计任务中的表现，并探索强化学习作为改进方法。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否能够学习创造复杂的机器，特别是在组合机器设计任务中。

Method: 通过构建BesiegeField测试平台，使用状态先进的LLMs进行基准测试，并探索强化学习微调方法。

Result: 识别了成功所需的的关键能力，包括空间推理、战略装配和指令遵循，并探索了强化学习作为改进路径。

Conclusion: 当前开源模型在机器设计任务中仍有不足，需要探索强化学习等方法进行改进，并且需要解决语言、机器设计和物理推理之间的开放性挑战。
...
### [140] [Detecting Early and Implicit Suicidal Ideation via Longitudinal and Information Environment Signals on Social Media](https://arxiv.org/abs/2510.14889)
*Soorya Ram Shimgekar,Ruining Zhao,Agam Goyal,Violeta J. Rodriguez,Paul A. Bloom,Hari Sundaram,Koustuv Saha*

Main category: cs.SI

TL;DR: 该研究提出了一种计算框架，通过建模用户的信息环境（包括其长期发布历史以及社会接近的同伴的讨论）来检测隐含的自杀意念。研究结果表明，同伴互动提供了有价值的预测信号，并对设计能够捕捉在线环境中间接和隐藏的风险表达的早期检测系统具有更广泛的意义。


<details>
  <summary>Details</summary>
Motivation: 在社交媒体上，许多经历自杀意念（SI）的人不会明确透露他们的痛苦。相反，迹象可能通过日常帖子或同伴互动间接出现。早期检测这种隐含信号至关重要，但仍然具有挑战性。

Method: 我们采用了一种复合网络中心性度量来识别用户的顶级邻居，并在时间上对齐用户和邻居的互动，将多层信号整合到微调的DeBERTa-v3模型中。

Result: 在Reddit的1000名用户（500名病例和500名对照）研究中，我们的方法比仅基于个体的基线提高了15%的早期和隐含SI检测效果。

Conclusion: 这些发现表明，同伴互动提供了有价值的预测信号，并对设计能够捕捉在线环境中间接和隐藏的风险表达的早期检测系统具有更广泛的意义。
...
### [141] [Joint Modeling of Big Five and HEXACO for Multimodal Apparent Personality-trait Recognition](https://arxiv.org/abs/2510.14203)
*Ryo Masumura,Shota Orihashi,Mana Ihori,Tomohiro Tanaka,Naoki Makishima,Taiga Yamane,Naotaka Kawata,Satoshi Suzuki,Taichi Katayama*

Main category: cs.CV

TL;DR: 本文提出了一种联合建模方法，用于自动识别多模态人类行为中的明显人格特质，结合了五大性格特质（Big Five）和HEXACO，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 以往的研究主要使用五大性格特质进行多模态明显人格特质识别，但没有研究专注于HEXACO，而HEXACO可以评估与转移攻击和报复性有关的诚实-谦逊特质和社会支配倾向等。此外，机器学习模型中五大性格特质和HEXACO之间的关系尚未明确。

Method: 本文提出了一种联合建模方法，用于自动识别多模态人类行为中的明显人格特质，结合了五大性格特质（Big Five）和HEXACO。

Result: 实验使用自我介绍视频数据集证明，所提出的方法可以有效地识别五大性格特质和HEXACO。

Conclusion: 本文提出了一种联合建模方法，将长期研究的五大性格特质（Big Five）和近年来在心理学中受到关注的HEXACO进行了自动识别多模态人类行为中的明显人格特质。实验表明，所提出的方法可以有效地识别出五大性格特质和HEXACO。
...
### [142] [Talking Points: Describing and Localizing Pixels](https://arxiv.org/abs/2510.14583)
*Matan Rusanovsky,Shimon Malnick,Shai Avidan*

Main category: cs.CV

TL;DR: 本文提出了一种新的框架，用于像素级别的关键点定位，通过 Point Descriptor 和 Point Localizer 实现，并引入了一个新的数据集 LlamaPointInPart。实验结果表明该框架性能优越。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言模型在对象级别或区域级别定位方面表现良好，但缺乏通过自然语言进行像素级关键点理解的能力。因此，本文旨在解决这一问题。

Method: 本文提出了一个包含 Point Descriptor 和 Point Localizer 的框架，用于生成丰富的上下文描述并回归精确的像素坐标。此外，还引入了 LlamaPointInPart 数据集，并通过 GRPO 优化 Point Descriptor。

Result: 实验结果表明，本文提出的框架在 LlamaPointInPart 数据集上表现出色，优于基线模型。

Conclusion: 本文提出了一种新的框架，用于像素级别的定位。该框架包括两个互补的组件：Point Descriptor 和 Point Localizer。实验表明，该框架在 LlamaPointInPart 数据集上优于基线模型。
...
### [143] [Benchmarking Multimodal Large Language Models for Face Recognition](https://arxiv.org/abs/2510.14866)
*Hatef Otroshi Shahreza,Sébastien Marcel*

Main category: cs.CV

TL;DR: 本文对最先进的多模态大语言模型在人脸识别任务中的表现进行了系统性基准测试，并发现它们在高精度识别场景中不如专门的模型。


<details>
  <summary>Details</summary>
Motivation: 尽管MLLM在多种视觉-语言任务中表现出色，但它们在人脸识别领域的潜力尚未得到充分探索。需要评估开源MLLM与现有面部识别模型在标准基准上的性能。

Method: 本文对最先进的MLLM进行了系统性基准测试，以评估其在人脸识别任务中的表现。

Result: 实验结果表明，虽然MLLM能够捕捉到对人脸相关任务有用丰富的语义线索，但在零样本应用的高精度识别场景中，它们的表现仍落后于专门的模型。

Conclusion: 本基准测试为推进基于MLLM的人脸识别提供了基础，并为设计更高精度和泛化能力的下一代模型提供了见解。
...
TL;DR: 本文提出了一种两阶段方法nlg2choice，用于解决在细粒度视觉分类任务中评估自回归模型自由形式响应的挑战，并在多个数据集上取得了改进。


<details>
  <summary>Details</summary>
Motivation: 尽管对零样本视觉分类的研究兴趣重新兴起，但评估自回归模型的自由形式响应仍然是一个持续的挑战。现有的工作主要集中在语言-only任务或不考虑超过5-way选项的多选题，而这对于细粒度视觉分类任务来说是关键能力。

Method: 本文提出了一种两阶段方法，首先让MLLM回答一个开放性问题，然后使用仅文本的受限解码来预测最可能的选择。在检索设置中，采用提前停止方法计算受限响应选择的概率，以显著提高吞吐量。

Result: 实验结果表明，该方法在七个细粒度视觉数据集上的分类和检索任务中均取得了改进，并且在各种自然语言实现方式中表现稳定。

Conclusion: 本文提出了一种名为nlg2choice的简单两阶段方法，用于解决在细粒度视觉分类任务中评估自回归模型自由形式响应的挑战。实验结果表明，该方法在多个数据集上均取得了改进，并且在各种自然语言实现方式中表现稳定。
...
### [145] [MathCanvas: Intrinsic Visual Chain-of-Thought for Multimodal Mathematical Reasoning](https://arxiv.org/abs/2510.14958)
...
TL;DR: 本文介绍了MathCanvas，一个全面的框架，旨在赋予统一的大规模多模态模型（LMMs）内在的视觉链式思维（VCoT）能力。通过两个阶段的训练，我们的模型在MathCanvas-Bench上取得了显著的性能提升，并展示了良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉链式思维（VCoT）方法通常受到刚性外部工具的限制，或者无法生成高保真度、战略时机的图表，这对于复杂的解决问题至关重要。为了弥合这一差距，我们引入了MathCanvas，一个全面的框架，旨在赋予统一的大规模多模态模型（LMMs）内在的VCoT能力。
...
Result: 在MathCanvas-Bench上，我们的模型BAGEL-Canvas相对于强大的LMM基线实现了86%的相对改进，展示了对其他公共数学基准的良好泛化能力。
...
TL;DR: 本文提出一种新的框架，通过检索系统视角解决ICD代码排序问题，并在性能上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 自动化分配和排序ICD代码对于医疗诊断和报销至关重要，但现有方法忽略了代码顺序的重要性。

Method: 本文从检索系统角度出发，将问题转化为分类和排序任务。

Result: 模型在正确排序主要诊断代码方面的准确率为47%，分类指标的微F1和宏F1分数分别为0.6065和0.2904。
...
TL;DR: BitDistill是一种轻量级的微调方法，能够将全精度LLM转换为1.58位精度的模型，在保持性能的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 为了在特定下游任务中实现高效且高性能的模型，需要一种轻量级的微调方法，以减少计算成本并提高效率。

Method: BitDistill通过三个关键技术：SubLN模块、基于MiniLM的多头注意力蒸馏以及持续预训练，将全精度LLM微调为1.58位精度的模型。

Result: 实验结果表明，BitDistill在模型大小方面与全精度模型性能相当，同时实现了高达10倍的内存节省和2.65倍的CPU推理速度提升。

Conclusion: BitDistill能够在保持强任务性能的同时，显著降低计算成本，实现内存节省和推理速度的提升。
...
### [148] [Towards Reversible Model Merging For Low-rank Weights](https://arxiv.org/abs/2510.14163)
*Mohammadsajad Alipour,Mohammad Mohammadi Amiri*

Main category: cs.LG

TL;DR: RMM is a new method for model merging that addresses the problem of performance degradation when merging low-rank models, allowing for the recovery of original task-specific models through linear combination.


<details>
  <summary>Details</summary>
Motivation: To address the issue of severe performance degradation in merged models when using conventional merging methods on low-rank weights, and to allow reverting to individual models when needed.
...
### [149] [MAFA: A Multi-Agent Framework for Enterprise-Scale Annotation with Configurable Task Adaptation](https://arxiv.org/abs/2510.14184)
*Mahmood Hegazy,Aaron Rodrigues,Azzam Naeem*

Main category: cs.LG

TL;DR: MAFA is a production-deployed system that transforms enterprise-scale annotation workflows using configurable multi-agent collaboration, effectively addressing annotation backlogs and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the critical challenge of annotation backlogs in financial services, where millions of customer utterances require accurate categorization.

Method: MAFA combines specialized agents with structured reasoning and a judge-based consensus mechanism, allowing dynamic task adaptation through configuration rather than code changes.
...
### [150] [Scaling Test-Time Compute to Achieve IOI Gold Medal with Open-Weight Models](https://arxiv.org/abs/2510.14232)
...
TL;DR: 本文提出了一种名为GenCluster的可扩展和可重复的测试时计算框架，该框架使用开放权重模型实现了IOI金牌级别的性能。


<details>
  <summary>Details</summary>
Motivation: Achieving comparable results with open-weight models remains a significant challenge, despite several proprietary models claiming to achieve gold medal-level performance at the IOI.

Method: GenCluster is a scalable and reproducible test-time compute framework that combines large-scale generation, behavioral clustering, ranking, and a round-robin submission strategy to efficiently explore diverse solution spaces under limited validation budgets.

Result: The performance of the proposed approach scales consistently with available compute, narrowing the gap between open and closed systems.

Conclusion: GenCluster can achieve a gold medal at IOI 2025 for the first time with an open-weight model gpt-oss-120b, setting a new benchmark for transparent and reproducible evaluation of reasoning in LLMs.
...
### [151] [CAST: Compositional Analysis via Spectral Tracking for Understanding Transformer Layer Functions](https://arxiv.org/abs/2510.14262)
*Zihao Fu,Ming Liao,Chris Russell,Zhenguang G. Cai*

Main category: cs.LG

TL;DR: 本文提出了一种名为CAST的新框架，通过直接变换矩阵估计和谱分析来研究Transformer层的功能，揭示了不同模型结构的行为差异以及层间的功能关系模式。


<details>
  <summary>Details</summary>
Motivation: 为了更好地理解大型语言模型的内部机制，研究人员提出了各种可解释性方法。本文旨在提供一种新的视角，以补充现有的方法。

Method: CAST（Compositional Analysis via Spectral Tracking）是一个无需探测器的框架，通过直接变换矩阵估计和全面的谱分析来分析Transformer层的功能。

Result: 分析显示，解码器模型表现出压缩-扩展周期，而编码器模型保持一致的高秩处理。核分析进一步展示了层间的功能关系模式，CKA相似性矩阵将层分为三个阶段：特征提取、压缩和专业化。

Conclusion: 本文介绍了CAST框架，通过直接变换矩阵估计和全面的谱分析，为Transformer层功能提供了新的视角。分析揭示了编码器-only和解码器-only模型之间的行为差异，并展示了层间的功能关系模式。
...
TL;DR: 本文首次系统分析了基于LLM的提示优化中的中毒风险，发现系统对操纵的反馈比对注入的查询更脆弱，并提出了一种简单的假奖励攻击和一种轻量级的防御方法。


<details>
  <summary>Details</summary>
Motivation: LLM-based prompt optimizers减少了设计提示的努力，但优化阶段的安全性尚未得到充分研究。

Method: 我们使用HarmBench进行了系统分析，以评估基于LLM的提示优化中的中毒风险，并引入了一种简单的假奖励攻击，以及一种轻量级的突出显示防御方法。

Result: 系统对操纵的反馈比对注入的查询更加脆弱，基于反馈的攻击将攻击成功率（ASR）提高了高达ΔASR = 0.48。假奖励攻击显著增加了脆弱性，而轻量级的突出显示防御方法将假奖励的ΔASR从0.23降低到0.07，同时不会降低效用。

Conclusion: 这些结果确立了提示优化管道作为一个首要的攻击面，并促使对反馈通道和优化框架采取更强的保护措施。
...
### [153] [Agentic Entropy-Balanced Policy Optimization](https://arxiv.org/abs/2510.14545)
...
TL;DR: The paper introduces AEPO, an agentic RL algorithm that balances entropy in both the rollout and policy update phases, leading to improved performance on multiple datasets.
...
### [154] [Reasoning with Sampling: Your Base Model is Smarter Than You Think](https://arxiv.org/abs/2510.14901)
*Aayush Karan,Yilun Du*

Main category: cs.LG

TL;DR: 本文提出了一种无需训练即可通过采样提升基础模型推理能力的方法，并在多个任务中表现出色。
...
Result: 我们的算法在多种单次任务上提供了显著的推理提升，几乎匹配甚至超过了RL的效果。

Conclusion: 我们的方法不需要训练、精心制作的数据集或验证器，表明它在易于验证的领域之外具有广泛的应用前景。
...
### [155] [Circuit Insights: Towards Interpretability Beyond Activations](https://arxiv.org/abs/2510.14936)
*Elena Golimblevskaia,Aakriti Jain,Bruno Puri,Ammar Ibrahim,Wojciech Samek,Sebastian Lapuschkin*

Main category: cs.LG

TL;DR: 本文提出了WeightLens和CircuitLens两种互补方法，以更系统地分析神经网络中的电路结构，提高可解释性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的方法依赖于手动检查，难以处理复杂的任务，而自动解释方法常常忽略了特征之间的相互作用，并且依赖外部LLM和数据集质量。

Method: 提出了一种直接从学习到的权重中解释特征的WeightLens和一种捕捉组件间相互作用导致特征激活的CircuitLens。
...
### [156] [Efficient Parallel Samplers for Recurrent-Depth Models and Their Connection to Diffusion Language Models](https://arxiv.org/abs/2510.14961)
*Jonas Geiping,Xinyu Yang,Guinan Su*

Main category: cs.LG

TL;DR: 本文研究了循环深度模型与扩散语言模型的关系，并提出了一种新的扩散强制采样器以加速生成。该采样器在每次前向传递中解码新标记，并通过循环并行优化潜在状态。实验结果表明，该方法在时间预算内比基线自回归生成更具表现力，并且可以在不调整的情况下应用于现有的3.5B循环深度变压器，实现高达5倍的速度提升。


<details>
  <summary>Details</summary>
Motivation: 最近的研究表明，这些架构可以扩展到现代语言建模任务，并在推理任务中表现出优势。然而，现有的生成方法在时间预算内可能不够高效。因此，我们需要一种更高效的生成方法。

Method: 我们研究了循环深度模型和扩散语言模型之间的关系，并开发了一种新的扩散强制采样器来加速生成。该采样器在每次前向传递中解码新标记，同时这些标记的潜在状态可以通过循环并行优化。

Result: 我们的采样器理论上比使用相同时间预算的基线自回归生成更具表现力。此外，该采样器可以直接应用于现有的3.5B循环深度变压器，无需调整，从而实现高达5倍的速度提升。

Conclusion: 我们的研究不仅提供了一种高效并行化循环深度模型在推理中的额外计算的机制，还表明这类模型可以自然地被视为强大的连续但因果的扩散语言模型。
