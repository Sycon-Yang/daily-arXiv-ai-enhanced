...
Motivation: 在对话转录流程中，大型语言模型常用于后处理以提高语法、标点和可读性。本文旨在探索一种互补的后处理步骤，即通过添加元数据标签来丰富转录对话，这些标签包括年龄、性别和情绪等说话者特征。

Method: 本文方法结合了冻结的音频基础模型（如Whisper或WavLM）和冻结的LLAMA语言模型，利用轻量高效的连接器来桥接音频和语言表示，以推断说话者属性。此外，还展示了冻结的LLAMA模型可以直接比较x向量。

Result: 本文方法在说话者分析任务上取得了具有竞争力的性能，同时保持了模块化和速度。此外，冻结的LLAMA模型可以直接比较x向量，在某些场景下实现了8.8%的等错误率。

Conclusion: 本文提出了一种在对话转录流程中添加元数据标签的互补后处理步骤，通过结合冻结的音频基础模型和冻结的LLAMA语言模型，无需任务特定的微调即可推断说话者属性。这种方法在保持模块化和速度的同时，在说话者分析任务上取得了具有竞争力的性能。此外，还展示了冻结的LLAMA模型可以直接比较x向量，实现了某些场景下的8.8%的等错误率。
...
<details>
  <summary>Details</summary>
Motivation: 为了提升使用半监督语音表示的自动语音识别（ASR）系统的性能，需要考虑音调重音等重要的语调线索。

Method: 引入了一个联合ASR和音调重音检测模型，以提升ASR系统的性能。

Result: 音调重音检测组件在该任务上取得了显著改进，将F1分数的差距缩小了41%。在有限资源微调下，联合训练中的ASR性能在LibriSpeech上将WER降低了28.3%。

Conclusion: 我们展示了通过引入联合ASR和音调重音检测模型，可以提升使用半监督语音表示的自动语音识别（ASR）系统的性能。我们的模型的音调重音检测组件在该任务上取得了显著改进，将F1分数的差距缩小了41%。此外，在有限资源微调下，联合训练中的ASR性能在LibriSpeech上将WER降低了28.3%。这些结果表明，扩展预训练语音模型以保留或重新学习重要的语调线索（如音调重音）的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要一致的行为模式以确保安全部署，但其类似性格的特征仍然不为人知。

Method: 通过PERSIST框架测试了25个以上的开源模型（1B-671B参数）在500,000+响应中的表现，使用传统和新型的LLM适应的性格工具进行系统性变化。

Result: 发现即使400B以上的模型也表现出显著的响应变异性，轻微的提示重新排序可以改变性格测量高达20%，一些预期能稳定行为的干预措施反而增加了变异性，LLM适应的工具显示与人类中心版本同样不稳定。

Conclusion: 当前大型语言模型在行为一致性方面存在根本性不足，基于性格的对齐策略可能不足以满足安全关键应用的需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有的协调方案依赖于静态或全上下文路由策略，导致过多的token消耗、冗余内存暴露以及跨交互轮次的适应性有限。因此，需要一种更高效、自适应的协作方法。

Method: RCR-Router是一种模块化且角色感知的上下文路由框架，它动态选择语义相关的记忆子集，并通过轻量级评分策略引导记忆选择，同时将智能体输出迭代地整合到共享内存存储中以促进渐进式上下文优化。

Result: 在三个多跳QA基准测试（HotPotQA、MuSiQue和2WikiMultihop）上的实验表明，RCR-Router减少了token使用量（高达30%），同时提高了或保持了答案质量。

Conclusion: 这些结果突显了结构化记忆路由和输出感知评估在推进可扩展多智能体LLM系统中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 本文旨在评估大型语言模型（LLMs）对语言shibboleths的反应，这些语言标记可能无意中揭示性别、社会阶层或地区背景等人口统计属性。

Method: 本文引入了一个全面的基准，用于评估大型语言模型（LLMs）对语言shibboleths的反应，通过精心构建的100个经过验证的问答对进行访谈模拟，生成受控的语言变化以隔离特定现象并保持语义等价性。

Result: 本文展示了LLMs如何系统地惩罚某些语言模式，尤其是模糊语言，尽管内容质量相当。结果表明，模糊回答的平均评分低25.6%，并且证明了该基准在识别模型特定偏见方面的有效性。

Conclusion: 本文建立了检测和测量人工智能系统中语言歧视的基础框架，并展示了其在自动化决策公平性中的广泛应用。
...
Method: 我们提出了一种视觉语言聚类框架，用于构建动词意义聚类，以提供更稳健的评估。

Result: 分析显示，每张图像平均映射到2.8个意义聚类，每个聚类代表图像的不同视角。此外，我们的聚类评估方法比标准评估方法更符合人类判断。

Conclusion: 我们的聚类评估方法更好地与人类判断对齐，提供了对模型性能的更细致评估。
...
<details>
  <summary>Details</summary>
Motivation: 了解导致自杀事件的社会决定因素（SDoH）对于早期干预和预防至关重要。然而，数据驱动的方法在这一目标上面临挑战，例如长尾因子分布、分析导致自杀事件的关键压力源以及模型可解释性有限。
...
Result: 我们展示了所提出的框架在提取SDoH因素的整体任务以及检索相关上下文的细粒度任务中的性能提升。此外，我们展示了微调一个较小的任务特定模型可以实现相当或更好的性能，同时减少推理成本。多阶段设计不仅增强了提取效果，还提供了中间解释，提高了模型的可解释性。

Conclusion: 我们的方法提高了从非结构化文本中提取与自杀相关的社会决定因素的准确性和透明度。这些进展有望支持早期识别处于风险中的个体，并制定更有效的预防策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法通常学习整个对话中的单词关系，假设情感元素的分布是均匀的。然而，我们发现对话中经常包含多个语义独立的子对话，这使得学习整个对话中的单词关系不可避免地引入额外的噪声。

Method: 我们提出了一种基于结构熵最小化的算法来划分对话，并引入了一个两步框架进行四元组提取。

Result: 实验结果表明，我们的方法在DiaASQ任务中表现优异，同时计算成本较低。
...
<details>
  <summary>Details</summary>
Motivation: 探索微调仅解码器的大型语言模型在AMR解析任务中的潜力，以提供一种简单且有效的解决方案。

Method: 对四个不同的LLM架构（Phi 3.5、Gemma 2、LLaMA 3.2和DeepSeek R1 LLaMA Distilled）进行微调，并使用LDC2020T02 Gold AMR3.0测试集进行评估。

Result: 微调后的模型在SMATCH F1指标上达到了0.804，与APT + Silver (IBM)相当，并接近Graphene Smatch (MBSE)的0.854。LLaMA 3.2在语义性能上优于其他模型，而Phi 3.5在结构有效性方面表现更佳。

Conclusion: 微调仅解码器的大型语言模型可以在AMR解析任务中达到与复杂最先进的AMR解析器相当的性能，其中LLaMA 3.2在语义性能上表现最佳。
...
<details>
  <summary>Details</summary>
Motivation: 参数高效微调（PEFT）对于适应大型语言模型（LLMs）至关重要。在实践中，LLMs经常需要处理来自多个领域的多样化任务，这自然由多任务学习（MTL）解决。然而，现有的多组件范式可能不是最优的。

Method: 我们提出了Align-LoRA，它在共享适配器空间中引入了显式的损失来对齐任务表示。

Result: 实验表明，Align-LoRA显著超越了所有基线，证明了其有效性。

Conclusion: 我们的研究结果表明，有效的多任务学习泛化依赖于学习稳健的共享表示，而不是隔离任务特定特征。Align-LoRA通过引入显式损失来对齐共享适配器空间中的任务表示，显著超越了所有基线，建立了一个更简单但更有效的适应大型语言模型到多个任务的范式。
...
<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息的增长率，其中声明由文本和图像支持，对依赖主要文本证据的验证系统构成了重大挑战。

Method: 我们的架构结合了用于文本和图像的专用编码器以及一个融合模块，该模块使用逐元素交互来捕捉跨模态关系。分类头然后预测声明的真伪，由对比学习目标支持，该目标鼓励共享潜在空间中的声明-证据对之间的语义对齐。

Result: 我们在Factify 2数据集上评估了我们的方法，获得了0.84的加权F1分数，明显优于基线。

Conclusion: 这些结果突显了显式多模态推理的有效性，并展示了我们的方法在复杂、现实场景中可扩展和可解释的假检查的潜力。
...
<details>
  <summary>Details</summary>
Motivation: RAG systems face challenges due to long context lengths causing entropy growth and attention dilution, which affect performance.

Method: BEE-RAG framework uses entropy invariance to improve adaptability to varying context lengths, along with a zero-shot inference strategy and parameter-efficient adaptive fine-tuning mechanism.

Result: Extensive experiments show that BEE-RAG is effective in improving RAG performance across multiple tasks.

Conclusion: BEE-RAG demonstrates effectiveness across multiple RAG tasks by addressing entropy growth and attention dilution issues.
...
<details>
  <summary>Details</summary>
Motivation: The performance of Large Language Models (LLMs) is significantly sensitive to the contextual position of information in the input. The paper aims to investigate the mechanism behind this positional bias and enhance model performance by addressing it.

Method: The paper introduces Attention-Driven Reranking (AttnRank), a two-stage framework that estimates a model's intrinsic positional attention preferences using a small calibration set and reorders retrieved documents or few-shot examples to align the most salient content with these high-attention positions.
...
<details>
  <summary>Details</summary>
Motivation: The integration of large language models into healthcare necessitates a rigorous evaluation of their ethical reasoning, an area current benchmarks often overlook.

Method: We introduce PrinciplismQA, a comprehensive benchmark with 3,648 questions designed to systematically assess LLMs' alignment with core medical ethics. Grounded in Principlism, our benchmark features a high-quality dataset. This includes multiple-choice questions curated from authoritative textbooks and open-ended questions sourced from authoritative medical ethics case study literature, all validated by medical experts.

Result: Our experiments reveal a significant gap between models' ethical knowledge and their practical application, especially in dynamically applying ethical principles to real-world scenarios. Most LLMs struggle with dilemmas concerning Beneficence, often over-emphasizing other principles. Frontier closed-source models, driven by strong general capabilities, currently lead the benchmark. Notably, medical domain fine-tuning can enhance models' overall ethical competence, but further progress requires better alignment with medical ethical knowledge.

Conclusion: PrinciplismQA offers a scalable framework to diagnose these specific ethical weaknesses, paving the way for more balanced and responsible medical AI.
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言生成方面取得了显著进展，但仍然容易产生幻觉，生成错误或误导性的内容。

Method: 本文探讨了使用外部上下文和不使用外部上下文的方法，利用少量样本提示与大型语言模型，以及基于合成数据的大型语言模型微调进行标记级分类。

Result: 本文的方法在西班牙语中取得了顶级排名，在英语和德语中获得了有竞争力的排名。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法主要依赖参数密集的多模态LLM进行情感分类，忽略了在资源受限环境中的自主多模态情感推理生成。因此，我们专注于资源受限的联合多模态情感推理和分类任务（JMSRC）。

Method: 提出了一种名为MulCoT-RD的多模态思维链推理蒸馏模型，采用“教师-助教-学生”蒸馏范式，在资源受限环境中进行多模态情感推理链生成和情感分类。

Result: 在四个数据集上的实验表明，仅具有3B参数的MulCoT-RD在JMSRC上表现强劲，并展现出稳健的泛化能力和增强的可解释性。

Conclusion: MulCoT-RD在资源受限环境中表现出强大的性能、泛化能力和可解释性，证明了其在联合多模态情感推理和分类任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前的结构化修剪方法通常忽视了人工神经元之间的交互和协作，导致LLM的宏观功能架构被破坏，从而影响修剪性能。本文旨在解决这一问题。

Method: 将LLM视为一个数字大脑，将其分解为功能网络，并保留这些功能网络中的关键神经元进行修剪。

Result: 实验结果表明，所提出的方法能够成功识别和定位LLMs中的功能网络和关键神经元，实现了高效的模型修剪。

Conclusion: 本文提出了一种通过识别和保留LLMs中的功能网络来修剪大型语言模型的方法，实验结果表明该方法能够成功识别和定位LLMs中的功能网络和关键神经元，从而实现高效的模型压缩。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于人工标注的指令，但高质量的编码指令收集成本高且难以扩展。而代码片段丰富可用，因此需要一种不依赖人工指令的后训练框架。

Method: CodeBoost 通过从代码片段中进行后训练，引入了最大团选择、双向预测、错误感知预测、异构增强和异构奖励等关键组件。

Result: 在多个代码 LLM 和基准测试中进行了广泛实验，验证了 CodeBoost 的有效性。

Conclusion: CodeBoost 是一种有效的训练管道，能够显著提升代码大语言模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 元评审是同行评审过程中的关键阶段，作为决定论文是否推荐接受的最后一步。以往的研究将元评审视为对评审报告的摘要问题，但元评审是一个需要权衡评审员论点并将其置于更广泛背景中的决策过程。

Method: 我们首先通过使用基于自我精炼策略的大型语言模型（LLMs）生成合成数据来解决训练对话代理的数据稀缺问题，以提高这些对话与专家领域的相关性。然后，我们利用这些数据训练针对元评审的对话代理。

Result: 我们的实验表明，这种方法可以生成高质量的合成数据，并可作为训练元评审助手的宝贵资源。随后，我们利用这些数据训练了针对元评审的对话代理，并发现这些代理在该任务上优于现有的基于LLM的助手。最后，我们在现实世界的元评审场景中应用了我们的代理，并确认了它们在提高元评审效率方面的有效性。

Conclusion: 我们的实验表明，这种方法可以生成高质量的合成数据，并可作为训练元评审助手的宝贵资源。随后，我们利用这些数据训练了针对元评审的对话代理，并发现这些代理在该任务上优于现有的基于LLM的助手。最后，我们在现实世界的元评审场景中应用了我们的代理，并确认了它们在提高元评审效率方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 提出SONAR-LLM是为了保留LCM的语义抽象，同时消除其扩散采样器并恢复基于似然的训练信号。

Method: SONAR-LLM是一种仅解码器的变压器，它在相同的连续SONAR嵌入空间中进行思考，但通过冻结的SONAR解码器传播的token级交叉熵进行监督。

Result: SONAR-LLM在从39M到1.3B参数的模型规模上达到了具有竞争力的生成质量。
...
<details>
  <summary>Details</summary>
Motivation: To mitigate the overthinking problem in LRLMs, which leads to redundant reasoning steps, increased token usage, and reduced practical utility.

Method: CGRS dynamically suppresses the generation of reflection triggers when the model exhibits high confidence in its current response.

Result: CGRS reduces token usage by an average of 18.5% to 41.9% while preserving accuracy and achieving the optimal balance between length reduction and performance compared to state-of-the-art baselines.

Conclusion: CGRS is a practical method for efficient reasoning in LRLMs, demonstrating effectiveness across different model architectures and scales.
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在评估在微软HoloLens 2设备上添加调整功能对现有手语虚拟人影响，特别是在可理解性、用户体验和系统接受度方面的效果。

Method: 研究分析了专家德国手语（DGS）用户在特定使用案例中与可调整和不可调整的虚拟人之间的互动，以确定影响可理解性、用户体验（UX）和系统接受度的关键因素。

Result: 尽管用户偏好可调整设置，但未观察到用户体验或可理解性的显著改善，仍然处于较低水平，这归因于缺少手语元素（如口型和面部表情）和实现问题（手形不清晰、缺乏反馈和菜单定位）。享乐质量高于实用质量，表明用户认为系统更具有情感或审美吸引力而非功能性。可调整虚拟人的压力水平更高，反映了较低的表现、更大的努力和更多的挫折感。此外，关于HoloLens调整手势是否直观和易于熟悉的问题也被提出。虽然调整概念的接受度总体积极，但强烈依赖于可用性和动画质量。

Conclusion: 研究指出，仅靠个性化是不够的，手语虚拟人必须默认可理解。关键建议包括增强口型和面部动画，改进交互界面，并应用参与式设计。
...
<details>
  <summary>Details</summary>
Motivation: 本文旨在研究将自主搜索系统应用于专业领域（如生物医学研究）时的挑战，并探索自我反馈策略是否能提高性能。

Method: 本文的方法包括一个自我反馈机制，其中LLM生成、评估并改进其输出以进行查询扩展和多种答案类型（是/否、事实性、列表、理想）。

Result: 初步结果表明，自我反馈策略在不同模型和任务中的表现各异。

Conclusion: 本文提供了关于LLM自我修正的见解，并为未来比较LLM生成的反馈与直接人类专家输入在这些搜索系统中的有效性提供了参考。
...
<details>
  <summary>Details</summary>
Motivation: 本文旨在创建一个大规模的并行语料库，以支持手语研究和相关技术的发展，特别是针对拉丁美洲和德国手语的语料库。

Method: 本文通过从多个在线来源收集和处理多种手语的视频来创建语料库，主要包括新闻节目、政府机构和教育频道的广播材料。准备过程包括数据收集、通知内容创作者并寻求使用许可、爬取和裁剪等阶段。

Result: 本文创建了一个包含12种手语的并行语料库，包括超过1,300小时的视频文件和1,3~M字幕，包含14~M个词。其中，8种拉丁美洲手语的语料库是首次一致的并行语料库，而德语手语语料库的规模是之前可用语料库的十倍。

Conclusion: 本文介绍了包含12种手语的并行语料库，以及对应国家主导口语语言的字幕。该语料库包括超过1,300小时的视频文件，配有1,3~M字幕，包含14~M个词。其中，8种拉丁美洲手语的语料库是首次一致的并行语料库，而德语手语语料库的规模是之前可用语料库的十倍。语料库通过从多个在线来源收集和处理多种手语的视频创建，主要包括新闻节目、政府机构和教育频道的广播材料。本文提供了该语料库的统计数据和数据收集方法的概述。
...
<details>
  <summary>Details</summary>
Motivation: 由于训练数据主要由高资源语言如英语和中文主导，大型语言模型（LLMs）往往表现出文化偏见，这在低资源语言环境中尤其具有挑战性。

Method: 我们引入了MyCulture，这是一个基准测试，用于全面评估马来西亚文化中的LLM，采用了一种新颖的开放性多选问题格式，而不是预定义选项。

Result: 我们的评估显示，不同地区和国际LLM在文化理解上存在显著差异。

Conclusion: 我们的评估揭示了在文化理解上的显著差异，强调了在LLM的开发和评估中需要文化和语言包容性的基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型在静态基准上的评估容易受到数据污染和排行榜过拟合的影响，这是掩盖真实模型能力的关键问题。

Method: LLMEval-3 是一个动态评估大型语言模型的框架，基于一个专有的 220k 研究生级问题库，从中动态采样未见过的测试集进行每次评估运行。其自动化流程通过抗污染数据整理、新颖的防作弊架构和经过校准的 LLM 作为裁判过程实现与人类专家 90% 的一致性，还补充了一个相对排名系统以实现公平比较。

Result: 一项为期 20 个月的纵向研究几乎涵盖了 50 个领先的模型，揭示了知识记忆的性能上限，并暴露了静态基准无法检测到的数据污染漏洞。该框架在排名稳定性和一致性方面表现出色，为动态评估范式提供了强有力的实证验证。

Conclusion: LLMEval-3 提供了一种稳健且可信的方法来评估大型语言模型的真实能力，超越了排行榜分数，促进了更值得信赖的评估标准的发展。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）在高层次语义任务中表现出色，但在需要精确和控制的应用中，它们常常在细粒度的token-level理解和结构推理方面表现不佳。

Method: 引入TASE基准，评估LLM在token-level信息感知和结构推理方面的能力，包括字符计数、token对齐、语法结构解析和长度约束满足等任务，并训练自定义的Qwen2.5-14B模型。

Result: 人类表现显著优于当前LLM，显示出在token-level推理方面的持续弱点。

Conclusion: TASE揭示了当前LLM在token-level推理方面的局限性，并为未来低层次语言理解和跨语言泛化的改进提供了新的诊断视角。
...
<details>
  <summary>Details</summary>
Motivation: 为了评估不同领域的创造力度量标准的有效性，并揭示它们的局限性，从而推动更准确的评估框架的发展。

Method: 我们系统地检查、分析和比较了代表性的创造力度量标准，包括创造力指数、困惑度、句法模板和LLM-as-a-Judge，在不同的创造性领域中进行比较，包括创造性写作、非常规问题解决和研究构想。

Result: 这些度量标准表现出有限的一致性，捕捉到创造力的不同方面。创造力指数侧重于词汇多样性，困惑度对模型置信度敏感，句法模板无法捕捉概念性创造力。此外，LLM-as-a-Judge表现出不稳定性和偏见。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在各种任务中表现出色，但在需要专业知识的任务中存在关键限制，常常在面对需要专门知识的问题时产生幻觉。虽然检索增强生成（RAG）通过整合外部知识缓解了这一问题，但由于其依赖直接语义检索和缺乏结构化的逻辑组织，在复杂推理场景中表现不佳。

Method: 本文引入了逻辑增强生成（LAG），通过系统的问题分解和依赖感知推理重新构想知识增强。LAG首先将复杂问题分解为按逻辑依赖关系排序的原子子问题，然后依次解决这些子问题，使用先前的答案来指导后续子问题的上下文检索，确保逐步在逻辑链中进行验证。为了防止错误传播，LAG包含一个逻辑终止机制，在遇到无法回答的子问题时停止推理，并减少过多推理上的浪费计算。最后，它综合所有子解决方案以生成经过验证的响应。

Result: 在四个基准数据集上的实验表明，LAG显著增强了推理鲁棒性，减少了幻觉，并使LLM的问题解决与人类认知相一致。

Conclusion: 实验表明，LAG显著增强了推理鲁棒性，减少了幻觉，并使LLM的问题解决与人类认知相一致，为现有的RAG系统提供了一种有原则的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法无法充分揭示模型中的隐性偏见，因此需要一种创新的、自由形式的评估框架来检测这些偏见。

Method: 通过20个问题游戏这一多轮推理任务来评估模型的表现，并使用新的数据集Geo20Q+进行系统评估。

Result: 研究发现，大型语言模型在从全球北方地区推断实体方面比全球南方地区更成功，且在西方世界比东方世界更成功。

Conclusion: 研究结果表明，大型语言模型在实体推理过程中存在地理和文化上的差异，这揭示了在标准提示设置中隐藏的细微偏见。
...
<details>
  <summary>Details</summary>
Motivation: LLMs can offer key benefits to the Legal domain, but their adoption has been hindered by their tendency to generate unfaithful, ungrounded, or hallucinatory outputs. While Retrieval-Augmented Generation offers a promising solution by grounding generations in external knowledge, it offers no guarantee that the provided context will be effectively integrated. To address this, context-aware decoding strategies have been proposed to amplify the influence of relevant context, but they usually do not explicitly enforce faithfulness to the context.

Method: CoCoLex is a decoding strategy that dynamically interpolates the model produced vocabulary distribution with a distribution derived based on copying from the context.

Result: Experimental results on five legal benchmarks demonstrate that CoCoLex outperforms existing context-aware decoding methods, particularly in long-form generation tasks.

Conclusion: CoCoLex outperforms existing context-aware decoding methods, particularly in long-form generation tasks.
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在多项选择题回答（MCQA）中表现出显著进展，但其固有的不可靠性（如幻觉和过度自信）限制了其在高风险领域的应用。因此，需要一种可靠的不确定性量化方法来提高LLMs的可信度。

Method: 本文提出了一种基于频率的不确定性量化方法，通过多次独立抽样模型输出分布，并使用最频繁的样本作为参考来计算预测熵（PE）。

Result: 实验评估显示，基于频率的PE在区分正确和错误预测方面优于基于logit的PE，同时该方法有效控制了用户指定风险水平下的经验误覆盖率。

Conclusion: 本文提出了一种基于频率的不确定性量化方法，在黑盒设置下利用合流预测（CP）来确保可证明的覆盖保证。实验结果表明，基于频率的PE在区分正确和错误预测方面优于基于logit的PE，并且该方法有效控制了用户指定风险水平下的经验误覆盖率，验证了采样频率可以作为黑盒场景中基于logit的概率的可行替代方案。本文提供了一个分布无关、模型无关的框架，用于MCQA中的可靠不确定性量化，提高了LLM在实际应用中的可信度。
...
<details>
  <summary>Details</summary>
Motivation: 研究多语言大语言模型是否在不同语言之间传递观点，或者是否每种语言都有独立的观点。

Method: 通过提示模型报告他们对投票建议应用程序中的政治陈述的（不）同意程度来评估多语言大语言模型的政治观点。同时，在使用直接偏好优化和仅用英语对齐数据对模型进行对齐之前和之后评估它们。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于模型的奖励和基于规则的奖励分别存在易受奖励黑客攻击和鲁棒性不足的问题，需要一种更有效的强化学习框架。

Method: 提出Cooper框架，联合优化策略模型和奖励模型，并引入混合标注策略和基于参考的奖励建模范式。

Result: Cooper框架能够缓解奖励黑客问题并提高端到端的强化学习性能，在Qwen2.5-1.5B-Instruct上平均准确率提高了0.54%。

Conclusion: 动态更新奖励模型是应对奖励黑客的有效方法，为更好地将奖励模型集成到强化学习中提供了参考。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在抽象推理方面表现出色，但其在具身代理推理方面的能力仍需探索。现有的基准测试提供预定义的工具集或明确的合作指令，而OmniEAR要求代理动态获取能力并自主决定协调策略。

Method: 本文提出了OmniEAR框架，用于评估语言模型在具身任务中的物理交互、工具使用和多智能体协调方面的推理能力。通过文本环境表示，建模了连续物理属性和复杂空间关系。
...
<details>
  <summary>Details</summary>
Motivation: R-LLMs在复杂推理任务上取得了显著进展，但在长格式事实性基准测试中往往产生更多的幻觉。将在线强化学习扩展到长格式事实性设置面临诸多挑战，因为缺乏可靠的验证方法。

Method: 我们提出了一种新的奖励函数，同时考虑事实精确度、响应详细程度和答案相关性，并应用在线强化学习来学习高质量的事实推理。

Result: 在六个长格式事实性基准测试中，我们的事实推理模型实现了幻觉率平均降低23.1个百分点，回答细节水平提高23%，且整体响应有用性没有下降。

Conclusion: 我们的事实推理模型在六个长格式事实性基准测试中实现了幻觉率平均降低23.1个百分点，回答细节水平提高23%，且整体响应有用性没有下降。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型已经展现出说服人类的能力，但对其动态的理解仍然有限。因此，研究旨在通过线性探针来分析这种说服过程。

Method: 研究使用了线性探针来分析大型语言模型在自然、多轮对话中的说服动态，并结合认知科学的见解来训练探针以关注说服成功、被说服者个性和说服策略等方面。

Result: 研究发现，尽管线性探针简单，但它们能够在样本和数据集层面捕捉各种说服方面的信息。此外，探针比基于提示的方法更快，并且在某些情况下表现更好或甚至超越基于提示的方法。

Conclusion: 研究结果表明，线性探针可以作为一种可行的方法来研究复杂的互动行为，如欺骗和操纵，特别是在多轮对话和大规模数据集分析中。
...
<details>
  <summary>Details</summary>
Motivation: 字节级语言模型消除了脆弱的分词器，但在形态丰富的语言中面临计算挑战。

Method: H-NET++是一种层次化动态分块模型，通过端到端训练学习语言信息分块。关键创新包括：(1) 轻量级Transformer上下文混合器，(2) 两级潜在超先验用于文档级一致性，(3) 对正字法伪影的特殊处理，(4) 基于课程的训练。

Result: 在1.4B个波斯语词元语料库上，H-NET++取得了最先进的结果：0.159 BPB减少，比基于BPE的GPT-2-fa好12%；ParsGLUE得分提高5.4pp；对ZWNJ损坏的鲁棒性提高53%；黄金形态边界F1得分为73.8%。

Conclusion: H-NET++展示了层次化动态分块在无标记化器的MRLs中的有效性，同时保持计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 由于仇恨言论在社交媒体上的广泛存在及其对社会和个人的严重影响，需要开发更有效的自动化检测方法。传统的深度学习方法（如RNN、LSTM和CNN）在处理长期依赖性和并行化方面存在局限性。

Method: 本研究使用MetaHate数据集（包含120万条社交媒体样本）对多种最先进的Transformer模型（如BERT、RoBERTa、GPT-2和ELECTRA）进行了评估，并对ELECTRA进行了微调以提高性能。

Result: 微调后的ELECTRA在仇恨言论检测任务中取得了最高的F1分数（0.8980），但研究还发现了一些挑战，如识别讽刺、编码语言和标签噪声。

Conclusion: 本研究展示了基于Transformer的模型在仇恨言论检测中的有效性，特别是微调后的ELECTRA在MetaHate数据集上表现出色。然而，研究也揭示了讽刺、编码语言和标签噪声等挑战。
...
### [43] [REINA: Regularized Entropy Information-Based Loss for Efficient Simultaneous Speech Translation](https://arxiv.org/abs/2508.04946)
*Nameer Hirschkind,Joseph Liu,Mahesh Kumar Nandwana,Xiao Yu*

Main category: cs.LG

TL;DR: The paper introduces REINA, a novel loss function that optimizes the tradeoff between translation quality and latency in SimulST systems, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Simultaneous Speech Translation (SimulST) systems face the challenge of balancing translation quality and latency, and the authors aim to optimize this tradeoff.

Method: REINA is a novel loss to train an adaptive policy using an existing non-streaming translation model, derived from information theory principles.

Result: REINA improves the latency/quality trade-off by as much as 21% compared to prior approaches, normalized against non-streaming baseline BLEU scores. The authors achieve state-of-the-art streaming results for models of comparable size.

Conclusion: REINA helps push the reported Pareto frontier of the latency/quality tradeoff over prior works, and achieves state-of-the-art streaming results for models of comparable size.
...
TL;DR: R-Zero是一种完全自主的框架，通过让两个模型协同进化来生成和优化训练数据，从而显著提升LLM的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的训练方法仍然严重依赖于大量人工标注的任务和标签，这限制了AI系统向超越人类智能的方向发展。因此，需要一种无需人工干预的自主训练框架。

Method: R-Zero是一个完全自主的框架，从一个基础LLM开始，初始化两个具有不同角色的独立模型——Challenger和Solver。Challenger通过提出接近Solver能力边缘的任务来获得奖励，而Solver则通过解决Challenger提出的更具挑战性的任务来获得奖励，从而实现协同进化。

Result: R-Zero在不同基础LLM上显著提升了推理能力，例如将Qwen3-4B-Base在数学推理基准测试中提高了6.49，在通用领域推理基准测试中提高了7.54。

Conclusion: R-Zero能够通过自我生成和优化训练数据，显著提升不同基础LLM的推理能力，展示了其在推动AI系统超越人类智能方面的潜力。
...
### [45] [Exploring Superior Function Calls via Reinforcement Learning](https://arxiv.org/abs/2508.05118)
*Bingguang Hao,Maolin Wang,Zengzhuang Xu,Yicheng Chen,Cunyin Peng,Jinjie GU,Chenyi Zhuang*

Main category: cs.LG

TL;DR: 本文提出了一种新的强化学习框架，用于改善函数调用任务中的策略优化，通过两阶段数据准备管道和战略熵基探索方法，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前的训练方法无法开发出稳健的推理策略，监督微调模型依赖于表面模式匹配，而标准的强化学习方法在结构化函数调用的复杂动作空间中表现不佳。因此，需要一种新的强化学习框架来解决这些问题。

Method: 本文提出了一种两阶段的数据准备管道，通过迭代的LLM评估和抽象语法树验证来确保高质量的训练样本，并采用基于战略熵的探索方法进行强化学习。

Result: 实验结果表明，该框架在伯克利函数调用排行榜上实现了最先进的性能，整体准确率达到86.02%，在复杂多函数场景中比标准GRPO高出6%。此外，该方法在代码预训练模型上表现出显著的改进。

Conclusion: 本文提出了一种新的强化学习框架，通过战略性熵基探索来增强群体相对策略优化，以解决函数调用中的三个关键挑战。实验结果表明，该框架在伯克利函数调用排行榜上实现了最先进的性能，并且在代码预训练模型上表现尤为突出。
...
### [46] [Aligning LLMs on a Budget: Inference-Time Alignment with Heuristic Reward Models](https://arxiv.org/abs/2508.05165)
*Mason Nakamura,Saaduddin Mahmud,Kyle H. Wray,Hamed Zamani,Shlomo Zilberstein*

Main category: cs.LG

TL;DR: HIA is a tuning-free method that balances alignment quality and computational cost for LLMs, outperforming existing approaches in multi-objective tasks with minimal inference calls.
...
### [47] [FAITH: A Framework for Assessing Intrinsic Tabular Hallucinations in finance](https://arxiv.org/abs/2508.05201)
*Mengao Zhang,Jiayu Fu,Tanya Warrier,Yuwen Wang,Tianhui Tan,Ke-wei Huang*

Main category: cs.LG

TL;DR: 本文提出了一种评估金融大型语言模型中内在幻觉的框架，并创建了新的数据集以改进金融生成式AI系统的可靠性。


<details>
  <summary>Details</summary>
Motivation: 准确提取和精确计算表格数据对于可靠的财务分析至关重要，因为即使是最小的数值错误也会损害决策和监管合规性。现有的幻觉基准很少捕捉到金融应用的独特需求。

Method: 本文开发了一个严格且可扩展的框架，用于评估金融LLMs中的内在幻觉，将其概念化为在现实世界金融文档上的上下文感知掩码跨度预测任务。

Result: 本文的主要贡献包括：(1) 使用掩码策略的新型自动化数据集创建范式；(2) 来自标准普尔500年报告的新幻觉评估数据集；以及(3) 对最先进的LLMs在金融表格数据上的内在幻觉模式的全面评估。

Conclusion: 本文提出了一个稳健的方法论，用于内部LLM评估，并为构建更值得信赖和可靠的金融生成式AI系统迈出了关键一步。
...
### [48] [Fairy$\pm i$: the First 2-bit Complex LLM with All Parameters in $\{\pm1, \pm i\}$](https://arxiv.org/abs/2508.05571)
*Feiyu Wang,Guoan Wang,Yihao Zhang,Shengfan Wang,Weitao Li,Bokai Huang,Shimao Chen,Zihan Jiang,Rui Xu,Tong Yang*

Main category: cs.LG

TL;DR: 本文提出了一种新的2比特量化框架Fairy±i，通过提高全精度模型的上限并实现高效的量化，从而突破了现有2比特量化方法的性能天花板。该方法在保持存储和计算效率的同时，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有的QAT研究主要集中在最小化全精度模型上的量化误差，其中全精度准确率作为性能上限（准确率天花板）。没有现有方法尝试超越这一天花板。为了打破这一限制，本文提出了一种新的范式：提高全精度模型的上限，然后将其高效地量化为2比特。

Method: 本文提出了Fairy±i，这是第一个针对复数值大语言模型的2比特量化框架。该方法利用复数域的表示优势来提升全精度模型的准确性，并将权重映射到单位根{±1, ±i}，形成一个完美对称且信息理论最优的2比特表示。此外，每个量化权重要么具有零实部，要么具有零虚部，从而实现了仅使用加法和元素交换的乘法自由推理。

Result: 实验结果表明，Fairy±i在PPL和下游任务方面均优于现有2比特量化方法的天花板，同时保持严格的存储和计算效率。

Conclusion: 本文提出了一种新的量化感知训练范式，通过提高全精度模型的上限，然后将其高效地量化为2比特，从而突破了现有2比特量化方法的性能天花板。该方法在保持存储和计算效率的同时，在PPL和下游任务中均优于现有方法，为在极低比特约束下构建高精度和实用的大语言模型开辟了新方向。
...
### [49] [Iterative Learning of Computable Phenotypes for Treatment Resistant Hypertension using Large Language Models](https://arxiv.org/abs/2508.05581)
...
<details>
  <summary>Details</summary>
Motivation: The potential of LLMs for generating interpretable computable phenotypes (CPs) is under-explored, despite their demonstrated capabilities in medical question answering and programming. This work aims to investigate whether LLMs can generate accurate and concise CPs for clinical phenotypes to enable scalable clinical decision support.
...
Conclusion: LLMs, when combined with iterative learning, can generate interpretable and reasonably accurate programs that approach the performance of state-of-the-art ML methods while requiring significantly fewer training examples.
...
### [50] [Understanding and Mitigating Errors of LLM-Generated RTL Code](https://arxiv.org/abs/2508.05266)
*Jiazheng Zhang,Cheng Liu,Huawei Li*

Main category: cs.AR

TL;DR: 本文分析了基于大语言模型的寄存器传输级代码生成中的错误原因，并提出了一系列针对性的错误修正技术，显著提高了性能。


<details>
  <summary>Details</summary>
Motivation: 尽管基于大语言模型（LLM）的寄存器传输级（RTL）代码生成具有前景，但整体成功率仍然不令人满意。错误源于各种因素，对具体失败原因的有限理解阻碍了改进。

Method: 我们提出了针对错误修正的技术，包括构建领域特定的知识库、采用检索增强生成（RAG）来提供必要的RTL知识、引入设计描述规则并实施规则检查机制、集成外部工具将输入转换为LLM兼容的元格式，以及采用迭代调试循环（仿真-错误定位-修正）。

Result: 将这些技术整合到一个LLM框架中显著提高了性能。实验结果表明，我们的增强框架在VerilogEval基准测试中实现了91.0%的准确率，比基线代码生成方法提高了32.7%。

Conclusion: 我们的增强框架在VerilogEval基准测试中实现了91.0%的准确率，比基线代码生成方法提高了32.7%，证明了我们方法的有效性。
...
### [51] [Speech LLMs in Low-Resource Scenarios: Data Volume Requirements and the Impact of Pretraining on High-Resource Languages](https://arxiv.org/abs/2508.05149)
*Seraphina Fong,Marco Matassoni,Alessio Brutti*

Main category: eess.AS

TL;DR: 本文研究了在低资源环境下使用Speech LLMs进行自动语音识别的潜力，提出了一种新的框架并验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在高资源语言中表现出色，但在低资源环境下的应用仍需探索。本文旨在研究如何利用Speech LLMs来解决低资源语音识别中的数据稀缺问题。

Method: 本文采用了SLAM-ASR框架，其中包含一个可训练的轻量级投影器，连接语音编码器和大语言模型。通过评估不同训练数据量和预训练模型的效果，验证了该方法的有效性。

Result: 实验结果表明，使用预训练的单语或多语投影器可以显著减少数据稀缺的影响，尤其是在小规模训练集的情况下。此外，结合多语种LLMs（如EuroLLM和Salamandra）提升了性能。

Conclusion: 本文研究了在低资源环境下使用语音大语言模型（Speech LLMs）进行自动语音识别的潜力，并提出了SLAM-ASR框架。实验表明，通过预训练的单语或多语投影器可以缓解数据稀缺问题，同时使用多语种LLMs（如EuroLLM和Salamandra）能够提升性能。
...
### [52] [Prescriptive Agents based on Rag for Automated Maintenance (PARAM)](https://arxiv.org/abs/2508.04714)
*Chitranshu Harbola,Anupam Purwar*

Main category: cs.AI

TL;DR: 本文提出了一种基于大型语言模型的智能系统，用于预测性维护，通过结合轴承振动频率分析和多智能体生成，提供可操作的维护建议。
...
Method: 该系统结合了轴承振动频率分析和多智能体生成，将轴承振动数据（BPFO、BPFI、BSF、FTF频率）序列化为自然语言以供大型语言模型处理，同时利用向量嵌入和语义搜索处理维护手册，并进行网络搜索以获取全面的程序知识和最新的维护实践。

Result: 实验验证表明，该系统在轴承振动数据集中实现了有效的异常检测和上下文相关的维护指导，成功弥合了状态监测与可操作维护计划之间的差距。

Conclusion: 该工作推进了大型语言模型在工业维护中的应用，为各种机械部件和工业领域提供了可扩展的预测性维护框架。
...
### [53] [Fine-Tuning Small Language Models (SLMs) for Autonomous Web-based Geographical Information Systems (AWebGIS)](https://arxiv.org/abs/2508.04846)
...
TL;DR: 该研究比较了三种实现AWebGIS的方法，并发现基于小型语言模型的客户端方法在准确性方面表现最佳，同时减少了对服务器的依赖。


<details>
  <summary>Details</summary>
Motivation: 当前大多数解决方案依赖于云基础大型语言模型（LLMs），这需要持续的互联网访问，并由于集中式服务器处理而引发用户的隐私和可扩展性问题。

Method: 该研究比较了三种方法：(1) 使用云基础大型语言模型（如Cohere）的完全自动化在线方法；(2) 使用经典机器学习分类器（如支持向量机和随机森林）的半自动化离线方法；(3) 基于微调的小型语言模型（SLM）的完全自主离线（客户端）方法，具体为T5-small模型，在客户端的网络浏览器中执行。

Result: 第三种方法，即利用SLMs的方法，在所有方法中达到了最高的准确性，包括精确匹配准确率为0.93，Levenshtein相似度为0.99，以及用于摘要评估的ROUGE-1和ROUGE-L分数分别为0.98。此外，这种客户端计算策略通过将处理任务卸载到用户设备上，减少了后端服务器的负载，消除了对基于服务器的推理的需求。
...
### [54] [ConfAgents: A Conformal-Guided Multi-Agent Framework for Cost-Efficient Medical Diagnosis](https://arxiv.org/abs/2508.04915)
*Huiya Zhao,Yinghao Zhu,Zixiang Wang,Yasha Wang,Junyi Gao,Liantao Ma*

Main category: cs.AI

TL;DR: 本文介绍了HealthFlow，一种自进化的AI代理，通过新颖的元级进化机制克服了传统AI代理在医疗研究中的局限性。实验表明，HealthFlow的自进化方法在复杂、现实的健康数据分析任务中显著优于最先进的代理框架。


<details>
  <summary>Details</summary>
Motivation: 传统AI代理在医疗研究中依赖静态、预定义的策略，导致它们无法成为更好的战略规划者，而这是复杂领域如医疗保健的关键技能。因此，需要一种能够自我进化的AI代理来解决这一问题。

Method: 本文引入了HealthFlow，一种自进化的AI代理，通过新颖的元级进化机制来克服传统AI代理的局限性。HealthFlow通过将程序成功和失败提炼成持久的战略知识库，自主优化其高层问题解决策略。此外，还引入了EHRFlowBench，一个新的基准测试，用于评估HealthFlow的表现。

Result: 实验结果表明，HealthFlow的自进化方法在复杂、现实的健康数据分析任务中显著优于最先进的代理框架。

Conclusion: 本文提出了一种自进化的AI代理HealthFlow，通过新颖的元级进化机制克服了传统AI代理在医疗研究中的局限性。实验表明，HealthFlow的自进化方法显著优于最先进的代理框架。这项工作标志着从构建更好的工具使用者转向设计更智能、自进化的任务管理者的重要转变，为科学发现中的更自主和有效的AI铺平了道路。
...
### [55] [Can Large Language Models Integrate Spatial Data? Empirical Insights into Reasoning Strengths and Computational Weaknesses](https://arxiv.org/abs/2508.05009)
*Bin Han,Robert Wolfe,Anat Caspi,Bill Howe*

Main category: cs.AI

TL;DR: 本研究探讨了大型语言模型（LLMs）在增强领域专家整合大规模、异构和噪声城市空间数据集方面的应用。我们的分析显示，LLMs在空间推理方面表现出色，但在将宏观环境与计算几何任务联系起来时存在困难。然而，当提供相关特征时，LLMs能够生成高性能的结果。我们还适应了一种审查和修正的方法，这在纠正错误的初始响应方面表现出色。我们的研究结果表明，LLMs可以作为传统基于规则的启发式方法的有前途且灵活的替代方案，推动自适应空间数据集成的能力。


<details>
  <summary>Details</summary>
Motivation: 传统的基于规则的集成方法无法涵盖所有边缘情况，需要人工验证和修复。机器学习方法需要收集和标记大量任务特定的样本。因此，我们需要一种更有效的方法来处理大规模、异构和噪声的城市空间数据集。

Method: 我们首先考虑了LLMs如何通过人类经验来推理环境空间关系，例如道路和人行道之间的关系。然后我们适应了一种审查和修正的方法，这在纠正错误的初始响应方面表现出色。

Result: 虽然LLMs表现出空间推理能力，但它们难以将宏观环境与相关的计算几何任务联系起来，经常产生逻辑不连贯的响应。但是当提供相关特征时，LLMs能够生成高性能的结果。

Conclusion: 我们的研究结果表明，LLMs可以作为传统基于规则的启发式方法的有前途且灵活的替代方案，推动自适应空间数据集成的能力。
...
### [56] [Cognitive Duality for Adaptive Web Agents](https://arxiv.org/abs/2508.05081)
*Jiarun Liu,Chunhong Zhang,Zheng Hu*

Main category: cs.AI

TL;DR: 本文提出了一种基于双过程理论的网络代理框架CogniWeb，该框架能够在快速直觉处理和有意识推理之间自适应切换，从而在保持高效率的同时实现良好的性能。


<details>
  <summary>Details</summary>
Motivation: 网络导航是评估人工通用智能（AGI）的关键且具有挑战性的领域，需要在高熵、动态环境中进行复杂的决策，并且动作空间呈组合爆炸式增长。目前构建自主网络代理的方法要么专注于离线模仿学习，要么专注于在线探索，但很少能有效地结合这两种范式。

Method: 受人类认知的双过程理论启发，我们推导出快速系统1和慢速系统2认知过程的原则性分解。这种分解提供了一个统一的视角，将现有的网络代理方法联系起来，弥合了离线学习直觉反应行为和在线获取推理规划能力之间的差距。我们将这个框架实现为CogniWeb，这是一种模块化的代理架构，可以根据任务复杂性在快速直觉处理和有意识推理之间自适应切换。

Result: CogniWeb在WebArena上的评估表明，它在保持显著更高的效率（令牌使用量减少75%）的同时，实现了具有竞争力的性能（43.96%的成功率）。

Conclusion: CogniWeb在WebArena上的评估表明，它在保持显著更高的效率（令牌使用量减少75%）的同时，实现了具有竞争力的性能（43.96%的成功率）。
...
### [57] [QA-Dragon: Query-Aware Dynamic RAG System for Knowledge-Intensive Visual Question Answering](https://arxiv.org/abs/2508.05197)
*Zhuohang Jiang,Pangjing Wu,Xu Yuan,Wenqi Fan,Qing Li*

Main category: cs.AI

TL;DR: 本文提出了QA-Dragon，一个查询感知的动态RAG系统，用于知识密集型视觉问答任务。该系统通过结合文本和图像搜索代理，实现了多模态、多轮和多跳推理，从而显著提升了答案准确性和知识重叠分数。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG方法通常单独从文本或图像中检索，限制了它们解决需要多跳推理或多源事实知识的复杂查询的能力。为了克服这一限制，我们提出了QA-Dragon，以提高知识密集型VQA的性能。

Method: 我们提出了QA-Dragon，这是一个查询感知的动态RAG系统，用于知识密集型VQA。具体来说，QA-Dragon引入了一个领域路由器来识别查询的主题领域以进行领域特定推理，以及一个搜索路由器来动态选择最佳检索策略。通过在混合设置中协调文本和图像搜索代理，我们的系统支持多模态、多轮和多跳推理，使其能够有效地处理复杂的VQA任务。

Result: 我们在KDD Cup 2025的Meta CRAG-MM挑战赛上评估了我们的QA-Dragon，结果表明它在具有挑战性的场景下显著提高了基础模型的推理性能。

Conclusion: 我们的框架在答案准确性和知识重叠分数上都取得了显著提升，在单源任务、多源任务和多轮任务中分别优于基线5.06%、6.35%和5.03%。
...
### [58] [A Novel Architecture for Symbolic Reasoning with Decision Trees and LLM Agents](https://arxiv.org/abs/2508.05311)
*Andrew Kiruluta*

Main category: cs.AI

TL;DR: 本文提出一种混合架构，结合决策树的符号推理和大型语言模型的生成能力，在多个基准测试中取得了显著提升，并展示了其在临床决策支持和科学发现中的应用。


<details>
  <summary>Details</summary>
Motivation: 现有方法将符号和神经模块松散耦合，而本文设计将决策树和随机森林作为可调用的预言机嵌入统一的推理系统中。
...
Conclusion: 该架构提供了一种稳健、可解释且可扩展的通用神经符号推理解决方案。
...
### [59] [Bench-2-CoP: Can We Trust Benchmarking for EU AI Compliance?](https://arxiv.org/abs/2508.05464)
*Matteo Prandi,Vincenzo Suriani,Federico Pierucci,Marcello Galisai,Daniele Nardi,Piercosma Bisconti*

Main category: cs.AI

TL;DR: 本研究揭示了现有AI评估基准与欧盟AI法案之间存在重大差距，特别是在系统性风险评估方面。


<details>
  <summary>Details</summary>
Motivation: 当前的AI评估实践严重依赖于已建立的基准测试，但这些工具并未设计用于衡量新监管环境关注的系统性风险。因此需要量化这种“基准-监管差距”。

Method: 引入了Bench-2-CoP框架，利用经过验证的LLM-as-judge分析，将广泛使用的基准测试中的194,955个问题映射到欧盟AI法案的模型能力分类法和倾向性。

Result: 评估生态系统主要集中在少数行为倾向上，如“幻觉倾向”（占整个语料库的53.7%）和“歧视性偏见”（28.9%），而关键功能能力被严重忽视。对于失控场景相关的功能能力，如规避人类监督、自我复制和自主AI开发，在整个基准语料库中完全没有覆盖。

Conclusion: 本研究提供了对这一差距的首次全面定量分析，为政策制定者优化CoP和开发者构建下一代评估工具提供了关键见解，最终促进更安全、合规的AI。
...
### [60] [Can Large Language Models Generate Effective Datasets for Emotion Recognition in Conversations?](https://arxiv.org/abs/2508.05474)
*Burak Can Kaplan,Hugo Cesar De Castro Carneiro,Stefan Wermter*

Main category: cs.AI

TL;DR: 本文提出了一种利用小型、资源高效且通用的LLM来合成ERC数据集的方法，以解决ERC数据稀缺和现有数据集的挑战。生成的数据集在ERC分类任务中表现出色，并有助于分析标签不平衡的影响。


<details>
  <summary>Details</summary>
Motivation: ERC数据仍然稀缺，现有数据集由于其高度偏倚的来源和软标签的固有主观性而面临诸多挑战。尽管大型语言模型（LLMs）在许多情感任务中表现出色，但它们通常训练成本高昂，且在ERC任务中的应用，特别是在数据生成方面，仍有限。

Method: 我们使用一个小型、资源高效且通用的LLM来合成具有多样特性的ERC数据集，补充了三个最常用的ERC基准测试。我们生成了六个新的数据集，其中两个专门用于增强每个基准测试。

Result: 实验结果表明，在生成的数据集上训练的ERC分类器模型表现出强大的鲁棒性，并在现有的ERC基准测试中 consistently 实现了统计上显著的性能提升。

Conclusion: 实验结果表明，在生成的数据集上训练的ERC分类器模型表现出强大的鲁棒性，并在现有的ERC基准测试中 consistently 实现了统计上显著的性能提升。
...
### [61] [A Study of the Framework and Real-World Applications of Language Embedding for 3D Scene Understanding](https://arxiv.org/abs/2508.05064)
*Mahmoud Chick Zaouali,Todd Charter,Yehor Karpichev,Brandon Haworth,Homayoun Najjjaran*

Main category: cs.GR

TL;DR: 本文综述了将语言指导与3D高斯散射结合的研究，指出了关键限制并提出了未来方向。


<details>
  <summary>Details</summary>
Motivation: 由于Gaussian Splatting在实时3D场景表示中的快速发展，以及大型语言模型和语言嵌入的集成带来的新可能性，需要一个全面的概述来推动语言引导的3D场景理解。

Method: 本文提供了对当前研究的结构化回顾，详细描述了理论基础、集成策略和实际用例。

Result: 本文详细描述了理论基础、集成策略和实际用例，并指出了计算瓶颈、泛化能力和语义标注3D高斯数据稀缺等关键限制。

Conclusion: 本文综述了将语言指导与3D高斯散射结合的当前研究努力，指出了关键限制，并提出了未来方向。
...
### [62] [Federal Reserve Communication and the COVID-19 Pandemic](https://arxiv.org/abs/2508.04830)
*Jonathan Benchimol,Sophia Kazinnik,Yossi Saadon*

Main category: econ.GN

TL;DR: 本研究分析了美联储在疫情期间的沟通策略，并将其与之前的经济压力时期进行比较。研究发现，美联储在疫情期间的沟通更加关注金融稳定、市场波动、社会福利和非常规货币政策，并且这种沟通方式已成为美联储的'新常态'。


<details>
  <summary>Details</summary>
Motivation: 研究旨在分析美联储在疫情期间的沟通策略，并将其与之前经济压力时期的沟通策略进行比较，以了解中央银行在危机期间如何调整其沟通策略。

Method: 研究使用了专门针对新冠疫情、非常规货币政策（UMP）和金融稳定的词典，结合情感分析和主题建模技术，对美联储在疫情期间的沟通策略进行了分析，并将其与之前的经济压力时期进行比较。

Result: 研究发现，美联储在疫情期间的沟通更加关注金融稳定、市场波动、社会福利和非常规货币政策，且这种沟通方式已成为美联储的'新常态'。此外，利率声明和会议纪要中与金融稳定相关的负面情绪预示了随后的宽松货币政策决策。

Conclusion: 研究发现，美联储在疫情时期的沟通策略更加关注金融稳定、市场波动、社会福利和非常规货币政策，并且这种沟通方式已成为美联储的'新常态'。这些发现有助于理解中央银行在危机期间如何演变其沟通策略以及如何适应特殊的经济情况。
...
### [63] [Navigating Through Paper Flood: Advancing LLM-based Paper Evaluation through Domain-Aware Retrieval and Latent Reasoning](https://arxiv.org/abs/2508.05129)
*Wuqiang Zheng,Yiyan Xu,Xinyu Lin,Chongming Gao,Wenjie Wang,Fuli Feng*

Main category: cs.IR

TL;DR: PaperEval是一种基于大型语言模型的自动论文评估框架，通过领域感知的论文检索和潜在推理机制来提高评估准确性，并在实际应用中取得了成功。


<details>
  <summary>Details</summary>
Motivation: 随着学术出版物的快速增长，识别高质量的研究变得越来越紧迫。现有的基于大型语言模型的自动论文评估方法受到过时领域知识和有限推理能力的限制。

Method: PaperEval是一个基于大型语言模型的框架，包含两个关键组件：领域感知的论文检索模块和潜在推理机制，并引入了一种渐进式排名优化策略。

Result: 实验表明，PaperEval在两个数据集上均优于现有方法，并在实际应用中取得了成功。

Conclusion: PaperEval在学术影响和论文质量评估方面均优于现有方法，并在实际应用中展示了其有效性。
...
### [64] [JPS: Jailbreak Multimodal Large Language Models with Collaborative Visual Perturbation and Textual Steering](https://arxiv.org/abs/2508.05087)
...
TL;DR: 本文提出了一种名为JPS的新型越狱攻击方法，通过协作视觉扰动和文本引导提示实现对MLLM的越狱攻击，显著提高了攻击成功率和恶意意图满足率。


<details>
  <summary>Details</summary>
Motivation: 当前研究主要关注最大化攻击成功率（ASR），而忽略了生成的响应是否实际上满足攻击者的恶意意图，这导致低质量的输出，这些输出虽然可以绕过安全过滤器，但缺乏实质性有害内容。

Method: JPS通过协作视觉扰动和文本引导提示实现对MLLM的越狱攻击，利用目标引导的对抗性图像扰动进行有效的安全绕过，并通过多智能体系统优化的“引导提示”来具体引导LLM响应以满足攻击者的意图。

Result: JPS在多个MLLM和基准测试中实现了ASR和MIFR的新最先进水平，分析证实了其有效性。

Conclusion: JPS在多个MLLM和基准测试中实现了ASR和MIFR的新最先进水平，分析证实了其有效性。
...
### [65] [MELLA: Bridging Linguistic Capability and Cultural Groundedness for Low-Resource Language MLLMs](https://arxiv.org/abs/2508.05502)
*Yufei Gao,Jiaying Fei,Nuo Chen,Ruirui Chen,Guohang Yan,Yunshi Lan,Botian Shi*

Main category: cs.CV

TL;DR: 本文提出了一个双源策略，通过收集特定数据来提升多模态大语言模型在低资源语言环境下的表现，实验结果表明这种方法有效。


<details>
  <summary>Details</summary>
Motivation: 当前的多语言增强方法通常仅限于文本模态或依赖机器翻译，这导致模型无法充分获取多模态信息和文化背景，而这对于有效服务低资源语言用户至关重要。

Method: 我们提出了一种双源策略，通过收集针对每个目标的数据来实现这两个目标，即从本地网络的替代文本中获取文化信息，并利用多模态大语言模型生成的描述来获取语言能力。

Result: 实验结果表明，在MELLA上微调后，各种多模态大语言模型在八种语言上的表现均有显著提升，模型能够生成“厚描述”。
...
### [66] [Uni-cot: Towards Unified Chain-of-Thought Reasoning Across Text and Vision](https://arxiv.org/abs/2508.05606)
*Luozheng Qin,Jia Gong,Yuqing Sun,Tianjiao Li,Mengping Yang,Xiaomeng Yang,Chao Qu,Zhiyu Tan,Hao Li*

Main category: cs.CV

TL;DR: Uni-CoT is a unified chain-of-thought framework that enables coherent and grounded multimodal reasoning within a single model, achieving SOTA performance on various benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with interpreting transitions of visual states due to limited capacity of modeling visual state transitions or incoherent visual trajectories caused by fragmented architectures.

Method: Uni-CoT introduces a novel two-level reasoning paradigm: A Macro-Level CoT for high-level task planning and A Micro-Level CoT for subtask execution. It also introduces a structured training paradigm combining interleaved image-text supervision for macro-level CoT with multi-task objectives for micro-level CoT.

Result: Uni-CoT achieves SOTA performance on reasoning-driven image generation benchmark (WISE) and editing benchmarks (RISE and KRIS), demonstrating scalable and coherent multi-modal reasoning.
...
TL;DR: 本文提出了一种新的测试时缩放方法GUI-RC和测试时强化学习方法GUI-RCPO，用于改进GUI接地任务的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的方法通过广泛的监督训练或带有标记奖励的强化学习实现强大的性能，但它们受到像素级注释的成本和可用性的限制。

Method: 我们提出了GUI-RC（区域一致性），一种测试时缩放方法，从多个采样预测中构建空间投票网格以识别模型显示最高一致性的共识区域。此外，我们引入了GUI-RCPO（区域一致性策略优化），将其转化为测试时强化学习的奖励。

Result: GUI-RC在ScreenSpot基准上提高了各种架构的准确性2-3%。GUI-RCPO进一步通过自监督优化提高了Qwen2.5-VL-3B-Instruct的准确性。

Conclusion: 我们的方法揭示了测试时缩放和测试时强化学习在GUI接地中的未开发潜力，为更鲁棒和数据高效的GUI代理提供了有希望的路径。
...
TL;DR: SPGISpeech 2.0是一个适用于金融领域说话人标记转录的数据集，包含3,780小时的专业转录收益电话，可用于端到端自动语音识别（ASR）任务。


<details>
  <summary>Details</summary>
Motivation: 为了提高适用建模任务的多样性，同时保持原始SPGISpeech数据集的核心特征，即音频片段及其对应的完整格式文本转录，适用于端到端自动语音识别（ASR）。

Method: SPGISpeech 2.0包含了每个音频片段的通话和说话人信息，以支持多说话人ASR。通过在SPGISpeech 2.0上微调，验证了其在提升流行语音识别模型的说话人标记ASR性能方面的有效性。

Result: SPGISpeech 2.0经过验证，能够提高流行语音识别模型在说话人标记ASR任务上的性能。

Conclusion: SPGISpeech 2.0免费用于非商业用途，预计将进一步推动语音识别技术的发展，并激发广泛的研究应用。
...
### [69] [Mixed-Initiative Dialog for Human-Robot Collaborative Manipulation](https://arxiv.org/abs/2508.05535)
*Albert Yu,Chengshu Li,Luca Macesanu,Arnav Balaji,Ruchira Ray,Raymond Mooney,Roberto Martín-Martín*

Main category: cs.RO

TL;DR: 本文提出了一种名为MICoBot的系统，用于长期人机协作。该系统通过混合主动性对话范式，让人类和机器人能够共同制定、接受或拒绝任务中的提议。MICoBot在三个层次上做出决策，包括高层合作策略、任务分配和低级动作执行。实验表明，该方法在任务成功率和用户体验方面优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 有效的机器人系统需要适应各种人类合作伙伴，他们的身体行为、协助意愿和对机器人能力的理解可能会随时间变化。这需要一个紧密耦合的通信循环，使两个代理能够灵活地提出、接受或拒绝请求以有效地完成任务。

Method: 我们应用了混合主动性对话范式来协作的人机团队，并提出了MICoBot系统，该系统处理双方使用自然语言主动制定、接受或拒绝提议的情况。MICoBot在三个层次上做出决策：(1) 一个元规划器考虑人类对话以形成和编码高层次的合作策略，(2) 一个规划器根据机器人的能力（通过模拟预训练的效用模型测量）和人类帮助的估计可用性，最优地将剩余步骤分配给任一代理，(3) 一个动作执行器决定执行低级动作或对人类说的话。

Result: 我们在模拟和现实世界中进行了广泛的评估——在物理机器人上，有18个独特的用户参与，持续27小时——证明了我们的方法有效与多样化的人类用户合作，任务成功率和用户体验显著优于纯LLM基线和其他代理分配模型。
...
### [70] [Posterior-GRPO: Rewarding Reasoning Processes in Code Generation](https://arxiv.org/abs/2508.05170)
*Lishui Fan,Yu Zhang,Mouxiang Chen,Zhongxin Liu*

Main category: cs.SE

TL;DR: 本文提出了一种新的强化学习框架，通过评估推理过程的质量来提升代码生成的效果，取得了显著的成果。


<details>
  <summary>Details</summary>
Motivation: 当前的强化学习方法依赖于基于结果的奖励，忽略了中间推理过程的质量。直接监督推理过程容易受到奖励黑客的影响，因此需要一种更有效的解决方案。

Method: 本文提出了LCB-RB基准、OD-based方法和Posterior-GRPO (P-GRPO) 强化学习方法，以评估和提升推理过程的质量。

Result: 使用7B参数的奖励模型在LCB-RB基准上取得了最先进的性能，并在其他基准上表现出良好的泛化能力。P-GRPO方法在多个代码生成任务中优于仅基于结果的基线，并与GPT-4-Turbo相当。

Conclusion: 本文提出了一种统一的框架，可以有效地在强化学习中融入推理过程的质量。通过引入LCB-RB基准和OD-based方法，以及P-GRPO强化学习方法，显著提升了代码生成的效果，并在多个任务中表现出色。
...
### [71] [Making Prompts First-Class Citizens for Adaptive LLM Pipelines](https://arxiv.org/abs/2508.05012)
*Ugur Cetintemel,Shu Chen,Alexander W. Lee,Deepti Raghavan*

Main category: cs.DB

TL;DR: SPEAR 是一种新的语言和运行时系统，旨在解决现代 LLM 流水线中提示管理的问题，通过结构化和自适应的提示管理提高系统的性能和灵活性。


<details>
  <summary>Details</summary>
Motivation: 现代 LLM 流水线越来越像数据导向的系统，但提示仍然是一个脆弱、不透明的字符串，与周围的数据显示脱节。这种脱节限制了重用、优化和运行时控制。

Method: SPEAR 是一种语言和运行时系统，它通过将提示结构化、自适应，并将其作为执行模型中的第一类组件来填补提示管理的空白。它定义了一个提示代数，用于指导提示在管道中的构建和适应，并支持多种精炼模式（手动、辅助和自动）。

Result: SPEAR 通过运行时提示精炼和结构化提示管理，提高了系统的灵活性和效率。初步实验量化了不同精炼模式与静态提示和代理重试的比较，以及提示级优化（如操作符融合）的影响。

Conclusion: SPEAR 提供了一种结构化、自适应的提示管理方法，使提示成为执行模型中的第一类组件，从而提高系统的可重用性、优化能力和运行时控制。
