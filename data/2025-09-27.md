...
Motivation: 公共情绪在外交中起着关键作用，良好的情绪为政策实施提供重要支持，有助于解决国际问题，并塑造国家的国际形象。传统的衡量公众情绪的方法，如大规模调查或人工内容分析，通常耗时、费力且缺乏前瞻性分析的能力。

Method: 我们训练了一个语言模型来预测公众对外交事件的反应，并构建了一个包含外交事件描述及其相关公众讨论的数据集。然后，我们根据传播理论并与领域专家合作，预先确定了几种文本特征进行修改，确保任何更改改变了事件的叙述框架，同时保留了其核心事实。我们开发了一种反事实生成算法，利用大型语言模型系统地生成原始文本的修改版本。

Result: 结果表明，该框架成功地将公众情绪转向更积极的状态，成功率高达70%。

Conclusion: 该框架可以作为外交官、政策制定者和传播专家的实用工具，提供数据驱动的见解，以如何构架外交举措或报道事件来促进更理想的公众情绪。
...
<details>
  <summary>Details</summary>
Motivation: 跨语言语音情感识别（SER）仍然是一个具有挑战性的任务，因为不同语言中的语音可变性和说话人特定的表达风格存在差异。在如此多样的条件下有效捕捉情感需要一个能够跨不同说话人和语言对齐情感外化的框架。

Method: 我们提出了一种说话人风格感知的音素锚定框架，该框架在语音和说话人层面对情感表达进行对齐。通过基于图的聚类构建特定于情感的说话人社区以捕捉共享的说话人特征，并在说话人和语音空间中应用双空间锚定以实现更好的跨语言情感迁移。

Result: 在MSP-Podcast（英语）和BIIC-Podcast（台湾普通话）语料库上的评估表明，我们的方法在跨语言情感识别任务中优于竞争基线，并提供了关于跨语言情感表示共性的有价值见解。

Conclusion: 我们的方法在MSP-Podcast（英语）和BIIC-Podcast（台湾普通话）语料库上的评估表明，它在跨语言情感识别任务中优于竞争基线，并提供了关于跨语言情感表示共性的有价值见解。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（如ChatGPT）的迅速采用，人类和AI生成的文本之间的界限变得模糊，引发了关于学术诚信、知识产权和虚假信息传播的紧迫问题。因此，需要可靠的AI文本检测来确保公平评估，保护人类的真实性，并培养对数字通信的信任。

Method: 本文研究了当前机器学习方法在区分ChatGPT-3.5生成的文本和人类撰写的文本方面的效果，测试并比较了经典方法（逻辑回归结合传统词袋、词性标注和TF-IDF特征）和基于变压器的方法（BERT增强的N-gram，DistilBERT，BERT自定义轻量级分类器，以及基于LSTM的N-gram模型）。

Result: 结果表明，DistilBERT表现最佳，而逻辑回归和BERT-Custom提供了稳健且平衡的替代方案；LSTM和BERT-N-gram方法则落后。三个最佳模型的最大投票集成未能超越DistilBERT本身，突显了单一基于变压器的表示优于单纯的模型多样性。

Conclusion: 本文通过全面评估这些AI文本检测方法的优缺点，为构建更强大的Transformer框架奠定了基础，以跟上不断改进的生成式AI模型的步伐。
...
<details>
  <summary>Details</summary>
Motivation: SAE features do not inherently align with human-understandable concepts, making their interpretation cumbersome and labor-intensive. To bridge the gap between SAE features and human concepts, we present ConceptViz, a visual analytics system designed for exploring concepts in LLMs.
...
TL;DR: 本文提出了一种细粒度情感建模框架Emo-FiLM，用于基于大语言模型的文本到语音系统，能够实现单词级情感控制，并在实验中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的情感文本到语音系统通常依赖于句子级别的控制，无法捕捉句子内部的动态变化。因此，需要一种更细粒度的情感建模框架来提高自然和可信的人机交互。

Method: Emo-FiLM是一种基于大语言模型的文本到语音系统，它通过将情绪2vec的帧级特征与单词对齐，获得单词级情绪注释，并通过特征线性调制（FiLM）层进行映射，从而实现单词级情绪控制。

Result: Emo-FiLM在全局和细粒度任务上均优于现有方法，证明了其在富有表现力的语音合成中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的方法主要集中在如何利用LLM的摘要和分析能力，而忽略了训练问题。

Method: 我们设计了一个基于LLM的偏好优化（PO）数据集构建策略用于RL训练，并提出了一种自我增强策略（SES）在推理阶段进一步挖掘从RL训练中获得的对话推荐潜力。

Result: 我们的方法在各种数据集上表现出色，优于之前最先进的方法。

Conclusion: 我们的方法在各种数据集上的实验表明，它始终优于之前最先进的方法。
...
Method: 我们引入了 Conformal Importance Summarization，这是第一个用于保留重要性的摘要生成框架，使用符合预测来提供严格的、分布无关的覆盖保证。通过校准句子级重要性评分的阈值，我们实现了具有用户指定覆盖和召回率的抽取式文档摘要。

Result: 实验表明，Conformal Importance Summarization 实现了理论上保证的信息覆盖率。

Conclusion: 我们的工作表明，Conformal Importance Summarization 可以与现有技术结合，实现可靠、可控的自动摘要，为在关键应用中安全部署 AI 摘要工具铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 短格式视频平台如TikTok由于其多模态、动态和噪声内容，给虚假信息检测带来了独特的挑战。

Method: ShortCheck集成了语音转录、OCR、物体和深度伪造检测、视频到文本摘要以及声明验证。

Result: 在两个手动标注的数据集上评估，ShortCheck在多语言环境下取得了F1加权分数超过70%的有希望的结果。

Conclusion: ShortCheck是一个模块化、仅推理的管道，能够自动识别值得检查的短视频，帮助人工事实核查。
...
Method: MARS (Multi-Agent Review System), a role-based collaboration framework inspired by the review process, where an author agent generates an initial solution, reviewer agents provide decisions and comments independently, and a meta-reviewer integrates the feedback to make the final decision and guide further revision.
...
<details>
  <summary>Details</summary>
Motivation: 尽管近年来机器翻译取得了重大进展，但许多低资源语言的进步仍然有限，因为缺乏大规模的训练数据和语言资源。本文旨在解决这一问题，提供一个可用于研究的资源。

Method: 本文通过使用母语者进行严格的注释过程，构建了包含错误跨度、错误类型和错误严重性注释的数据集。

Result: 本文提出了SiniticMTError数据集，并通过分析注释者之间的一致性、迭代反馈以及错误类型和严重性的模式来验证其有效性。

Conclusion: 本文介绍了SiniticMTError数据集，该数据集为机器翻译社区提供了用于微调具有错误检测能力的模型的资源，并支持翻译质量估计、误差感知生成和低资源语言评估的研究。
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of automatic disease diagnosis from clinical text in multilingual healthcare environments, where there is a scarcity of annotated medical data in low-resource languages and linguistic variability across populations.

Method: SwasthLLM is a unified, zero-shot, cross-lingual, and multi-task learning framework for medical diagnosis that uses a multilingual XLM-RoBERTa encoder with a language-aware attention mechanism and a disease classification head. It also incorporates a Siamese contrastive learning module, a translation consistency module, and a contrastive projection head. The model is trained using a multi-task learning strategy and MAML for rapid adaptation.
...
<details>
  <summary>Details</summary>
Motivation: 当前的变压器架构对所有输入应用相同的处理深度，导致效率低下并限制了推理质量。简单事实查询与复杂逻辑问题一样受到多层计算的影响，浪费资源并限制深度推理。

Method: DS-MoE 是一种基于深度专业化的专家混合（Mixture of Experts）框架，引入了针对不同推理深度的专家模块，并通过学习的路由网络动态组装定制的推理链。

Result: 实验结果表明，DS-MoE 在计算节省和推理速度方面优于统一深度的变压器，同时在复杂多步骤推理基准上实现了更高的准确性。此外，路由决策产生了可解释的推理链，增强了透明度和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: Transformer 架构在自然语言任务中取得了最先进的性能，但它们从根本上错误地表示了人类语言的层次结构，通过将文本作为扁平的标记序列进行处理。这导致了二次计算成本、弱计算成本、弱组合泛化和不足的语篇级建模。

Method: HRT（层次分辨率变压器）是一种受小波启发的神经架构，它同时在多个分辨率上处理语言，从字符到话语级单位。HRT 构建了一个多分辨率注意力机制，实现了自底向上的组合和自顶向下的上下文化。

Result: HRT 在 GLUE、SuperGLUE、Long Range Arena 和 WikiText-103 等多样化的基准测试中进行了评估，结果显示 HRT 平均比标准 transformer 基线高出 +3.8%（GLUE）、+4.5%（SuperGLUE）和 +6.1%（Long Range Arena），同时与 BERT 和 GPT 风格的模型相比，内存使用减少了 42%，推理延迟减少了 37%。

Conclusion: HRT 是第一个将计算结构与人类语言的分层组织对齐的架构，展示了多尺度、小波启发式处理在理论效率增益和语言理解的实际改进方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统离散扩散模型需要数百到数千次模型评估才能达到高质量，导致效率低下。而自回归语言模型生成速度慢，难以处理长序列。因此，需要一种既快又不失质量的语言生成方法。

Method: FS-DFM是一种基于步骤数的离散流匹配模型，通过训练模型在不同步骤预算下保持一致性，实现快速且高质量的语言生成。

Result: FS-DFM在8步采样下实现了与1024步离散流基线相当的困惑度，采样速度提高了128倍，显著提升了延迟和吞吐量。
...
Method: 我们创建了PRECOG，这是一个包含红化描述-性能对的语料库，涵盖了多种任务、领域和指标。我们研究了文本仅性能预测：从红化的任务描述和预期配置中估计模型的分数，而无需访问数据集实例。

Result: 实验表明该任务具有挑战性但可行：配备检索模块的模型（排除源论文）在高置信度阈值下达到最低8.7的平均绝对误差。我们的分析表明，更强的推理模型会进行多样化的、迭代的查询，而当前的开源模型则落后，并且经常跳过检索或收集证据的多样性有限。
...
<details>
  <summary>Details</summary>
Motivation: 虽然日语资源丰富，但用于训练模型生成包含声调标记的准确音素转录的数据量很小。因此，我们需要提出新的方法来解决这个问题。

Method: 我们提出了两种方法来缓解数据稀缺问题：一种是多任务训练方案，引入辅助损失函数以估计输入信号的正字法文本标签和音高模式；另一种是融合两个估计器，一个基于语音字母字符串，另一个基于文本标记序列，并开发了一种基于有限状态转换器框架的算法。

Result: 实验结果表明，多任务学习和融合对于构建准确的音素识别器是有效的。我们的方法相比通用多语言识别器具有优势。
...
<details>
  <summary>Details</summary>
Motivation: 尽管GNN和自监督学习方法在分子属性预测方面取得了进展，但人类先验知识的整合仍然是不可或缺的。然而，LLM在知识缺口和幻觉方面存在限制，特别是在研究较少的分子属性上。因此，需要一种新的方法来整合LLM的知识和结构特征。

Method: 我们提出了一种新颖的框架，首次将从大型语言模型中提取的知识与预训练分子模型的结构特征相结合，以增强分子属性预测。我们的方法提示大型语言模型生成领域相关的知识和可执行代码用于分子向量化，生成基于知识的特征，并将其与结构表示融合。

Result: 广泛的实验表明，我们的集成方法优于现有的方法，证实了LLM衍生知识与结构信息的结合提供了一种稳健且有效的解决方案。

Conclusion: 我们的集成方法在分子属性预测任务中优于现有方法，证实了LLM衍生知识与结构信息的结合提供了一种稳健且有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的攻击检测模型的可靠性尚未得到充分探索。因此，本文提出了一种新的攻击设置RedHerring，以测试攻击检测模型的可靠性，并提供新的见解，了解对手可能如何针对检测模型。

Method: 本文提出了一种新的攻击方法RedHerring，通过修改文本使得检测模型预测为攻击，而保持分类器的正确性。此外，还提出了一种简单的置信度检查作为初步防御措施。

Result: 实验结果表明，RedHerring能够在4个数据集上对3个检测器进行攻击，将检测准确率降低了20-71个百分点，同时保持（或提高）分类器的准确性。此外，简单的置信度检查显著提高了检测准确率。

Conclusion: 本文提出了一个新颖的攻击设置RedHerring，旨在使攻击检测模型不可靠。实验结果表明，RedHerring能够显著降低检测准确率，同时保持（或提高）分类器的准确性。此外，本文还提出了一种简单的置信度检查作为初步防御措施，无需重新训练分类器或检测器即可显著提高检测准确率。
...
TL;DR: 本文提出两种新的攻击选择策略，混合选择和动态选择，以减少攻击测试所需的查询次数，同时保持攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有的流行黑盒攻击方法通常需要大量查询，这使得它们对资源有限的研究人员来说效率低下且不切实际。由于基于transformer的架构的复杂性增加，攻击测试的计算成本显著上升。

Method: 我们提出了两种新的攻击选择策略，称为混合选择和动态选择。混合选择通过引入一个大小阈值来决定使用哪种选择算法，将广义的BinarySelect技术与GreedySelect结合。动态选择则通过学习每种选择方法应应用于哪些文本长度来提供另一种结合广义Binary和GreedySelect的方法。

Result: 在4个数据集和6个目标模型上，我们的最佳方法（句子级混合选择）能够平均减少每个攻击所需的查询次数25.82%，同时保持攻击效果。

Conclusion: 我们的最佳方法（句子级混合选择）能够在不损失攻击效果的情况下，平均减少每个攻击所需的查询次数25.82%。
...
<details>
  <summary>Details</summary>
Motivation: 在实际部署中，SER经常因领域不匹配而失败，而强大的LALM只能通过API访问。

Method: 提出了一种去噪标签融合框架MI-Fuse，结合了源域训练的SER分类器作为辅助教师。

Result: 实验显示学生模型超过了LALM，并且比最强基线高出3.9%。

Conclusion: 该方法在没有源数据的情况下增强了情感感知的语音系统，实现了现实世界的适应。
...
<details>
  <summary>Details</summary>
Motivation: 现有的模型面临表达瓶颈，导致语法过大但表现不佳。我们识别出概率分布崩溃是这一限制的根本原因。

Method: 我们引入了一种有针对性的解决方案，称为'崩溃放松神经参数化'，以缓解概率分布崩溃问题。

Result: 通过广泛的实证分析，我们的方法在多种语言中显著提高了解析性能，并允许使用更紧凑的语法。
...
<details>
  <summary>Details</summary>
Motivation: 提出一种适用于文本、图像和视频领域问答任务的训练-free 框架，通过构建和优化子QAs来提高目标答案的置信度评分。

Method: C2R首先整理了一组子QAs以探索多样的推理路径，然后比较结果答案候选者的置信度分数，以选择最可靠的最终答案。

Result: C2R能够提高目标答案的置信度评分，并在多种模型和基准测试中实现性能提升。

Conclusion: C2R可以无缝集成到各种现有的QA模型中，并在不同的模型和基准测试中表现出一致的性能提升。此外，我们提供了关于利用子QAs如何影响模型行为的重要但未被充分探索的见解。
...
<details>
  <summary>Details</summary>
Motivation: 监督微调（SFT）在特定领域数据集上是常见的方法，但被认为会损害模型的通用能力。本文重新审视这种权衡，并提供实证和理论见解。

Method: 本文通过实验和理论分析，评估了多种策略以减少通用能力的损失，包括L2正则化、LoRA、模型平均、FLOW和提出的TALR。

Result: 实验结果表明，虽然没有方法能完全消除这种权衡，但TALR在平衡领域特定收益和通用能力方面表现最佳。

Conclusion: 本文提出了TALR方法，在平衡领域特定性能和通用能力方面表现优于其他方法，并给出了适应LLM到新领域的实用指南。
...
<details>
  <summary>Details</summary>
Motivation: LLMs的内部表示的基本单位尚未明确，限制了对其机制的进一步理解。神经元或特征常被视为这些单位，但神经元存在多义性，而特征则面临不可靠重建和不稳定的问题。

Method: 本文提出了Atoms理论，引入了原子内积（AIP）来纠正表示偏移，正式定义了原子，并证明了原子满足受限等距性质（RIP）的条件，确保了原子集上的稳定稀疏表示，并与压缩感知相关联。

Result: 在Gemma2-2B、Gemma2-9B和Llama3.1-8B上训练阈值激活的SAEs，平均在各层实现了99.9%的稀疏重建，超过99.8%的原子满足唯一性条件，相比之下，神经元和特征分别为0.5%和68.2%，表明原子更真实地捕捉了LLMs的内在表示。

Conclusion: 本文系统地引入并验证了LLMs的Atoms理论，为理解内部表示提供了理论框架，并为机械可解释性奠定了基础。
...
Method: 我们提出了对话提示，这是一种轻量级方法，将用户评论重新表述为多轮对话。其简单变体SCP仅依赖于用户的自身评论，而对比变体CCP则插入其他用户的评论或LLMs作为错误回复，然后让模型进行纠正，鼓励模型以用户风格生成文本。

Result: 实验结果显示，传统的非对话提示往往产生与随机用户撰写的评论相似的评论，而SCP和CCP产生的评论更接近目标用户的评论，即使每个用户只有两条评论。当有高质量的负例时，CCP进一步提高了性能，而当无法收集此类数据时，SCP仍然具有竞争力。

Conclusion: 这些结果表明，对话提示为在少量样本和无训练约束下的评论生成提供了一个实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为了有效利用LLM的优势来解决sLMs的召回率挑战，提出了PoCO方法，该方法战略性地平衡召回率和精确度。

Method: PoCO首先通过LLM有意触发过度纠正以最大化召回率，然后通过微调较小的模型进行有针对性的后纠正步骤，以识别和精炼错误输出。

Result: 实验结果表明，PoCO在增加召回率的同时保持了竞争性的精确度，从而提高了语法错误纠正的整体质量。
...
<details>
  <summary>Details</summary>
Motivation: 为了应对许多示例 ICL 带来的高计算需求问题，提出了 cheat-sheet ICL 方法。

Method: 提出了一种称为 cheat-sheet ICL 的方法，该方法将许多示例 ICL 的信息提炼成一个简洁的文本摘要（便签）作为推理时的上下文。

Result: 实验表明，cheat-sheet ICL 在使用更少标记的情况下实现了与许多示例 ICL 相当或更好的性能，并且无需测试时检索即可匹配基于检索的 ICL。

Conclusion: 这些发现表明，cheat-sheet ICL 是在下游任务中利用 LLM 的实用替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的引用方法在生成引用时存在两个问题：一是引用通常以句子或段落级别提供，可能导致包含大量不相关信息；二是句子级别的引用可能遗漏验证输出所需的关键信息，迫使用户阅读上下文。因此，需要一种更简洁且充分的引用方式。

Method: 本文首先制定了此类引用的注释指南并构建了相应的数据集。然后，提出了一个符合我们标准的引用生成框架。该框架利用大语言模型自动生成微调数据，并采用信用模型过滤低质量示例。

Result: 实验结果表明，所提出的方法能够生成高质量且更易读的引用。

Conclusion: 本文提出了一种生成子句级引用的方法，以提高检索增强生成（RAG）问答系统中大语言模型（LLM）输出的可验证性。实验结果表明，该方法能够生成高质量且更易读的引用。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在语言建模中表现出色，但其在每个去噪步骤中缺乏精确的概率估计，这使得监督微调变得困难。此外，生成过程不够可预测和一致，因此需要控制关键标记来引导生成方向。

Method: WeFT 是一种基于熵的加权监督微调方法，用于扩散语言模型。

Result: 在四个广泛使用的推理基准测试（Sudoku、Countdown、GSM8K 和 MATH-500）上，WeFT 在使用 s1K、s1K-1.1 和 3k 样本进行训练时，相对于标准 SFT 分别实现了 39%、64% 和 83% 的相对改进。

Conclusion: WeFT 是一种有效的加权 SFT 方法，可以在扩散语言模型上实现显著的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 临床决策很少依赖单一答案，而是考虑多个选项，以减少狭隘观点的风险。然而，当前的MRMs通常被训练只产生一个答案，即使在开放性设置中也是如此。

Method: 本文提出了两种方法：提示和微调。SFT教模型模仿注释响应，而RFT通过最大化奖励来激励探索。我们还提出了针对排序列表答案格式的新奖励函数，并进行了RFT的消融研究。

Result: 结果表明，虽然一些SFT模型可以推广到某些答案格式，但使用RFT训练的模型在多种格式上更加稳健。我们还在修改后的MedQA上进行了一项案例研究，发现尽管MRMs可能无法选择基准的首选真实答案，但它们可以识别有效答案。

Conclusion: 本文是首次系统地研究如何使医学推理模型生成排序答案列表。我们希望这项工作能为开发超越单一答案的替代答案格式提供第一步。
...
TL;DR: 本文提出了一种新的对抗性多智能体框架SummQ，用于解决长文档摘要中的信息丢失、事实不一致和连贯性问题。实验结果表明，该框架在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在处理过长文档时，现有的方法通常会遇到信息丢失、事实不一致和连贯性问题。因此，需要一种新的方法来解决这些问题。

Method: 本文提出了SummQ，一个基于对抗性多智能体框架的方法，包括总结生成器、评审员、测验生成器和评审员，以及一个验证生成的摘要是否包含回答测验问题所需信息的考生代理。

Result: 实验结果表明，SummQ在ROUGE和BERTScore指标以及LLM-as-a-Judge和人工评估中均显著优于现有最先进的方法。

Conclusion: 本文提出了一种新的对抗性多智能体框架SummQ，通过总结和测验领域的协作智能解决了长文档摘要中的信息丢失、事实不一致和连贯性问题。实验结果表明，该框架在多个基准测试中显著优于现有最先进的方法，并建立了基于对抗性智能协作的长文档摘要新方法。
...
Result: 污染样本表现出“捷径”行为，而干净样本则表现出更渐进的证据积累。

Conclusion: MemLens 提供了检测记忆行为的真正信号，而不是偶然相关性。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在探讨句子级记忆负荷是更好地由句法相关词之间的线性接近度还是中间材料的结构密度来解释。

Method: 研究使用了统一依存树库和跨语言混合效应框架，联合评估了句子长度、依赖长度和介入复杂度作为记忆负荷的预测因素。

Result: 所有三个因素都与记忆负荷正相关，其中句子长度影响最广泛，而介入复杂度在解释线性距离之外提供了额外的解释力。

Conclusion: 研究结果表明，句子长度对记忆负荷的影响最大，而介入复杂度在解释线性距离之外提供了额外的解释力。概念上，研究调和了线性和层次化的地方性观点，将依赖长度视为一个重要表面特征，同时将介入头作为整合和维护需求的更直接指标。方法上，研究展示了如何使用基于UD的图度量和跨语言混合效应建模来区分线性和结构贡献，为评估句子理解中的记忆负荷竞争理论提供了一条有原则的路径。
...
<details>
  <summary>Details</summary>
Motivation: 当前的研究和资源主要集中在英语上，这导致了对如何在其他语言（如阿拉伯语）中实现此功能的理解不足。本文旨在填补这一空白。

Method: 本文通过使用基础和后训练的开放权重阿拉伯语语言模型进行广泛的实验来解决这些问题。此外，为了促进这项研究，我们将两个开源工具调用数据集翻译并适应为阿拉伯语。

Result: 本文的结果提供了关于为阿拉伯语开发强大工具增强代理的最佳策略的重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在探索LLM在学术文本输入问题中的自动评估系统的应用，以提高评估的效率和准确性。

Method: 本研究提出了五种基于LLM的自动评估系统，并在自定义数据集上进行了测试，包括JudgeLM评估、参考辅助评估、无参考评估、加法评估和自适应评估。

Result: 结果显示，参考辅助评估在与人类评估比较时表现出最低的中位数绝对偏差（0.945）和均方根偏差（1.214），提供了公平且全面的评估。其他方法如加法和自适应评估在简短答案中表现不佳，无参考评估缺乏必要的信息，而JudgeLM评估由于模型限制未能提供良好的结果。

Conclusion: 人工智能驱动的自动评估系统，在适当的方法论辅助下，显示出作为其他学术资源的补充工具的潜力。
...
TL;DR: 本文探讨了如何利用大语言模型和OnPrem$.$LLM框架来加速联邦资助研究和开发中心的文本处理，并在敏感政府环境中保持数据主权和可审计性。
...
Method: 应用OnPrem$.$LLM框架，这是一个用于生成式AI安全和灵活应用的开源框架。

Result: 案例研究表明，这种方法可以提高监督和战略分析能力。

Conclusion: 本文展示了如何利用大语言模型加速对联邦资助研究和开发中心的文本工作负载的处理，同时在敏感政府环境中保持数据主权和可审计性。
...
<details>
  <summary>Details</summary>
Motivation: 我们想了解因果掩码是否可以提供位置信息，以及它如何影响Transformer解码器中的注意力模式。

Method: 我们通过理论分析和实证分析来研究因果掩码对注意力分数的影响。

Result: 我们的分析表明，因果掩码可以诱导位置依赖的注意力模式，即使输入中没有参数或因果依赖。此外，我们发现因果掩码和RoPE的交互会扭曲RoPE的相对注意力模式。

Conclusion: 我们的研究表明，因果掩码可以作为位置信息的来源，与显式的位置编码一样重要。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在现实场景中的应用增加，理解它们同时遵循多个指令的能力变得至关重要。

Method: 本文介绍了两个专门的基准测试（ManyIFEval和StyleMBPP），并开发了三种类型的回归模型来估计性能。

Result: 实验结果表明，随着指令数量的增加，性能会持续下降。使用逻辑回归模型可以以大约10%的误差预测性能。

Conclusion: 本文结论是，通过使用回归模型可以有效地估计大型语言模型在不同指令组合下的性能，即使这些组合在训练中未出现过。
...
<details>
  <summary>Details</summary>
Motivation: 当前的基础模型在理解复杂的视觉信息方面能力有限，因此需要一种更有效的策略来评估它们在工程问题上的表现。

Method: 本文提出了一个名为DoI的新提示策略，该策略为视觉图提供了严格的专家生成文本描述作为上下文，并评估了八种代表性的基础模型。

Result: 当前的基础模型在这些工程问题上表现出显著的困难，最佳模型仅达到56.6%的准确率。有趣的是，当LLMs提供DoI时，它们经常比提供视觉图的VLMs表现更好。

Conclusion: 本文建立了工程AI的严格基准，并突显了在科学和工程背景下开发更强大多模态推理能力的迫切需求。
...
Result: 实证评估显示，虽然模型在88%以上的美国情境脚本中采用内部语气，但它们在较少主导的文化中主要采用外部立场。基于代理的方法在减轻生成式LLM中的偏见方面表现出有效性。
...
TL;DR: 本文介绍了PerHalluEval，这是第一个针对波斯语的动态幻觉评估基准。通过三阶段LLM驱动的流程和人工验证，该基准能够检测问答和摘要任务中的外在和内在幻觉。评估结果显示，LLMs在检测波斯语幻觉文本方面存在困难，但提供外部知识可以部分缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 幻觉是影响所有大型语言模型（LLMs）的一个持续性问题，特别是在低资源语言如波斯语中。目前缺乏针对波斯语的幻觉评估基准，因此需要一个专门的评估工具来检测和分析LLMs在波斯语中的幻觉问题。

Method: 我们提出了PerHalluEval（波斯语幻觉评估）基准，这是一个针对波斯语的动态幻觉评估基准。该基准采用三阶段LLM驱动的流程，并结合人工验证，生成关于问答和摘要任务的合理答案和摘要，以检测外在和内在幻觉。此外，我们使用生成标记的日志概率来选择最可信的幻觉实例，并让人工标注者突出问答数据集中的波斯语特定上下文，以评估LLMs在与波斯文化相关内容上的表现。

Result: 对12个LLMs（包括开源和闭源模型）使用PerHalluEval进行评估显示，模型在检测波斯语幻觉文本方面普遍表现不佳。提供外部知识（例如摘要任务的原始文档）可以部分缓解幻觉问题。此外，专门针对波斯语训练的模型与其他模型在幻觉方面没有显著差异。

Conclusion: 我们的评估显示，模型在检测波斯语幻觉文本方面普遍表现不佳。提供外部知识可以部分缓解幻觉问题，且专门针对波斯语训练的模型与其他模型在幻觉方面没有显著差异。
...
TL;DR: 本文提出 BESPOKE 基准，用于评估搜索增强型 LLM 的个性化能力，通过真实数据和细粒度反馈提升信息检索的个性化体验。


<details>
  <summary>Details</summary>
Motivation: 现有的搜索增强型 LLM 在满足多样化的用户需求方面仍有不足，需要更好地识别同一查询可能反映的不同意图，并提供符合用户偏好的信息。

Method: BESPOKE 通过收集真实的聊天和搜索历史，并配以细粒度的偏好评分和反馈，构建了一个现实且诊断性的基准。

Result: BESPOKE 基准通过长期深入的人类注释构建，揭示了有效个性化在信息检索任务中的关键要求。

Conclusion: BESPOKE 提供了一个细粒度评估个性化搜索增强型 LLM 的基础，有助于改进信息检索任务中的个性化体验。
...
TL;DR: VoiceBBQ is a spoken extension of the BBQ dataset that measures social bias in spoken language models by considering both content and acoustic aspects.


<details>
  <summary>Details</summary>
Motivation: Social bias in Spoken Language Models (SLMs) can emerge from two distinct sources: content aspect and acoustic aspect.

Method: The dataset converts every BBQ context into controlled voice conditions, enabling per-axis accuracy, bias, and consistency scores that remain comparable to the original text benchmark.

Result: Using VoiceBBQ, we evaluate two SLMs - LLaMA-Omni and Qwen2-Audio - and observe architectural contrasts: LLaMA-Omni resists acoustic bias while amplifying gender and accent bias, whereas Qwen2-Audio substantially dampens these cues while preserving content fidelity.
...
TL;DR: 本文分析了SpeechLM在语音交互中可能存在的性别偏差问题，并发现模型在不同情境下表现出矛盾的性别倾向。


<details>
  <summary>Details</summary>
Motivation: 本文旨在分析SpeechLM在语音交互中可能存在的基于声音的性别差异，并探索其原因。

Method: 本文提出了一种新的数据集，用于系统分析这一现象，并评估了LLaMA-Omni系列模型，发现了一个矛盾模式。此外，通过比较SpeechLM与相应的骨干LLM，确认了这些矛盾模式主要源自Whisper语音编码器生成的男性导向声学标记。

Result: 研究发现，在性别刻板印象问题中，所有模型都表现出男性导向的回应；而在性别依赖问题中，模型的回应却与性别无关。此外，即使在允许中性回应的情况下，模型在性别依赖问题中也倾向于中性回应，但矛盾模式依然存在。

Conclusion: 当前的SpeechLM可能未能成功消除性别偏见，尽管它们优先考虑了普遍的公平原则而非情境适当性，这突显了需要更复杂的技巧来正确利用语音技术中的性别信息。
...
TL;DR: 本文提出了一种新的多模态关系抽取框架ROC，通过检索任务来解决传统分类方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统方法在多模态场景中仍然采用基于分类的范式，这有两个显著的局限性：(1) 忽略了如实体类型和位置线索之类的结构约束，(2) 缺乏细粒度关系理解的语义表达。
...
TL;DR: 本研究探讨了语法和领域之间的虚假相关性对LLM性能的影响，并提出了检测和预防这种相关性的方法。


<details>
  <summary>Details</summary>
Motivation: 为了使LLM正确响应指令，必须理解任务指令对的语义和领域。然而，语法也可以传达隐含信息，因此需要研究语法和领域之间的关系。

Method: 我们通过合成训练数据集和评估框架来检测这种现象，并在多个模型中进行了实验验证。

Result: 我们发现语法-领域相关性可能会降低OLMo-2模型在实体知识任务上的性能，并且在FlanV2数据集的部分子集中观察到了这种现象。此外，我们展示了语法-领域相关性可能被用来绕过拒绝行为。
...
TL;DR: 本文综述了计算幽默在生成和解释任务方面的现状，指出尽管幽默是自然语言处理的重要任务，但相关研究仍较少，且现有模型的表现仍不及人类。


<details>
  <summary>Details</summary>
Motivation: 幽默是人类的基本特征，其计算理解是自然语言处理中最具挑战性的任务之一。幽默需要广泛的推理来理解和创造，使其成为评估现代大型语言模型常识知识和推理能力的相关任务。

Method: 对计算幽默领域的文献进行综述，重点放在生成和解释幽默的任务上。

Result: 尽管理解幽默具有基础自然语言处理任务的所有特点，但除了双关语之外，生成和解释幽默的工作仍然很少，而最先进的模型仍然无法达到人类的能力。

Conclusion: 计算幽默处理作为自然语言处理的一个子学科的重要性，以及需要考虑幽默的主观性和伦理模糊性的未来研究方向。
...
TL;DR: 本研究探讨了基于SLM的聊天机器人中的PII泄露问题，提出了一种新的GEP方法，该方法在PII泄露检测方面表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 由于小语言模型（SLMs）在某些领域表现出与大型语言模型（LLMs）相当的性能，同时消耗更少的能量和时间，因此它们变得越来越受欢迎。然而，SLMs在下游任务中的个人可识别信息（PII）泄露尚未被探索。

Method: 本研究首先微调了一个新的聊天机器人ChatBioGPT，基于BioGPT的主干网络使用医疗数据集Alpaca和HealthCareMagic。然后，提出了GEP（贪心坐标梯度方法）以专门用于PII提取。

Result: 实验结果显示，GEP方法相比之前的基于模板的方法，PII泄漏增加了多达60倍。在自由风格插入的情况下，GEP仍能揭示高达4.53%的PII泄漏率。

Conclusion: 本研究证明了传统的基于模板的PII攻击方法在SLM条件下无法有效提取数据集中的PII，而提出的GEP方法显著提高了PII泄漏检测的效果。此外，在更复杂和现实的情况下，GEP仍然能够揭示高达4.53%的PII泄漏率。
...
TL;DR: 本文提出了一种统一的框架，结合了隐式检索和结构化协作，以提高大型语言模型在科学推理中的性能，并显著减少了资源消耗。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在科学推理中虽然取得了进展，但仍然存在两个主要瓶颈：显式检索片段推理导致额外的token和步骤，以及多代理管道通过平均所有候选者稀释了强大的解决方案。

Method: 本文提出了基于Monitor的检索模块和分层解决方案优化（HSR）以及质量感知迭代推理（QAIR）方法，以实现隐式检索和结构化协作。

Result: 在Humanity's Last Exam (HLE) Bio/Chem Gold数据集上，该框架达到了48.3%的准确率，这是目前报告的最高成绩，超过了最强的代理基线13.4分，并领先于前沿LLM多达18.1分，同时减少了53.5%的token使用和43.7%的代理步骤。在SuperGPQA和TRQA上的结果证实了其在不同领域的鲁棒性。

Conclusion: 本文提出了一种统一的框架，结合了隐式检索和结构化协作，以解决大型语言模型在科学推理中的两个主要瓶颈。实验结果表明，该框架在多个数据集上取得了最先进的性能，并且在减少token使用和代理步骤方面表现出色。
...
TL;DR: 本文提出了一个名为CLaw的基准测试，用于评估大型语言模型在中文法律知识和推理应用方面的表现，并指出当前模型在准确检索和引用法律条款方面存在明显不足，需要通过监督微调或检索增强生成等方法来提高其可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在分析法律文本和引用相关法规方面越来越被依赖，但它们的可靠性常常因一般性的预训练而受损，这种预训练没有专注于法律文本，掩盖了它们真正的法律知识深度。

Method: 本文引入了CLaw，这是一个专门设计用于评估中文法律知识和推理应用的基准测试。它包括两个关键组成部分：一个全面的、细粒度的全国306部法律条文语料库，以及一个基于中国最高法院精选材料的254个案例推理实例集。

Result: 实证评估表明，大多数现代大型语言模型在忠实再现法律条款方面存在显著困难。
...
TL;DR: SGMem is a novel method for long-term conversational agents that uses sentence-level graphs to represent dialogue and improve memory management, leading to better performance in question answering tasks.
...
Method: SGMem (Sentence Graph Memory) represents dialogue as sentence-level graphs within chunked units, capturing associations across turn-, round-, and session-level contexts. It combines retrieved raw dialogue with generated memory such as summaries, facts, and insights to supply LLMs with coherent and relevant context for response generation.
...
TL;DR: QCG-RAG是一种以查询为中心的图RAG框架，通过可控粒度的查询中心图和多跳检索机制，提高了问答准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法面临粒度困境：细粒度实体级图成本高且丢失上下文，粗粒度文档级图无法捕捉细微关系。

Method: 引入了QCG-RAG框架，通过Doc2Query和Doc2Query--构建以查询为中心的图，实现查询粒度的索引和多跳块检索。

Result: 在LiHuaWorld和MultiHop-RAG数据集上的实验表明，QCG-RAG在问答准确率上优于之前的基于块和基于图的RAG方法。
...
TL;DR: 本文研究了扩散模型中的同音异义词重复问题，并提出了一种测量重复率的方法。通过自动评估和人工评估对不同的扩散模型进行了评估，并研究了通过提示扩展来缓解这一问题的方法。结果表明，这种方法也能有效减少与英语中心偏见相关的重复。


<details>
  <summary>Details</summary>
Motivation: 同音异义词在提示中出现时，扩散模型可能会同时生成该词的多个含义，这被称为同音异义词重复。这个问题因英语中心偏见而变得更加复杂，这包括在文本到图像模型管道之前的一个额外翻译步骤。因此，即使在原始语言中不是同音异义词的词语，在翻译成英语后也可能变成同音异义词并失去其意义。

Method: 本文引入了一种测量重复率的方法，并使用基于视觉-语言模型的自动评估和人工评估对不同的扩散模型进行了评估。同时，我们研究了通过提示扩展来缓解同音异义词重复问题的方法。

Result: 本文通过自动评估和人工评估对不同的扩散模型进行了评估，并研究了通过提示扩展来缓解同音异义词重复问题的方法。结果表明，这种方法也能有效减少与英语中心偏见相关的重复。

Conclusion: 本文提出了一种测量重复率的方法，并通过自动评估和人工评估对不同的扩散模型进行了评估。此外，我们研究了通过提示扩展来缓解同音异义词重复问题的方法，证明这种方法也能有效减少与英语中心偏见相关的重复。
...
TL;DR: 本文提出了任务分类法和任务锚定的功能多样性，以改善输出同质化的评估和缓解。


<details>
  <summary>Details</summary>
Motivation: 先前的研究在处理输出同质化时往往未能以任务依赖的方式概念化多样性，因此本文旨在填补这一研究空白。

Method: 本文提出了一个包含八个任务类别的任务分类法，引入了任务锚定的功能多样性来更好地评估输出同质化，并提出了一种任务锚定的采样技术来增加功能多样性。

Result: 本文通过任务依赖的方法改进了输出同质化的评估和缓解，并挑战了多样性和质量之间的权衡观念。
...
TL;DR: LLMTrace is a new large-scale, bilingual corpus for AI-generated text detection that addresses the limitations of existing datasets by providing character-level annotations for precise localization of AI-generated segments.
...
TL;DR: 本文理论分析了输入扰动对CoT输出的影响，并在LSA模型上验证了相关结论。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示，Chain-of-Thought (CoT) 的输出受到输入扰动的显著影响。尽管许多方法旨在通过优化提示来减轻这种影响，但对这些扰动如何影响CoT输出的理论解释仍然是一个开放的研究领域。这种差距限制了我们对输入扰动在推理过程中传播的深入理解，并阻碍了提示优化方法的进一步改进。因此，本文旨在理论上分析输入扰动对CoT输出波动的影响。

Method: 本文首先推导了在输出波动在可接受范围内的条件下输入扰动的上限，并证明了该上限与CoT中的推理步骤数正相关。此外，还证明了即使无限长的推理过程也无法消除输入扰动的影响。接着，将这些结论应用于LSA模型，并证明了输入扰动的上限与输入嵌入和隐藏状态向量的范数负相关。

Result: 本文通过理论分析和实验验证，证明了输入扰动对CoT输出的影响，并在LSA模型上得出了相关结论。实验结果与理论分析一致，实证证明了研究结果的正确性。

Conclusion: 本文通过理论分析和实验验证，证明了输入扰动对CoT输出的影响，并在LSA模型上得出了相关结论。
...
TL;DR: DisCoCLIP通过张量网络编码语言结构，提高了视觉-语言模型对动词语义和词序的敏感性，实现了更优的组合推理。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在大规模图像-文本对齐方面表现优异，但往往忽视语言的组合结构，导致在依赖词序和谓词-论元结构的任务中失败。

Method: 引入DisCoCLIP，结合冻结的CLIP视觉变压器和一种新型张量网络文本编码器，显式编码句法结构。使用组合范畴语法解析器生成分布词张量，通过张量分解保持模型效率。

Result: DisCoCLIP在SVO-Probes任务中将CLIP的动词准确率从77.6%提高到82.4%，在ARO属性和关系分数上提升超过9%和4%，并在新的SVO-Swap基准测试中达到93.7%。

Conclusion: 通过张量网络嵌入显式语言结构可以产生可解释且参数高效的表示，显著改善视觉-语言任务中的组合推理。
...
TL;DR: 本文介绍了Updesh，一个高质量的大规模合成指令遵循数据集，包含13种印度语言的950万个数据点，旨在提高多语言AI系统的性能，特别是在低资源语言中。


<details>
  <summary>Details</summary>
Motivation: 开发能够在多种语言中有效运作并保持文化根基的AI系统是一个长期挑战，特别是在资源匮乏的环境中。合成数据提供了一个有希望的途径，但其在多语言和多元文化环境中的有效性仍需探索。

Method: 我们通过自下而上的生成策略创建和影响了印度语言的合成文化情境数据集，该策略提示大型开源LLM（>=235B参数）在语言特定的维基百科内容中进行数据生成。

Result: Updesh数据集包含13种印度语言的950万个数据点，涵盖各种推理和生成任务，强调长上下文、多轮能力以及与印度文化背景的一致性。评估表明生成的数据质量高，但人类评估指出了进一步改进的领域。此外，我们在数据集上微调模型，并在15个多样化的多语言数据集上评估性能，结果表明模型在生成任务上取得了显著提升，并在多项选择风格的NLU任务中保持竞争力。

Conclusion: 这些发现提供了实证证据，表明有效的多语言AI需要多方面的数据整理和生成策略，这些策略应结合上下文感知、文化基础的方法。
...
TL;DR: 本文研究了大型语言模型中的奉承行为，发现其由不同的表示构成，并且可以独立调节。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型中的奉承行为是否由单一机制或多个不同过程引起。

Method: 通过差异均值方向、激活添加和子空间几何学，在多个模型和数据集上分析了奉承行为。

Result: （1）三种行为在潜在空间中沿不同的线性方向编码；（2）每种行为可以独立增强或抑制而不影响其他行为；（3）它们的表征结构在模型家族和规模上是一致的。

Conclusion: 这些结果表明，奉承行为对应于不同的、可独立调节的表示。
...
TL;DR: RLBFF是一种结合人类偏好和规则验证的新方法，能够提高奖励模型的性能并实现更灵活的定制。


<details>
  <summary>Details</summary>
Motivation: RLHF和RLVR各有优缺点，RLHF缺乏可解释性和奖励黑客，而RLVR局限于基于正确性的验证器。因此需要一种新的方法来克服这些限制。

Method: RLBFF结合了人类驱动的偏好和基于规则的验证，通过从自然语言反馈中提取二进制问题来训练奖励模型。

Result: RLBFF在RM-Bench（86.2%）和JudgeBench（81.4%，#1在排行榜上）上取得了顶级性能。此外，RLBFF提供了一个完全开源的配方，用于对齐Qwen3-32B，以匹配或超过o3-mini和DeepSeek R1的性能。

Conclusion: RLBFF可以超越Bradley-Terry模型，并在RM-Bench和JudgeBench上取得顶级性能。此外，RLBFF提供了一个完全开源的配方，用于对齐Qwen3-32B，以匹配或超过o3-mini和DeepSeek R1的性能。
...
TL;DR: 本文提出了一种科学推理基础模型，该模型能够将自然语言与异构科学表示对齐，并通过多种技术提升其性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 为了提供一个能够将自然语言与异构科学表示对齐的科学推理基础模型，以提高跨领域泛化能力和保真度。

Method: 模型在206B个标记的语料库上进行预训练，然后通过SFT在40M个指令上进行对齐，使用annealed cold-start bootstrapping来引发长形式的思维链，并通过任务特定的奖励塑造进行强化学习，从而培养出有意识的科学推理。

Result: 模型支持四个能力家族，涵盖多达103个任务，包括文本与科学格式之间的忠实翻译、文本/知识提取、属性预测、属性分类、无条件和条件序列生成和设计。

Conclusion: 我们的方法拓宽了指令覆盖范围，提高了跨领域泛化能力，并增强了保真度。跨学科学习加强了迁移和下游可靠性。
...
TL;DR: 本研究探讨了验证设计和策略如何影响代码生成模型的性能，发现当前验证方式过于僵化，需要重新校准以突破验证天花板。


<details>
  <summary>Details</summary>
Motivation: 大型代码生成语言模型越来越多地依赖合成数据，但这种数据生成方式引入了一个之前未探索的瓶颈：验证天花板，即训练数据的质量和多样性受到合成验证器能力的根本限制。

Method: 系统研究了验证设计和策略如何影响模型性能，分析了测试复杂性和数量的影响，探索了放松通过阈值的方法，并通过对比正式正确和错误的解决方案以及人类评估来验证验证的必要性。

Result: 更丰富的测试套件可以提高代码生成能力（平均提高3个pass@1），而单纯的数量增加则收益递减；允许放松的阈值或结合基于LLM的软验证可以恢复有价值的训练数据，带来2-4点的pass@1性能提升；保留每个问题的多样化正确解决方案可以持续提升泛化能力。

Conclusion: 当前的验证方式过于僵化，过滤掉了有价值的多样性。但不能被抛弃，只能重新校准。通过结合校准后的验证与多样且具有挑战性的问题-解决方案对，我们可以突破验证天花板，解锁更强大的代码生成模型。
...
### [68] [On Theoretical Interpretations of Concept-Based In-Context Learning](https://arxiv.org/abs/2509.20882)
*Huaze Tang,Tianren Peng,Shao-lun Huang*

Main category: cs.IT

TL;DR: 本文研究了基于概念的ICL（CB-ICL）的理论机制，并通过实验证明了其在少样本提示任务中的有效性，为模型预训练和提示工程提供了重要见解。


<details>
  <summary>Details</summary>
Motivation: 尽管ICL已成为自然语言处理和大语言模型应用的重要范式，但对其机制的理论理解仍然有限。本文旨在通过研究CB-ICL来解决这一问题。

Method: 本文研究了一种特定的ICL方法，称为基于概念的ICL（CB-ICL），并通过理论分析解释了其为何以及何时在仅有少量演示的提示中表现良好。此外，还提出了一个相似性度量，用于衡量提示演示与查询输入之间的关系。

Result: 理论分析量化了LLMs可以利用的知识，并提供了关于提示演示大小和LLM嵌入维度在ICL中的影响的探索。此外，通过实际数据实验验证了CB-ICL及其相应理论的实用性。

Conclusion: 本文通过理论分析和实验证明了CB-ICL在少样本提示任务中的有效性，并提供了对模型预训练和提示工程的重要见解。
...
### [69] [Blueprints of Trust: AI System Cards for End to End Transparency and Governance](https://arxiv.org/abs/2509.20394)
*Huzaifa Sidhpurwala,Emily Fox,Garth Mollett,Florencio Cano Gabarda,Roman Zhukov*

Main category: cs.CY

TL;DR: 本文介绍了HASC框架，用于提高AI系统的透明度和问责性，并将其与ISO/IEC 42001:2023标准进行比较。


<details>
  <summary>Details</summary>
Motivation: 提高AI系统开发和部署的透明度和问责性。

Method: HASC框架通过集成AI系统安全和安全状态的全面动态记录，扩展了现有的模型卡和系统卡概念，并提出了一个标准化的标识符系统，包括新的AI安全危险（ASH）ID。

Result: HASC为开发者和利益相关者提供了单一、可访问的真相来源，使他们能够更明智地做出关于AI系统安全性的决策。
...
### [70] [Communication Bias in Large Language Models: A Regulatory Perspective](https://arxiv.org/abs/2509.21075)
*Adrian Kuenzler,Stefan Schmid*

Main category: cs.CY

TL;DR: 本文探讨了大型语言模型可能带来的偏见风险及其社会影响，并呼吁加强竞争和设计治理以确保公平和可信的人工智能。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在许多应用中变得越来越重要，人们对偏见、公平性和监管合规性产生了担忧。

Method: 本文回顾了偏见输出的风险及其社会影响，并聚焦于欧盟的《人工智能法案》和《数字服务法案》等框架。

Result: 本文强调了在监管之外，需要更强的竞争和设计治理来确保公平和可信的人工智能。

Conclusion: 本文认为，除了持续的监管外，还需要加强对竞争和设计治理的关注，以确保公平、可信的人工智能。
...
### [71] [Visual Authority and the Rhetoric of Health Misinformation: A Multimodal Analysis of Social Media Videos](https://arxiv.org/abs/2509.20724)
*Mohammad Reza Zarei,Barbara Stead-Coyle,Michael Christensen,Sarah Everts,Majid Komeili*

Main category: cs.SI

TL;DR: 本研究分析了短格式视频平台上营养和补充视频中可信度的包装方式，发现自信的单个主持人在工作室或家庭环境中占主导地位，权威线索如头衔、幻灯片和图表以及证书经常与说服性元素如术语、参考文献、恐惧或紧迫感、对主流医学的批评和阴谋论一起出现，并与销售链接和订阅呼吁等货币化手段结合。


<details>
  <summary>Details</summary>
Motivation: 短格式视频平台是健康建议的核心场所，其中混合了有用、误导性和有害的内容。本研究不评判真相，而是研究可信度在营养和补充视频中的包装方式。

Method: 研究通过分析跨平台的152个公共视频（来自TikTok、Instagram和YouTube）来探讨可信度的包装方式，每个视频都标注了26个特征，包括视觉权威、演讲者属性、叙述策略和参与提示。透明的标注流程整合了自动语音识别、原则性的帧选择和多模态模型，并通过分层子样本的人工验证显示出高度的一致性。

Result: 描述性结果表明，自信的单个主持人在工作室或家庭环境中占主导地位，临床背景很少见。分析结果显示，权威线索如头衔、幻灯片和图表以及证书经常与说服性元素如术语、参考文献、恐惧或紧迫感、对主流医学的批评和阴谋论一起出现，并与销售链接和订阅呼吁等货币化手段结合。参考文献和类似科学的视觉效果往往伴随着情感和对立的叙述，而不是表明克制。

Conclusion: 研究发现，可信度信号、叙述技巧和货币化在营养和补充视频中交织在一起，这些视频通常由自信的单个主持人在工作室或家庭环境中呈现，临床背景很少见。权威线索如头衔、幻灯片和图表以及证书经常与说服性元素如术语、参考文献、恐惧或紧迫感、对主流医学的批评和阴谋论一起出现，并与销售链接和订阅呼吁等货币化手段结合。参考文献和类似科学的视觉效果往往伴随着情感和对立的叙述，而不是表明克制。
...
### [72] [Perspectra: Choosing Your Experts Enhances Critical Thinking in Multi-Agent Research Ideation](https://arxiv.org/abs/2509.20553)
*Yiren Liu,Viraj Shah,Sangho Suh,Pao Siangliulue,Tal August,Yun Huang*

Main category: cs.HC

TL;DR: Perspectra是一个交互式多代理系统，通过论坛风格的界面促进批判性思维和跨学科合作，实验结果显示其优于传统的群聊方式。


<details>
  <summary>Details</summary>
Motivation: 当前多代理系统（MAS）在信息搜索和创意生成方面取得了进展，但如何让用户有效控制、引导和批判性评估多个领域专家代理之间的协作仍研究不足。

Method: 本文介绍了Perspectra，这是一个交互式多代理系统，通过论坛风格的界面可视化和结构化LLM代理之间的讨论，并支持@提及邀请目标代理、并行探索的线程以及实时思维图来可视化论点和理由。

Result: 在一项针对18名参与者的内部研究中，与群聊基线相比，Perspectra显著增加了批判性思维行为的频率和深度，引发了更多的跨学科回复，并导致研究提案的频繁修改。

Conclusion: 本文讨论了设计支持用户控制多代理对抗性话语的多代理工具的意义，强调了Perspectra在促进批判性思维和跨学科回复方面的有效性。
...
### [73] [Interactive Recommendation Agent with Active User Commands](https://arxiv.org/abs/2509.21317)
...
TL;DR: 本文提出了一种新的推荐系统，称为交互式推荐流（IRF），通过自然语言命令让用户主动控制推荐策略，并开发了RecBot双代理架构来实现这一目标。实验表明，该方法在用户满意度和业务成果方面都有显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统依赖于被动反馈机制，限制用户只能进行简单的选择，如喜欢和不喜欢。然而，这些粗粒度信号无法捕捉用户的细微行为动机和意图。当前系统也无法区分哪些特定的项目属性驱动用户满意或不满意，导致偏好建模不准确。这些根本性限制造成了用户意图和系统解释之间的持续差距，最终损害用户满意度和系统效果。

Method: 我们开发了RecBot，这是一个双代理架构，其中解析器代理将语言表达转换为结构化偏好，规划器代理动态协调自适应工具链以进行实时策略调整。为了实现实际部署，我们采用模拟增强的知识蒸馏来实现高效性能同时保持强大的推理能力。
...
### [74] [Every Character Counts: From Vulnerability to Defense in Phishing Detection](https://arxiv.org/abs/2509.20589)
*Maria Chiper,Radu Tudor Ionescu*

Main category: cs.CR

TL;DR: 本文研究了字符级深度学习模型在检测网络钓鱼攻击中的有效性，发现CharGRU在所有场景中表现最佳，并且通过对抗训练可以提高模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前的自动检测方法在检测新型网络钓鱼攻击时缺乏可解释性和鲁棒性。因此，我们需要一种更有效的方法来应对这一威胁。

Method: 我们评估了三种字符级神经架构：CharCNN、CharGRU和CharBiLSTM，并在自建的电子邮件数据集上进行了测试。我们还分析了它们在三种场景下的性能：标准训练和测试、标准训练和测试下的对抗攻击，以及使用对抗样本的训练和测试。

Result: CharGRU在所有场景中表现最佳，所有模型都对对抗攻击表现出脆弱性，但对抗训练显著提高了它们的鲁棒性。此外，我们还能够通过Grad-CAM技术可视化模型的决策过程。

Conclusion: 我们的研究表明，字符级深度学习模型在检测网络钓鱼攻击方面具有潜力，尤其是在提供可解释性和鲁棒性方面。CharGRU在所有场景中表现最佳，并且通过对抗训练可以显著提高其鲁棒性。此外，我们还能够通过Grad-CAM技术可视化模型的决策过程。
...
### [75] [PMark: Towards Robust and Distortion-free Semantic-level Watermarking with Channel Constraints](https://arxiv.org/abs/2509.21057)
*Jiahao Huo,Shuliang Liu,Bin Wang,Junyan Zhang,Yibo Yan,Aiwei Liu,Xuming Hu,Mingxun Zhou*

Main category: cs.CR

TL;DR: PMark is a new SWM method that offers better robustness and text quality compared to existing methods by using proxy functions and dynamic median estimation.
...
Result: PMark achieves the desired distortion-free property and improves the robustness against paraphrasing-style attacks. It also provides an empirically optimized version that removes the requirement for dynamical median estimation for better sampling efficiency.
...
TL;DR: 本文介绍了InsightGUIDE，这是一种新型的AI辅助工具，旨在作为阅读助手而非替代品。该系统通过将专家的阅读方法直接嵌入其核心AI逻辑中，提供简洁、结构化的见解，帮助研究人员更有效地理解论文的关键内容。


<details>
  <summary>Details</summary>
Motivation: The proliferation of scientific literature presents an increasingly significant challenge for researchers. While Large Language Models (LLMs) offer promise, existing tools often provide verbose summaries that risk replacing, rather than assisting, the reading of the source material.

Method: The paper introduces InsightGUIDE, a novel AI-powered tool designed to function as a reading assistant, not a replacement. The system provides concise, structured insights that act as a 'map' to a paper's key elements by embedding an expert's reading methodology directly into its core AI logic.
...
### [77] [CLAUSE: Agentic Neuro-Symbolic Knowledge Graph Reasoning via Dynamic Learnable Context Engineering](https://arxiv.org/abs/2509.21035)
*Yang Zhao,Chengxiao Dai,Wei Zhuo,Yue Xiu,Dusit Niyato*

Main category: cs.AI

TL;DR: CLAUSE is an agentic three-agent neuro-symbolic framework that treats context construction as a sequential decision process over knowledge graphs, allowing per-query adaptation to trade-offs among accuracy, latency, and cost without retraining.
...
### [78] [Disagreements in Reasoning: How a Model's Thinking Process Dictates Persuasion in Multi-Agent Systems](https://arxiv.org/abs/2509.21054)
*Haodong Zhao,Jidong Li,Zhaomin Wu,Tianjie Ju,Zhuosheng Zhang,Bingsheng He,Gongshen Liu*

Main category: cs.AI

TL;DR: 本研究挑战了说服效果主要取决于模型规模的假设，提出这些动力学本质上由模型的基本认知过程决定，特别是其显式推理能力。通过多智能体说服实验，我们发现了说服二元性，揭示了推理过程的抗说服性和透明度对说服能力的影响，以及多跳说服中的影响传播和衰减动态。


<details>
  <summary>Details</summary>
Motivation: 随着最近多智能体系统（MAS）的迅速增长，其中大型语言模型（LLMs）和大型推理模型（LRMs）通常协作解决复杂问题，需要深入了解支配其相互作用的说服动力学。

Method: 通过一系列多智能体说服实验，我们揭示了称为说服二元性的基本权衡。我们进一步考虑了更复杂的传输说服情况，揭示了在多个代理网络之间的多跳说服中影响传播和衰减的复杂动态。

Result: 我们的发现表明，LRMs中的推理过程表现出对说服的显著抵抗力，更稳固地保持其初始信念。相反，通过分享“思考内容”使这种推理过程透明化，大大增加了它们说服他人的能力。

Conclusion: 本研究提供了系统证据，将模型的内部处理架构与其外部说服行为联系起来，为先进模型的易受攻击性提供了新的解释，并突出了对未来多智能体系统安全、稳健性和设计的关键影响。
...
### [79] [TrustJudge: Inconsistencies of LLM-as-a-Judge and How to Alleviate Them](https://arxiv.org/abs/2509.21117)
...
TL;DR: 本文分析了LLM-as-a-judge评估框架中的不一致性问题，并提出了TrustJudge框架，通过分布敏感评分和似然感知聚合来解决这些问题。实验结果表明，TrustJudge在降低评分比较和成对传递性不一致性方面效果显著，同时保持了高评估准确性。


<details>
  <summary>Details</summary>
Motivation: 当前评估框架存在关键不一致性，包括评分比较不一致和成对传递性不一致。这些问题源于离散评分系统中的信息丢失和成对评估中的模糊平局判断。

Method: 我们提出了TrustJudge，这是一个概率框架，通过两个关键创新来解决这些问题：1）分布敏感评分，从离散评分概率计算连续期望，保留信息熵以实现更精确的评分；2）似然感知聚合，使用双向偏好概率或困惑度来解决传递性违规。

Result: 当使用Llama-3.1-70B-Instruct作为法官进行评估时，TrustJudge将评分比较不一致性降低了8.43%（从23.32%降至14.89%），成对传递性不一致性降低了10.82%（从15.22%降至4.40%），同时保持了更高的评估准确性。

Conclusion: 我们的工作提供了对LLM-as-a-judge范式中评估框架不一致性的首次系统分析，提供了理论见解和实用解决方案，以实现可靠的自动化评估。该框架在各种模型架构和规模上表现出一致的改进，使更可信的LLM评估成为可能，而无需额外训练或人工注释。
...
### [80] [Expanding Reasoning Potential in Foundation Model by Learning Diverse Chains of Thought Patterns](https://arxiv.org/abs/2509.21124)
*Xuemiao Zhang,Can Ren,Chengying Tu,Rongxiang Weng,Shuo Wang,Hongfei Yan,Jingang Wang,Xunliang Cai*

Main category: cs.AI

TL;DR: 本文提出了一种新方法，通过利用高价值推理模式数据提升模型推理能力，在挑战性任务中取得显著效果。


<details>
  <summary>Details</summary>
Motivation: 当前的方法往往不加区分地使用CoT数据，因此需要确定哪些数据类型最有效地增强模型的推理能力。

Method: 本文定义了基础模型的推理潜力，并提出了一个双粒度算法，用于从数据池中高效选择与核心集对齐的高价值CoT数据（CoTP）。

Result: 仅使用10B-token的CoTP数据，85A6B MoE模型在AIME 2024和2025任务上的性能提高了9.58%，并提升了下游RL性能的上限7.81%。

Conclusion: 本文提出了一种新的方法，通过利用高价值的推理模式数据来提升模型的推理能力。实验结果表明，这种方法在挑战性的AIME 2024和2025任务上取得了显著的性能提升。
...
TL;DR: 本文提出RadAgents框架，用于胸片解释，通过多模态推理和上下文冲突的验证与解决，提高输出的可靠性、透明度和与临床实践的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的胸片解释方法在推理的临床可解释性、多模态证据的融合以及检测和解决跨工具不一致方面存在不足。

Method: RadAgents是一个多代理框架，用于胸片解释，它结合了临床先验和任务感知的多模态推理，并集成了接地和多模态检索增强以验证和解决上下文冲突。

Result: RadAgents框架能够生成更可靠、透明且与临床实践一致的输出，通过多模态证据的融合和上下文冲突的验证与解决。

Conclusion: RadAgents框架通过结合临床先验和任务感知的多模态推理，以及整合接地和多模态检索增强来验证和解决上下文冲突，从而提高了胸片解释的可靠性、透明度和与临床实践的一致性。
...
### [82] [Intercept Cancer: Cancer Pre-Screening with Large Scale Healthcare Foundation Models](https://arxiv.org/abs/2506.00209)
*Liwen Sun,Hao-Ren Yao,Gary Gao,Ophir Frieder,Chenyan Xiong*

Main category: cs.LG

TL;DR: CATCH-FM is a pre-screening method using EHR foundation models to identify high-risk cancer patients, achieving strong efficacy with low risk and outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: Existing cancer screening techniques are expensive, intrusive, and not globally available, leading to many preventable deaths. The goal is to develop an effective and accessible pre-screening method.

Method: CATCH-FM is a cancer pre-screening methodology that uses historical medical records and EHR foundation models pretrained on medical code sequences. It is finetuned on clinician-curated cancer risk prediction cohorts.

Result: In a retrospective evaluation of thirty thousand patients, CATCH-FM achieved 60% sensitivity, 99% specificity, and a high negative predictive value. It outperformed feature-based tree models and general and medical large language models.

Conclusion: CATCH-FM demonstrates robustness in various patient distributions and the ability to capture non-trivial cancer risk factors. It achieves state-of-the-art pancreatic cancer risk prediction and will be open-sourced.
...
### [83] [Can Federated Learning Safeguard Private Data in LLM Training? Vulnerabilities, Attacks, and Defense Evaluation](https://arxiv.org/abs/2509.20680)
*Wenkai Guo,Xuefeng Liu,Haolin Wang,Jianwei Niu,Shaojie Tang,Jing Yuan*

Main category: cs.LG

TL;DR: 本文研究了联邦学习框架下大语言模型的隐私泄露问题，发现攻击者可以从中提取训练数据，提出增强攻击策略，并评估了隐私保护技术的效果，为降低隐私风险提供了实用指南。


<details>
  <summary>Details</summary>
Motivation: 组织希望将大语言模型适应其特定领域，但不愿共享本地数据，因此需要一种隐私保护的方法来实现协同微调。联邦学习提供了一种可能的解决方案，但其隐私保护效果尚不明确。

Method: 本文通过实验分析了联邦学习框架下大语言模型的隐私泄露问题，提出了增强的攻击策略，并评估了隐私保护技术的效果。

Result: 实验表明，攻击者可以利用简单的生成方法从全局模型中提取训练数据，且数据泄露随着模型规模的增大而增加。此外，本文提出的增强攻击策略能够进一步加剧隐私泄露。评估结果显示，差分隐私、正则化约束更新和采用安全对齐的大语言模型等隐私保护技术有助于降低隐私风险。

Conclusion: 本文通过实验展示了即使在联邦学习框架下，攻击者仍能从全局模型中提取训练数据，且随着模型规模的增大，数据泄露风险增加。此外，本文提出了针对联邦学习的增强攻击策略，并评估了隐私保护技术的效果，为使用联邦学习训练大语言模型提供了有价值的见解和实用指南。
...
### [84] [CE-GPPO: Controlling Entropy via Gradient-Preserving Clipping Policy Optimization in Reinforcement Learning](https://arxiv.org/abs/2509.20712)
*Zhenpeng Su,Leiyu Pan,Minxuan Lv,Yuntao Li,Wenping Hu,Fuzheng Zhang,Kun Gai,Guorui Zhou*

Main category: cs.LG

TL;DR: CE-GPPO 是一种新的强化学习算法，通过重新引入被剪切的标记的梯度来控制熵，从而实现探索与利用的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法如 PPO 及其变体由于剪切机制会丢弃低概率标记的梯度信号，而这些标记在调节熵演化中起着关键但被忽视的作用。

Method: CE-GPPO 是一种新算法，通过以温和且有限的方式重新引入原始 PPO 中被剪切的标记的梯度，从而控制熵。

Result: CE-GPPO 在数学推理基准测试中表现优于强基线，并提供了理论证明和实证证据。

Conclusion: CE-GPPO 有效地缓解了熵的不稳定性，并在数学推理基准测试中表现出色。
...
### [85] [StyleBench: Evaluating thinking styles in Large Language Models](https://arxiv.org/abs/2509.20868)
*Junyu Guo,Shangding Gu,Ming Jin,Costas Spanos,Javad Lavaei*

Main category: cs.LG

TL;DR: 研究介绍了StyleBench基准，评估了五种推理风格在多种任务和模型上的表现，发现不同风格的效果取决于模型规模和任务类型。
...
Method: 我们引入了StyleBench，这是一个全面的基准，用于系统评估不同任务和模型中的推理风格。

Result: 我们的大规模分析表明，没有一种风格是普遍最优的。基于搜索的方法在开放式问题中表现优异，但需要大规模模型，而简洁的风格在定义明确的任务中实现了显著的效率提升。

Conclusion: 我们的研究提供了选择最优推理策略的路线图，这取决于特定的约束条件。
...
### [86] [CLUE: Conflict-guided Localization for LLM Unlearning Framework](https://arxiv.org/abs/2509.20977)
*Hang Chen,Jiaying Zhu,Xinyu Yang,Wenya Wang*

Main category: cs.LG

TL;DR: CLUE is a framework that uses circuit discovery to identify and manipulate important neurons for better LLM unlearning.


<details>
  <summary>Details</summary>
Motivation: To address the issue of uniform interventions in existing localization-based methods, which risk catastrophic over-forgetting or incomplete erasure of the target knowledge.

Method: CLUE framework identifies the forget and retain circuit composed of important neurons, and then the circuits are transformed into conjunctive normal forms (CNF). The assignment of each neuron in the CNF satisfiability solution reveals whether it should be forgotten or retained. We then provide targeted fine-tuning strategies for different categories of neurons.
...
### [87] [Binary Autoencoder for Mechanistic Interpretability of Large Language Models](https://arxiv.org/abs/2509.20997)
*Hakaze Cho,Haolin Yang,Brian M. Kurkoski,Naoya Inoue*

Main category: cs.LG

TL;DR: This paper introduces Binary Autoencoder (BAE), a novel autoencoder that promotes feature sparsity and independence by enforcing minimal entropy on hidden activations, showing effectiveness in feature extraction and analysis of LLMs.


<details>
  <summary>Details</summary>
Motivation: Existing works rely on autoencoders with implicit training-time regularization, which do not guarantee global sparsity among instances, leading to dense features that harm feature sparsity and atomization.
...
Result: BAE was empirically demonstrated in two major applications: feature set entropy calculation and feature untangling. It showed effectiveness in extracting atomized features and characterizing inference dynamics of LLMs and In-context Learning.
...
TL;DR: 本文从信息移除的角度分析了In-context Learning (ICL)的机制，发现ICL通过选择性地移除冗余信息来提高输出质量，并识别出关键的注意力头（Denoising Heads）。


<details>
  <summary>Details</summary>
Motivation: 尽管ICL是一种新兴的少样本学习范式，但其内部机制尚不明确。本文旨在揭示ICL的核心机制，以增进对现代语言模型的理解。

Method: 本文通过信息移除的视角研究ICL机制，利用低秩滤波器选择性地移除隐藏状态中的特定信息，并通过测量隐藏状态来观察ICL的效果。

Result: 本文发现，在零样本场景中，语言模型会将查询编码为包含所有可能任务信息的非选择性表示，导致输出不聚焦于目标任务。而通过低秩滤波器选择性地移除信息可以引导模型关注目标任务。此外，本文还发现了关键的注意力头（Denoising Heads），它们在信息移除过程中起着重要作用。

Conclusion: 本文揭示了In-context Learning (ICL)的关键机制，即通过信息去除过程实现任务导向的表示。
...
### [89] [DELTA-Code: How Does RL Unlock and Transfer New Programming Algorithms in LLMs?](https://arxiv.org/abs/2509.21016)
*Yiyou Sun,Yuhan Cao,Pohao Huang,Haoyue Bai,Hannaneh Hajishirzi,Nouha Dziri,Dawn Song*

Main category: cs.LG

TL;DR: DELTA-Code 是一个用于测试 LLM 是否能通过强化学习获得新推理策略的基准，实验发现模型在经历长时间无进展后能突然提高性能，但其在转化任务中的表现仍有不足。


<details>
  <summary>Details</summary>
Motivation: 研究 LLM 是否能够获得或泛化真正的新推理策略，而不仅仅是预训练或后训练期间编码的技能。

Method: DELTA-Code 是一个受控基准，使用模板化问题生成器来隔离推理技能，并引入完全 OOD 的问题家族，以测试学习能力和可转移性。

Result: 实验揭示了一个显著的掌握阶段转变：在长时间的近零奖励后，RL 训练的模型突然达到接近完美的准确性。结果还显示了在家庭内和重组技能上的显著提升，但在转化案例中仍有持续的弱点。

Conclusion: DELTA 提供了一个干净的测试平台，用于探测 RL 驱动推理的极限，并理解模型如何超越现有先验来获得新的算法技能。
...
### [90] [ScaleDiff: Scaling Difficult Problems for Advanced Mathematical Reasoning](https://arxiv.org/abs/2509.21070)
*Qizhi Pei,Zhuoshi Pan,Honglin Lin,Xin Gao,Yu Li,Zinan Tang,Conghui He,Rui Yan,Lijun Wu*

Main category: cs.LG

TL;DR: 本文提出了一种名为ScaleDiff的简单而有效的管道，用于大规模生成困难问题，并展示了其在性能上的优势。


<details>
  <summary>Details</summary>
Motivation: 现有的方法在生成困难问题时面临高计算/API成本、提示复杂性和生成问题难度有限等挑战。因此，我们需要一种更有效和可扩展的方法来生成困难问题。

Method: 我们提出了ScaleDiff，这是一个简单但有效的管道，用于扩展困难问题的创建。我们使用自适应思维模型仅通过一次前向传递来高效地从现有数据集中识别困难问题，并训练一个专门的问题生成器DiffGen-8B来大规模生成新的困难问题。

Result: 在ScaleDiff-Math数据集上微调Qwen2.5-Math-7B-Instruct后，性能提高了11.3%，并在多个基准测试中达到了65.9%的平均准确率，超过了最近的大型推理模型。
...
### [91] [CARINOX: Inference-time Scaling with Category-Aware Reward-based Initial Noise Optimization and Exploration](https://arxiv.org/abs/2509.17458)
...
TL;DR: CARINOX是一种新的文本到图像扩散模型方法，通过结合噪声优化和探索以及基于人类判断的相关奖励选择程序，提高了图像与文本提示的对齐度。


<details>
  <summary>Details</summary>
Motivation: 现有的推理时间方法在单独使用时存在内在限制，例如优化可能因不良初始化或不利的搜索轨迹而停滞，而探索可能需要过多的样本才能找到满意的输出。此外，单一奖励指标或随意组合无法可靠地捕捉所有方面的组成性，导致指导效果弱或不一致。

Method: CARINOX是一个统一的框架，结合了噪声优化和探索，并采用了基于与人类判断相关的相关性的奖励选择程序。

Result: CARINOX在T2I-CompBench++基准上平均对齐分数提高了+16%，在HRS基准上提高了+11%。

Conclusion: CARINOX在两个互补基准上提高了平均对齐分数，并且在所有主要类别中都优于最先进的优化和探索方法，同时保持了图像质量和多样性。
...
### [92] [Leveraging NTPs for Efficient Hallucination Detection in VLMs](https://arxiv.org/abs/2509.20379)
*Ofir Azachi,Kfir Eliyahu,Eyal El Ani,Rom Himelstein,Roi Reichart,Yuval Pinter,Nitay Calderon*

Main category: cs.CV

TL;DR: 本文提出了一种基于VLM下一个标记概率的高效幻觉检测方法，并展示了其在实际应用中的有效性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）的幻觉会损害VLM的可靠性。目前常见的检测方法计算成本高且增加模型延迟。因此，我们需要一种更高效的方法来检测幻觉。

Method: 我们通过在基于VLM的下一个标记概率（NTP）的信号上训练传统机器学习模型，探索了一种高效的实时幻觉检测方法。此外，我们还引入了一个包含1,400个由VLM生成内容得出的人类标注语句的数据集，并使用它来测试我们的NTP基础轻量级方法。

Result: NTP基础特征是幻觉的有效预测因子，使快速简单的机器学习模型能够达到与强大VLM相当的性能。此外，将这些NTP与语言NTP结合，通过仅将生成文本重新输入VLM计算，可以增强幻觉检测性能。最后，将VLM的幻觉预测分数集成到NTP基础模型中，比单独使用VLM或NTP表现更好。

Conclusion: 我们希望这项研究为简单、轻量级的解决方案铺平道路，以提高VLM的可靠性。
...
### [93] [Human Semantic Representations of Social Interactions from Moving Shapes](https://arxiv.org/abs/2509.20673)
*Yiling Yun,Hongjing Lu*

Main category: cs.CV

TL;DR: 本研究探讨了人类如何利用语义表示来补充视觉特征，以理解简单的移动形状的社会互动。结果显示，动词嵌入能最好地解释人类的相似性判断，表明社会感知反映了社会互动的语义结构。


<details>
  <summary>Details</summary>
Motivation: 先前的研究往往关注视觉特征，而我们研究了人类用来补充视觉特征的语义表示。

Method: 在研究1中，我们直接让人类参与者根据移动形状的印象对动画进行标记。在研究2中，我们通过人类相似性判断测量了27种社会互动的表征几何，并将其与基于视觉特征、标签和动画描述语义嵌入的模型预测进行了比较。

Result: 人类响应是分布的。语义模型提供了补充信息来解释人类判断。其中，从描述中提取的动词嵌入最好地解释了人类相似性判断。
...
### [94] [Seeing Through Words, Speaking Through Pixels: Deep Representational Alignment Between Vision and Language Models](https://arxiv.org/abs/2509.20751)
*Zoe Wanying He,Sean Trott,Meenakshi Khosla*

Main category: cs.CV

TL;DR: 本研究探讨了深度视觉-only和语言-only模型如何在部分对齐的表示空间中投影输入，并发现它们在中后期层达到对齐峰值，这种对齐是语义性的，并且随着示例聚合而增强。


<details>
  <summary>Details</summary>
Motivation: 我们想要了解深度视觉-only和语言-only模型如何在部分对齐的表示空间中投影输入，以及这种对齐是否能捕捉人类偏好。

Method: 我们系统地研究了这些问题，包括分析对齐在模型层中的出现位置、支持对齐的视觉或语言提示、对齐是否捕捉人类偏好以及聚合同一概念的示例如何影响对齐。

Result: 我们发现对齐在两种模型类型的中后期层达到峰值，反映了从模态特定到概念共享表示的转变。这种对齐对于仅外观的变化是稳健的，但当语义被改变时（例如对象移除或词序混乱）会崩溃，这表明共享代码确实是语义性的。此外，平均嵌入可以增强对齐而不是模糊细节。
...
### [95] [TABLET: A Large-Scale Dataset for Robust Visual Table Understanding](https://arxiv.org/abs/2509.21205)
*Iñigo Alonso,Imanol Miranda,Eneko Agirre,Mirella Lapata*

Main category: cs.CV

TL;DR: TABLET is a large-scale VTU dataset with 4 million examples across 20 tasks, designed to improve the robustness and generalization of vision-language models on real-world table visualizations.
...
### [96] [Sigma: Semantically Informative Pre-training for Skeleton-based Sign Language Understanding](https://arxiv.org/abs/2509.21223)
*Muxin Pu,Mei Kuan Lim,Chun Yong Chong,Chen Change Loy*

Main category: cs.CV

TL;DR: Sigma is a new framework for sign language understanding that improves upon existing methods by addressing their limitations through a combination of techniques.


<details>
  <summary>Details</summary>
Motivation: To address the three key limitations of current SLU methods: weak semantic grounding, imbalance between local details and global context, and inefficient cross-modal learning.
...
### [97] [Evaluating the Evaluators: Metrics for Compositional Text-to-Image Generation](https://arxiv.org/abs/2509.21227)
...
TL;DR: 本文对文本-图像生成评估度量进行了广泛研究，发现没有一种度量在所有任务中表现一致，且某些度量在特定情况下表现更好。


<details>
  <summary>Details</summary>
Motivation: 评估文本-图像生成是否真正捕捉提示中描述的对象、属性和关系仍然是一个核心挑战。然而，目前的评估依赖于自动化度量，这些度量通常是根据惯例或流行程度采用的，而不是基于与人类判断的验证。因此，了解这些度量如何反映人类偏好至关重要。

Method: 本文对广泛使用的组合文本-图像评估度量进行了广泛的研究，分析不仅限于简单的相关性，还考察了它们在各种组合挑战中的行为，并比较了不同度量家族与人类判断的对齐情况。

Result: 研究结果显示，没有单一的度量在所有任务中表现一致；性能随着组合问题的类型而变化。值得注意的是，尽管VQA-based度量很受欢迎，但它们并不总是更优，而某些基于嵌入的度量在特定情况下表现更强。图像-only度量由于设计用于感知质量而非对齐，因此在组合评估中的贡献很小。

Conclusion: 研究结果强调了在评估和生成过程中仔细和透明地选择度量标准的重要性。
...
### [98] [Hallucination as an Upper Bound: A New Perspective on Text-to-Image Evaluation](https://arxiv.org/abs/2509.21257)
*Seyed Amir Kasaei,Mohammad Hossein Rohban*

Main category: cs.CV

TL;DR: 本文探讨了文本到图像生成模型中的幻觉现象，并提出了一种新的分类法来更全面地评估这些模型。


<details>
  <summary>Details</summary>
Motivation: 现有的评估主要关注对齐问题，即检查提示中指定的元素是否出现，但忽略了模型在提示之外生成的内容。因此，需要一种新的框架来更全面地评估文本到图像生成模型。

Method: 本文提出了一个分类法，将文本到图像生成模型中的幻觉分为属性、关系和对象三类，并讨论了这种框架如何提供评估的上限并揭示隐藏的偏见。

Result: 本文提出了一种新的框架来定义文本到图像生成模型中的幻觉，并引入了一个分类法，以更好地评估这些模型。

Conclusion: 本文提出了一种新的框架来定义文本到图像生成模型中的幻觉，并引入了一个分类法，以更好地评估这些模型。
...
### [99] [Automotive-ENV: Benchmarking Multimodal Agents in Vehicle Interface Systems](https://arxiv.org/abs/2509.21143)
