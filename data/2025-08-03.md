...
Motivation: 在线房产预订平台依赖于来自第三方供应商的准确、最新的信息，但这些外部数据源经常受到不完整或不一致细节的影响，这可能会让用户感到沮丧并导致市场份额损失。

Method: 我们评估了两种著名的大型语言模型：使用QLoRA微调的Mistral 7B和使用优化系统提示的Mixtral 8x7B。

Result: Mixtral 8x7B在完整性（99.6% vs. 93%）、精确度（98.8% vs. 96%）和幻觉率（1.2% vs. 4%）方面优于Mistral 7B，生成的内容更短但更简洁（平均249 vs. 277个单词）。然而，这带来了显著更高的计算成本：50GB VRAM和每小时1.61美元，而Mistral 7B则是5GB和每小时0.16美元。
...
<details>
  <summary>Details</summary>
Motivation: Current systems for electric power marketing customer service face challenges such as slow response times, inflexible procedures, and limited accuracy. While large language models (LLMs) have strong general capabilities, they lack domain expertise and empathy required in this field.

Method: Introduce ElectriQ, a benchmark designed to evaluate and enhance LLMs in electric power marketing scenarios. It includes a dialogue dataset covering six key service categories and four evaluation metrics: professionalism, popularity, readability, and user-friendliness. Additionally, a domain-specific knowledge base and a knowledge augmentation method are proposed.

Result: Experiments on 13 LLMs show that smaller models like LLama3-8B, when fine-tuned and augmented, can surpass GPT-4o in terms of professionalism and user-friendliness.
...
<details>
  <summary>Details</summary>
Motivation: 非法市场已经越来越多地转移到互联网的隐蔽部分，包括深网和暗网，以及Telegram、Reddit和Pastebin等平台。检测和分类这些内容仍然具有挑战性，因为标记数据有限，非法语言不断演变，以及在线来源的结构异质性。

Method: 本文提出了一种分层分类框架，结合了微调的语言模型和半监督集成学习策略。首先使用基于熵的加权投票的半监督集成XGBoost、随机森林和SVM来检测销售相关的文档，然后进一步将这些文档分类为毒品、武器或凭证销售。

Result: 实验结果表明，该模型在三个数据集（包括我们的多源语料库、DUTA和CoDA）上优于多个基线模型，包括BERT、ModernBERT、DarkBERT、ALBERT、Longformer和BigBird。模型达到了0.96489的准确率，0.93467的F1分数和0.95388的TMCC，证明了其在现实世界非法内容检测中的有效性。

Conclusion: 本文提出了一种分层分类框架，结合了微调的语言模型和半监督集成学习策略，以在多样化的平台上检测和分类非法市场内容。实验结果表明，该模型在多个数据集上优于多个基线模型，表现出强大的泛化能力、在有限监督下的鲁棒性和在现实世界非法内容检测中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在分类和摘要任务中已被广泛使用，但它们在执行主题分析方面的能力尚未得到充分探索。传统的机器学习模型在多标签分类中用于主题分析，但难以处理未见过的情况。LLMs提供了另一种选择，但常常过度生成和幻觉。

Method: 提出了一种混合框架，将基于嵌入的机器学习模型与大型语言模型（LLMs）集成。该方法使用机器学习模型来预测最佳的LCSH标签数量以指导LLM预测，并用实际的LCSH术语对预测的术语进行后期编辑以减轻幻觉。

Result: 实验结果显示，提供初始预测来引导LLM生成并施加后期编辑可以产生更受控和词汇对齐的输出。
...
<details>
  <summary>Details</summary>
Motivation: 现有的实体匹配方法在评估时通常不考虑来源、大小和信息密度等因素，而这些因素在现实世界的KG中可能有很大差异。因此，现有方法在需要整合多样且复杂上下文的场景中可能表现不佳。

Method: 我们提出了一种新的KG集成方法，包括标签匹配和三元组匹配。我们使用字符串操作、模糊匹配和向量相似性技术来对齐实体和谓词标签。然后，我们识别传达相似信息的三元组之间的映射，利用这些映射来提高实体匹配的准确性。

Result: 我们的方法在OAEI竞赛和监督方法中表现出色，实现了高精度。此外，我们引入了一个新的数据集来更全面地评估三元组匹配步骤。

Conclusion: 我们的方法在OAEI竞赛和监督方法中表现出色，实现了高精度。此外，我们引入了一个新的数据集来更全面地评估三元组匹配步骤。
...
<details>
  <summary>Details</summary>
Motivation: 幻觉是大型语言模型生成的内容与输入或现实世界事实不一致的现象，这影响了模型的可靠性和实用性。因此，需要对幻觉进行严谨的研究和处理。

Method: 本文通过学习理论框架（如PAC-Bayes和Rademacher复杂性）推导了幻觉风险的界限，并调查了检测策略，如token级别的不确定性估计、置信度校准和注意力对齐检查。同时，讨论了缓解方法，包括检索增强生成、幻觉感知微调、logit校准以及事实验证模块的整合。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于语义的检索方法无法收集既主题相关又时间连贯的证据，因此本文旨在解决这一问题，提升RAG系统处理纵向查询的能力。

Method: 本文的方法首先将用户的查询分解为核心主题和时间窗口，然后使用专门的检索器，将语义匹配与时间相关性校准，确保收集到一个连续的证据集，覆盖整个查询时期。

Result: 在ADQAB基准测试中，本文的方法在答案准确性上取得了显著提升，比标准RAG实现高出13%至27%。

Conclusion: 本文提出了一个重新设计的RAG管道框架，以注入时间逻辑，使RAG系统能够进行复杂、现实世界问题所需的细微和演进分析。
...
Method: 使用稀疏自编码器（SAE）字典学习流程，在每个模型的残差流激活上使用SAE，通过激活相关性对单义特征进行对齐，并通过SVCCA和RSA比较匹配的特征空间。

Result: 中间层表现出最强的重叠，而早期和晚期层显示较少的相似性。初步实验将分析从单个标记扩展到多标记子空间，表明语义相似的子空间与语言模型的交互方式相似。

Conclusion: 大型语言模型尽管规模不同，但会将世界划分为广泛相似且可解释的特征，这加强了模型间可解释性的基础。
...
Method: 我们分析了来自ClinicalTrials.gov的22,107个双臂平行干预性临床试验，并开发了两个预测模型：一个分类器用于预测实验组是否有更高的SAE率，以及一个回归模型用于预测对照组中SAE的比例。使用预训练语言模型（如ClinicalT5、BioBERT）进行特征提取，并结合下游模型进行预测。为了保持超出局部语言模型输入限制的长试验文本的语义表示，开发了一种滑动窗口方法进行嵌入提取。
...
Conclusion: 总结结果数据在ClinicalTrials.gov中仍被低估利用。在试验开始前估计结果的潜力是一个改进试验设计和标记预期与报告的安全结果之间差异的机会。
...
<details>
  <summary>Details</summary>
Motivation: 由于LLM的快速发展，需要有效的机制将连续的多模态数据转换为适合语言处理的离散表示。然而，目前缺乏一个全面的调查来系统地检查LLM系统中的VQ技术。

Method: 本文对8种典型的VQ变体进行了分类，并分析了它们的算法原理、训练动态和与LLM流水线的集成挑战。此外，还讨论了现有研究在无LLMs的经典应用、基于LLM的单模态系统和基于LLM的多模态系统中的情况。

Result: 本文提出了第一个针对LLM的离散标记化方法的结构化分类和分析，讨论了量化策略如何影响对齐、推理和生成性能，并识别了关键挑战，如代码本崩溃、不稳定的梯度估计和模态特定编码约束。

Conclusion: 本文通过构建第一个结构化的分类和分析，填补了现有研究的空白，为高效且通用的多模态系统的发展提供了基础参考。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型虽然可以捕捉文本中的复杂关系，但它们以成本高和产生不存在的信息（即幻觉）而闻名。如果这些信息不正确，投入的资源将被浪费。因此，我们需要一种方法来解决这些问题。

Method: 我们提出了语言模型链（LMC）算法，其中语言模型对给定文本的响应只有在存在于可能的答案集合中时才是正确的，错误的响应对应的文本会被输入到一个更可预测（但更慢）的语言模型中。这个过程重复进行，直到所有关于文本的预测都是正确的。

Result: 我们使用LMC算法从医疗文档中提取患者的出生日期，将一组语言模型组合成多阶段级联，显著提高了预测速度和准确性，同时大大减少了相应的幻觉。

Conclusion: 我们认为LMC算法对知识提取领域有显著贡献，并应在未来进一步探索。
...
<details>
  <summary>Details</summary>
Motivation: 2021年GameStop做空挤兑事件引发了关于在线情绪对股价影响的疑问。本文旨在探讨从社交媒体讨论中提取的情绪是否能有意义地预测股市波动。

Method: 本研究聚焦于Reddit的r/wallstreetbets论坛，分析与GameStop（GME）和AMC娱乐（AMC）相关的社交媒体情感。我们采用了两种现有的基于文本的情感分析方法，并引入了一种新的模型，即使用ChatGPT标注并微调的RoBERTa模型，以更好地理解社交媒体讨论中常见的非正式语言和表情符号。我们使用相关性和因果性度量来确定这些模型的预测能力。

Result: 研究发现，社交媒体情绪与股票价格之间只有微弱的相关性。同时，简单的指标如评论数量和谷歌搜索趋势表现出更强的预测信号。

Conclusion: 研究结果表明，社交媒体情感与股票价格之间的相关性较弱，而简单的指标如评论数量和谷歌搜索趋势则表现出更强的预测信号。这突显了零售投资者行为的复杂性，并表明传统的情感分析可能无法完全捕捉市场变动的在线讨论的细微差别。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）在多语言能力方面取得了进展，但它们在不同语言和任务中的表现差异很大。在多语言检索增强生成（RAG）系统中，知识库（KB）通常从高资源语言（如英语）共享到低资源语言，导致从KB中检索的信息与上下文的语言不同。因此，需要评估不同的提示翻译策略的影响。

Method: 本文系统评估了不同提示翻译策略对多语言系统中基于RAG增强的LLM进行分类任务的影响。

Result: 实验结果表明，优化的提示策略可以显著提高跨语言的知识共享，从而提升下游分类任务的性能。

Conclusion: 研究结果表明，优化的提示策略可以显著提高跨语言的知识共享，从而提升下游分类任务的性能。研究提倡更广泛地利用多语言资源共享和跨语言提示优化，尤其是对于非英语语言，特别是资源较少的语言。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在探讨母语与非母语英语使用者在在线美国计算机课程中的同伴反馈体验差异。

Method: 使用基于Twitter-roBERTa的模型分析了500名学生的同伴评论的情感，并将情感评分和同伴反馈评分与学生的语言背景相关联。

Result: 结果表明，母语为英语的学生对反馈的评价较低，而非母语者写作更积极，但收到的积极情感较少。当控制性别和年龄时，出现了显著的交互作用。

Conclusion: 研究发现，母语为英语的学生对反馈的评价不如非母语者积极，而非母语者虽然写作更积极，但收到的积极情感较少。当控制性别和年龄时，出现了显著的交互作用，表明语言背景在塑造同伴反馈体验中起着适度但复杂的作用。
...
<details>
  <summary>Details</summary>
Motivation: 长期记忆是影响大型语言模型代理（LLM Agents）推理能力的关键因素。结合有效的记忆机制可以显著增强LLM Agents的决策能力和上下文连贯性。然而，现有的方法在结构化记忆组织和高效检索方面存在不足。

Method: 我们提出了一个分层记忆（H-MEM）架构，该架构基于语义抽象程度以多级方式组织和更新记忆。每个记忆向量都嵌入了一个位置索引编码，指向下一层中语义相关的子记忆。在推理阶段，基于索引的路由机制实现了高效的逐层检索，而无需进行全面的相似性计算。

Result: 我们在LoCoMo数据集的五个任务设置上评估了我们的方法。实验结果表明，我们的方法始终优于五种基线方法。

Conclusion: 我们的方法在长期对话场景中表现出色，优于五种基线方法，证明了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 在文档级关系抽取中，实体可能在文档中多次出现，它们的关系可以从一个上下文转移到另一个上下文。准确预测整个文档中两个实体之间的关系需要构建跨越所有相关句子的全局上下文。然而，以往的方法只关注提及实体的句子，无法捕捉准确的关系抽取所需的完整文档上下文。

Method: 该研究引入了一种新的输入嵌入方法，通过将实体表示为独立于其在文档中位置的独立段，利用全局关系和多句推理来捕获文档中提到的实体的位置，而不是仅关注它们出现的范围。

Result: 所提出的方法在三个基准关系抽取数据集（DocRED、Re-DocRED 和 REBEL）上进行了测试，实验结果表明该方法能够准确预测文档级设置中实体之间的关系。

Conclusion: 该研究在理论和实践上都有重要意义，理论上推进了文档级关系抽取中的全局上下文建模和多句推理，实践中增强了关系检测，从而提高了需要全面实体级洞察和可解释性的现实世界自然语言处理应用的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前基准主要关注RAG系统的整体性能，很少评估LLM特定的能力。现有的基准强调诸如噪声鲁棒性等广泛方面，但缺乏对文档利用的系统性和细粒度评估框架。
...
Method: 结合稀疏自编码器和激活修补，我们从Pythia-70M和Pythia-2.8B中提取单义特征，同时它们在CoT和plain（noCoT）提示下解决GSM8K数学问题。

Result: 将一小部分CoT推理特征交换到noCoT运行中，在2.8B模型中显著提高了答案的对数概率，但在70M模型中没有可靠的效果，这揭示了一个清晰的规模阈值。CoT还导致更大的模型中激活稀疏性和特征可解释性得分显著提高，表明内部计算更加模块化。例如，模型生成正确答案的信心从1.2提高到4.3。
...
<details>
  <summary>Details</summary>
Motivation: 由于缺乏公开的标记数据集，研究PII泄露文本的识别和检索受到阻碍。本文旨在创建一种合成数据集，以促进可重复的研究。

Method: 本文开发了一种新方法，通过三个文本生成大型语言模型（LLMs）生成合成PII标记的多文本跨度数据集，并使用顺序指令提示来模仿原始Reddit帖子。

Result: 本文创建了一个包含19个PII泄露类别的分类法，并生成并发布了从三个LLMs生成的合成PII标记数据集。评估了该方法在可重复性等价性、不可链接性和不可区分性方面的有效性。

Conclusion: 本文提出了一种创建合成PII泄露数据的方法，以促进对在线社交媒体中PII隐私风险的可重复研究。
...
<details>
  <summary>Details</summary>
Motivation: Existing methods apply fixed compression rates, over-compressing simple queries or under-compressing complex ones.

Method: ACC-RAG combines a hierarchical compressor (for multi-granular embeddings) with a context selector to retain minimal sufficient information, akin to human skimming.

Result: Evaluated on Wikipedia and five QA datasets, ACC-RAG outperforms fixed-rate methods and matches/unlocks over 4 times faster inference versus standard RAG while maintaining or improving accuracy.
...
<details>
  <summary>Details</summary>
Motivation: 旨在通过结合情感信号和传统市场指标，提高投资组合优化的效果，并增强系统的稳定性。

Method: 提出了一种分层框架，将轻量级大语言模型（LLM）与深度强化学习（DRL）结合，利用金融新闻中的情感信号和传统市场指标进行投资组合优化。

Result: 在2018年至2024年的数据上，该框架实现了26%的年化收益率和1.2的夏普比率，优于均等加权和标普500基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言机器学习模型在解释性、适应性和资源效率方面存在不足，因此需要探索将神经网络与外部符号信息系统的结合。

Method: 本文采用系统文献综述的方法，对相关研究进行了分类和分析。

Result: 本文总结了多种通过与外部符号信息系统的交互来提升视觉-语言理解的技术，并讨论了其优势和挑战。

Conclusion: 本文综述了通过与外部符号信息系统的交互来改进视觉-语言理解的技术，强调了神经符号系统在提高推理和记忆能力方面的潜力。
...
Method: 本文调查了深度学习方法在意图识别中的应用，涵盖了从单模态到多模态技术的转变、相关数据集、方法、应用和当前挑战。

Result: 本文总结了多模态意图识别的最新进展，并为研究人员提供了未来研究的方向。

Conclusion: 本文提供了关于多模态意图识别（MIR）的最新发展和未来研究方向的见解。
...
<details>
  <summary>Details</summary>
Motivation: 从组织数据存储中获取操作智能是一个关键挑战，因为数据保密性与数据集成目标之间的二分法，以及自然语言处理（NLP）工具在特定领域知识结构方面的局限性。

Method: 我们讨论了知识图谱的构建，并将知识提取过程分解为命名实体识别、共指消解、命名实体链接和关系提取的功能组件。然后，我们评估了十六个NLP工具与大型语言模型（LLMs）的能力进行比较。

Result: 我们评估了可以在受控、保密环境中运行的NLP和LLM工具的零样本性能。基于我们观察到的重大性能限制，我们讨论了可信NLP和LLM工具及其技术成熟度水平在航空等关键行业中的广泛使用相关挑战。

Conclusion: 我们得出结论，需要增强可信NLP和LLM工具的信任度，并提供我们的开源数据集以支持进一步的基准测试和评估。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）在金融自然语言处理（FinNLP）任务中表现出色，但对常用LLMs的系统比较仍较少。鉴于LLMs在金融分析中的快速发展和日益增长的影响，本研究对五个领先的LLMs进行了全面的比较评估。

Method: 本研究通过三种方法评估模型性能：人工标注、自动的词汇-语义度量（ROUGE、余弦相似度、Jaccard）以及模型行为诊断（提示级方差和跨模型相似性）。

Result: 研究结果表明，GPT提供了最连贯、语义对齐且上下文相关的答案；其次是Claude和Perplexity。Gemini和DeepSeek则表现出更多的变异性且一致性较低。此外，输出的相似性和稳定性因公司而异，并随时间变化，表明它们对提示的编写和所用源材料非常敏感。

Conclusion: 研究发现，GPT在回答的连贯性、语义对齐和上下文相关性方面表现最佳；其次是Claude和Perplexity。Gemini和DeepSeek则表现出更多的变异性且一致性较低。此外，输出的相似性和稳定性因公司而异，并随时间变化，表明它们对提示的编写和所用源材料非常敏感。
...
<details>
  <summary>Details</summary>
Motivation: 由于领域特定知识的限制，单一模型只能处理特定任务的需求，如日志解析、根本原因分析。而结合多个模型可以实现更高效的结果，这在之前的集成学习和最近的LLM训练领域已被证明。因此，本文旨在解决AIOps中的类似挑战。

Method: 本文首先提出了一个协作专家框架(CoE-Ops)，结合了一个通用的大语言模型任务分类器。引入了检索增强生成机制，以提高框架在处理高层（如代码、构建、测试等）和低层（如故障分析、异常检测等）问答任务的能力。最后，将所提出的方法应用于AIOps领域，并在DevOps-EVAL数据集上进行了广泛的实验。

Result: 实验结果表明，CoE-Ops在高层次AIOps任务的路由准确性方面相比现有CoE方法提高了72%，在DevOps问题解决中比单一AIOps模型提高了高达8%的准确性，并且在准确性上超过了大规模的Mixture-of-Experts (MoE)模型高达14%。

Conclusion: CoE-Ops在AIOps领域表现出色，相比现有的CoE方法在高层次AIOps任务的路由准确性提高了72%，在DevOps问题解决中比单一AIOps模型提高了高达8%的准确性，并且在准确性上超过了大规模的Mixture-of-Experts (MoE)模型高达14%。
...
Method: 我们利用从视觉大语言模型（VLMs）获得的流程图的图表示，并将它们融入基于文本的RAG系统中，以展示这种方法可以实现电信领域的问答图像检索。

Result: 结果表明，使用微调的VLM模型获得的图表示与真实情况的编辑距离较低，这说明这些表示对于流程图图像具有鲁棒性。此外，使用这些表示的问答方法在使用基于文本的嵌入模型（包括电信领域适应的模型）时表现出良好的检索性能。

Conclusion: 我们的方法减轻了推理过程中对VLM的需求，这对于部署的QA系统来说是一个重要的成本优势。
...
Result: Extensive evaluation across ten state-of-the-art models reveals that even leading models like Claude-3.7 and GPT-o1 demonstrate reasoning factual accuracy of only 81.93% and 82.57% respectively. RELIANCE significantly enhances factual robustness (up to 49.90% improvement) while maintaining or improving performance on challenging benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: 通过手动审查来自关联电子健康记录的自由文本注释来创建参考标准标签通常需要大量的时间和资源分配，因此需要一种更有效的方法来验证基于代码的算法的测量特性。

Method: 我们描述了一个加速过程，引入了两种不同的机制：1）使用自然语言处理（NLP）以减少人类审阅者审查每张病历的时间，2）一种多波适应性抽样方法，具有预定义的标准，在性能特征被识别出足够精确时停止验证研究。

Result: 我们实证证明，NLP辅助的注释过程将每张病历的审查时间减少了40%，并且使用预定义的停止规则和多波样本可以防止审查77%的患者病历，而对推导出的测量特征的精度影响有限。

Conclusion: 这种方法可以促进对用于定义关键研究参数的基于代码的算法的更常规验证，最终提高对从数据库研究中得出的发现的可靠性的理解。
...
<details>
  <summary>Details</summary>
Motivation: 本文旨在重新定义任意性，将其视为一种结构功能机制，而非规范缺陷或支配的症候。

Method: 本文基于费迪南·德·索绪尔的'符号任意性'概念，扩展了这一原则到法律和社会动态领域，并引入了'Motivation -> Constatability -> Contestability'链条来分析任意性。

Result: 本文通过Shannon熵模型形式化了任意性，并展示了其在法律、社会动态和人工智能系统中的应用。

Conclusion: 本文提出了一种新的关于任意性的理论，将其视为控制和关怀的核心操作符，并为分析高级人工智能系统的可解释性提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏对SDM在理解和模拟人类对话方面的实际效果的全面研究，特别是在与文本型大语言模型相比时。

Method: 本文提出了一种基于LLM的评估方法，该方法与人类判断紧密对齐，并结合了一个包含1,079个实例的基准数据集。

Result: 该数据集和评估方法有助于全面探索SDM在应对这些实际挑战中的表现。

Conclusion: 本文提出了一个基准数据集，以阐明当前SDM的发展状况并解决这些挑战。
...
<details>
  <summary>Details</summary>
Motivation: To determine if contemporary LLMs can perform natural language inference on mathematical texts and assess the quality of corpora generated by LLMs.

Method: The study constructs a corpus of Math NLI pairs with human-labeled hypotheses and evaluates LLM performance, including inter-group consistency.

Result: In some settings, LLMs using majority vote perform similarly to human-labeled data, but they still have difficulties with mathematical language and basic inferences.

Conclusion: LLMs show some ability in Math NLI tasks but still struggle with mathematical language and basic inferences. The study provides corpora for future research.
...
<details>
  <summary>Details</summary>
Motivation: 本文旨在探索使用In-Context Learning (ICL)与大型语言模型（LLMs）进行Frame Semantic Parsing（FSP），而无需对模型进行微调。
...
Result: 在与暴力事件相关的框架子集上进行了实验，该方法取得了具有竞争力的结果，FI的F1得分为94.3%，FSRL的F1得分为77.4%。

Conclusion: 研究结果表明，ICL为特定领域的FSP任务提供了一种实用且有效的替代传统微调的方法。
...
<details>
  <summary>Details</summary>
Motivation: RoPE relies on static, input-independent sinusoidal frequency patterns, limiting its ability to model context-sensitive relationships.
...
<details>
  <summary>Details</summary>
Motivation: 现有模型主要进行局部编辑，而SMART-Editor旨在通过全局一致性保持跨结构化和非结构化领域的组合布局和内容编辑。

Method: SMART-Editor框架采用两种策略：Reward-Refine（推理时的奖励引导细化方法）和RewardDPO（使用奖励对齐布局对的训练时偏好优化方法）。

Result: SMART-Editor在SMARTEdit-Bench基准测试中优于InstructPix2Pix和HIVE等强基线模型，RewardDPO在结构化设置中实现了高达15%的提升，Reward-Refine在自然图像上表现出优势。
...
<details>
  <summary>Details</summary>
Motivation: 尽管在基于大型语言模型的自然语言数据库接口方面取得了进展，但扩展到企业级数据目录仍然是一个研究不足的挑战。先前的工作依赖于领域特定的微调，这使得部署变得复杂，并且未能利用数据库元数据中包含的重要语义上下文。

Method: 我们引入了一个基于组件的检索架构，将数据库模式和元数据分解为离散的语义单元，并分别进行索引以实现有针对性的检索。
...
<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型（LLMs）在遇到模糊叙述文本时的行为，特别是中文文本的模糊性，以了解它们在处理模糊性时的表现。

Method: 我们通过收集和生成带有上下文的模糊句子及其对应的消歧对，创建了一个基准数据集，并将其系统地分为3个主要类别和9个子类别。

Result: 实验发现大型语言模型在处理模糊性时表现出显著的脆弱性，其行为与人类有显著差异。具体来说，大型语言模型无法可靠地区分模糊文本和非模糊文本，对模糊文本的解释过于自信，认为其只有一个含义而不是多个含义，并且在尝试理解各种可能含义时表现出过度思考。
...
Result: 对十种前沿视觉-语言模型的评估结果表现出令人失望的表现：最佳零样本F1仅为0.57，思维链推理仅带来适度提升（最高0.62 F1），远落后于人类（0.98 F1）。
...
TL;DR: 本文提出了一种名为LENS的新方法，通过分析内部表示来学习估计模型置信度，从而更精确地加权模型预测。该方法不需要修改模型参数，并且计算开销很小。实验结果表明，LENS在多项选择和布尔问答任务上优于传统集成方法。
...
Method: 我们提出了LENS（从神经状态中学习集成置信度），这是一种新颖的方法，通过分析内部表示来学习估计模型置信度。对于每个LLM，我们训练了一个轻量级的线性置信度预测器，该预测器利用逐层隐藏状态和归一化概率作为输入。

Result: 在多项选择和布尔问答任务上的实验结果表明，LENS在传统集成方法上取得了显著的提升。

Conclusion: 我们的研究结果表明，内部表示为确定模型置信度提供了有价值的信号，并可以有效地用于集成学习。
...
TL;DR: 本文提出了一种利用负样本更好选择正样本的方法，以增强少样本ICL的性能。通过构建正负样本语料库，并基于语义相似性选择例子，实验结果证明了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 最近的研究主要集中在利用正样本上，而忽视了负样本中的额外信息对上下文学习的潜在帮助。

Method: 我们构建了基于零样本思维链的正样本和负样本语料库。在推理过程中，我们采用基于语义相似性的方法从正样本和负样本语料库中选择最相似的例子。然后，我们根据与负样本的语义相似性从正样本语料库中进一步检索正样本，并将它们与之前选择的正样本拼接起来作为ICL演示。

Result: 实验结果表明，我们的方法超越了仅依赖最相似正例的方法，验证了负样本中的额外信息有助于提高ICL性能。

Conclusion: 实验结果表明，我们的方法优于仅依赖最相似正例的方法，验证了负样本中的额外信息通过改进正例选择来增强ICL性能。
...
TL;DR: MTMs are a new class of topic models that use interpretable features from sparse autoencoders to reveal deeper conceptual themes and enable controllable text generation. They outperform traditional and neural baselines in evaluations.


<details>
  <summary>Details</summary>
Motivation: Traditional topic models struggle to capture semantically abstract features, and neural variants are constrained by expressing topics as word lists. MTMs aim to overcome these limitations by using interpretable features and enabling controllable text generation.

Method: Mechanistic Topic Models (MTMs) operate on interpretable features learned by sparse autoencoders (SAEs) and use a semantically rich space to define topics. They also introduce 'topic judge,' an LLM-based pairwise comparison evaluation framework.

Result: MTMs match or exceed traditional and neural baselines on coherence metrics, are consistently preferred by topic judge, and enable effective steering of LLM outputs across five datasets.

Conclusion: MTMs can reveal deeper conceptual themes with expressive feature descriptions and enable controllable text generation using topic-based steering vectors. They match or exceed traditional and neural baselines on coherence metrics and are consistently preferred by topic judge.
...
TL;DR: 本文提出了一种名为TAP-GPT的新框架，用于利用结构化生物标志物数据进行阿尔茨海默病诊断。该框架通过使用上下文学习示例构建少样本表格提示，并使用参数高效的qLoRA适应进行微调，以执行AD或认知正常(CN)的临床二分类任务。TAP-GPT在AD诊断任务中表现出色，优于更先进的通用LLM和用于预测任务的表格基础模型(TFM)。这是首次将LLM应用于使用表格生物标志物数据的预测任务，为未来基于LLM的多智能体框架在生物医学信息学中的应用铺平了道路。


<details>
  <summary>Details</summary>
Motivation: 早期和准确诊断阿尔茨海默病(AD)需要分析异构生物标志物，通常以表格格式表示。大型语言模型(LLM)提供了前所未有的机会，用于具有结构化生物医学数据的预测。

Method: TAP-GPT框架通过使用来自结构化生物医学数据的上下文学习示例构建少样本表格提示，并使用参数高效的qLoRA适应进行微调，以执行AD或认知正常(CN)的临床二分类任务。

Result: TAP-GPT框架在AD诊断任务中表现出色，优于更先进的通用LLM和用于预测任务的表格基础模型(TFM)。

Conclusion: TAP-GPT框架利用TableGPT2强大的表格理解能力和LLM的编码先验知识，在AD诊断任务中优于更先进的通用LLM和用于预测任务的表格基础模型(TFM)。这是首次将LLM应用于使用表格生物标志物数据的预测任务，为未来基于LLM的多智能体框架在生物医学信息学中的应用铺平了道路。
...
TL;DR: 本研究探讨了大型语言模型在心理健康领域的实用推理能力，并引入了P-ReMe数据集。实验结果显示，Mistral和Qwen表现良好，而Claude-3.5-haiku在处理心理健康污名方面更为负责。


<details>
  <summary>Details</summary>
Motivation: 尽管最近在可解释性和个性化心理健康聊天机器人的开发方面取得了进展，但尚未探索可解释性和对话话语方面的推理方面。因此，我们正在研究大型语言模型（LLMs）在该领域的实用推理能力。

Method: 引入了P-ReMe数据集，并提出了适用于心理健康领域的隐含意义和预设的修改定义。随后，制定了两个隐含意义任务和一个预设任务。对四个模型（Llama3.1、Mistral、MentaLLaMa和Qwen）进行了基准测试。还提出了StiPRompts来研究心理健康污名，并评估了GPT-4o mini、Deepseek-chat和Claude-3.5-haiku的表现。
...
TL;DR: 本研究探讨了孟加拉语NLP面临的挑战，评估了多个大型语言模型的表现，并发现了令牌化效率与模型准确性之间的关系。研究结果表明，需要改进数据集质量和评估方法，以支持资源较少语言的NLP发展。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语在NLP研究中是资源较少的语言，但由于其独特的语言结构和计算限制，仍然面临挑战。缺乏标准化的评估基准是影响孟加拉语NLP性能的关键问题。

Method: 本研究系统地调查了阻碍孟加拉语NLP性能的挑战，重点关注缺乏标准化评估基准的问题。然后评估了10个最近的开源大型语言模型（LLMs）在8个翻译数据集上的表现，并进行了全面的错误分析以确定其主要失败模式。

Result: 研究发现，与英语相比，孟加拉语的表现存在一致的差距，尤其是对于小型模型和特定模型家族如Mistral。同时，某些架构如DeepSeek表现出有希望的鲁棒性，能够在多种语言中保持更稳定的性能。此外，研究还发现了一种令牌化效率与LLM准确性之间的反比关系。

Conclusion: 本研究揭示了当前模型在处理孟加拉语时的不足，并强调了改进数据集质量和评估方法的必要性。这项工作将促进对资源较少语言的NLP研究，有助于在全球范围内普及先进的语言技术。
...
TL;DR: 本研究发现了 MoE 模型中的 Super Experts (SEs)，这些专家在模型前向推理中起着关键作用，尽管数量有限，但修剪它们会导致性能显著下降。研究还揭示了 SEs 对注意力汇聚点的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖经验标准来识别关键专家，缺乏对专家异质性重要性的深入探索。本研究旨在揭示 SEs 的作用及其对模型性能的影响。

Method: 通过分析 SEs 在模型前向推理中的作用，研究者发现了 SEs 的特性，并通过修剪 SEs 来评估其对模型性能的影响。

Result: SEs 是在输出中表现出罕见但极端激活异常值的专家，它们在解码器层之间的隐藏状态中产生大量激活。修剪 SEs 会导致模型性能显著下降，尤其是在数学推理任务中。

Conclusion: 研究发现，MoE LLMs 依赖 Super Experts (SEs) 来诱导注意力汇聚点，这对注意力分数的分布至关重要，但 SEs 的修剪会显著破坏这一过程。
...
TL;DR: 本研究分析了GPT-4o-mini在改写敏感内容时的隐式调节行为，并评估了其敏感性变化的程度。结果表明，GPT-4o-mini会系统性地减少侮辱性和禁忌语言。此外，LLM在零样本分类句子敏感性方面表现出与传统方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管之前的研究主要集中在显式训练模型来调节和净化敏感内容，但很少有研究探讨LLM是否会在没有明确指令的情况下隐式净化语言。

Method: 本研究通过实验证明了GPT-4o-mini在改写敏感内容时的隐式调节行为，并评估了其敏感性变化的程度。同时，还评估了LLM在零样本情况下分类句子敏感性的能力，并将其性能与传统方法进行了比较。

Result: 实验结果表明，GPT-4o-mini在改写敏感内容时会系统性地将其调节为更不敏感的类别，显著减少了侮辱性和禁忌语言。此外，LLM在零样本分类句子敏感性方面表现出与传统方法相当的性能。

Conclusion: 研究发现GPT-4o-mini在改写敏感内容时会系统性地将内容向更不敏感的类别进行调节，显著减少了侮辱性和禁忌语言。此外，LLM在零样本分类句子敏感性方面表现出与传统方法相当的性能。
...
TL;DR: 本文提出MusT-RAG框架，利用检索增强生成技术以及专门的音乐向量数据库，以提升大型语言模型在音乐问答任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 由于大型语言模型在训练数据中音乐相关知识比例较小，因此在音乐相关应用中效果有限，需要改进。

Method: 提出MusT-RAG框架，结合检索增强生成技术，并构建了专门的音乐向量数据库MusWikiDB。

Result: 实验表明MusT-RAG在音乐问答任务中优于传统微调方法，且MusWikiDB比通用维基百科语料更有效。

Conclusion: MusT-RAG显著提升了大型语言模型在音乐领域适应能力，并且在音乐问答任务中表现出色。
...
TL;DR: 本文提出 TeQoDO 方法，利用大型语言模型的 SQL 编程能力和对话理论，无需监督构建任务导向对话本体，并在多个数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的任务导向对话系统依赖于外部数据库和显式本体，但构建这些本体需要手动标签或监督训练。本文旨在解决这一问题，提出一种无需监督的方法来构建本体。

Method: TeQoDO 方法利用大型语言模型的内在 SQL 编程能力结合提示中的对话理论，自主构建任务导向对话本体。

Result: TeQoDO 在下游对话状态跟踪任务中表现出色，且其构建的本体在规模上可以扩展，适用于 Wikipedia 和 ArXiv 数据集。

Conclusion: TeQoDO 是一种无需监督即可从头构建任务导向对话本体的方法，展示了其在下游对话状态跟踪任务中的竞争力，并且能够扩展到构建更大的本体，为提高大型语言模型的可解释性提供了一个步骤。
...
TL;DR: 本文介绍了MPCC，这是一个新的基准，用于评估多模态大语言模型（MLLM）在处理多模态约束方面的规划能力。实验显示，MLLM在面对复杂约束时表现不佳，这表明需要改进约束感知推理。


<details>
  <summary>Details</summary>
Motivation: 当前基准面临两个关键挑战：(1) 无法直接评估多模态现实规划能力，(2) 缺乏或隐含的跨模态约束。为解决这些问题，我们引入了MPCC，这是第一个系统评估MLLM处理多模态约束能力的基准。

Method: 我们引入了Multimodal Planning with Complex Constraints (MPCC)，这是第一个系统评估MLLM处理多模态约束能力的基准。针对第一个挑战，MPCC专注于三个现实任务：航班规划、日历规划和会议规划。为了解决第二个挑战，我们在这些任务中引入了复杂的约束（如预算、时间和空间），并设有不同难度级别（EASY、MEDIUM、HARD）以将约束复杂度与搜索空间扩展分开。

Result: 实验表明，封闭源代码模型仅实现了21.3%的可行计划，而开源模型平均低于11%。此外，我们观察到MLLM对约束复杂度高度敏感，并且传统的多模态提示策略在多约束场景中失败。

Conclusion: 我们的工作形式化了规划中的多模态约束，提供了严格的评估框架，并强调了在现实世界MLLM应用中进行约束感知推理的必要性。
...
TL;DR: Causal2Vec是一种通用的嵌入模型，旨在增强仅解码器LLM的性能，而无需更改其原始架构或引入显著的计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有的方法主要关注于移除LLM中的因果注意力掩码以实现双向注意力，这可能会削弱模型在预训练期间获得的语义信息提取能力。此外，领先的单向方法通常依赖于额外的输入文本来克服因果注意力的固有局限性，这不可避免地增加了计算成本。

Method: Causal2Vec采用了一个轻量级的BERT风格模型来预编码输入文本为一个上下文标记，然后将其添加到LLM的输入序列中，以允许每个标记捕捉上下文信息。此外，为了减轻最后标记池引入的近期偏差，将上下文标记和EOS标记的最后隐藏状态连接起来作为最终的文本嵌入。
...
TL;DR: 本研究探讨了本地部署的开源语言模型作为商业云服务的可行替代方案的可行性与性能，结果显示其在功能性能上具有竞争力，并提供了增强的数据控制、隐私保护和减少对云服务的依赖等优势。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的迅速普及，翻译领域既有机会也有挑战。虽然商业云基AI聊天机器人在翻译研究中引起了广泛关注，但数据隐私、安全性和公平获取的问题需要探索替代的部署模型。因此，本研究旨在探讨本地可部署的免费语言模型作为专有云服务的可行替代方案的可行性与性能。

Method: 本研究评估了三种安装在基于CPU的平台上的开源模型，并将其与商业在线聊天机器人进行了比较。评估重点是功能性能，而不是人机翻译质量的比较，这一领域已有大量研究。所选平台因其跨各种操作系统的可访问性和易用性而被选中。

Result: 本地部署虽然有自己的挑战，但增强的数据控制、改进的隐私和减少对云服务的依赖等优势具有说服力。研究结果表明，本地部署的开源语言模型在功能性能上可以与商业云服务相媲美，同时提供了额外的优势。

Conclusion: 本研究的结论是，本地部署的开源语言模型在功能性能上可以作为专有云服务的可行替代方案，同时提供了增强的数据控制、隐私保护和减少对云服务的依赖等优势。这些发现有助于推动AI技术的民主化，并为未来的研究和开发提供参考，以使LLM更易于使用和实用，特别是针对个人翻译者和小型企业的需求。
...
TL;DR: 提出了一种基于多关系图和结构熵最小化的无监督多文档摘要框架MRGSEM-Sum，通过构建多关系图和应用结构熵最小化算法进行聚类，生成高质量摘要。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只考虑单关系图并需要预定义聚类数量，限制了其全面表示丰富关系信息和自适应划分句子组的能力。

Method: 提出了一种基于多关系图和结构熵最小化的无监督多文档摘要框架MRGSEM-Sum，构建了融合语义和话语关系的多关系图，并应用二维结构熵最小化算法进行聚类，最后引入位置感知压缩机制生成摘要。

Result: 在四个基准数据集（Multi-News、DUC-2004、PubMed和WikiSum）上的实验表明，该方法优于之前的无监督方法，并在某些情况下达到与监督模型和大型语言模型相当的性能。人类评估显示生成的摘要具有高一致性和覆盖度。

Conclusion: MRGSEM-Sum在多个基准数据集上表现出色，生成的摘要质量接近人类水平。
...
TL;DR: 本文提出了一种针对阿拉伯语的增强型密集段落检索框架，通过引入一种新的注意力相关性评分机制，提高了问答任务中的排名准确性。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语由于其复杂的形态学、可选的元音符号以及现代标准阿拉伯语和各种方言共存，对自然语言处理和信息检索构成了挑战。然而，阿拉伯语在NLP研究和基准资源中仍处于欠代表状态。

Method: 本文提出了一种增强型密集段落检索（DPR）框架，核心是一种新的注意力相关性评分（ARS）机制，该机制用自适应评分函数替代了标准交互机制，以更有效地建模问题与段落之间的语义相关性。此外，还集成了预训练的阿拉伯语语言模型和架构改进，以提高检索性能。

Result: 该方法在回答阿拉伯语问题时显著提高了排名准确性，并且代码已公开在GitHub上。

Conclusion: 本文提出的增强型DPR框架为阿拉伯语的自然语言处理任务提供了一个有效的解决方案，有助于推动阿拉伯语在NLP领域的研究和发展。
...
TL;DR: 本文介绍了主动批判性思维的概念，并提出了两个新的基准测试来评估模型在不完整或误导性条件下进行数学推理的能力。实验表明，虽然现有模型在传统任务中表现良好，但在主动批判性思维方面存在困难。通过强化学习，我们可以显著提高模型在这方面的性能。


<details>
  <summary>Details</summary>
Motivation: 批判性思维对于构建稳健的AI系统至关重要，但之前的工作主要集中在被动批判性思维上，即模型只是拒绝有问题的查询而没有采取建设性的步骤来解决用户请求。

Method: 我们引入了主动批判性思维，这是一种模型主动从用户那里寻求缺失或澄清信息以更好地解决查询的范式。我们提出了GSM-MC和GSM-MCE两个新的基准测试，用于评估数学推理能力。此外，我们还使用增强的强化学习算法来提高模型的能力。

Result: 实验表明，尽管这些模型在传统推理任务中表现优异，但由于缺乏主动批判性思维，它们在处理不完整或误导性条件下的数学问题时表现出困难。然而，我们证明了强化学习可以显著提高这种能力。使用我们的增强强化学习算法，Qwen3-1.7B的准确率在GSM-MC上从0.15%提升到了73.98%。

Conclusion: 我们希望这项工作能通过主动批判性思维推动模型在解决问题时更有效地与用户合作。
...
TL;DR: 本文研究了如何通过微调大型语言模型来生成符合不同组织角色访问权限的响应，并构建了两个数据集来评估方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在企业环境中的广泛应用，根据用户角色控制模型行为变得至关重要。现有的安全方法通常假设统一访问，并专注于防止有害或有毒的输出，而没有解决特定角色的访问限制。

Method: 本文研究了三种建模策略：基于BERT的分类器、基于LLM的分类器和角色条件生成。

Result: 本文构建了两个互补的数据集来评估这些方法，并分析了模型在不同组织结构下的性能以及对提示注入、角色不匹配和越狱尝试的鲁棒性。

Conclusion: 本文结论是，通过微调大型语言模型可以生成反映不同组织角色访问权限的响应。
...
TL;DR: 本研究开发了临床安全-效果双轨基准（CSEDB），用于评估医学大语言模型的安全性和效果性。测试结果表明，大语言模型在临床应用中存在一定的安全和效果问题，领域特定的模型表现优于通用模型。研究结果有助于推动大语言模型在医疗环境中的更安全和有效的部署。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在临床决策支持中具有潜力，但在安全评估和效果验证方面面临重大挑战。需要一种标准化的指标来评估医学大语言模型的临床应用，以便进行比较分析、风险暴露识别和改进方向的确定。

Method: 开发了临床安全-效果双轨基准（CSEDB），这是一个基于临床专家共识的多维框架，包含30个涵盖关键领域的标准，如危重疾病识别、指南遵循和药物安全，并带有加权后果测量。32名专科医生开发并审查了2,069个开放式问答项目，这些项目与这些标准一致，涵盖了26个临床部门以模拟现实场景。对六种大语言模型进行了基准测试。

Result: 六种大语言模型的基准测试结果显示总体表现中等（平均总分57.2%），安全性能为54.7%，效果性能为62.3%。在高风险场景中，性能显著下降（13.3%，p < 0.0001）。领域特定的医学大语言模型在安全性和效果性方面表现出相对较高的最高得分（分别为0.912和0.861）。

Conclusion: 本研究的发现不仅为评估医学大语言模型的临床应用提供了标准化指标，还促进了不同场景下的比较分析、风险暴露识别和改进方向的确定，并有望推动大语言模型在医疗环境中的更安全和有效的部署。
...
TL;DR: 本文提出了一种新的文本检测方法T-Detect，通过理论验证和实验评估，证明了其在对抗性文本检测中的优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本检测器依赖于假设高斯分布的统计度量，但面对对抗性或非母语英语文本的重尾统计特征时效果不佳。

Method: T-Detect通过将标准高斯归一化替换为基于学生t分布的重尾差异分数，重新设计了基于曲率的检测器的统计核心。

Result: T-Detect在RAID基准和HART数据集上进行了验证，实验结果表明其性能优于强基线，在目标领域中AUROC提高了3.9%。

Conclusion: 本文提出了T-Detect，一种新的文本检测方法，通过理论验证和实验评估，证明了其在对抗性文本检测中的优越性能。
...
TL;DR: DiffLoRA是一种参数高效的微调方法，结合了差分注意力机制和低秩适配器，在某些任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 为了在保持LoRA效率的同时，从差分注意力的性能提升中获益。

Method: 引入DiffLoRA，这是对差分注意力机制的参数高效适应，结合了正负注意力项的低秩适配器。

Result: DiffLoRA在广泛的语言模型任务中进行了评估，包括通用基准、多示例上下文学习、RAG和长上下文测试。虽然在大多数任务中表现不如其他方法，但在某些领域表现优异。
...
TL;DR: 本文研究了阿拉伯语文本中的仇恨言论识别和清理问题，采用深度学习和机器翻译方法取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 随着社交媒体上仇恨言论问题的日益严重，需要有效的方法来检测和清理这些内容。

Method: 针对第一个问题，使用深度学习模型和变压器进行了实验以确定最佳模型；针对第二个问题，将其视为机器翻译任务，输入是包含脏话的句子，输出是屏蔽脏话的句子。

Result: 在仇恨言论检测方面，最佳模型达到了92%的宏F1分数和95%的准确率；在文本清理方面，最佳模型在BLEU评分上达到了0.3。
...
TL;DR: 本文研究了大型语言模型生成知识图谱逻辑规则自然语言解释的能力，结果表明其在正确性和清晰度方面表现良好，但仍需进一步研究。


<details>
  <summary>Details</summary>
Motivation: 知识图谱（KGs）通常包含足够的信息来支持新事实的推断。识别逻辑规则不仅提高了知识图谱的完整性，还能够检测潜在错误，揭示细微的数据模式，并增强整体的推理和解释能力。然而，这些规则的复杂性以及每个KG的独特标记约定，使得它们对人类来说难以理解。因此，本文探讨了大型语言模型在生成自然语言解释方面的潜力。

Method: 本文使用AMIE 3.5.1规则发现算法从基准数据集FB15k-237和两个大规模数据集FB-CVT-REV和FB+CVT-REV中提取逻辑规则，并研究了各种提示策略，包括零样本和少量样本提示、变量实体类型以及思维链推理。

Result: 实验结果表明，大型语言模型在解释的正确性和清晰度方面表现出色，但仍有若干挑战需要未来研究解决。

Conclusion: 本文展示了大型语言模型在生成逻辑规则的自然语言解释方面具有良好的表现，尽管仍存在一些挑战需要未来研究解决。
...
Method: 我们提出了一个基于级联问题披露的全面且可推广的框架，以提供更准确的模型问题解决能力估计，同时保持可扩展性和自动化。这种方法以分阶段的方式收集模型响应，每个阶段揭示关于问题的部分信息，旨在激发LLM的通用推理。

Result: 我们发现，我们的方法不仅提供了更好的LLM比较，而且与标准QA范式相比，诱导了更好的中间轨迹。我们在各种推理和知识密集型QA数据集上通过比较不同大小和家族的LLM来验证这种行为。

Conclusion: 我们的方法缩小了标准QA评估设置中观察到的性能差距，表明普遍的间接QA范式高估了模型之间性能的差异。
...
TL;DR: 本文提出了一种基于EEG和LLM的混合框架，用于语言康复，以实现实时适应用户的认知和语言需求。


<details>
  <summary>Details</summary>
Motivation: 传统的增强和替代沟通（AAC）系统和语言学习平台在适应用户的认知和语言需求方面存在不足，特别是在中风后失语症或肌萎缩侧索硬化症等神经系统疾病中。

Method: 本文结合了非侵入性脑电图（EEG）脑机接口（BCI）和基于Transformer的大语言模型（LLM），提出了一种新的混合框架，利用实时EEG信号驱动LLM-powered语言康复助手。

Result: 本文评估了一个新颖的混合框架，该框架利用实时EEG信号驱动LLM-powered语言康复助手，旨在实现用户通过心理命令导航语言学习模块、动态个性化语言内容以及根据认知努力调整任务难度。

Conclusion: 本文提出了一种新颖的混合框架，利用实时EEG信号驱动基于LLM的语言康复助手，旨在帮助有严重言语或运动障碍的用户通过心理命令导航语言学习模块，动态个性化词汇、句子构造练习和纠正反馈，并监测认知努力的神经标记以实时调整任务难度。
...
TL;DR: 本文介绍了一种语音驱动的人工智能系统，用于指导非医疗志愿者进行中风评估，并通过智能手机视频捕获关键检查部分。结果表明，该系统在中风检测方面表现良好，但需要人类监督。


<details>
  <summary>Details</summary>
Motivation: 当前急救人员对中风的识别不一致且经常不准确，导致治疗延误。

Method: 开发了一个语音驱动的人工智能（AI）系统，该系统通过自然对话引导任何人（从急救人员到家庭成员）进行专家级中风评估，同时允许使用智能手机视频捕获关键检查部分以进行记录和潜在专家审查。

Result: AI系统正确识别了84%的个体中风迹象，并检测到了75%的可能大血管阻塞（LVO）中风，评估时间仅超过6分钟。用户报告了高信心（中位数4.5/5）和易用性（平均4.67/5）。系统成功识别了86%的实际中风，但也错误地将3个非中风案例中的2个标记为中风。当专家医生审查AI报告和视频时，他们100%识别了正确的诊断，但由于观察到的AI错误包括错误评分和虚假信息，只有40%的案例中感到足够自信做出初步治疗决定。

Conclusion: 虽然当前系统需要人类监督，但语音到语音的人工智能模型的快速进步表明，未来版本有望实现高度准确的评估。达到人类水平的语音交互可能会改变紧急医疗服务，将专家指导的评估能力带给每个人。
...
TL;DR: 本研究评估了一种基于多代理LLM的AI框架是否可以在虚拟紧急护理环境中自主运作为AI医生。结果表明，AI医生在诊断和治疗方案上与人类医生具有高度一致性，且在某些情况下表现更优。这表明多代理AI系统在临床决策方面可以与人类提供者相媲美，并可能成为解决医疗人员短缺问题的潜在解决方案。


<details>
  <summary>Details</summary>
Motivation: 全球预计到2030年将面临1100万名医疗保健专业人员的短缺，行政负担消耗了50%的临床时间。人工智能（AI）有潜力帮助缓解这些问题。然而，还没有经过严格评估的端到端自主大型语言模型（LLM）为基础的AI系统在现实临床实践中使用。

Method: 我们回顾性地比较了多代理AI系统Doctronic和董事会认证的临床医生在500次连续的紧急护理远程医疗会诊中的表现。主要终点：诊断一致性和治疗方案一致性以及安全指标，通过盲法LLM基于判决和专家人工评审进行评估。

Result: Doctronic和临床医生的最高诊断在81%的病例中匹配，治疗方案在99.2%的病例中一致。没有发生临床幻觉（例如，诊断或治疗不受临床发现支持）。在对不一致病例的专家评审中，AI表现优于36.1%，而人类表现优于9.3%；其余病例的诊断是等效的。

Conclusion: 在对自主AI医生的首次大规模验证中，我们展示了与人类临床医生相当的诊断和治疗计划一致性，AI的表现与人类临床医生相当，在某些情况下甚至超过了他们。这些发现表明，多代理AI系统在临床决策方面可以与人类提供者相媲美，并可能成为解决医疗人员短缺问题的潜在解决方案。
...
### [71] [DSBC : Data Science task Benchmarking with Context engineering](https://arxiv.org/abs/2507.23336)
*Ram Mohan Rao Kadiyala,Siddhant Gupta,Jebish Purbey,Giulio Martini,Suman Debnath,Hamza Farooq*

Main category: cs.AI

TL;DR: 本文提出一个全面的基准测试，用于评估数据科学代理的性能，并揭示了不同模型和方法之间的性能差异及影响实际部署的关键因素。


<details>
  <summary>Details</summary>
Motivation: 尽管数据科学代理已被广泛采用，但系统性的基准测试仍较为缺乏。本文旨在填补这一空白，提供一个评估数据科学代理性能的基准测试。

Method: 本文通过观察商业应用中的使用情况，构建了一个反映真实用户交互的基准测试。评估了三种LLM（Claude-4.0-Sonnet、Gemini-2.5-Flash和OpenAI-o4-Mini）在三种方法（零样本带上下文工程、多步骤带上下文工程和SmolAgent）下的表现，并探讨了温度参数对整体和任务特定结果的影响。

Result: 研究发现不同模型和方法在性能上存在显著差异，并揭示了影响实际部署的关键因素。此外，还探索了温度参数对模型表现的影响。

Conclusion: 本文引入了一个全面的基准测试，旨在评估数据科学代理的有效性和局限性。研究发现不同模型和方法之间存在显著的性能差异，并强调了影响实际部署的关键因素。该基准数据集和评估框架为未来更强大和有效的数据科学代理研究提供了基础。
...
### [72] [TextQuests: How Good are LLMs at Text-Based Video Games?](https://arxiv.org/abs/2507.23701)
*Long Phan,Mantas Mazeika,Andy Zou,Dan Hendrycks*

Main category: cs.AI

TL;DR: TextQuests是一个新的基准，用于评估AI代理在需要持续、自主推理的探索性环境中的能力，它基于Infocom互动小说游戏套件。


<details>
  <summary>Details</summary>
Motivation: 现有的代理基准无法充分捕捉代理在需要持续、自主推理的探索性环境中操作的能力，因此需要一个新的基准来推动更强大的内在推理能力的发展。

Method: TextQuests基于Infocom互动小说游戏套件，用于评估LLM代理的自我包含问题解决能力，不允许使用外部工具。

Result: TextQuests可以作为评估AI代理在专注于状态化任务上的有效代理，它要求代理进行试错学习和持续的问题解决。

Conclusion: TextQuests是一个有效的基准，用于评估AI代理在需要持续、自主推理的探索性环境中的能力。
...
### [73] [Seed-Prover: Deep and Broad Reasoning for Automated Theorem Proving](https://arxiv.org/abs/2507.23726)
...
TL;DR: 本文介绍了Seed-Prover和Seed-Geometry，它们在数学定理证明和几何推理方面取得了显著成果。


<details>
  <summary>Details</summary>
Motivation: LLMs在定理证明方面仍然存在困难，因为仅使用自然语言时缺乏明确的监督信号。

Method: 提出了一种基于引理的完整证明推理模型Seed-Prover，以及一个几何推理引擎Seed-Geometry。

Result: Seed-Prover在形式化过去的IMO问题上证明了78.1%，在MiniF2F上达到饱和，并在PutnamBench上超过50%。Seed-Geometry在形式几何引擎中表现优于之前的方法。

Conclusion: 本工作代表了自动化数学推理的重大进展，展示了形式验证与长链式思维推理的有效性。
...
### [74] [CoT-Self-Instruct: Building high-quality synthetic prompts for reasoning and non-reasoning tasks](https://arxiv.org/abs/2507.23751)
*Ping Yu,Jack Lanchantin,Tianlu Wang,Weizhe Yuan,Olga Golovneva,Ilia Kulikov,Sainbayar Sukhbaatar,Jason Weston,Jing Xu*

Main category: cs.AI

TL;DR: 我们提出了CoT-Self-Instruct，一种基于链式思维的合成数据生成方法，能够生成高质量的合成提示，并在多个任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有训练数据集在可验证推理任务和不可验证指令遵循任务中的表现有限，需要一种更有效的合成数据生成方法来提升大型语言模型的性能。

Method: 我们提出了CoT-Self-Instruct，一种合成数据生成方法，通过链式思维（CoT）指导大型语言模型进行推理和规划，并生成高质量和复杂度相似的新合成提示，随后使用自动指标进行过滤。

Result: 在可验证推理任务中，我们的合成数据显著优于现有的训练数据集，如s1k和OpenMathReasoning，在MATH500、AMC23、AIME24和GPQA-D上表现优异；在不可验证指令遵循任务中，我们的方法在AlpacaEval 2.0和Arena-Hard上超越了人类或标准自指导提示。

Conclusion: 我们的方法在可验证推理和不可验证指令遵循任务中均表现出色，超越了现有的训练数据集和人类或标准自指导提示。
...
### [75] [SimuRA: Towards General Goal-Oriented Agent via Simulative Reasoning Architecture with LLM-Based World Model](https://arxiv.org/abs/2507.23773)
...
### [76] [Counterfactual Evaluation for Blind Attack Detection in LLM-based Evaluation Systems](https://arxiv.org/abs/2507.23453)
*Lijia Liu,Takumi Kondo,Kyohei Atarashi,Koh Takeuchi,Jiyi Li,Shigeru Saito,Hisashi Kashima*

Main category: cs.CR

TL;DR: 本文提出了一种新的评估框架SE+CFE，以提高基于LLM的评估系统对提示注入攻击的防御能力。
...
Method: 我们提出了一个框架，将标准评估（SE）与反事实评估（CFE）相结合，该框架对提交的答案重新评估以对抗故意错误的地面真实答案。

Result: 实验表明，标准评估极易受到攻击，而我们的SE+CFE框架显著提高了安全性，同时保持了最小的性能损失。
...
### [77] [ELMES: An Automated Framework for Evaluating Large Language Models in Educational Scenarios](https://arxiv.org/abs/2507.22947)
...
TL;DR: 本文介绍了ELMES，一个开源的自动化评估框架，旨在评估教育环境中大型语言模型的表现。ELMES通过模块化架构和混合评估引擎，帮助研究人员灵活设计教育场景并客观量化教学指标。实验结果展示了不同模型在各种教育任务中的能力分布。
...
Method: 引入ELMES，一个开源的自动化评估框架，专门设计用于评估教育环境中的LLMs。该框架具有模块化架构，使研究人员能够通过简单的配置文件创建动态、多代理对话，同时结合混合评估引擎，使用LLM-as-a-Judge方法客观量化传统上主观的教学指标。

Result: 对最先进的LLMs在四个关键教育场景中的系统基准测试结果表明，模型在不同情境下表现出不同的能力分布，揭示了特定情境下的优势和限制。

Conclusion: ELMES为教育者和研究人员提供了一个可访问的评估框架，显著降低了多样化教育应用的适应障碍，同时推进了LLMs在教学中的实际应用。
...
### [78] [SequenceLayers: Sequence Processing and Streaming Neural Networks Made Easy](https://arxiv.org/abs/2507.23292)
...
TL;DR: 本文介绍了 SequenceLayers，一种用于序列建模的神经网络层 API 和库，旨在简化序列模型的创建，并支持流式处理和自动回归采样。


<details>
  <summary>Details</summary>
Motivation: 为了使复杂的模型能够立即流式处理，减少在流式处理和并行序列处理中出现的常见错误，并且可以在任何深度学习库中实现。

Method: 通过定义显式的随时间变化的状态（如 Transformer KV 缓存、卷积缓冲区、RNN 隐藏状态）和一个逐步演化该状态的方法，实现序列模型的流式处理。

Result: SequenceLayers 的当前实现（JAX、TensorFlow 2）已在 https://github.com/google/sequence-layers 上提供。

Conclusion: SequenceLayers 提供了一种可组合和声明式的 API，使得从简单的可流式处理组件构建生产规模的模型变得更加容易，并保持了强大的正确性保证。
...
### [79] [Deep Learning-based Prediction of Clinical Trial Enrollment with Uncertainty Estimates](https://arxiv.org/abs/2507.23607)
*Tien Huu Do,Antoine Masquelier,Nae Eoun Lee,Jonathan Crowther*

Main category: cs.LG

TL;DR: 本文提出了一种基于深度学习的新型方法，用于预测临床试验中的患者招募情况。该方法利用预训练语言模型和概率层来提高预测准确性，并在实际数据上表现出色。


<details>
  <summary>Details</summary>
Motivation: 准确预测患者招募是临床试验规划阶段的主要挑战之一。由于临床试验需要大量的资金投入和细致的规划，因此需要准确预测试验结果。

Method: 本文提出的方法是一种神经网络模型，利用预训练语言模型（PLMs）来捕捉临床文档的复杂性和细微差别，并通过注意力机制将这些表示与编码的表格特征相结合。此外，还引入了一个基于Gamma分布的概率层，以考虑招募预测中的不确定性。

Result: 本文的方法在实际临床试验数据上进行了广泛的实验，结果表明该方法能够有效地预测给定临床试验中多个站点的患者招募数量，并且优于现有的基线模型。

Conclusion: 本文提出了一种基于深度学习的新方法，用于预测临床试验中的患者招募情况。实验结果表明，该方法在实际数据上表现优于传统基线模型。
...
### [80] [TweakLLM: A Routing Architecture for Dynamic Tailoring of Cached Responses](https://arxiv.org/abs/2507.23674)
*Muhammad Taha Cheema,Abeer Aamir,Khawaja Gul Muhammad,Naveed Anwar Bhatti,Ihsan Ayyub Qazi,Zafar Ayyub Qazi*

Main category: cs.LG

TL;DR: TweakLLM 是一种用于大型语言模型的高效缓存解决方案，通过动态调整缓存响应来提高性能。


<details>
  <summary>Details</summary>
Motivation: 由于聊天机器人的个性化互动和语义相似性搜索的有限准确性，使用这种方法保持与用户查询的相关性具有挑战性。

Method: TweakLLM 是一种新颖的路由架构，使用轻量级 LLM 动态调整缓存响应以适应传入提示。

Result: 通过全面的评估，包括用户研究、并排比较、满意度投票以及多代理 LLM 辩论，我们证明 TweakLLM 保持了与前沿模型相当的响应质量，同时显著提高了缓存效果。

Conclusion: TweakLLM 是一种可扩展且资源高效的缓存解决方案，适用于高吞吐量的 LLM 部署，同时不会影响用户体验。
...
### [81] [Holistic Evaluations of Topic Models](https://arxiv.org/abs/2507.23364)
*Thomas Compton*
...
<details>
  <summary>Details</summary>
Motivation: 主题模型在商业和学术领域越来越受欢迎，但它们可能成为'黑箱'，用户输入数据并接受输出作为准确的摘要而无需审查。因此，需要评估主题模型以识别优化模型参数的权衡，并反思这些发现对主题模型解释和负责任使用的意义。

Method: 本文通过分析1140次BERTopic模型运行，从数据库的角度评估了主题模型。

Result: 本文通过分析1140次BERTopic模型运行，从数据库的角度评估了主题模型，识别了优化模型参数的权衡，并反思了这些发现对主题模型解释和负责任使用的意义。
...
TL;DR: 本文通过使用极坐标参数对频谱子带重心频率进行表征，结合Mel频率倒谱系数，提出了新的动态参数，以提高越南语自动语音识别的性能。结果表明，这些参数能有效降低词错误率，并具有更好的性别独立性。


<details>
  <summary>Details</summary>
Motivation: 语音信号的动态特性提供时间信息，在增强自动语音识别（ASR）中起重要作用。

Method: 使用极坐标参数对频谱子带重心频率（SSCFs）的比率平面进行表征，以捕捉语音的动态特性并最小化频谱变化。这些动态参数与Mel频率倒谱系数（MFCCs）结合用于越南语自动语音识别（ASR），以捕获更详细的频谱信息。SSCF0被用作基频（F0）的伪特征，以稳健地描述调制信息。
...
TL;DR: 本文提出了MECAT基准和DATE度量标准，以改进音频模型的细粒度理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前基准由于数据注释和评估指标的限制，无法可靠地区分通用和高度详细的模型输出，因此需要一个新的基准和度量标准来提高音频模型的理解能力。

Method: 通过集成专业专家模型的分析与链式思维大型语言模型推理的管道生成MECAT基准，并提出DATE度量标准来惩罚通用术语并奖励详细描述。

Result: MECAT基准提供了多角度、细粒度的字幕和开放集问答对，并通过DATE度量标准对最先进的音频模型进行了全面评估，揭示了它们当前的能力和局限性。

Conclusion: 本文介绍了MECAT，一个用于细粒度音频理解任务的多专家构建基准，并提出了DATE度量标准，以更准确地评估音频模型的性能。
...
### [84] [Generalized Reinforcement Learning for Retriever-Specific Query Rewriter with Unstructured Real-World Documents](https://arxiv.org/abs/2507.23242)
*Sungguk Cha,DongWook Kim,Taeseung Hahn,Mintae Kim,Youngsub Han,Byoung-Ki Jeon*

Main category: cs.CV

TL;DR: 本文介绍了RL-QR，这是一种基于强化学习的检索器特定查询重写框架，无需人工标注数据集，适用于文本和多模态数据库。实验显示其在不同检索器上的性能提升，但也指出在语义检索方面的挑战。


<details>
  <summary>Details</summary>
Motivation: RAG系统依赖于有效的查询制定来获取外部知识，但优化多样化、非结构化的现实文档中的查询仍然是一个挑战。

Method: 我们引入了RL-QR，这是一个基于强化学习的检索器特定查询重写框架，通过合成场景-问题对并利用广义奖励策略优化（GRPO）来训练针对特定检索器的查询重写器。

Result: 实验表明，RL-QR在工业内部数据上表现出显著的改进，其中RL-QR_{multi-modal}在多模态RAG中实现了NDCG@3的11%相对增益，而RL-QR_{lexical}则为词汇检索器带来了9%的增益。然而，在语义和混合检索器中，重写器未能提高性能，这可能是由于训练不一致所致。

Conclusion: 我们的研究展示了RL-QR在RAG系统查询优化中的潜力，提供了一种可扩展的、无需注释的解决方案，同时指出了在语义检索上下文中进一步改进的方向。
...
### [85] [SWE-Debate: Competitive Multi-Agent Debate for Software Issue Resolution](https://arxiv.org/abs/2507.23348)
...
### [86] [SWE-Exp: Experience-Driven Software Issue Resolution](https://arxiv.org/abs/2507.23361)
*Silin Chen,Shaoxin Lin,Xiaodong Gu,Yuling Shi,Heng Lian,Longfei Yun,Dong Chen,Weiguo Sun,Lin Cao,Qianxiang Wang*

Main category: cs.SE

TL;DR: 本文提出了一种名为SWE-Exp的增强经验方法，通过从先前代理轨迹中提炼经验，实现跨问题的持续学习。该方法引入了一个多方面的经验库，捕捉成功和失败的修复尝试，并在不同层次上提取可重用的问题解决知识。实验表明，SWE-Exp在SWE-bench-Verified上实现了最先进的解决率（41.6% Pass@1）。


<details>
  <summary>Details</summary>
Motivation: 当前的代理作为无记忆的探索者，单独处理每个问题，不保留或重用以前的修复经验，导致冗余探索失败轨迹并错过将成功的解决方案方法适应到类似问题的机会。

Method: 我们引入了SWE-Exp，一种增强经验的方法，它从先前的代理轨迹中提炼出简洁且可操作的经验，以实现跨问题的持续学习。我们的方法引入了一个多方面的经验库，捕捉成功和失败的修复尝试，并在不同层次上提取可重用的问题解决知识。

Result: 实验表明，SWE-Exp在SWE-bench-Verified上实现了最先进的解决率（41.6% Pass@1）。

Conclusion: 我们的方法建立了一个新范式，其中自动化软件工程代理系统地积累和利用修复专业知识，从根本上从试错探索转向战略性、经验驱动的问题解决。
