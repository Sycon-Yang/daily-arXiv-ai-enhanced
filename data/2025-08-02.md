...
Motivation: 在线房产预订平台依赖于一致且最新的住宿设施信息，但这些外部数据源经常受到不完整或不一致细节的影响，这可能会让用户感到沮丧并导致市场损失。

Method: 我们评估了两种知名的大型语言模型：Mistral 7B，使用QLoRA进行微调，以及Mixtral 8x7B，使用优化的系统提示。

Result: Mixtral 8x7B在完整性（99.6% vs. 93%）、精确度（98.8% vs. 96%）和幻觉率（1.2% vs. 4%）方面优于Mistral 7B，生成的内容更短但更简洁（平均249 vs. 277个单词）。然而，这需要更高的计算成本：50GB VRAM和1.61美元/小时，而Mistral 7B只需5GB和0.16美元/小时。

Conclusion: 我们的研究提供了在生产环境中部署大型语言模型的实用见解，并展示了它们在提高住宿数据的一致性和可靠性方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: Current systems like China's 95598 hotline struggle with slow response times, inflexible procedures, and limited accuracy in domain-specific tasks. While large language models (LLMs) demonstrate strong general capabilities, they lack the domain expertise and empathy required in this field.

Method: Introduce ElectriQ, a benchmark designed to evaluate and enhance LLMs in electric power marketing scenarios. It consists of a dialogue dataset covering six key service categories and introduces four evaluation metrics: professionalism, popularity, readability, and user-friendliness. Additionally, a domain-specific knowledge base and a knowledge augmentation method are incorporated.
...
<details>
  <summary>Details</summary>
Motivation: 非法市场内容的检测和分类面临挑战，包括有限的标记数据、非法语言的演变以及在线来源的结构异质性。因此，需要一种有效的分类框架来应对这些挑战。

Method: 本文提出了一种分层分类框架，结合微调的语言模型和半监督集成学习策略。首先使用XGBoost、随机森林和SVM的半监督集成进行销售相关文档检测，然后进一步分类为毒品、武器或凭证销售。

Result: 实验在三个数据集上进行，包括多源语料库、DUTA和CoDA，结果表明该模型在准确率、F1分数和TMCC方面均优于多种基线模型，如BERT、ModernBERT、DarkBERT、ALBERT、Longformer和BigBird。

Conclusion: 本文提出了一种分层分类框架，结合微调的语言模型和半监督集成学习策略，以在多样的平台上检测和分类非法市场内容。实验结果表明，该模型在多个数据集上优于多种基线模型，表现出强大的泛化能力、在有限监督下的鲁棒性和在现实世界非法内容检测中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在分类和摘要任务中被广泛使用，但它们在执行主题分析方面的能力尚未得到充分探索。传统的机器学习（ML）模型用于多标签分类进行主题分析，但在处理未见过的情况时存在困难。LLMs提供了另一种选择，但常常过度生成和产生幻觉。

Method: 我们提出了一种混合框架，将基于嵌入的机器学习模型与大型语言模型（LLMs）集成。该方法使用机器学习模型来预测最佳的LCSH标签数量以指导LLM预测，并通过实际的LCSH术语对预测的术语进行后期编辑以减轻幻觉。

Result: 实验结果表明，提供初始预测来引导LLM生成并施加后期编辑可以产生更受控和词汇对齐的输出。

Conclusion: 实验结果表明，提供初始预测来引导LLM生成并施加后期编辑可以产生更受控和词汇对齐的输出。
...
<details>
  <summary>Details</summary>
Motivation: 现有的实体匹配方法在评估时通常不考虑真实世界知识图谱在来源、大小和信息密度方面的显著差异，因此需要一种新的方法来解决这一问题。

Method: 本文提出了一种新的知识图谱集成方法，包括标签匹配和三元组匹配。标签匹配使用字符串操作、模糊匹配和向量相似性技术来对齐实体和谓词标签。三元组匹配则用于识别传达相似信息的三元组，从而提高实体匹配的准确性。

Result: 本文的方法在OAEI竞赛和监督方法中表现出了竞争力，并在各种测试案例中实现了高精度。此外，还引入了一个新的数据集来更全面地评估三元组匹配步骤。

Conclusion: 本文提出了一种新的知识图谱集成方法，通过标签匹配和三元组匹配来提高实体匹配的准确性，并在多个测试案例中取得了高精度。此外，还引入了一个新的数据集来更全面地评估三元组匹配步骤。
...
Method: 本文通过学习理论框架（如PAC-Bayes和Rademacher复杂性）推导了幻觉风险的界限，并调查了检测策略，如token级别的不确定性估计、置信度校准和注意力对齐检查。此外，还讨论了缓解方法，包括检索增强生成、幻觉感知微调、logit校准和事实验证模块的整合。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于语义的检索方法无法收集既主题相关又时间连贯的证据，因此需要一种新的方法来处理纵向查询。

Method: 本文提出了一种新的框架，从根本上重新设计RAG管道以注入时间逻辑。方法包括将用户的查询解耦为核心主题和时间窗口，并使用专门的检索器校准语义匹配与时间相关性。

Result: 在ADQAB上的实证结果表明，我们的方法在答案准确性上取得了显著提升，比标准RAG实现高出13%到27%。

Conclusion: 本文提供了一种验证的路径，使RAG系统能够执行复杂现实问题所需的细微和演化的分析。
...
<details>
  <summary>Details</summary>
Motivation: 研究Gemma-2语言模型（Gemma-2-2B和Gemma-2-9B）中的特征普遍性，探讨规模相差四倍的模型是否仍收敛于类似的内部概念。

Method: 使用稀疏自编码器（SAE）字典学习流程，在每个模型的残差流激活上使用SAE，通过激活相关性对产生的单义特征进行对齐，并通过SVCCA和RSA比较匹配的特征空间。

Result: 中间层产生最强的重叠，而早期和晚期层显示较少的相似性。初步实验将分析从单个标记扩展到多标记子空间，显示语义相似的子空间与语言模型的交互方式类似。

Conclusion: 这些结果加强了大型语言模型尽管规模不同，但仍将世界划分为广泛相似且可解释的特征的观点，强化了普遍性作为跨模型可解释性的基础。
...
<details>
  <summary>Details</summary>
Motivation: 通过准确估计预期的安全结果，可以设计临床试验以避免终止并限制参与者暴露于不必要的风险。

Method: 我们分析了22,107个两臂平行干预性临床试验，使用了结构化摘要结果。开发了两个预测模型：一个分类器预测实验组是否有更高的SAE率，以及一个回归模型预测对照组中SAE的比例。使用预训练语言模型（如ClinicalT5、BioBERT）进行特征提取，并结合下游模型进行预测。为了保持超出局部语言模型输入限制的长试验文本的语义表示，开发了一种滑动窗口方法进行嵌入提取。
...
Conclusion: 总结结果数据在ClinicalTrials.gov中仍被低估利用。在试验开始前估计试验结果的潜力是一个改进试验设计和标记预期和报告的安全性结果之间差异的机会。
...
<details>
  <summary>Details</summary>
Motivation: 由于LLM的快速发展，需要有效的机制将连续多模态数据转换为适合语言处理的离散表示。然而，目前缺乏对VQ技术在LLM系统中的全面调查。

Method: 本文对8种代表性的VQ变体进行了分类，并分析了它们的算法原理、训练动态和与LLM流水线的集成挑战。此外，还讨论了现有研究在传统应用、单模态系统和多模态系统中的情况。

Result: 本文提出了第一个结构化的分类和分析，涵盖了VQ方法的算法原理、训练动态、集成挑战以及量化策略对对齐、推理和生成性能的影响。

Conclusion: 本文通过构建第一个结构化的分类和分析，填补了现有研究的空白，为高效且通用的多模态系统的开发提供了基础参考。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型可以捕捉给定文本中的复杂关系，但它们以成本高昂和产生不存在的信息（即幻觉）而闻名。此外，投入到生成这些信息的资源如果错误的话就会浪费。

Method: 我们提出了LMC算法，其中语言模型对给定文本的响应只有在存在于可能的答案集合中时才是正确的，并且将对应于错误响应的文本输入到更可预测（但更慢）的语言模型中。此过程重复进行一组语言模型，或者直到所有关于文本的预测都正确为止。

Result: 我们使用LMC算法从医疗文档中提取患者出生日期，将一组语言模型组合成多阶段级联显著提高了预测速度和准确性，同时大大减少了相应的幻觉数量。

Conclusion: 我们相信新颖的LMC算法对知识提取领域有显著贡献，并且应该在未来进一步探索。
...
<details>
  <summary>Details</summary>
Motivation: 2021年GameStop做空挤压事件引发了对在线情绪如何影响股价的疑问。本文旨在探讨从社交媒体讨论中得出的情绪是否能有意义地预测股市波动。

Method: 本研究聚焦于Reddit的r/wallstreetbets板块，分析与GameStop (GME)和AMC Entertainment (AMC)相关的社交媒体情感。我们采用两种现有的基于文本的情感分析方法，并引入第三种方法，即使用ChatGPT标注并微调的RoBERTa模型，以更好地理解社交媒体讨论中的非正式语言和表情符号。我们使用相关性和因果度量来评估这些模型的预测能力。

Result: 研究发现，社交媒体情感与股票价格之间只有较弱的相关性。同时，简单的指标如评论数量和谷歌搜索趋势表现出更强的预测信号。

Conclusion: 研究结果表明，社交媒体情感与股票价格之间的关联性较弱，而简单的指标如评论数量和谷歌搜索趋势则表现出更强的预测信号。这突显了散户投资者行为的复杂性，并表明传统的情感分析可能无法完全捕捉市场动向的细微差别。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）在多语言能力方面取得了进展，但它们在不同语言和任务中的表现差异很大。在多语言检索增强生成（RAG）系统中，知识库（KB）通常从高资源语言（如英语）共享到低资源语言，导致从KB中检索的信息与上下文的语言不同。在这种情况下，常见的做法是预翻译以创建单语提示或跨语言提示进行直接推理，但这些选择的影响仍不明确。

Method: 本文系统评估了不同提示翻译策略对多语言系统中基于RAG增强的LLM进行分类任务的影响。

Result: 实验结果表明，优化的提示策略可以显著提高跨语言的知识共享，从而提高下游分类任务的性能。

Conclusion: 研究结果表明，优化的提示策略可以显著提高跨语言的知识共享，从而提高下游分类任务的性能。研究倡导更广泛地利用多语言资源共享和跨语言提示优化，特别是对于非英语语言，尤其是资源较少的语言。
...
<details>
  <summary>Details</summary>
Motivation: 研究母语和非母语英语学习者在在线美国计算机课程中的同行反馈体验差异。

Method: 使用基于Twitter-roBERTa的模型分析了500名学生的同行评审情感。

Result: 母语英语学习者对反馈评分较低，而非母语学习者写作更积极但收到的积极情感较少。

Conclusion: 语言背景在塑造同行反馈体验中起着适度但复杂的作用。
...
<details>
  <summary>Details</summary>
Motivation: 长期记忆是影响大型语言模型代理（LLM Agents）推理能力的关键因素。结合一种能够有效整合过去交互的记忆机制可以显著增强LLM Agents的决策能力和上下文连贯性。虽然最近的研究在记忆存储和检索方面取得了进展，例如将记忆编码为密集向量进行基于相似性的搜索或以图的形式组织知识，但这些方法在结构化记忆组织和高效检索方面往往不足。

Method: 我们提出了一种分层记忆（H-MEM）架构，该架构基于语义抽象程度以多级方式组织和更新记忆。每个记忆向量都嵌入了一个位置索引编码，指向下一层的语义相关子记忆。在推理阶段，基于索引的路由机制实现了高效的逐层检索，而无需进行广泛的相似性计算。

Result: 我们在LoCoMo数据集的五个任务设置上评估了我们的方法。实验结果表明，我们的方法始终优于五种基线方法，证明了其在长期对话场景中的有效性。

Conclusion: 我们的方法在长期对话场景中表现出色，证明了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅关注提及实体的句子，无法捕捉完整的文档上下文，导致关系抽取不准确。

Method: 该研究引入了一种新的输入嵌入方法，通过将实体表示为独立的段落，而不是仅关注它们出现的范围，从而捕捉整个文档中提到的实体的位置。

Result: 该方法在三个基准数据集（DocRED、Re-DocRED 和 REBEL）上进行了测试，并展示了其在文档级别上准确预测实体之间关系的能力。

Conclusion: 该研究在文档级关系抽取任务中具有理论和实际意义，能够提高现实世界自然语言处理应用中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 目前的基准主要关注整体RAG系统性能，很少评估LLM特定能力。当前的基准强调诸如噪声鲁棒性等广泛方面，但缺乏对文档利用的系统性和细致的评估框架。

Method: 我们引入了	extit{Placeholder-RAG-Benchmark}，这是一个多级细粒度基准，强调以下渐进维度：(1) 多级过滤能力，(2) 组合能力，以及(3) 参考推理。我们提出了一个基于占位符的方法，以解耦LLM的参数知识和外部知识的贡献。

Result: 实验表明，代表性LLM在RAG系统的生成能力上存在局限性，特别是在错误恢复和上下文忠实性方面。
...
<details>
  <summary>Details</summary>
Motivation: Chain-of-thought (CoT) prompting提升大型语言模型在多步骤任务中的准确性，但生成的“思考”是否反映真实的内部推理过程尚不清楚。

Method: 结合稀疏自编码器和激活修补，我们从Pythia-70M和Pythia-2.8B中提取单义特征，同时它们在CoT和无CoT提示下解决GSM8K数学问题。

Result: 将一小部分CoT推理特征交换到无CoT运行中，在2.8B模型中显著提高了答案的对数概率，但在70M模型中没有可靠效果，显示出明显的规模阈值。CoT还导致更大的模型中激活稀疏性和特征可解释性评分显著提高，表明内部计算更模块化。例如，模型生成正确答案的信心从1.2提高到4.3。

Conclusion: 我们的结果表明，CoT可以在高容量LLM中引发更可解释的内部结构，验证了其作为结构化提示方法的作用。
...
Result: 实验结果表明，我们的多智能体框架显著减轻了两种类型的幻觉，提高了准确性、可解释性和可靠性。

Conclusion: 我们的多智能体框架显著减轻了两种类型的幻觉，提高了准确性、可解释性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 由于缺乏开放源代码的标记数据集，研究PII自我披露的识别和检索受到阻碍。因此，需要一种方法来创建安全共享的合成数据。

Method: 本文提出了一种新的方法，通过三个文本生成大型语言模型（LLMs）生成合成等效数据，并使用顺序指令提示来模仿原始Reddit帖子。

Result: 本文创建了一个包含19个PII披露类别的分类法，并生成并发布了从三个LLMs生成的合成PII标记多文本跨度数据集。

Conclusion: 本文提出了一个生成合成PII标记多文本跨度数据集的方法，以促进对在线社交媒体中PII隐私风险的可重复研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法应用固定的压缩率，对简单查询过度压缩或对复杂查询压缩不足，导致推理成本高。

Method: ACC-RAG结合了分层压缩器（用于多粒度嵌入）和上下文选择器，以保留最小足够的信息，类似于人类的快速浏览。

Result: 在Wikipedia和五个QA数据集上评估，ACC-RAG优于固定率方法，并且在保持或提高准确性的同时实现了更快的推理速度。

Conclusion: ACC-RAG在保持或提高准确性的同时，实现了比标准RAG快四倍以上的推理速度。
...
<details>
  <summary>Details</summary>
Motivation: 为了提高投资组合优化的效果，结合情感信号和传统市场指标，并通过分层结构增强稳定性。

Method: 该论文提出了一种分层框架，结合轻量级大语言模型（LLM）和深度强化学习（DRL），将金融新闻的情感信号与传统市场指标相结合。三层架构包括基础RL代理、元代理和超级代理，分别处理混合数据、聚合决策和基于市场数据和情感分析的决策合并。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言机器学习模型在解释输出、整合新信息和逻辑推理方面存在局限性，因此需要一种更有效的解决方案。

Method: 本文采用系统文献综述的方法，对相关研究进行了分析和分类。

Result: 本文总结了通过与外部符号信息系统的交互来改进视觉-语言理解的不同技术，并讨论了它们的优势和挑战。

Conclusion: 本文通过系统文献综述，旨在分类通过与外部符号信息系统的交互来改进视觉-语言理解的技术。
...
<details>
  <summary>Details</summary>
Motivation: 随着对自然人机交互需求的增长，意图识别领域需要更先进的方法来处理多模态数据。

Method: 本文回顾了深度学习方法在意图识别中的应用，包括多模态技术的使用。

Result: 本文提供了关于多模态意图识别（MIR）最新进展的见解，并指出了未来研究的方向。

Conclusion: 本文为研究人员提供了多模态意图识别领域的最新发展和未来研究方向的洞察。
...
<details>
  <summary>Details</summary>
Motivation: 从组织数据存储中获取操作智能是一个关键挑战，因为数据保密性与数据集成目标之间的矛盾，以及自然语言处理（NLP）工具在特定领域知识结构方面的局限性。

Method: 我们讨论了知识图谱的构建，并将知识提取过程分解为命名实体识别、共指消解、命名实体链接和关系提取的功能组件。我们评估了十六种NLP工具，并与大型语言模型（LLMs）的能力进行了比较。

Result: 我们观察到NLP和LLM工具在零样本性能方面存在显著限制，这表明在航空等关键任务行业中的技术成熟度水平较低。

Conclusion: 我们得出结论，需要增强可信NLP和LLM工具的信任度，并提供了开源的经过筛选的数据集以支持进一步的基准测试和评估。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）在金融自然语言处理（FinNLP）任务中表现出色，但对广泛使用的LLMs进行系统比较的研究仍较少。鉴于LLMs在金融分析中的快速发展和日益增长的影响，本研究对五个领先的LLMs进行了全面的比较评估。

Method: 本研究通过三种方法评估模型性能：人工标注、自动的词汇-语义指标（ROUGE、余弦相似度、Jaccard）以及模型行为诊断（提示级方差和跨模型相似性）。

Result: GPT在生成连贯、语义对齐和上下文相关的答案方面表现最佳；其次是Claude和Perplexity。Gemini和DeepSeek则表现出更多的变异性且一致性较低。此外，输出的相似性和稳定性因公司而异，并随时间变化，表明它们对提示的编写方式和所使用的源材料非常敏感。

Conclusion: 研究结果显示，GPT在生成连贯、语义对齐和上下文相关的答案方面表现最佳；其次是Claude和Perplexity。Gemini和DeepSeek则表现出更多的变异性且一致性较低。此外，输出的相似性和稳定性因公司而异，并随时间变化，表明它们对提示的编写方式和所使用的源材料非常敏感。
...
<details>
  <summary>Details</summary>
Motivation: 由于领域专业知识的限制，单个模型只能处理特定任务，而结合多个模型可以实现更高效的结果。本文旨在解决AIOps中的类似挑战，通过协作专家框架提高任务处理效率。

Method: 本文提出了一个协作专家框架(CoE-Ops)，结合了一个通用的大语言模型任务分类器，并引入了检索增强生成机制，以提高框架在处理高阶和低阶AIOps任务的能力。

Result: 实验结果表明，CoE-Ops在高阶AIOps任务的路由准确性上比现有CoE方法提高了72%，在DevOps问题解决中比单一AIOps模型提高了最多8%的准确率，并在准确性上超过了更大规模的Mixture-of-Experts (MoE)模型最多14%。

Conclusion: CoE-Ops在AIOps领域表现出色，相比现有的CoE方法、单一AIOps模型以及更大规模的Mixture-of-Experts (MoE)模型，取得了显著的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于文本的RAG系统可能无法回答包含图表答案的问题，因此需要一种更有效的方法来处理这类问题。

Method: 该方法涉及处理技术文档、分类图像类型、构建图表示，并将其与文本嵌入管道结合，以实现高效的检索。

Result: 使用微调的VLM模型获得的图表示与真实情况之间的编辑距离较低，表明这些表示对于流程图图像具有鲁棒性。此外，使用这些表示进行问答的方法在基于文本的嵌入模型上表现出良好的检索性能。

Conclusion: 该方法通过利用从视觉大语言模型获得的流程图图表示，提高了在电信领域中基于文本的RAG系统对包含图表答案的问题的问答性能，并且减少了推理过程中对VLM的需求，从而降低了成本。
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the critical vulnerability in Large Language Models (LLMs): the prevalence of factual inaccuracies within intermediate reasoning steps despite correct final answers, which poses substantial risks in high-stakes domains.
...
Result: Extensive evaluation across ten state-of-the-art models reveals concerning patterns: even leading models like Claude-3.7 and GPT-o1 demonstrate reasoning factual accuracy of only 81.93% and 82.57% respectively. RELIANCE significantly enhances factual robustness (up to 49.90% improvement).

Conclusion: RELIANCE significantly enhances factual robustness while maintaining or improving performance on challenging benchmarks, and provides actionable insights into how factual enhancements reshape reasoning trajectories within model architectures.
...
<details>
  <summary>Details</summary>
Motivation: 通过手动审查来自关联电子健康记录的自由文本注释来创建参考标准标签通常需要大量的时间和资源分配，因此需要一种更高效的方法来验证基于代码的算法的测量特性。

Method: 我们描述了一种加快验证研究过程的方法，引入了两种不同的机制：1）使用自然语言处理（NLP）减少人类评审员审查每张病历的时间，2）使用预定义的标准进行多波适应性抽样以在性能特征被识别出足够精确时停止验证研究。

Result: 我们实证证明，NLP辅助的注释过程将每张病历的审查时间减少了40%，并且使用预定义的停止规则和多波样本可以防止审查77%的患者病历，而对推导出的测量特性精度的影响有限。

Conclusion: 这种方法可以促进对用于定义关键研究参数的代码算法的更常规验证，最终增强对从数据库研究中得出的发现的可靠性理解。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏对语音对话模型在理解和模拟人类对话方面的实际效果的全面研究，尤其是在与文本大型语言模型相比时。

Method: 本文提出了一个包含1079个实例的基准数据集，并采用基于大型语言模型的评估方法来评估语音对话模型的性能。

Result: 本文提出的基准数据集和评估方法能够帮助全面探索语音对话模型在处理实际挑战中的性能。

Conclusion: 本文提出了一种基准数据集，用于评估语音对话模型在处理实际挑战中的性能，并通过基于大型语言模型的评估方法来接近人类判断。
...
<details>
  <summary>Details</summary>
Motivation: To determine if contemporary LLMs can perform natural language inference tasks on mathematical texts and to evaluate the quality of corpora generated by LLMs compared to human-labeled data.

Method: The study constructs a corpus of Math NLI pairs with premises from mathematical texts and hypotheses provided by humans with expertise in mathematics and NLI. It also evaluates corpora where hypotheses are generated by LLMs, assessing both performance and inter-group consistency among LLMs.

Result: Positive findings include that a majority vote of LLMs is approximately equivalent to human-labeled data in some settings. Negative findings indicate that LLMs still struggle with mathematical language and occasionally fail at basic inferences. They are less prone to hypothesis-only 'inference' compared to previous generations.

Conclusion: LLMs show some capability in Math NLI tasks but still struggle with mathematical language and basic inferences. However, using a majority vote of LLMs can be approximately equivalent to human-labeled data in certain settings.
...
<details>
  <summary>Details</summary>
Motivation: 本文旨在探索使用In-Context Learning (ICL)与大型语言模型（LLMs）进行Frame Semantic Parsing（FSP）而不进行模型微调的可能性。
...
<details>
  <summary>Details</summary>
Motivation: RoPE has limitations in modeling context-sensitive relationships due to its static, input-independent sinusoidal frequency patterns.
...
Result: CARoPE consistently outperforms RoPE and other common positional encoding baselines, achieving significantly lower perplexity, even at longer context lengths, and enables faster training throughput without sacrificing model stability.
...
<details>
  <summary>Details</summary>
Motivation: 现有的模型通常只进行局部编辑，而SMART-Editor旨在通过全局一致性来改进跨结构化和非结构化领域的布局和内容编辑。

Method: SMART-Editor采用两种策略：Reward-Refine（推理时的奖励引导优化方法）和RewardDPO（训练时使用奖励对齐布局对的偏好优化方法）。

Result: SMART-Editor在SMARTEdit-Bench基准测试中优于强大的基线模型，如InstructPix2Pix和HIVE，在结构化设置中RewardDPO实现了高达15%的提升，而Reward-Refine在自然图像上表现出优势。

Conclusion: SMART-Editor在结构化和非结构化领域中表现出色，通过奖励引导的规划方法实现了语义一致和视觉对齐的编辑。
...
TL;DR: 本文提出了一种基于组件的检索架构，用于解决大规模数据库自然语言接口的可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的方法依赖于领域特定的微调，这使得部署复杂，并且未能利用数据库元数据中的重要语义上下文。

Method: 将数据库模式和元数据分解为离散的语义单元，并分别进行索引以实现有针对性的检索。

Result: 实验表明，该方法保持了高召回率和准确性，并在具有不同结构和可用元数据的大规模数据库上优于基线方法。

Conclusion: 该解决方案使无需专门微调的实用文本到SQL系统在各种企业环境中部署成为可能，解决了自然语言数据库接口中的关键可扩展性差距。
...
<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在遇到模糊叙述文本时的行为，特别是针对中文文本的模糊性，以了解它们在处理语言歧义方面的表现。

Method: 我们通过收集和生成带有上下文的模糊句子及其对应的消歧对，创建了一个基准数据集，并将其系统地分为3个主要类别和9个子类别。然后进行了实验，以研究大型语言模型在处理模糊文本时的表现。

Result: 我们发现大型语言模型在处理模糊性时表现出显著的脆弱性，其行为与人类有显著差异。具体来说，大型语言模型无法可靠地区分模糊文本和非模糊文本，对模糊文本的解释过于自信，认为其只有一个意义而非多个意义，并且在尝试理解各种可能的意义时表现出过度思考。

Conclusion: 我们的研究揭示了当前大型语言模型在处理语言歧义方面的根本性局限，这对其在现实世界应用中的部署具有重要意义，需要改进处理语言理解中不确定性的方法。
...
<details>
  <summary>Details</summary>
Motivation: 理解跨模态的因果关系是多模态模型在现实环境中面临的核心挑战。

Method: 引入ISO-Bench，一个评估模型能否推断视觉观察和程序文本之间因果依赖性的基准。

Result: 对十种前沿视觉-语言模型的评估结果表现不佳：最佳零样本F1仅为0.57，思维链推理仅带来适度提升（最高0.62 F1），远低于人类（0.98 F1）。
...
<details>
  <summary>Details</summary>
Motivation: 用户反馈可以提高模型性能，但直接请求用户反馈可能具有干扰性，因此本文研究如何从用户-LM交互日志中提取隐式反馈。

Method: 本文研究了从用户-LM交互日志中获取隐式用户反馈的方法，并分析了用户反馈在不同情境下的效果。

Result: 研究发现，用户反馈的内容（例如用户希望得到澄清）可以提高模型在短问题上的性能，但在长而复杂的问题上效果不佳。此外，用户反馈的有用性与用户初始提示的质量密切相关。

Conclusion: 本文提供了对隐式用户反馈的深入研究，展示了其潜力和局限性。
...
TL;DR: 本文提出了一种新方法，利用负例更好地选择正例，从而提高少量样本ICL的性能。通过构建正负例语料库并基于语义相似性选择例子，实验结果表明该方法优于仅依赖正例的方法。
...
Method: 我们构建了基于零样本Cot的正例和负例语料库。在推理过程中，我们采用基于语义相似性的方法从正例和负例语料库中选择与给定查询最相似的例子。然后，我们进一步从正例语料库中根据与负例的语义相似性检索正例，并将它们与之前选择的正例拼接起来作为ICL演示。
...
TL;DR: 本文介绍了机制主题模型(MTMs)，它基于稀疏自编码器(SAEs)学习的可解释特征运行，能够揭示更深层次的概念主题，并且能够使用基于主题的引导向量进行可控文本生成。在五个数据集上，MTMs在连贯性指标上与传统和神经基线相当或超过，并且被主题法官一致偏好，同时能够有效引导LLM输出。


<details>
  <summary>Details</summary>
Motivation: 传统主题模型依赖于词袋表示，难以捕捉语义抽象特征。虽然一些神经变体使用更丰富的表示，但它们同样受到将主题表达为词列表的限制，这限制了它们阐述复杂主题的能力。因此，需要一种新的主题模型来解决这些问题。

Method: 引入了机制主题模型(MTMs)，它基于稀疏自编码器(SAEs)学习的可解释特征运行。MTMs可以通过定义这个语义丰富的空间来揭示更深层次的概念主题，并且能够使用基于主题的引导向量进行可控文本生成。为了正确评估MTM主题与基于词列表的方法，提出了一个基于LLM的成对比较评估框架(topic judge)。

Result: 在五个数据集上，MTMs在连贯性指标上与传统和神经基线相当或超过，并且被主题法官一致偏好，同时能够有效引导LLM输出。

Conclusion: MTMs可以揭示更深层次的概念主题，并且能够通过基于主题的引导向量进行可控文本生成。在五个数据集上，MTMs在连贯性指标上与传统和神经基线相当或超过，并且被主题法官一致偏好，同时能够有效引导LLM输出。
...
TL;DR: 本文提出了一种名为TAP-GPT的新框架，该框架适应了TableGPT2，这是一种专为商业智能任务开发的多模态表格专用LLM，用于使用结构化生物标志物数据进行AD诊断。TAP-GPT框架利用TableGPT2强大的表格理解能力和LLM的编码先验知识，在AD诊断任务中表现优于更先进的通用LLM和为预测任务开发的表格基础模型(TFM)。这是首次将LLM应用于使用表格生物标志物数据的预测任务，为未来基于LLM的多智能体框架在生物医学信息学中的应用铺平了道路。


<details>
  <summary>Details</summary>
Motivation: 早期和准确诊断阿尔茨海默病(AD)需要分析异构生物标志物，这些生物标志物通常以表格形式表示。大型语言模型(LLM)提供了前所未有的机会，用于具有结构化生物医学数据的预测。

Method: TAP-GPT框架通过使用从结构化生物医学数据中获取的上下文学习示例构建少量样本表格提示，并使用参数高效的qLoRA适应进行微调，以执行AD或认知正常(CN)的临床二分类任务。

Result: TAP-GPT框架在AD诊断任务中表现出色，优于更先进的通用LLM和为预测任务开发的表格基础模型(TFM)。

Conclusion: TAP-GPT框架利用TableGPT2强大的表格理解能力和LLM的编码先验知识，在AD诊断任务中表现优于更先进的通用LLM和为预测任务开发的表格基础模型(TFM)。这是首次将LLM应用于使用表格生物标志物数据的预测任务，为未来基于LLM的多智能体框架在生物医学信息学中的应用铺平了道路。
...
TL;DR: 本文研究了大型语言模型在心理健康领域的实用推理能力，引入了P-ReMe数据集，并提出了关于暗示和预设的修改定义。实验结果显示，Mistral和Qwen表现良好，而Claude-3.5-haiku在处理心理健康污名方面更为负责。


<details>
  <summary>Details</summary>
Motivation: 近年来，可解释性和个性化心理健康聊天机器人的发展有所增加。然而，可解释性和对话话语方面的推理方面尚未在心理健康领域得到探索。因此，我们正在研究大型语言模型（LLMs）在该领域的实用推理能力。

Method: 引入了P-ReMe数据集，并提出了对心理卫生中暗示（隐含意义）和预设（隐含假设）的修改定义。随后，制定了两个关于暗示的任务和一个关于预设的任务。为了基准测试数据集和提出的任务，考虑了四个模型：Llama3.1、Mistral、MentaLLaMa和Qwen。还提出了StiPRompts来研究心理健康中的污名，并使用了最先进的LLMs进行评估。

Result: 实验结果表明，Mistral和Qwen在该领域表现出显著的推理能力。此外，Claude-3.5-haiku在处理心理健康污名方面比其他两个LLMs更负责任。

Conclusion: 研究结果显示，Mistral和Qwen在心理健康领域表现出显著的推理能力。此外，Claude-3.5-haiku在处理心理健康污名方面比其他两个LLMs更负责任。
...
TL;DR: 本研究分析了孟加拉语NLP面临的挑战，评估了多个大型语言模型的表现，并发现了令牌化效率与模型准确性之间的关系。研究结果表明，需要改进数据集质量和评估方法，以更好地支持多语言环境下的NLP研究。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语在NLP研究中是欠代表的语言，但由于其独特的语言结构和计算限制，仍然面临挑战。本研究旨在通过系统分析来解决这一问题。

Method: 本研究系统地调查了阻碍孟加拉语NLP性能的挑战，重点关注缺乏标准化评估基准的问题。然后评估了10个最近的开源大型语言模型（LLMs）在8个翻译数据集上的表现，并进行了全面的错误分析，以确定其主要失败模式。

Result: 研究发现，与英语相比，孟加拉语的表现存在持续的差距，尤其是在较小的模型和特定模型家族（如Mistral）中。同时，某些架构（如DeepSeek）表现出有希望的鲁棒性，能够在不同语言中保持更稳定的性能。此外，研究还发现了一种令牌化效率和LLM准确性之间的反比关系。

Conclusion: 本研究揭示了当前模型在处理孟加拉语时的不足，并强调了需要改进数据集质量和评估方法，以适应多语言环境。这项工作将促进对欠代表语言的NLP研究，帮助全球普及先进的语言技术。
...
TL;DR: 本文发现并研究了一类在MoE LLMs中起关键作用的专家（SEs），这些专家在模型的前向推理中起着重要作用，尽管数量有限，但它们的剪枝会导致模型性能显著下降。SEs的分布具有模型特定性，并且不受后训练过程的影响。SEs对于诱导注意力焦点至关重要，而SEs的剪枝会显著干扰注意力分数的分布。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖经验标准来识别关键专家，缺乏对专家异质重要性的深入探索和理解。本文旨在发现并研究一类在MoE LLMs中起关键作用的专家，以提高对模型内部机制的理解。

Method: 本文通过分析MoE LLMs中的专家重要性，发现了SEs，并对其特性进行了深入研究。通过剪枝SEs，评估了它们在各种任务中的重要性，并进一步探讨了SEs压缩的影响。

Result: SEs在开放源代码的MoE LLMs中普遍存在，尽管数量有限，但它们的剪枝会导致模型性能显著下降。SEs的分布具有模型特定性，并且不受后训练过程的影响。SEs对于诱导注意力焦点至关重要，而SEs的剪枝会显著干扰注意力分数的分布。

Conclusion: 本文发现并研究了一类在MoE LLMs中起关键作用的专家，称为Super Experts (SEs)。SEs在模型的前向推理中起着重要作用，尽管数量有限，但它们的剪枝会导致模型性能显著下降。此外，SEs的分布具有模型特定性，并且不受后训练过程的影响。SEs对于诱导注意力焦点至关重要，而SEs的剪枝会显著干扰注意力分数的分布。
...
TL;DR: 本研究分析了GPT-4o-mini在改写敏感内容时的隐式内容审核行为，并评估了其在零样本分类句子敏感性方面的表现。结果表明，GPT-4o-mini会系统性地减少敏感内容，并且在分类任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 以往的研究主要集中在显式训练模型以审核和净化敏感内容上，而本研究旨在探索LLM是否会在没有明确指令的情况下隐式地净化语言。

Method: 本研究通过实验证明了GPT-4o-mini在改写敏感内容时的隐式内容审核行为，并评估了敏感性变化的程度。同时，还评估了LLM在零样本情况下对句子敏感性的分类能力，并与传统方法进行了比较。

Result: 实验结果表明，GPT-4o-mini会系统性地将内容向不那么敏感的类别调整，并显著减少侮辱性和禁忌语言。此外，LLM在零样本分类句子敏感性方面表现出色，与传统方法相比具有竞争力。

Conclusion: 本研究发现GPT-4o-mini在改写敏感内容时会系统性地将其向不那么敏感的类别进行调整，并显著减少了侮辱性和禁忌语言。此外，研究还评估了LLM在零样本分类句子敏感性方面的表现，并与传统方法进行了比较。
...
TL;DR: 本文提出了一种基于检索增强生成（RAG）的框架MusT-RAG，用于将通用大语言模型适配到仅文本音乐问答任务。通过引入音乐专业的向量数据库MusWikiDB以及在推理和微调过程中利用上下文信息，MusT-RAG在音乐领域表现优异，优于传统微调方法。


<details>
  <summary>Details</summary>
Motivation: 由于音乐特定知识在训练数据中的比例相对较小，大语言模型（LLMs）在音乐相关应用中的有效性仍然有限。因此，需要一种方法来提升LLMs在音乐领域的表现。

Method: 我们提出了MusT-RAG，这是一个基于检索增强生成（RAG）的综合框架，用于将通用大语言模型适配到仅文本音乐问答（MQA）任务。为了优化RAG以适应音乐领域，我们（1）提出了MusWikiDB，一个专门用于检索阶段的音乐专业向量数据库，并且（2）在推理和微调过程中利用上下文信息，以有效地将通用大语言模型转化为音乐特定模型。

Result: 实验表明，MusT-RAG在增强LLMs的音乐领域适应能力方面显著优于传统的微调方法，在域内和域外MQA基准测试中均表现出稳定的提升。此外，我们的MusWikiDB比一般的维基百科语料库更有效，提供了优越的性能和计算效率。

Conclusion: MusT-RAG显著优于传统的微调方法，在增强LLMs的音乐领域适应能力方面表现出色，并在域内和域外MQA基准测试中都取得了稳定的提升。此外，我们的MusWikiDB比一般的维基百科语料库更有效，提供了优越的性能和计算效率。
...
TL;DR: 本文提出 TeQoDO 方法，利用大型语言模型的 SQL 编程能力和对话理论，无需监督即可构建任务导向对话本体，提高了模型的可解释性和可控性。


<details>
  <summary>Details</summary>
Motivation: 现有的任务导向对话系统依赖于外部数据库和显式本体，但构建这些本体需要手动标记或监督训练，限制了其应用。本文旨在解决这一问题，提出一种无需监督的方法来构建本体。

Method: TeQoDO 利用大型语言模型的内在 SQL 编程能力和对话理论，在没有监督的情况下从零开始构建任务导向对话本体。

Result: TeQoDO 在下游对话状态跟踪任务中表现出色，其构建的本体具有竞争力，并且能够扩展到构建更大的本体。消融研究证明了对话理论的关键作用。

Conclusion: TeQoDO 是一种无需监督即可自主构建任务导向对话本体的方法，展示了其在对话状态跟踪任务中的竞争力，并且能够扩展到构建更大的本体，为提高大型语言模型的可解释性提供了一种新途径。
...
TL;DR: 本文提出了MPCC基准，用于评估MLLM处理多模态约束的能力。实验发现MLLM在面对复杂约束时表现不佳，需要改进约束感知推理。


<details>
  <summary>Details</summary>
Motivation: 当前基准面临两个关键挑战：(1) 无法直接评估多模态现实规划能力，(2) 缺乏或隐含的跨模态约束。为了解决这些问题，我们提出了MPCC基准。

Method: 我们引入了Multimodal Planning with Complex Constraints (MPCC)，这是第一个系统评估MLLM处理规划中多模态约束能力的基准。MPCC专注于三个现实任务：航班规划、日历规划和会议规划，并在这些任务中引入了复杂的约束（如预算、时间和空间），并设有不同难度级别以区分约束复杂度与搜索空间扩展。

Result: 实验表明，封闭源代码模型仅能实现21.3%的可行计划，而开源模型平均低于11%。此外，我们观察到MLLM对约束复杂度高度敏感，并且传统的多模态提示策略在多约束场景中失败。

Conclusion: 我们的工作形式化了规划中的多模态约束，提供了严格的评估框架，并突出了在现实世界MLLM应用中进行约束感知推理的必要性。
...
TL;DR: Causal2Vec是一种通用的嵌入模型，旨在增强解码器-only LLM的性能，而无需更改其原始架构或引入显著的计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有的方法主要关注于移除LLM中的因果注意力掩码以实现双向注意力，这可能削弱模型在预训练中获取语义信息的能力。此外，领先的单向方法通常依赖于额外的输入文本来克服因果注意力的固有局限性，这不可避免地增加了计算成本。

Method: Causal2Vec通过使用轻量级的BERT风格模型对输入文本进行预编码，生成一个上下文标记，并将其附加到LLM的输入序列中，以提高解码器-only LLM的性能。此外，为了减轻最后标记池引入的近期偏差，将上下文标记和EOS标记的最后隐藏状态连接起来作为最终的文本嵌入。

Result: Causal2Vec在MTEB基准测试中实现了最先进的性能，同时减少了所需的序列长度和推理时间。

Conclusion: Causal2Vec在仅使用公开检索数据集训练的模型中，在MTEB基准测试中达到了最先进的性能，同时减少了所需的序列长度和推理时间。
...
TL;DR: 本研究探讨了本地部署的开源语言模型作为专有云服务的可行替代方案，发现其在功能性能上表现良好，并提供了更好的数据控制和隐私保护。


<details>
  <summary>Details</summary>
Motivation: 由于数据隐私、安全性和公平访问的问题，需要探索专有云服务的替代部署模型。本地部署的开源语言模型可能提供更好的数据控制和隐私保护，同时减少对云服务的依赖。

Method: 本研究评估了三种安装在基于CPU的平台上的开源语言模型，并将其与商用在线聊天机器人进行了比较。评估重点在于功能性能，而非人机翻译质量的比较。

Result: 研究结果表明，本地部署的开源语言模型在功能性能上可以与商用在线聊天机器人相媲美，同时提供了增强的数据控制、隐私保护和减少对云服务的依赖等优势。

Conclusion: 本研究的结论表明，本地部署的开源语言模型在功能性能上可以作为专有云服务的可行替代方案。本地部署提供了增强的数据控制、隐私保护和减少对云服务的依赖等优势，有助于推动AI技术的民主化，并为未来的研究和开发提供参考，特别是针对个人翻译者和小型企业的需求。
...
TL;DR: 本文提出了一种基于多关系图和结构熵最小化的无监督多文档摘要框架MRGSEM-Sum，通过构建多关系图、应用结构熵最小化算法和引入位置感知压缩机制，有效解决了文档间复杂关系和信息冗余的问题，并在多个基准数据集上取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多文档摘要方法通常只考虑单关系图并需要预定义的聚类数，这限制了它们全面表示丰富的关系信息和自适应划分句子组的能力。

Method: MRGSEM-Sum基于多关系图和结构熵最小化提出了一种无监督多文档摘要框架。具体来说，构建了一个整合语义和话语关系的多关系图，应用二维结构熵最小化算法进行聚类，自动确定最佳聚类数，并引入位置感知压缩机制以生成简洁的信息摘要。

Result: 在四个基准数据集（Multi-News、DUC-2004、PubMed和WikiSum）上的广泛实验表明，该方法在多个方面优于之前的无监督方法，并且在某些情况下达到与监督模型和大型语言模型相当的性能。人类评估显示，MRGSEM-Sum生成的摘要具有高一致性和覆盖率，接近人工水平。

Conclusion: MRGSEM-Sum能够生成高质量的多文档摘要，其性能在多个基准数据集上优于之前的无监督方法，并且在某些情况下可以与监督模型和大型语言模型相媲美。
...
TL;DR: 本文提出了一种针对阿拉伯语的增强型密集段落检索（DPR）框架，核心是新颖的注意相关性评分（ARS）方法，以更有效地建模问题与段落之间的语义相关性。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语由于其复杂的形态学、可选的变音符号以及现代标准阿拉伯语（MSA）和各种方言的共存，对自然语言处理（NLP）和信息检索（IR）构成了特殊挑战。尽管阿拉伯语在全球的重要性不断增加，但在NLP研究和基准资源中仍被低估。

Method: 本文提出了一个专门针对阿拉伯语的增强型DPR框架，核心是新颖的ARS方法，该方法用自适应评分函数替代了标准的交互机制，以更有效地建模问题与段落之间的语义相关性。此外，还集成了预训练的阿拉伯语语言模型和架构改进，以提高检索性能并显著提高回答阿拉伯语问题的排名准确性。

Result: 本文的方法通过集成预训练的阿拉伯语语言模型和架构改进，显著提高了回答阿拉伯语问题的排名准确性。

Conclusion: 本文提出的增强型DPR框架和ARS方法为阿拉伯语的自然语言处理提供了有效的解决方案。
...
TL;DR: 本文介绍了主动批判性思维的概念，并提出了两个新基准来评估模型在不完整或误导条件下的数学推理能力。实验表明，强化学习可以显著提升模型的主动批判性思维能力。


<details>
  <summary>Details</summary>
Motivation: 先前的研究主要关注被动批判性思维，而本文旨在探索主动批判性思维，以更好地解决用户的问题。

Method: 本文引入了主动批判性思维的概念，并提出了两个新的基准GSM-MC和GSM-MCE来评估数学推理能力。此外，还使用强化学习算法来提升模型的主动批判性思维能力。

Result: 实验结果显示，尽管这些模型在传统推理任务中表现优异，但在主动批判性思维方面存在困难。然而，通过强化学习算法，显著提升了模型的能力，例如Qwen3-1.7B在GSM-MC上的准确率从0.15%提升到了73.98%。

Conclusion: 本文希望通过主动批判性思维，使模型在解决问题时能更有效地与用户协作。
...
TL;DR: 本文研究了如何通过微调大型语言模型来实现基于角色的访问控制，并评估了多种方法的有效性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在企业环境中的广泛应用，根据用户角色控制模型行为变得至关重要。现有的安全方法通常假设统一的访问权限，而没有解决特定角色的访问限制问题。

Method: 本文研究了三种建模策略：基于BERT的分类器、基于LLM的分类器以及角色条件生成。此外，还构建了两个互补的数据集以评估这些方法。

Result: 本文评估了模型在不同组织结构下的性能，并分析了对提示注入、角色不匹配和越狱尝试的鲁棒性。

Conclusion: 本文探讨了通过微调大型语言模型（LLMs）来生成反映不同组织角色访问权限的响应的可能性，并评估了多种建模策略的有效性。
...
TL;DR: 本研究开发了临床安全-效果双轨基准（CSEDB），用于评估医疗大型语言模型的临床应用。结果显示，大型语言模型在安全性和效果方面表现中等，领域特定的模型在安全性和效果方面表现更好。研究结果有助于促进医疗环境中大型语言模型的更安全、更有效部署。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在临床决策支持中具有潜力，但在安全评估和效果验证方面面临重大挑战。需要一种标准化的指标来评估医疗大型语言模型的临床应用，以便进行比较分析、风险暴露识别和改进方向的确定。

Method: 本研究开发了临床安全-效果双轨基准（CSEDB），这是一个基于临床专家共识的多维框架，涵盖了30个标准，包括危重疾病识别、指南遵循和药物安全等关键领域，并采用加权后果测量。32名专科医生开发并审查了2,069个开放式问答项目，这些项目与这些标准一致，涵盖26个临床部门以模拟现实场景。对六种大型语言模型的基准测试显示总体表现中等（平均总分57.2%，安全54.7%，效果62.3%），在高风险场景中表现下降显著（p < 0.0001）。

Result: 对六种大型语言模型的基准测试显示总体表现中等（平均总分57.2%，安全54.7%，效果62.3%），在高风险场景中表现下降显著（p < 0.0001）。领域特定的医疗大型语言模型在安全（0.912）和效果（0.861）方面表现出相对较高的最高分数。

Conclusion: 本研究的发现不仅为评估医疗大型语言模型的临床应用提供了标准化指标，还促进了不同场景下比较分析、风险暴露识别和改进方向的确定，同时具有促进医疗环境中大型语言模型更安全、更有效部署的潜力。
...
TL;DR: 本文提出了 Med-R$^3$，一个基于渐进强化学习的医学检索增强推理框架，解决了现有模型在检索和推理联合优化方面的不足，并在实验中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要集中在增强模型的检索或推理能力，而很少关注两者的联合优化，导致两个过程之间的协调有限。此外，当前方法严重依赖监督微调，这可能导致模型记忆现有的解决问题路径，从而限制其在面对新问题情境时的泛化能力。虽然一些研究探索了通过强化学习改进一般领域的检索增强推理，但它们的奖励函数设计未能充分满足医学领域的特定需求。

Method: 引入了 Med-R$^3$，这是一个基于渐进强化学习的医学检索增强推理框架。首先开发模型在医学问题上的逻辑推理能力，然后在此基础上自适应优化检索能力，以更好地与知识语料库和外部信息利用相匹配，最后进行检索和推理协调的联合优化。

Result: Med-R$^3$ 实现了最先进的性能，LLaMA3.1-8B-Instruct + Med-R$^3$ 在参数规模相当的情况下超越了封闭的 GPT-4o-mini 3.93%，而 Qwen2.5-14B 增强的 Med-R$^3$ 显示出更显著的 13.53% 的提升。

Conclusion: Med-R$^3$ 可以实现最先进的性能，LLaMA3.1-8B-Instruct + Med-R$^3$ 在参数规模相当的情况下超越了封闭的 GPT-4o-mini 3.93%，而 Qwen2.5-14B 增强的 Med-R$^3$ 显示出更显著的 13.53% 的提升。
...
TL;DR: 本文提出 T-Detect，一种基于学生 t 分布的文本检测方法，显著提升了对抗性文本检测的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的零样本检测器依赖于隐含高斯分布的统计度量，这在面对对抗性或非英语文本的重尾统计特征时失效。因此需要一种更稳健的检测方法。

Method: T-Detect 通过将标准高斯归一化替换为基于学生t分布的重尾差异分数，重新设计了基于曲率的检测器的统计核心。

Result: T-Detect 在 RAID 基准和 HART 数据集上验证，表现优于强基线，在目标领域 AUROC 提升高达 3.9%。在二维检测框架中，达到了 0.926 的 AUROC。

Conclusion: T-Detect 提供了一种新的、理论上合理的统计基础用于文本检测，并展示了其在对抗条件下的性能分析。代码已发布。
...
TL;DR: DiffLoRA is a parameter-efficient adaptation of differential attention that shows promising results in specific domains.


<details>
  <summary>Details</summary>
Motivation: To retain the efficiency of LoRA while benefiting from the performance gains of differential attention.

Method: DiffLoRA is a parameter-efficient adaptation of the differential attention mechanism with low-rank adapters on both positive and negative attention terms.

Result: DiffLoRA shows +11 pts on LoRA for HumanEval in certain domains, but falls short of other methods in most tasks.

Conclusion: DiffLoRA shows interesting results in certain domains despite falling short of other parameter-efficient fine-tuning methods in most evaluation tasks.
...
TL;DR: 本文研究了阿拉伯语文本中的仇恨言论识别和清理问题，采用深度学习和机器翻译方法，取得了较高的准确率和BLEU分数。


<details>
  <summary>Details</summary>
Motivation: 随着社交媒体上仇恨言论问题的日益严重，需要有效的识别和清理方法。

Method: 针对第一个问题，使用深度学习模型和变压器进行了实验以确定最佳模型；针对第二个问题，将其视为机器翻译任务，输入是包含脏话的句子，输出是屏蔽脏话的相同句子。

Result: 在仇恨言论检测中，最佳模型达到了92%的宏F1分数和95%的准确率；在文本清理实验中，最佳模型达到了0.3的BLEU分数（1-gram）。

Conclusion: 本文提出了两种方法来解决阿拉伯语文本中的仇恨言论识别和清理问题，并取得了良好的效果。
...
TL;DR: 本文研究了大型语言模型生成知识图谱中逻辑规则的自然语言解释的能力，结果显示其在正确性和清晰度方面表现良好，但仍有改进空间。


<details>
  <summary>Details</summary>
Motivation: 知识图谱（KGs）通常包含足够的信息来支持新事实的推断。识别逻辑规则不仅提高了知识图谱的完整性，还能够检测潜在错误，揭示细微的数据模式，并增强整体推理和解释能力。然而，这些规则的复杂性以及每个KG的独特标记约定可能使它们对人类难以理解。因此，本文探索了大型语言模型生成自然语言解释的潜力。

Method: 本文使用AMIE 3.5.1规则发现算法从基准数据集FB15k-237和两个大规模数据集FB-CVT-REV和FB+CVT-REV中提取逻辑规则，并探讨了各种提示策略，包括零样本和少量样本提示、变量实体类型以及思维链推理。

Result: 本文的结果表明，大型语言模型在解释的正确性和清晰度方面表现出色，但仍然存在一些挑战需要未来研究解决。

Conclusion: 本文展示了大型语言模型在生成逻辑规则的自然语言解释方面表现出色，但仍存在一些挑战需要未来研究解决。
...
TL;DR: 本文提出了一种基于级联问题披露的框架，以更准确地估计LLM的问题解决能力，同时保持可扩展性和自动化。
...
Method: 我们提出了一个基于级联问题披露的全面且可推广的框架，通过分阶段收集模型响应，每个阶段揭示部分问题信息，以激发LLM的通用推理。

Result: 我们的方法不仅提供了更好的LLM比较，而且相比标准QA范式，诱导了更好的中间轨迹。

Conclusion: 我们的方法缩小了在标准QA评估设置中观察到的性能差距，表明普遍的间接QA范式高估了模型之间的性能差异。
...
### [68] [Hybrid EEG--Driven Brain--Computer Interface: A Large Language Model Framework for Personalized Language Rehabilitation](https://arxiv.org/abs/2507.22892)
*Ismail Hossain,Mridul Banik*

Main category: cs.HC

TL;DR: 本文提出了一种结合实时EEG信号和大型语言模型的混合框架，用于语言康复助手，以更好地适应用户的认知和语言需求。


<details>
  <summary>Details</summary>
Motivation: 传统辅助和替代沟通（AAC）系统和语言学习平台在实时适应用户认知和语言需求方面存在不足，尤其是在中风失语症或肌萎缩侧索硬化等神经疾病中。

Method: 提出了一种新的混合框架，利用实时EEG信号驱动基于LLM的语言康复助手。

Result: 该系统能够使用户通过思维指令导航语言学习模块，动态个性化词汇、句子构造练习和纠正反馈，并监测认知努力的神经标记以实时调整任务难度。

Conclusion: 该系统旨在通过实时EEG信号驱动基于LLM的语言康复助手，以满足严重言语或运动障碍用户的需求，并动态调整任务难度。
...
### [69] [Voice-guided Orchestrated Intelligence for Clinical Evaluation (VOICE): A Voice AI Agent System for Prehospital Stroke Assessment](https://arxiv.org/abs/2507.22898)
...
TL;DR: 开发了一个语音驱动的人工智能系统，用于指导非医疗志愿者进行中风评估，并通过智能手机视频捕捉关键检查部分。该系统在评估中表现良好，但需要专家审查以确保准确性。


<details>
  <summary>Details</summary>
Motivation: 当前急救人员对中风的识别不一致且经常不准确，检测中风的敏感性低至58%，导致治疗延误。

Method: 开发了一个语音驱动的人工智能（AI）系统，该系统通过自然对话引导任何人（从急救人员到家庭成员）进行专家级中风评估，同时允许智能手机视频捕捉关键检查部分以进行记录和潜在专家审查。

Result: AI系统正确识别了84%的个体中风迹象，并检测到了75%的可能大血管阻塞（LVO）中风，完成评估仅需6分钟多一点。用户报告了高信心（中位数4.5/5）和易用性（平均4.67/5）。系统成功识别了86%的实际中风，但也错误地将3个非中风病例中的2个标记为中风。当专家医生审查AI报告和视频时，他们100%确定了正确的诊断，但由于观察到的AI错误，只在40%的病例中足够自信做出初步治疗决定。

Conclusion: 虽然当前系统需要人工监督，但语音到语音AI模型的快速发展表明，未来版本有望实现高度准确的评估。实现人类水平的语音交互可能会改变紧急医疗护理，将专家指导的评估能力带给每个人。
...
### [70] [Toward the Autonomous AI Doctor: Quantitative Benchmarking of an Autonomous Agentic AI Versus Board-Certified Clinicians in a Real World Setting](https://arxiv.org/abs/2507.22902)
*Hashim Hayat,Maksim Kudrautsau,Evgeniy Makarov,Vlad Melnichenko,Tim Tsykunou,Piotr Varaksin,Matt Pavelle,Adam Z. Oskowitz*

Main category: cs.HC

TL;DR: 本研究评估了一种基于多代理LLM的AI框架是否可以在虚拟紧急护理环境中自主作为AI医生运作。结果显示，AI在诊断和治疗方案方面与人类医生高度一致，并在某些情况下表现更优，这表明多代理AI系统可以实现与人类提供者相当的临床决策，并可能成为解决医疗人员短缺问题的潜在解决方案。


<details>
  <summary>Details</summary>
Motivation: 全球预计到2030年将面临1100万医疗从业者短缺的问题，而行政负担消耗了50%的临床时间。人工智能（AI）有潜力缓解这些问题。然而，还没有端到端的自主大型语言模型（LLM）为基础的AI系统在现实临床实践中经过严格评估。

Method: 我们回顾性比较了多代理AI系统Doctronic和认证临床医生在500次连续的紧急护理远程医疗会诊中的表现。主要终点：诊断一致性和治疗方案一致性以及安全指标，通过盲法LLM基于判决和专家人工评审进行评估。

Result: Doctronic和临床医生的最高诊断在81%的病例中匹配，治疗方案在99.2%的病例中一致。没有发生临床幻觉（例如，诊断或治疗不受临床发现支持）。在对不一致案例的专家评审中，AI表现优于36.1%，而人类表现优于9.3%；其余案例的诊断是等效的。

Conclusion: 在首次大规模验证自主AI医生的研究中，我们展示了与人类临床医生强大的诊断和治疗计划一致性，AI性能与执业临床医生相当，并在某些情况下超过了他们。这些发现表明，多代理AI系统可以实现与人类提供者相当的临床决策，并可能成为解决医疗人员短缺问题的潜在解决方案。
...
### [71] [SWE-Debate: Competitive Multi-Agent Debate for Software Issue Resolution](https://arxiv.org/abs/2507.23348)
...
<details>
  <summary>Details</summary>
Motivation: Existing agent-based issue resolution approaches often get stuck in local solutions and fail to identify issue patterns that span across different parts of the codebase.
...
### [72] [SWE-Exp: Experience-Driven Software Issue Resolution](https://arxiv.org/abs/2507.23361)
*Silin Chen,Shaoxin Lin,Xiaodong Gu,Yuling Shi,Heng Lian,Longfei Yun,Dong Chen,Weiguo Sun,Lin Cao,Qianxiang Wang*

Main category: cs.SE

TL;DR: 本文介绍了一种名为SWE-Exp的方法，该方法通过从先前代理轨迹中提炼出简洁且可操作的经验，实现跨问题的持续学习，从而提高了软件问题解决的效率。


<details>
  <summary>Details</summary>
Motivation: 当前的代理作为无记忆的探索者，单独处理每个问题，而不保留或重用以前修复经验的知识。这导致了对失败轨迹的冗余探索，并错过了将成功的解决问题方法适应到类似问题的机会。

Method: 我们引入了SWE-Exp，一种增强经验的方法，它从先前的代理轨迹中提炼出简洁且可操作的经验，实现了跨问题的持续学习。我们的方法引入了一个多方面经验库，捕捉成功和失败的修复尝试，并在不同层次上提取可重用的问题解决知识。

Result: 实验表明，SWE-Exp在SWE-bench-Verified上实现了最先进的解决率（41.6% Pass@1）。

Conclusion: 我们的方法建立了一种新范式，其中自动软件工程代理系统地积累和利用修复专业知识，从根本上从试错探索转向战略性的、以经验为导向的问题解决。
...
### [73] [ELMES: An Automated Framework for Evaluating Large Language Models in Educational Scenarios](https://arxiv.org/abs/2507.22947)
...
TL;DR: 本文介绍了ELMES，一个开源的自动化评估框架，用于评估教育环境中的大型语言模型。该框架通过模块化架构和混合评估引擎，解决了现有评估指标不足的问题，并展示了不同模型在教育场景中的表现差异。


<details>
  <summary>Details</summary>
Motivation: 当前的基准测试主要衡量一般智能而非教学能力，而许多新兴的教学场景缺乏适当的评估指标。因此，需要一种专门针对教育场景的评估框架。

Method: 引入ELMES，一个开源的自动化评估框架，专门设计用于评估教育环境中的LLMs。该框架具有模块化架构，允许研究人员通过简单的配置文件创建动态、多代理对话，同时结合混合评估引擎，使用LLM-as-a-Judge方法客观量化传统的主观教学指标。

Result: 在四个关键教育场景中对最先进的LLMs进行了系统基准测试，结果揭示了模型在不同情境下的特定优势和局限性。

Conclusion: ELMES为教育者和研究人员提供了一个可访问的评估框架，显著降低了多种教育应用的适应障碍，同时推动了LLMs在教学中的实际应用。
...
### [74] [Holistic Evaluations of Topic Models](https://arxiv.org/abs/2507.23364)
*Thomas Compton*

Main category: cs.IR

TL;DR: 本文从数据库的角度评估了主题模型，并从1140次BERTopic模型运行中获得了见解，以识别优化模型参数时的权衡，并反思这些发现对主题模型的解释和负责任使用的意义。


<details>
  <summary>Details</summary>
Motivation: 本文的动机是识别在优化模型参数时的权衡，并反思这些发现对主题模型的解释和负责任使用的意义。

Method: 本文从数据库的角度评估了主题模型，并从1140次BERTopic模型运行中获得了见解。

Result: 本文的结果是，从1140次BERTopic模型运行中获得了关于主题模型优化的见解。

Conclusion: 本文结论是，主题模型在优化模型参数时存在权衡，并且这些发现对主题模型的解释和负责任使用具有重要意义。
...
### [75] [Generalized Reinforcement Learning for Retriever-Specific Query Rewriter with Unstructured Real-World Documents](https://arxiv.org/abs/2507.23242)
*Sungguk Cha,DongWook Kim,Taeseung Hahn,Mintae Kim,Youngsub Han,Byoung-Ki Jeon*

Main category: cs.CV

TL;DR: 本文介绍了一种名为 RL-QR 的强化学习框架，用于检索器特定的查询重写，以提高 RAG 系统的检索性能，无需人工标注数据集，并在实验中展示了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的 RAG 系统在优化用于多样化和非结构化现实文档的查询方面仍然面临挑战，而传统方法需要人工标注的数据集。

Method: RL-QR 是一种基于强化学习的检索器特定查询重写框架，它通过合成场景-问题对并利用广义奖励策略优化（GRPO）来训练针对特定检索器的查询重写器，从而提高检索性能。

Result: 实验结果显示，RL-QR 在工业内部数据上表现出显著的改进，其中 RL-QR_{multi-modal} 在多模态 RAG 中实现了 NDCG@3 的 11% 相对增益，而 RL-QR_{lexical} 为词汇检索器带来了 9% 的增益。然而，在语义和混合检索器中，重写器未能提高性能，这可能是因为训练不一致。

Conclusion: RL-QR 的研究结果表明，它有潜力彻底改变 RAG 系统的查询优化，提供一种可扩展的、无需注释的解决方案，用于现实世界的检索任务，同时在语义检索环境中指明了进一步改进的方向。
...
### [76] [SequenceLayers: Sequence Processing and Streaming Neural Networks Made Easy](https://arxiv.org/abs/2507.23292)
...
TL;DR: 本文介绍了 SequenceLayers，一种用于序列建模的神经网络层 API 和库，支持流式处理和并行处理，提高模型的正确性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 为了实现序列模型的流式处理，减少在流式处理和并行序列处理中出现的常见错误，并在任何深度学习库中实现。

Method: 通过定义显式的随时间变化的状态（如 Transformer KV 缓存、卷积缓冲区、RNN 隐藏状态）和一个演变该状态的步骤方法，实现序列模型的流式处理。

Result: SequenceLayers 可以立即实现流式处理，减少常见的错误，并且可以用于构建生产规模的模型。

Conclusion: SequenceLayers 提供了一种可组合和声明式的 API，使从简单的可流式处理组件构建生产规模模型变得容易，同时保持强大的正确性保证。
...
### [77] [Deep Learning-based Prediction of Clinical Trial Enrollment with Uncertainty Estimates](https://arxiv.org/abs/2507.23607)
*Tien Huu Do,Antoine Masquelier,Nae Eoun Lee,Jonathan Crowther*

Main category: cs.LG

TL;DR: 本文提出了一种基于深度学习的方法，用于预测临床试验中的患者招募情况，该方法利用预训练语言模型和概率层来提高预测准确性，并在实际数据上表现出优越的性能。


<details>
  <summary>Details</summary>
Motivation: 临床试验需要大量的资金和细致的规划，因此准确预测试验结果非常重要。而准确预测患者招募是试验成功的关键挑战之一，因此需要一种有效的方法来解决这个问题。

Method: 本文提出了一种基于神经网络模型的方法，利用预训练语言模型（PLMs）来捕捉临床文档的复杂性和细微差别，并通过注意力机制将这些表示与编码的表格特征结合。此外，还通过基于Gamma分布的概率层来处理招募预测中的不确定性。

Result: 本文在真实世界的临床试验数据上进行了广泛的实验，结果表明所提出的方法可以有效地预测给定临床试验中多个地点的患者招募数量，并且优于现有的基线模型。
...
### [78] [TweakLLM: A Routing Architecture for Dynamic Tailoring of Cached Responses](https://arxiv.org/abs/2507.23674)
*Muhammad Taha Cheema,Abeer Aamir,Khawaja Gul Muhammad,Naveed Anwar Bhatti,Ihsan Ayyub Qazi,Zafar Ayyub Qazi*

Main category: cs.LG

TL;DR: TweakLLM 是一种利用轻量级 LLM 动态调整缓存响应的路由架构，能够有效提高缓存效率，同时保持响应质量。


<details>
  <summary>Details</summary>
Motivation: 由于聊天机器人的个性化交互和语义相似性搜索的有限准确性，使用响应缓存来保持与用户查询的相关性具有挑战性。

Method: TweakLLM 是一种新颖的路由架构，利用轻量级 LLM 动态调整缓存响应以适应传入提示。

Result: 通过全面评估，包括用户研究、并排比较、满意度投票以及多代理 LLM 辩论，我们证明 TweakLLM 在保持响应质量方面与前沿模型相当，同时显著提高了缓存效果。
...
TL;DR: 本文提出了一种新的评估框架SE+CFE，用于检测和防范基于LLM的评估系统中的提示注入攻击。


<details>
  <summary>Details</summary>
Motivation: 本文研究了针对基于LLM的评估系统的防御措施，以应对提示注入攻击。我们形式化了一类称为盲攻击的威胁，其中候选答案是独立于真实答案构建的，以欺骗评估器。

Method: 我们提出了一个框架，将标准评估（SE）与反事实评估（CFE）相结合，该框架重新评估提交的答案 against 一个故意错误的基准答案。

Result: 实验显示，标准评估极易受到攻击，而我们的SE+CFE框架在几乎没有性能损失的情况下显著提高了安全性。
...
### [80] [DSBC : Data Science task Benchmarking with Context engineering](https://arxiv.org/abs/2507.23336)
*Ram Mohan Rao Kadiyala,Siddhant Gupta,Jebish Purbey,Giulio Martini,Suman Debnath,Hamza Farooq*

Main category: cs.AI

TL;DR: 本文介绍了一个专门用于评估数据科学代理的基准，分析了不同LLM和方法在多种任务类别中的表现，并探讨了温度参数对结果的影响。


<details>
  <summary>Details</summary>
Motivation: 尽管数据科学代理迅速被采用，但系统性基准评估其效果和局限性仍然稀缺。本文旨在填补这一空白。

Method: 本文通过观察我们商业应用中的使用情况，设计了一个全面的基准来反映现实世界中用户与数据科学代理的交互。评估了三种LLM：Claude-4.0-Sonnet、Gemini-2.5-Flash和OpenAI-o4-Mini，采用了三种方法：零样本上下文工程、多步骤上下文工程和SmolAgent。

Result: 研究结果揭示了所评估模型和方法之间的显著性能差异，突显了影响实际部署的关键因素。

Conclusion: 本文引入的基准数据集和评估框架旨在为未来更强大和有效的数据科学代理研究提供基础。
...
### [81] [TextQuests: How Good are LLMs at Text-Based Video Games?](https://arxiv.org/abs/2507.23701)
*Long Phan,Mantas Mazeika,Andy Zou,Dan Hendrycks*

Main category: cs.AI

TL;DR: TextQuests 是一个基于互动小说游戏的基准测试，用于评估 AI 代理在复杂、交互式环境中的内在长上下文推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的代理基准测试无法充分捕捉代理在探索性环境中进行持续、自主推理的能力，因此需要一个新的基准测试来推动更强大的内在推理能力的发展。

Method: TextQuests 基于 Infocom 互动小说游戏套件，通过文本冒险游戏来评估 AI 代理的自我-contained 问题解决能力，不允许使用外部工具。

Result: TextQuests 被设计为一个有效的代理评估基准，能够评估 LLM 代理在探索性环境中进行试错学习和持续解决问题的能力。

Conclusion: TextQuests 是一个用于评估 AI 代理在复杂、交互式环境中自主操作能力的基准测试，它专注于内在的长上下文推理能力。
...
### [82] [Seed-Prover: Deep and Broad Reasoning for Automated Theorem Proving](https://arxiv.org/abs/2507.23726)
...
TL;DR: 本文介绍了Seed-Prover和Seed-Geometry两个系统，用于解决数学定理证明问题，并在多个基准测试中取得了显著成果。


<details>
  <summary>Details</summary>
Motivation: LLMs在利用强化学习进行长链式思维推理方面表现出强大的数学推理能力，但仍然在定理证明方面存在困难，因为仅使用自然语言时缺乏清晰的监督信号。而像Lean这样的专用领域语言可以通过形式化验证提供清晰的监督，从而通过强化学习进行有效训练。

Method: 提出了一种基于引理的完整证明推理模型Seed-Prover，并设计了三种测试时推理策略以解决IMO级别的竞赛问题。此外，引入了一个几何推理引擎Seed-Geometry来解决Lean中缺乏几何支持的问题。

Result: Seed-Prover证明了78.1%的形式化过去IMO问题，达到了MiniF2F的饱和状态，并在PutnamBench上实现了50%以上的成绩，大大超过了之前的最先进水平。Seed-Geometry在形式化几何引擎中表现优于之前的引擎。使用这两个系统参与了IMO 2025并成功证明了6个问题中的5个。

Conclusion: 本工作代表了自动数学推理的重大进展，展示了形式化验证与长链式思维推理的有效性。
...
### [83] [CoT-Self-Instruct: Building high-quality synthetic prompts for reasoning and non-reasoning tasks](https://arxiv.org/abs/2507.23751)
*Ping Yu,Jack Lanchantin,Tianlu Wang,Weizhe Yuan,Olga Golovneva,Ilia Kulikov,Sainbayar Sukhbaatar,Jason Weston,Jing Xu*

Main category: cs.AI

TL;DR: CoT-Self-Instruct是一种合成数据生成方法，通过链式思维推理和规划生成高质量的合成提示，并在多个任务中表现出色，优于现有数据集和人类提示。


<details>
  <summary>Details</summary>
Motivation: 现有训练数据集在可验证推理任务和非可验证指令遵循任务中的表现有限，需要一种更有效的合成数据生成方法来提升大语言模型的性能。

Method: CoT-Self-Instruct是一种合成数据生成方法，通过让大语言模型首先进行链式思维推理和规划，然后生成类似质量和复杂度的新合成提示，并使用自动指标进行过滤以获得高质量数据。

Result: 在可验证推理任务中，CoT-Self-Instruct生成的合成数据显著优于s1k和OpenMathReasoning等现有数据集；在非可验证指令遵循任务中，其性能超过了人类或标准自我指导提示。

Conclusion: CoT-Self-Instruct方法在可验证推理和非可验证指令遵循任务中均表现出色，优于现有的训练数据集和人类或标准自我指导提示。
...
### [84] [SimuRA: Towards General Goal-Oriented Agent via Simulative Reasoning Architecture with LLM-Based World Model](https://arxiv.org/abs/2507.23773)
*Mingkai Deng,Jinyu Hou,Yilin Shen,Hongxia Jin,Graham Neubig,Zhiting Hu,Eric Xing*

Main category: cs.AI

TL;DR: 本文提出SimuRA，一种基于世界模型的通用AI代理架构，能够克服自回归LLM的局限性，并在复杂任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前的AI代理方法在可扩展性和通用性方面存在不足，并且受到自回归LLM的根本限制。而人类是能够通过心理模拟行动和计划结果的通用代理。

Method: 引入SimuRA，一种目标导向的通用代理推理架构，利用世界模型进行规划。

Result: 实验表明，SimuRA在困难的网页浏览任务中显著提高了航班搜索的成功率，并且基于世界模型的规划相比自回归规划有明显优势。

Conclusion: 我们对基于LLM训练单一、通用的智能体模型的可能性感到兴奋，这种模型可以在所有环境中表现出超级智能行为。
...
### [85] [Exploring Dynamic Parameters for Vietnamese Gender-Independent ASR](https://arxiv.org/abs/2507.22964)
*Sotheara Leang,Éric Castelli,Dominique Vaufreydaz,Sethserey Sam*

Main category: eess.AS

TL;DR: 本文提出了一种新的方法，通过在频谱子带重心频率的比率平面上使用极坐标参数来捕捉语音的动态特性，并将其与Mel频率倒谱系数结合用于越南语自动语音识别，从而提高了识别性能。


<details>
  <summary>Details</summary>
Motivation: 语音信号的动态特性提供时间信息，在增强自动语音识别（ASR）中起着重要作用。

Method: 通过使用极坐标参数在频谱子带重心频率（SSCFs）的比率平面上表征声学过渡，以捕捉语音的动态特性并最小化频谱变化。这些动态参数与Mel频率倒谱系数（MFCCs）结合用于越南语自动语音识别，以捕获更详细的频谱信息。SSCF0被用作基频（F0）的伪特征来稳健地描述调制信息。

Result: 研究结果表明，所提出的参数显著降低了词错误率，并且比基线MFCCs具有更大的性别独立性。

Conclusion: 研究结果表明，所提出的参数显著降低了词错误率，并且比基线MFCCs具有更大的性别独立性。
...
### [86] [MECAT: A Multi-Experts Constructed Benchmark for Fine-Grained Audio Understanding Tasks](https://arxiv.org/abs/2507.23511)
*Yadong Niu,Tianzi Wang,Heinrich Dinkel,Xingwei Sun,Jiahao Zhou,Gang Li,Jizhong Liu,Xunying Liu,Junbo Zhang,Jian Luan*

Main category: eess.AS

TL;DR: 本文介绍了MECAT，一个用于精细音频理解任务的多专家构建基准，并提出了一种新的度量标准DATE，以更准确地评估音频模型的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基准由于数据注释和评估指标的限制，无法可靠地区分通用和高度详细的模型输出，因此需要一种更细致的音频理解基准。

Method: MECAT是通过集成专业专家模型的分析与链式思维大语言模型推理的管道生成的，提供了多角度、细致的字幕和开放式问答对。

Result: MECAT提供了多角度、细致的字幕和开放式问答对，并引入了DATE度量标准来惩罚通用术语并奖励详细描述。

Conclusion: 本文提出了MECAT基准，用于更细致的音频理解任务，并通过DATE度量标准评估了最先进的音频模型的能力和局限性。
